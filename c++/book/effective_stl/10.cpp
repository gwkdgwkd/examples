...
// 在C++中，一个类型为T的对象，默认分配子（allocator<T>）提供了两个类型定义，
...
// 事实上，C++标准很明确地指出，允许库实现者假定每个分配子的指针类型等同于T*，
// 而分配子的引用类型就是T&，没错，库实现者可以忽略类型定义，而直接使用指针和引用。
// 所以，即便能找到一个办法，从而可以成功地提供新的指针和引用类型，
// 也是无济于事，因为STL实现可能忽略了类型定义，太简洁了。

// 分配子是对象，可以有成员函数、嵌套类型和类型定义（如pointer和reference）等等。
// C++规定STL所有属于同一种类型的分配子对象都是等价的，相互比较的结果总是相等的。
// template <typename T>
...
// list的链接操作没有拷贝任何东西，只有指针被调整，既快速又是异常安全的。
...
// pointer是个类型定义，总是T*，new的参数指明需要多少字节，
...
// 需要记住的内容：
// 1.分配子是一个模板，模板参数T代表为它分配内存的对象的类型；
// 2.提供类型定义pointer和reference，但始终是pointer为T*，reference为T&；
// 3.千万别让分配子拥有随对象而不同的状态，通常，分配子不应该有非静态的数据成员；
// 4.传给分配子的allocate成员函数的是要求内存的对象的个数，而不是所需的字节数，
//   返回的T*指针，尚未有T对象被构造出来；

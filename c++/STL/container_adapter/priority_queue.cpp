...
// 即使用此容器适配器存储元素只能从一端进（称为队尾），从另一端出（称为队头），
// 且每次只能访问priority_queue中位于队头的元素。
// 但是，priority_queue容器适配器中元素的存和取，
// 遵循的并不是First in,First out（先入先出）原则，
// 而是First in，Largest out原则。
// 直白的翻译，指的就是先进队列的元素并不一定先出队列，
// 而是优先级最大的元素最先出队列。
...
// 每当有新元素进入，它都会根据既定的排序规则找到优先级最高的元素，
// 并将其移动到队列的队头；
...
//                    但更多情况下是使用自定义的排序规则。
//  其中，std::less<T>和std::greater<T>都是以函数对象的方式，
//  定义在<function>头文件中。

// priority_queue容器适配器模板位于<queue>头文件中，
// 并定义在std命名空间里，提供的成员函数：
// empty() 	               如果priority_queue为空的话，返回true；
//                         反之，返回false。
// size() 	               返回priority_queue中存储元素的个数。
// top() 	                 返回priority_queue中第一个元素的引用形式。
// push(const T& obj) 	   根据既定的排序规则，
//                         将元素obj的副本存储到priority_queue中适当的位置。
// push(T&& obj) 	         根据既定的排序规则，
//                         将元素obj移动存储到priority_queue中适当的位置。
// emplace(Args&&... args) Args&&... args 表示构造一个存储类型的元素所需要的数据。
//                         此函数的功能是根据既定的排序规则，
//                         在容器适配器适当的位置直接生成该新元素。
// pop()                   移除priority_queue容器适配器中第一个元素。
// swap(priority_queue<T>& other) 将两个priority_queue容器适配器中的元素进行互换，
...
// 值得庆幸的是，STL已经为封装好了可以使用堆存储结构的方法，
// 它们都位于<algorithm>头文件中。
// 常用的几个和堆存储结构相关的方法：
...
int main() {
  ...
  // 另外，用来初始化的数组或容器中的数据不需要有序，
  // priority_queue会自动对它们进行排序。
  ...
  // 结合vector容器提供的成员函数，
  // 模拟了priority_queue容器适配器部分成员函数的底层实现：

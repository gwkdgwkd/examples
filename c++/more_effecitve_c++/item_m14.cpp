...
// 而且如果一个函数抛出一个不在异常规格范围里的异常，系统在运行时能够检测出这个错误，然后一个特殊函数unexpected将被自动地调用。
// 异常规格既可以做为一个指导性文档同时也是异常使用的强制约束机制，它好像有着很诱人的外表。
// 函数unexpected缺省的行为是调用函数terminate，而terminate缺省的行为是调用函数abort，所以一个违反异常规格的程序其缺省的行为就是halt（停止运行）。
// 在激活的栈中的局部变量没有被释放，因为abort在关闭程序时不进行这样的清除操作。对异常规格的触犯变成了一场并不应该发生的灾难。
// 不幸的是，我们很容易就能够编写出导致发生这种灾难的函数。编译器仅仅部分地检测异常的使用是否与异常规格保持一致。
// 一个函数调用了另一个函数，并且后者可能抛出一个违反前者异常规格的异常，编译器不对此种情况进行检测，并且语言标准也禁止编译器拒绝这种调用方式：
void f1() {}
void f2() throw(int) {
...
// 能够避免调用unexpected函数的第二个方法是如果在一个函数内调用其它没有异常规格的函数时应该去除这个函数的异常规格。
...
void CallBack::makeCallBack(int eventXLocation, int eventYLocation) const
    throw() {
  // 在makeCallBack内调用func，要冒违反异常规格的风险，因为无法知道func会抛出什么类型的异常。
  func(eventXLocation, eventYLocation, data);
}
// 通过在程序在CallBackPtr typedef中采用更严格的异常规格来解决问题：
// typedef void (*CallBackPtr1)(int eventXLocation, int eventYLocation, void* dataToPassBack) throw();
...
// CallBack c2(callBackFcn2, callBackData);  // 正确，callBackFcn2没有异常规格
// 传递函数指针时进行这种异常规格的检查，是语言的较新的特性，所以有可能你的编译器不支持这个特性。如果它们不支持，那就依靠你自己来确保不能犯这种错误。

// 避免调用unexpected的第三个方法是处理系统本身抛出的异常。这些异常中最常见的是bad_alloc，当内存分配失败时它被operator new和operator new[]抛出。
// 如果你在函数里使用new操作符，你必须为函数可能遇到bad_alloc异常作好准备。

// 虽然防止抛出unexpected异常是不现实的，但是C++允许你用其它不同的异常类型替换unexpected异常，你能够利用这个特性。
...
// Unexpected异常被一种UnexpectedException新异常类型替换。如果被违反的异常规格包含UnexpectedException异常，那么异常传递将继续下去，好像异常规格总是得到满足。
// 如果异常规格没有包含UnexpectedException，terminate将被调用，就好像你没有替换unexpected一样。
// 另一种把unexpected异常转变成知名类型的方法是替换unexpected函数，让其重新抛出当前异常，这样异常将被替换为bad_exception。你可以这样编写：
void convertUnexpected1() {  // 如果一个unexpected异常被抛出，这个函数被调用
  throw;                     // 它只是重新抛出当前
}
// 如果这么做，你应该在所有的异常规格里包含bad_exception（或它的基类，标准类exception）。你将不必再担心如果遇到unexpected异常会导致程序运行终止。
// 任何不听话的异常都将被替换为bad_exception，这个异常代替原来的异常继续传递。

// 以全面的角度去看待异常规格是非常重要的。它们提供了优秀的文档来说明一个函数抛出异常的种类，并且在违反它的情况下，会有可怕的结果，程序被立即终止，在缺省时它们会这么做。
// 同时编译器只会部分地检测它们的一致性，所以他们很容易被不经意地违反。而且他们会阻止high-level异常处理器来处理unexpected异常，即使这些异常处理器知道如何去做。

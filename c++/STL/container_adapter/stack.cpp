...
// stack适配器的开头端通常称为栈顶。
// 由于数据的存和取只能从栈顶处进行操作，因此对于存取数据，stack适配器有这样的特性，
// 即每次只能访问适配器中位于最顶端的元素，也只有移除stack顶部的元素之后，才能访问位于栈中的元素。
// 栈中存储的元素满足“后进先出（简称LIFO）”的准则，stack适配器也同样遵循这一准则。

// stack适配器以模板类stack<T,Container=deque<T>>的形式位于<stack>头文件中，并定义在std命名空间里。
...
//  swap(stack<T> & other_stack) 将两个stack适配器中的元素进行互换，需要注意的是，
//                     进行互换的2个stack适配器中存储的元素类型以及底层采用的基础容器类型，都必须相同。
...
int main() {
  ...
  // 只要该容器支持empty()、size()、back()、push_back()、pop_back()这5个成员函数即可：
  ...
  // 第3、4种初始化方法中，my_stack适配器的数据是经过拷贝得来的，也就是说，操作my_stack适配器，
  // 并不会对l容器以及my_stack1适配器有任何影响；反过来也是如此。

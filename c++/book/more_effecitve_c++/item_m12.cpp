...
// 一些catch子句，用来捕获异常，异常为Widget，Widget&或Widget*：
// catch (Widget w)
// catch (Widget& w)
...
// 但是当传递参数和异常时，系统所要完成的操作过程则是完全不同的。
// 产生这个差异的原因是：
// 调用函数时，程序的控制权最终还会返回到函数的调用处，
// 但是当抛出一个异常时，控制权永远不会回到抛出异常的地方。

std::istream operator>>(std::istream& s, Widget& w) {}
namespace n1 {
void passAndThrowWidget() {
  Widget localWidget;
  std::cin >> localWidget;  // 传递localWidget到operator>>
...
// 不论通过传值捕获异常还是通过引用捕获，都将进行localWidget的拷贝操作，
...
// 这个子句接收到的只是一个被析构了的Widget，这是无法使用的。
...
// 仅仅能修改localWidget的拷贝。
// 对异常对象进行强制复制拷贝，有助于理解参数传递与抛出异常的第二个差异：
// 抛出异常运行速度比参数传递要慢。
}
...
void passAndThrowWidget() {
  ...
  // 这种行为可能与期待的不一样，但是这与在其他情况下拷贝构造函数的行为一致。
  throw rw;
}
}

// 异常是其它对象的拷贝，这个事实影响到如何在catch块中再抛出一个异常。
...
//   一般应该用throw来重新抛出当前的异常，因为这样不会改变传出去的异常类型，
//   而且更有效率，因为不用生成一个新拷贝。
...
// 不论哪种方法都是一个指针的拷贝被传递。
// 但不能认为抛出的指针是一个指向局部对象的指针，
// 因为当异常离开局部变量的生存空间时，该局部变量已经被释放。
// catch子句将获得一个指向已经不存在的对象的指针。
// 这种行为在设计时应该予以避免，也就是说必须是全局的或堆中的。

// 对象从函数调用处传到函数参数与从异常抛出点传到catch所采用的方法不同，
// 这只是参数传递与异常传递的区别的一个方面；
// 第二个差异是在函数调用或抛出异常与被调用或异常捕获之间的类型匹配过程不同。
...
//   // 这个块永远不会被执行，因为所有的异常都被上面的catch子句捕获。
// }
...
// 与上面这种行为相反，当调用一个虚拟函数时，
// 被调用的函数位于与发出函数调用的对象的动态类型(dynamic type)最相近的类里。
// 可以这样说虚拟函数采用最优适合法，而异常处理采用的是最先适合法。
...
// 2.对象做为异常被抛出与做为参数传递给函数相比，前者（两种）类型转换比后者要少。

...
// multimap容器也用于存储pair<const K, T>类型的键值对，
// 其中各个键值对的键的值不能做修改；
...
//                  增加了const属性，不能用于修改容器内存储的键值对。
// find(key)        在multimap容器中查找首个键为key的键值对，
//                  如果成功找到，则返回指向该键值对的双向迭代器；
//                  反之，则返回和end()方法一样的迭代器。
//                  另外，如果multimap容器用const限定，
//                  则该方法返回的是const类型的双向迭代器。
// lower_bound(key) 返回一个指向当前multimap容器中，
//                  第一个大于或等于key的键值对的双向迭代器。
//                  如果multimap容器用const限定，
//                  则该方法返回的是const类型的双向迭代器。
// upper_bound(key) 返回一个指向当前multimap容器中，
//                  第一个大于key的键值对的迭代器。
//                  如果multimap容器用const限定，
//                  则该方法返回的是const类型的双向迭代器。
...
// max_size()       返回multimap容器所能容纳键值对的最大个数，
//                  不同的操作系统，其返回值亦不相同。
// insert()         向multimap容器中插入键值对。
// erase() 	        删除multimap容器指定位置、
//                  指定键（key）值或者指定区域内的键值对。
// swap() 	        交换2个multimap容器中存储的键值对，
//                  这意味着，操作的2个键值对的类型必须相同。
// clear() 	        清空multimap容器中所有的键值对，
//                  使multimap容器的size()为0。
// emplace()        在当前multimap容器中的指定位置处构造新键值对。
//                  其效果和插入键值对一样，但效率更高。
// emplace_hint()   在本质上和emplace()构造新键值对的方式是一样的，
//                  不同之处在于，
...
void func1() {
  ...
  // 这意味着，map容器中通过指定键获取指定指定键值对的方式，
  // 将不再适用于multimap容器。
  // 因为multimap容器中指定的键可能对应多个键值对，而不再是1个。
  // 只要是multimap容器提供的成员方法，
  // map容器都提供，并且它们的用法是相同的。
  // 由于multimap容器可存储多个具有相同键的键值对，
  // 因此lower_bound()、upper_bound()、
  // equal_range()以及count()成员方法会经常用到。

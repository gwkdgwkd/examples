...
// 所以，它不能用于由输入迭代器指定的区间，也不适用与map或multimap，同样不适用于某些set和mutliset的实现。
...
// 只要确保提供给它们的排序的区间，并保证这些算法所用的比较函数与排序所使用的比较函数一致，
// 就可以有效地使用这些与查找、集合操作以及区间合并有关的算法，并且你会惊喜地发现，
// unique和unique_copy如愿地删除了所有重复的值。

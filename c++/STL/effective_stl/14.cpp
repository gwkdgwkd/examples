...
// STL容器在放入数据时，
// 只要没有超出它们的最大限制（调用max_size成员函数）就可以自动增长。
// 对于vector和string，每次需要更多空间时，
// 就调用与realloc类似的操作，类似以下4个部分：
...
int main() {
  ...
  // 大小和容量之间的关系使我们能够预知，
  // 什么时候插入操作会导致vector或string执行重新分配的动作，

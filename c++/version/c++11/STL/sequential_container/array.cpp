...
// 和其它容器不同，array容器的大小是固定的，无法动态的扩展或收缩，这意味着，
// 在使过程无法借由增加或移除元素而改变其大小，它只允许访问或者替换存储的元素。
// 在array<T,N>类模板中，T用于指明容器中的存储的具体数据类型，
// N用于指明容器的大小，N必须是常量，不能用变量表示。
void func() {
  // array容器有多种初始化方式：
  std::array<int, 5> ia;  // 各个值是不确定的,不会做默认初始化操作
  std::array<double, 4> da{};  // 所有的元素都会被初始化为0.0
  std::array<char, 5> ca{'a', 'b', 'c', 'd', '3'};  // 像创建常规数组一样
  std::array<float, 5> fa{2.1f, 3.4f};  // 只初始化前2个元素，其他元素初始化为0
...
// size() 	   返回容器中当前元素的数量，始终等于array类的第二个模板参数N
// max_size()  返回容器可容纳元素的最大数量，始终等于array类的第二个模板参数N
// empty() 	   判断容器是否为空，和通过size()==0功能相同，但其效率可能更快
// at(n)       返回容器中n位置处元素的引用，自动检查索引，越界抛出out_of_range
// front() 	   返回容器中第一个元素的直接引用，该函数不适用于空的array容器
// back()      返回容器中最后一个元素的直接应用，该函数同样不适用于空的array容器
// data()      返回指向容器首元素的指针，利用该指针可实现复制所有元素等类似功能
// fill(val)   将val这个值赋值给容器中的每个元素
// a1.swap(a2) 交换a1和a2容器中的所有元素，但前提是它们具有相同的长度和类型
void func() {
  ...
  // 访问单个元素方法二，为了能够有效地避免越界访问的情况，
  // 可以使用array容器提供的at()成员函数：
  for (int i = 0; i < a.size(); ++i) {
    std::cout << a.at(i) << " ";
  }
  ...
  // 因此当需要访问容器中某个指定元素时，建议使用at()，除非确定索引没有越界。
  // 为什么array容器在重载[]运算符时，没有实现边界检查的功能呢？
  // 答案很简单，因为性能，如果每次访问元素，都去检查索引值，会产生很多开销。
  ...
  *(data + 3) += 9;
  ...
  // 很少会创建空的array，因为当生成一个array时，它的元素个数就固定了，
  // 而且无法改变，所以生成空array的唯一方法是将模板的第二个参数指定为0，
  // 但这种情况基本不可能发生：
  std::array<int, 0> zero;
  // std::cout << zero.front() << std::endl;  // 崩溃
  // std::cout << zero.back() << std::endl;  // 崩溃

  // array容器之所以提供empty()，对于其他元素可变或者元素可删除的容器来说，
...
// 同样end()返回指向数组中最后一个元素之后一个位置的指针，不是最后一个元素。
// 另外，在<array>头文件中还重载了get()全局函数，
// 该重载函数的功能是访问容器中指定的元素，并返回该元素的引用。
// 由于array容器中包含了at()这样的成员函数，使得操作元素时比普通数组更安全。
void func() {
  ...
  // 该模板函数中，参数的实参必须是一个在编译时可以确定的常量表达式，
  // 所以它不能是一个循环变量，也就是说，它只能访问模板参数指定的元素，
  // 编译器在编译时会对它进行检查。
  std::get<4>(a) = 8;
...
void func() {
  ...
  // 两个array容器可以直接做赋值操作，即将一个容器中的元素赋值给另一个容器：
  addr1 = addr2;

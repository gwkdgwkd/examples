...
// 而在C++引入类（class）类型之后，sizeof的定义也随之进行了拓展。不过在C++98标准中，对非静态成员变量使用sizeof是不能够通过编译的。
...
int main() {
  ...
  cout << sizeof(People::hand) << endl;  // C++98中错误(不报错？), C++11中通过
  ...
  // 在C++ 11中，我们无需这样的技巧，因为sizeof可以作用的表达式包括了类成员表达式。如下代码：
  sizeof(People::hand);

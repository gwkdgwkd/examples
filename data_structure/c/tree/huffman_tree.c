...
//   从根结点到结点c的路径长度为3。
// 结点的权：给每一个结点赋予一个新的数值，被称为这个结点的权，a的权为7，b的权为5。
...
// 如果构建的这棵树的带权路径长度最小，称这棵树为最优二叉树，有时也叫赫夫曼树或者哈夫曼树。
...
// 2.在原有的n个权值中删除那两个最小的权值，同时将新的权值加入到n–2个权值中，以此类推；
// 3.重复1和2，直到所以的结点构建成了一棵二叉树为止，这棵树就是哈夫曼树。

// 构建哈夫曼树时，首先需要确定树中结点的构成。
// 由于哈夫曼树的构建是从叶子结点开始，不断地构建新的父结点，
// 直至树根，所以结点中应包含指向父结点的指针。
// 但是在使用哈夫曼树时是从树根开始，根据需求遍历树中的结点，
// 因此每个结点需要有指向其左孩子和右孩子的指针。
typedef struct {
  int weight;               // 结点权重
...
// 这样，用到哪个字符时，从哈夫曼树的根结点开始，
// 依次写出经过结点的标记，最终得到的就是该结点的哈夫曼编码。

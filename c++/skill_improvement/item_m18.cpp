...
// 这个条款的核心就是over-eager evaluation（过度热情计算法）：
// 在要求你做某些事情以前就完成它们。
...
//   最大值和平均值，这样当min,max或avg被调用时，
//   我们可以不用计算就立刻返回正确的数值。
// 如果频繁调用min,max和avg，我们把跟踪集合最小值、
// 最大值和平均值的开销分摊到所有这些函数的调用上，
...
// 例如磁盘控制器从磁盘读取数据时，
// 它们会读取一整块或整个扇区的数据，即使程序仅需要一小块数据。
// 这是因为一次读取一大块数据比在不同时间读取两个或三个小块数据要快。
// 而且经验显示如果需要一个地方的数据，
// 则很可能也需要它旁边的数据，这是位置相关现象。
...
T& DynArray<T>::operator[](int index) {
  ...
  if (index > 5) {  // 当前最大的索引值
    ...
    // operator new(和 operator delete)的调用通常开销很大。

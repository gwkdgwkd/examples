...
// lambda源自希腊字母表中第11位的λ，
// 在计算机科学领域，它则是被用来表示一种匿名函数。
// 所谓匿名函数，简单地理解就是没有名称的函数，
// 又常被称为lambda函数或者lambda表达式。
// 继Python、Java、C#、PHP等众多高级语言都支持lambda后，
// C++11标准终于引入了lambda。
...
//   不允许在lambda表达式内部修改它们的值，
//   可以理解为这部分变量都是const常量。
//   而如果想修改它们，就必须使用mutable关键字。
//   注意，对于以值传递方式引入的外部变量，
//   lambda表达式修改的是拷贝的那一份，并不会修改真正的外部变量。
// 4.noexcept/throw()
...
namespace n3 {
void testN3_1() {
  // 如果不使用noexcept或者throw()，
  // 则lambda函数体中允许发生任何类型的异常。
  try {
    []() { throw 10; }();
  } catch (int i) {

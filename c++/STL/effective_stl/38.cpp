...
// 按值传递的行为并非不可改变，
// for_each的调用者在调用点上可以显式地指明模板参数的类型。
...
// 由于函数对象往往会按值传递和返回，所以，
// 你必须确保编写的函数对象在经过了传递之后还能正常工作。
// 意味这两件事：
// 1.首先，你的函数对象必须尽可能地小，否则拷贝的开销会非常昂贵；
// 2.其次，函数对象必须是单态的（不是多态的），也就是说，不能使用虚函数。
//   如果参数的类型是基类类型，而实参是派生类对象，
//   那么在传递过程中会产生剥离问题：
//   对象拷贝过程中，派生部分可能会被去掉。
// 当然，效率很重要，而避免剥离问题同样也很重要，
// 但并不是所有的函数子都非常小巧，而且，也不是所有的函数子都是单态的。
// 与普通函数相比，函数对象的一大优点是，函数子可以包含所需要的状态信息。
// 有些函数对象生来就显得非常繁重，
// 所以要能够像传递普通的函数指针那样方便地将这样的函数子传递给STL算法。
// 试图禁止多态的函数子同样也是不切实际的。
// 所以必须找到一个两全其美的办法，既允许函数对象可以很大并且保留多态性，
// 有可以与STL所采用的按值传递函数子的习惯保持一致。方法是：
// 将数据和虚函数从函数子中分类出来，放到一个新的类中；
// 然后在函数子类中包含指向这个新类的指针。
struct Widget {};
template <typename T>
class BPFC1 : public std::unary_function<T, void> {
 private:
  Widget w;
  int x;
  // 很多数据
...
class BPFC : public std::unary_function<T, void> {
 ...
 public:
  ...
  // 必须以某种合理的方式来支持拷贝动作。
  // 确保BPFC的拷贝构造函数正确地处理了所指向的BPFCImpl对象。
  // 简单合理的做法是使用引用计数。
};

// 需要谨慎处理BPFC的拷贝构造函数，
// 因为函数对象在STL中作为参数传递或者返回的时候总是按值方式传递拷贝的。

...
// 默认情况下，map容器选用std::less<T>排序规则（其中T表示键的数据类型），
// 其会根据键的大小对所有键值对做升序排序。
// 当然，也可以手动指定map容器的排序规则，
// 既可以选用STL中提供的其它排序规则（std::greater<T>），也可以自定义排序规则。
...
void func2() {
  ...
  //   不同之处在于，如果查找失败，该方法不会向容器中添加新的键值对，而是直接抛出out_of_range异常。
  ...
  // 4.如果以上方法都不适用，还可以遍历整个map容器，找到包含指定键的键值对，进而获取该键对应的值：
  ...
  // 从“在map容器存储的键值对中，获取指定键对应的值”的角度出发，
  // 更推荐使用at()成员方法，因为该方法既简单又安全。
}

void func3() {
  // 除了使用[]运算符实现向map容器中添加新键值对外，
  // map类模板中还提供有insert()成员方法，该方法专门用来向map容器中插入新的键值对。
  ...
  // 其中val为要插入的键值对变量。
  // 注意，和第1种方式的语法格式不同，这里insert()方法返回的是迭代器，而不再是pair对象：
...
void func6() {
  // map类模板中还提供有lower_bound(key)和upper_bound(key)成员方法，
  // 它们的功能是类似的，唯一的区别在于：
  ...
  // 因为map容器中各键值对的键的值都是唯一的，因此通过map容器调用此方法，
  // 其返回的范围内最多也只有1个键值对。
}

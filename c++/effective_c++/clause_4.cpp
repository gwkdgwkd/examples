...
// 有多个构造函数，会导致重复代码，可以在初始化列表中遗漏那些（赋值和初始化一样好）的成员变量，
...
// static对象，从被构造出来直到程序结束为止，因此stack和heap对象都被排除。
// 包括global对象、定义namespace内的对象、class内、函数内以及文件作用内被声明为static的对象。
// 函数内的static对象被称为local static对象，其他static对象称为non-local static对象。
// 程序结束时static对象会被自动销毁，析构函数会在main()结束时被自动调用。
// 所谓编译单元是指产出单一目标文件的源码。基本上是单一源码文件加上包含的头文件。
// 如果某个编译单元内的某个non-local static对象的初始化使用了另一个编译单元内的某个non-local static对象，
// 用到的对象可能尚未被初始化。
// 因为C++对“定义于不同编译单元内的non-local static对象”的初始化次序并无明确定义。
// 因为决定次序非常困难，根本无解。
// 消除这个问题需要做的是，将每个non-local static对象搬到自己的专属函数内（在函数内被声明为static），
// 这些函数返回一个引用指向所包含的对象。
// 然后用户调用这些函数，而不直接接触这些对象。
// 换句话说，non-local static对象被local static对象替换了（单例模式常见的实现手法）。
// C++保证，函数内的local static对象会在“该函数被调用期间”“首次遇上该对象的定义式”时被初始化。
// 使用函数调用代替访问non-local static对象，就可以保证获取已经初始化的对象。
// 更棒的是，如果从未调用non-local static对象的“仿真函数”，就绝不会引发构造和析构成本。

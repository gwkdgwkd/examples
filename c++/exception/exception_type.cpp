...
// 当程序抛出异常时，会创建一份数据，这份数据包含了错误信息，可以根据这些信息来判断出了什么问题，接下来怎么处理。
// 异常既然是一份数据，那么就应该有数据类型。

// C++规定，异常类型可以是int、char、float、bool等基本类型，也可以是指针、数组、字符串、结构体、类等聚合类型。
...
// 2.而对于catch，异常是在运行阶段产生的，它可以是任何类型，没法提前预测，
//   所以不能在编译阶段判断类型是否正确，只能等到程序运行后，真的抛出异常了，
//   再将异常类型和catch能处理的类型进行匹配，匹配成功的话就“调用”当前的catch，否则就忽略当前的catch。
...
namespace n3 {
// 当异常发生时，程序会按照从上到下的顺序，将异常类型和catch所能接收的类型逐个匹配。
// 一旦找到类型匹配的catch就停止检索，并将异常交给当前的catch处理（其他的catch不会被执行）。
// 如果最终也没有找到匹配的catch，就只能交给系统处理，终止程序的运行。

// catch在匹配过程中的类型转换:
// C/C++中存在多种多样的类型转换，普通函数（非模板函数）发生调用时，
// 如果实参和形参的类型不是严格匹配，那么会将实参的类型进行适当的转换，以适应形参的类型，包括：
...
void func3() {  // 数组指针转换和const转换
  ...
  try {
    ...
    // catch中没有严格匹配的类型，所以把int[3]先转换为int*，再转换为const int*。

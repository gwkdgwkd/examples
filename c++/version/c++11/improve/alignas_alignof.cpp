...
#pragma pack(pop)  // 指定恢复保存（1字节）的对齐方式
struct A4 {
  int a;
  char b;
  int c;
...
// 为了省事有人不push和pop，直接改，其他人包含了这个头文件，进而受影响。
// 于是编译器突破规范定义了扩展：__declspec(align(n))等，但也是各做各的，不统一。
...
// struct alignas(3) E {};  // 非法的非零对齐，对齐数3不是2的N次方

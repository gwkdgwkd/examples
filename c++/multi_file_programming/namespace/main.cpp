...
// 当进行多文件编程时，通常是将声明部分（例如变量、
// 函数和类等）划分到.h文件中，将实现部分划分到.cpp文件中。
// 在此基础上，如果要给变量、函数或者类指定命名空间，
// 则该命令空间应至少包含它们的声明部分。
// 所以当进行多文件编程时，命名空间常位于.h头文件中。

// 当类的声明位于指定的命名空间中时，如果要在类的外部实现其成员方法，
// 需同时注明所在命名空间名和类名。
namespace n1 {
class A {
 public:
...
// 不同的头文件中可以使用的是不同的命名空间，
// 除此之外，不同头文件中也可以使用名称相同的命名空间，

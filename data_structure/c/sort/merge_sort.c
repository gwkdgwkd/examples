...
// 归并排序算法是在分治算法基础上设计出来的一种排序算法，对应的时间复杂度为O(nlogn)。
// 归并排序算法实现排序的思路是：
// 1.将整个待排序序列划分成多个不可再分的子序列，每个子序列中仅有1个元素；
// 2.所有的子序列进行两两合并，合并过程中完成排序操作，最终合并得到的新序列就是有序序列。

// 对于含有n个记录的无序表，首先默认表中每个记录各为一个有序表（只不过表的长度都为1），
// 然后进行两两合并，使n个有序表变为⌈n/2⌉个长度为2或者1的有序表，
// 通过不断地进行两两合并，直到得到一个长度为n的有序表为止，这种方法称为2-路归并排序。

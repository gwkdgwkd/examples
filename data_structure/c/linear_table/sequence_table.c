...
// 线性表，基于数据在实际物理空间中的存储状态，
// 又可细分为顺序表（顺序存储结构）和链表（链式存储结构）。
// 1.将数据依次存储在连续的整块物理空间中，这种存储结构称为顺序存储结构（简称顺序表）；
// 2.数据分散的存储在物理空间中，通过一根线保存着它们之间的逻辑关系，
//   这种存储结构称为链式存储结构（简称链表）；

// 对于具有“一对一”逻辑关系的数据，一直在用“某一元素的左侧（前边）或右侧（后边）”这样不专业的词，
// 其实线性表中有更准确的术语：
...
//   顺序表中，插入、删除和移动数据可能会牵涉到大量元素的整体移动，因此时间复杂度至少为O(n);
//   链表适合问题中主要涉及元素的插入、删除和移动，访问元素的需求很少；
//   因为在链表中访问数据元素，需要从表头依次遍历，直到找到指定节点，花费的时间复杂度为O(n);
...
// 顺序表存储数据时，会提前申请一整块足够大小的物理空间，然后将数据依次存储起来，
// 存储时做到数据元素之间不留一丝缝隙。

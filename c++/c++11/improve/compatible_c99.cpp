...
// __STDC__：C中表示编译器是否与C标准一致，C++中由编译器决定这个宏是否定义，定义成什么值。
// __STDC__VERSION__：C中表示编译器支持的标准版本，C++中由编译器决定这个宏是否定义，定义成什么值。
// __STDC_ISO_10646__：用于表示C++编译环境符合某个版本的ISO/IEC 10646标准。

void testN1() {
  std::cout << "Standard Clib: " << __STDC_HOSTED__
            << std::endl;                                // Standard Clib: 1
  std::cout << "Standard C: " << __STDC__ << std::endl;  // Standard C: 1
  // std::cout << "C Standard Version: " << __STDC__VERSION__ << std::endl;
  std::cout << "ISO/IEC " << __STDC_ISO_10646__ << std::endl;  // ISO/IEC 201706
}
}  // namespace n1
...
struct TestStruct {  // 在C++11标准中，还允许在类和结构体中使用__func__
  TestStruct() : name(__func__) {}
  const char *name;
};
// 将__func__标识符作为函数参数的默认值是不允许的:
// ‘__func__’ is not defined outside of function scope
// void FuncFail(std::string func_name = __func__){};  // 编译不能通过
...
void testN3() {
  LOG("x = %d", 3);  // compatible_c99.cpp: Line 61:    x = 3
...
namespace n4 {
// 宏__cpluscplus:C/C++代码混合编程时运用
// 主要用途就是实现用C方式进行编译，因为用C++方式编译它会将函数进行名字重组（为了实现函数重载），
// 所以要混合编程必须统一按C方式编译代码。
#ifdef __cplusplus
extern "C" {
#endif
...
// _Pragma操作符:_Pragma(字符串字面值)
// 下面3中方法，都能保证头文件只被包含一次
// _Pragma("once")      // 1
// #pragma once         // 2
...
// 11标准中，char字符串和wchar_t字符串连接，会先将char转换为wchar_t，再连接字符串。

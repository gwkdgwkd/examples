...
// 由于拷贝控制操作是由三个特殊的成员函数来完成的，所以我们称此为C++三法则。
// 在C++11标准中，为了支持移动语义，又增加了移动构造函数和移动赋值运算符，
// 这样共有五个特殊的成员函数，所以又称为C++五法则。
// 也就是说，三法则是针对较旧的C++89标准说的，五法则是针对较新的C++11标准说的。
// 为了统一称呼，后来人们干把它叫做C++三/五法则。
// 如果一个类没有定义所有这些拷贝控制成员，
// 编译器会自动为它定义默认的操作，因此很多类会忽略这些拷贝控制操作。
// 但是，对于一些持有其他资源（例如动态分配的内存、打开的文件、指向其他数据的指针、
// 网络连接等）的类来说，依赖这些默认的操作会导致灾难，必须显式的定义这些操作。

// C++并不要求定义所有的这些操作，你可以只定义其中的一个或两个。

...
// 外部extern这个概念在C中就有了，
// 在一个文件a.c中定义了一个变量int i，
// 要在另外一个文件b.c中使用它，
// 这个时候就会在b.c中做一个外部变量的声明，例如：
extern int i;
// 这样做的好处是在分别编译了a.c和b.c之后，
// 它们生成的目标文件a.o和b.o中只有i这个符号的一份定义。
// 具体说就是a.o中的i实际存在于a.o目标文件的数据区中的数据，
// 而在b.o中只是记录了i会引用其他目标文件数据区中名为i的数据。
// 在链接器将a.o和b.o链接成一个可执行文件c的时候，
// c的数据区中也只有一个i的数据。
// 如果在b.c中声明i时不叫extern，
// 那么i既存在于a.o的数据区中，也存在于b.o的数据区中。
// 在链接器链接a.o和b.o的时候，就会报错，
// 因为无法决定相同的符号是否需要合并。
...
// 所以在目标文件test1.o和test2.o中，
// 就会有两份相同的函数fun<int>(int)代码。
// 代码重复，为了节省空间，值保留其中一个。
// 在链接的时候，链接器会通过一些手段，
// 将重复的的函数模板fun<int>(int)删掉，
// 只保留一个，这样就解决了模板实例化产生的代码冗余问题。
// 编译器要对每一处模板进行实例化，链接器还要移除重复的实例化代码。
// 当在广泛使用模板的项目中，编译器会产生大量的冗余代码，
// 这会极大的增加编译时间和链接时间。

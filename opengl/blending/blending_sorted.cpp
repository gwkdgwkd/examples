...
int main()
{
    ...
    glEnable(GL_BLEND); // 用GL_BLEND来启用混合
    ...
    unsigned int transparentTexture = loadTexture("../source/resources/window.png");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // ---------------------------------------------
        // 把距离和它对应的位置向量存储到一个STL库的map数据结构中。map会自动根据键值(Key)对它的值排序，所以只要我们添加了所有的位置，并以它的距离作为键，它们就会自动
        // 根据距离值排序了。结果就是一个排序后的容器对象，它根据distance键值从低到高储存了每个窗户的位置。
...
// glBlendFunc(GLenum sfactor, GLenum dfactor)函数接受两个参数，来设置源和目标因子。OpenGL为我们定义了很多个选项，下面列出大部分最常用的选项。
...
// L_FUNC_ADD：默认选项，将两个分量相加：C¯result=Src+Dst。
...
// 不要打乱顺序:
// 要想让混合在多个物体上工作，我们需要最先绘制最远的物体，最后绘制最近的物体。普通不需要混合的物体仍然可以使用深度缓冲正常绘制，所以它们不需要排序。但我们仍要保证它
// 们在绘制（排序的）透明物体之前已经绘制完毕了。当绘制一个有不透明和透明物体的场景的时候，大体的原则如下：
// 先绘制所有不透明的物体。
// 对所有透明的物体排序。(排序透明物体的一种方法是，从观察者视角获取物体的距离。这可以通过计算摄像机位置向量和物体的位置向量之间的距离所获得)
// 按顺序绘制所有透明的物体。
// 虽然按照距离排序物体这种方法对我们这个场景能够正常工作，但它并没有考虑旋转、缩放或者其它的变换，奇怪形状的物体需要一个不同的计量，而不是仅仅一个位置向量。
// 在场景中排序物体是一个很困难的技术，很大程度上由你场景的类型所决定，更别说它额外需要消耗的处理能力了。完整渲染一个包含不透明和透明物体的场景并不是那么容易。更高级
// 的技术还有次序无关透明度(Order Independent Transparency, OIT)

...
// BOOST_STATIC_ASSERT和LOKI_STATIC_CHECK，但由于它们都是利用了一些编译器的隐晦特性实现的trick，可移植性、简便性都不是太好，还会降低编译速度，而且功能也不够完善。
...
int main() {
  ...
  double varB;
  // int varB = 5;
  bit_copy(varA, varB);  // template parameter size no equal!
  return 0;
}
...
// const多才多艺，可以：
// 1.在class外部修饰全局或命名空间中的常量，或修饰文件、函数、或区块作用域中被声明为static的对象；
...
// 在一个函数内，const可以和函数返回值、各参数、函数自身（成员函数）产生关联。
// 令函数返回一个常量值，往往可以降低因客户错误而造成的意外，而不至于放弃安全性和高效型。
class Rational {};
const Rational operator*(const Rational& lhs, const Rational& rhs){};
Rational operator/(const Rational& lhs, const Rational& rhs){};

// const参数，没有什么特别新颖的概念，与local const对象一样，除非需要改动，否则请将它们声明为const。

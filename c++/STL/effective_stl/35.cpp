...
int ciStringCompareImpl(const std::string& s1, const std::string& s2) {
  ...
  // 最繁重的工作由mismatch完成，它返回一对迭代器，
  // 指示了这两个区间对应字符第一次比较失败的位置：
  ...
  // 因为当返回false时mismatch会停下。
  // 不能直接调用ciCharCompare，因为它返回-1、1或者0，
  // 而且当两个字符匹配的时候返回0。
  // 如果直接用ciCharCompare作为判别式，
  // 那么C++将会把它的返回类型转换成bool。
...
int ciStringCompare1(const std::string& s1, const std::string& s2) {
  ...
  // 特别是，如果两个字符串的长度不一样，那么必须把短的字符串作为第一个区间传入。
  if (s1.size() <= s2.size()) {
...
int ciStringCompare2(const std::string& s1, const std::string& s2) {
  // lexicographical_compare名字第二长，
  // 最长的是set_symmetric_difference。
  // lexicographical_compare是strcmp的一个泛化版本。
  // strcmp只能与字符数组一起工作，
  // 而lexicographical_compare则可以与任何类型的值的区间一起工作。
  // strcmp只是比较两个字符串的关系，
  // 而lexicographical_compare则可以接受一个判别式。
  return std::lexicographical_compare(s1.begin(), s1.end(), s2.begin(),
                                      s2.end(), ciCharLess);
}

// C库的非标准扩展中，有忽略大小写的字符串比较函数。
// 具有像strcmp或者strcmpi这样的名字。

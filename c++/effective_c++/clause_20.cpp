...
//  当一个派生类对象以by value方式传递并被视为一个基类对象，基础类copy构造函数会被调用，派生类属性被切割，仅仅留下基类对象。
void printNameAndDisplay1(Window w) {
  // 不论传递过来的对象原本是什么类型，参数w就是一个Window对象。因此，函数内调用的是基类版本的函数。
...
// 即使小型对象拥有并不昂贵的copy构造函数，还是可能有效率上的争议。编译器对待内置类型和用户自定义类型的态度截然不同，哪怕两者拥有相同的底层类型。
...
int main() {
  ...
  // const很重要，以by value方式，调用者不会修改实参，只会修改副本。以by reference方式，调用者需要考虑会不会改变实参数。

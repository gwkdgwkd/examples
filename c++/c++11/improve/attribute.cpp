...
// 随着C++语言的演化和编译器的发展，
// 人们常会发现标准提供的语言能力不能完全满足要求。
// 于是编译器厂商或组织为了满足编译器客户的需求，
// 设计出一系列的语言扩展语法。
// 这些扩展语法并不存在于C++/C标准中，却有可能拥有较多的用户。
// 扩展语法中比较常见的就是属性，
// 属性是对语言中的实体对象，比如函数、变量、类型等，
// 附加一些的额外注解信息，其用来实现一些语言及非语言层面的功能，
// 或是实现优化代码等的一种手段。
// 不同编译器有不同的属性语法，比如对于g++，
// 属性是通过GNU的关键字__attribute__来声明的。
// 程序员只需要简单地声明：__attribute__((attribute-list))
// 即可为程序中的函数、变量和类型设定一些额外信息，
// 以便编译器可以进行错误检查和性能优化等。
extern int area(int n) __attribute__((const));
// 这里的const属性告诉编译器：本函数返回值只依赖于输入，
// 不会改变任何函数外的数据，因此没有任何副作用。
// 在此情况下，编译器可以对area函数进行优化处理，
// area(3)的值只需要计算一次，
// 编译之后可以将area(3)视为循环中的常量而只使用其计算结果，
// 从而大大提高了程序的执行性能。

// 在GNU对C/C++的扩展中可以看到很多不同的attribute属性：
// 常见的如format、noreturn、const和aligned等，
// 具体含义和用法可以参考GNU的在线文档。

// 在Windows平台上，使用另外一种关键字__declspec。
// __declspec是微软用于指定存储类型的扩展属性关键字，
// 用户只要简单地在声明变量时加上：
...
// 对于作用于声明的通用属性，既可以写在声明的起始处，
// 也可以写在声明的标识符之后。
// 而对于作用于整个语句的通用属性，则应该写在语句起始处。
// 而出现在以上两种规则描述的位置之外的通用属性，
// 作用于哪个实体跟编译器具体的实现有关。
...
// 按照C++11的定义，[[attr1]]和[[attr4]]会作用于c1，
// [[attr1]]和[[attr5]]会作用于c2,
// [[attr2]]出现在声明之后，仅作用于类C，
// 而[[attr3]]所作用的对象则跟具体实现有关。

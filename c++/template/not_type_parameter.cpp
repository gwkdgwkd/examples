...
// 多出来的形参len给编码带来了不便，我们可以借助模板中的非类型参数将它消除：
...
// 换句话说，数组容量不能动态地增大或者减小，这样的数组称为静态数组（Static array）。
// 静态数组有时候会给编码代码不便，可以通过自定义的Array类来实现动态数组（Dynamic array）。

...
// 使用此变量前，必须调用pthread_attr_init()函数进行初始化，该函数的语法格式如下：
// int pthread_attr_init(pthread_attr_t * attr);
// 此函数定义在<pthread.h>头文件中，函数执行成功时，返回数字0，反之返回非零数。
// 通过调用pthread_attr_init()函数，myAttr变量就拥有了系统默认的线程属性。
...
//   可以直接将目标线程的__detachstate属性改为PTHREAD_CREATE_DETACHED，语法如下：
...
//   SCHED_RR：轮转法。
//   其中，SCHED_OTHER调度算法不支持为线程设置优先级，而另外两种调度算法支持。
//   <pthread.h>头文件提供了如下两个函数，专门用于访问和修改__schedpolicy属性：
...
//   该属性仅当线程的__schedpolicy属性为SCHED_FIFO，或者SCHED_RR时才能发挥作用。
...
//   默认情况下规定新线程的调度属性遵循父线程，也可以修改__inheritsched的值，
...
//   PTHREAD_EXPLICIT_SCHED：新线程的调度属性继承自myAttr规定的值。
...
//   通常称这块内存为警戒缓冲区，它的功能是：
//   一旦我们使用的栈空间超出了额定值，警戒缓冲区可以确保线程不会因栈溢出立刻执行崩溃。
...
//   某些场景中，线程执行可能需要较大的栈内存，此时就需要自定义线程拥有的栈的大小。

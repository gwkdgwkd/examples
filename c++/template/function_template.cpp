...
// 为了交换不同类型的变量的值，通过函数重载定义了四个名字相同、参数不同的函数。
// 这些函数虽然在调用时方便了一些，
// 但从本质上说还是定义了三个功能相同、函数体相同的函数，
...
// 所谓函数模板，实际上是建立一个通用函数，
// 它所用到的数据的类型（包括返回值类型、形参类型、局部变量类型）可以不具体指定，
...
// 说得直白一点，原来使用int、float、char等内置类型的地方，
// 都可以用类型参数来代替。
...
// 这样做显得不太友好，所以后来C++又引入了一个新的关键字typename，
// 专门用来定义类型参数。
...
template <typename T>
// 模板头和函数定义（声明）是一个不可分割的整体，
// 它们可以换行，但中间不能有分号。
// 在该函数模板中，最后一个参数的类型为具体类型int，
// 而不是泛型，并不是所有的模板参数都必须被泛型化。
// 模板头和函数头可以合并到一行，不过为了让代码格式更加清晰，
// 一般是将它们分成两行。
...
// 因为已经显式地指明了T1、T2的具体类型，
// 所以编译器就不用再自己推断了，直接拿来使用即可。
...
// 2.派生类向基类的转换：也就是向上转型。
// 3.const转换：也即将非const类型转换为const类型，
//   例如将char *转换为const char *。
...
void func4() {
  int str1[20];
  int str2[10];
  // 由于str1、str2的类型分别为int[20]和int[10]，
  // 在函数调用过程中又不会转换为指针，
  // 所以编译器不知道应该将T实例化为int[20]还是int[10]，导致调用失败：
  // fun6(str1, str2);
  // no matching function for call to ‘fun6(int [20], int [10])’

  // fun6()有两个类型参数，分别是T1和T2，
  // 但是编译器只能从函数调用中推断出T1的类型来，
  // 不能推断出T2的类型来，所以这种调用是失败的，
  // 必须显式地指明T1、T2的具体类型：
  // fun6(10);
  // no matching function for call to ‘fun6(int)’
}

// 函数模板的实参推断是指在函数调用过程中，
// 根据实参的类型来寻找类型参数的具体类型的过程，
...
void func5() {
  ...
  // fun7<int>(10);
  // no matching function for call to ‘fun7<int>(int)’
  ...
  // fun9(10, 23.5);
  // no matching function for call to ‘fun9(int, double)’
}

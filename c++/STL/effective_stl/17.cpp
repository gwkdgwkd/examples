...
// 为了避免占用不再需要的内存，
// 需要把容器的容量从以前的最大值缩减到当前需要的数量。
// 这种对容量的缩减通常被称为shrink to fit。
// 这一技术并不保证一定能去除多余的容量。
// STL实现者如果愿意的话，
// 可以自由地为vector和string保留多余的容量，有时确实希望这样做。
...
int main() {
  ...
  // vector<int>(v)创建了一个临时变量，
  // 是v的拷贝，有vector的拷贝构造函数完成。
  // 然而vector的拷贝构造函数只为所拷贝的元素分配所需要的内存，
  // 所以这个临时变量没有多余的容量。
  // 然后把临时变量中的数据和v中的数据做swap操作。
  // 最后，v具有了被去除之后的容量，即临时变量的容量。
  // 而临时变量具有原先v臃肿的容量。

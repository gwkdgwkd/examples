...
// 这个设计由三个部分组成:
// 1.Printer类的构造函数是private，这样能阻止建立对象。
// 2.全局函数thePrinter被声明为类的友元，
//   让thePrinter避免私有构造函数引起的限制。
// 3.最后thePrinter包含一个静态Printer对象，
//   这意味着只有一个对象被建立。
// 客户端代码无论何时要与系统的打印机进行交互访问，
// 它都要使用thePrinter函数：
class PrintJob {
 public:
...
// thePrinter使用全局命名空间完全是多余的，
// 想把所有与打印有关的功能都放到Printer类里。
...
//   与此相反，只有第一次执行函数时，才会建立函数中的静态对象，
//   所以如果没有调用函数，就不会建立对象。
//   不过你得为此付出代价，每次调用函数时都得检查是否需要建立对象。
//   建立C++一个理论支柱是你不需为你不用的东西而付出，在函数里，
//   把类似于Printer这样的对象定义为静态成员就是坚持这样的理论，
//   你应该尽可能坚持这种理论。
//   与一个函数的静态成员相比，
//   把Printer声明为类中的静态成员还有一个缺点，它的初始化时间不确定。
//   我们能够准确地知道函数的静态成员什么时候被初始化：
//   在第一次执行定义静态成员的函数时。
//   而没有定义一个类的静态成员被初始化的时间。
//   C++为一个编译单元内的静态成员的初始化顺序提供某种保证，
//   但是对于在不同编译单元中的静态成员的初始化顺序则没有这种保证。
//   在实际使用中，这会给我们带来许多麻烦。
//   当函数的静态成员能够满足我们的需要时，就能避免这些麻烦。
// 2.另一个细微之处是内联与函数内静态对象的关系。
//   thePrinter的非成员函数形式，除了第一次执行这个函数时（也就是构造p时），
...
// 但是你可能认为建立函数来返回一个隐藏对象的引用，
// 这种限制对象的数量的方法是错误的。
// 也许你认为只需简单地计算对象的数目，一旦需要太多的对象，
// 就抛出异常，这样做也许会更好。
class Printer2 {
 public:
...
// 另一个是很容易推广它的用途，
// 可以允许建立对象最多的数量不是一，而是其它大于一的数字。

// 建立对象的环境
// 这种方法也有一个问题，定义基类和派生类对象会产生多少Pritner对象？
// 答案是两个。
// 在运行时，当构造派生类的基类部分时，会抛出TooManyObjects异常。
...
// 每个makeFSA的伪构造函数则是返回一个指向对象的指针，
// 指向的对象都是惟一的，不相同的。
// 也就是说允许建立的FSA对象数量没有限制。
// 不过每个伪构造函数都调用new这个事实暗示调用者必须记住调用delete，
// 否则就会发生资源泄漏。
// 如果调用者希望退出生存空间时delete会被自动调用，
// 他可以把makeFSA返回的指针存储在auto_ptr中。
...
Printer3::Printer3() {
  if (numObjects >= 1) {
    throw TooManyObjects();
    // 当需要的对象过多时，会抛出异常，
    // 如果你认为这种方式给你的感觉是unreasonablyharsh，
    // 你可以让伪构造函数返回一个空指针，当然用户在使用之前应该进行检测。
  }
  // 继续运行正常的构造函数;
  ++numObjects;
}
...
// 我们只需把hard-wired常量值1改为根据某个类而确定的数量，
// 然后消除拷贝对象的约束。
class Printer4 {
 public:
  class TooManyObjects {
...
class Counted {
 ...
 private:
  ...
  // private成员函数init用来避免两个Counted构造函数的语句重复：
  void init();
};
template <class BeingCounted>
...
class Printer5 : private Counted<Printer5> {
 public:
  ...
  void reset();
  void performSelfTest();
  // 让这个函数对于Printer是public：
  using Counted<Printer5>::objectCount;
  // 老式的访问权声明语法：让objectCount在Printer中是public：
  // Counted<Printer5>::objectCount;
...
// 不对maxObject进行初始化，而是让此类的客户端提供合适的初始化。
...
// 坦率地说，除了Printer的编写者，没有人关心这个事实。
// 它的实现细节最好是private，这就是为什么这里使用private继承的原因。
// 另一种方法是在Printer和counted<Printer>之间使用public继承，
// 但是我们必须给Counted类一个虚拟析构函数。
// 否则如果有人通过Counted<Printer>*指针删除一个Printer对象，
// 我们就有导致对象行为不正确的风险。
// 在Counted中存在虚函数，几乎肯定影响从Counted继承的对象的大小和布局。
// 我们不想引入这些额外的负担，所以使用private继承来避免这些负担。
// Counted所做的大部分工作对于Printer的用户来说都是隐藏的，
// 但是这些用户可能很想知道有当前多少Printer对象存在。
// Counted模板提供了objectCount函数，用来提供这种信息，
// 但是因为我们使用private继承，这个函数在Printer类中成为了private。
// 当Printer继承Counted<Printer>时，它可以忘记有关对象计数的事情。
// 编写Printer类时根本不用考虑对象计数，就好像有其他人会为它计数一样。
// Printer的构造函数可以是这样的：
// Printer5::Printer5() {
//   // 进行正常的构造函数运行
// }
// 这里有趣的不是你所见到的东西，而是你看不到的东西。
// 不检测对象的数量就好像限制将被超过，
// 执行完构造函数后也不增加存在对象的数目。
// 所有这些现在都由Counted<Printer>的构造函数来处理，
// 因为Counted<Printer>是Printer的基类，

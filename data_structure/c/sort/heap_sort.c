...
// 在含有n个元素的序列中，如果序列中的元素满足下面其中一种关系时，此序列可以称之为堆：
// 1.在n个记录的范围内，第i个关键字的值小于第2*i个关键字，同时也小于第2*i+1个关键字；
// 2.在n个记录的范围内，第i个关键字的值大于第2*i个关键字，同时也大于第2*i+1个关键字。
// 对于堆也可以使用完全二叉树来解释，在完全二叉树中第i个结点的左孩子恰好是第2i个结点，
// 右孩子恰好是2i+1个结点，如果该序列可以被称为堆，则使用该序列构建的完全二叉树中，
// 每个根结点的值都必须不小于（或者不大于）左右孩子结点的值。
// 堆用完全二叉树表示时，其表示方法不唯一，
// 但是可以确定的是树的根结点要么是无序表中的最小值，要么是最大值。

// 通过将无序表转化为堆，可以直接找到表中最大值或者最小值，然后将其提取出来，
// 令剩余的记录再重建一个堆，取出次大值或者次小值，如此反复执行就可以得到一个有序序列，
// 此过程为堆排序，堆排序过程的代码实现需要解决两个问题：
// 1.如何将得到的无序序列转化为一个堆？
// 2.在输出堆顶元素之后（完全二叉树的树根结点），如何调整剩余元素构建一个新的堆？

// 堆分为大根堆和小根堆，大根堆是一棵完全二叉树，且对于任意一个子节点，
// 其值均不大于父节点的值，如此递推，就是根节点的值是最大的，即大根堆。
// 小根堆也是一棵完全二叉树,且对于任意一个子节点，其数值均不小于父节点的值，
// 这样层层递推，就是根节点的值最小，即小根堆。

// 完全二叉树通过起始索引为0的一维数组来存储时有如下特性，当然堆就也有了：
// 1.父节点i的左子节点在位置2*i+1；
// 2.父节点i的右子节点在位置2*i+2；
// 3.子节点i的父节点在位置(i-1)/2，i/2-1也行。

// 堆排序在最坏的情况下，其时间复杂度仍为O(nlogn)，这是相对于快速排序的优点所在。
// 同时堆排序相对于树形选择排序，其只需要一个用于记录交换（rc）的辅助存储空间，
// 比树形选择排序的运行空间更小。

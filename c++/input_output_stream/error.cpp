...
//  当我们输入时，可能会由于人的失误（错误理解了指令、打字错误、允许自家的小猫在键盘上散步等）、
//  文件格式不符、错误估计了情况等原因造成读取失败。
//  当我们输出时，如果输出设备不可用、队列满或者发生了故障等，都会导致写入失败。
// 发生输入输出错误的可能情况是无限的！但C++将所有可能的情况归结为四类，称为流状态（stream state）。
// 每种流状态都用一个iostate类型的标志位来表示。流状态对应的标志位：
...
//  如果输入操作遇到一个简单的格式错误，则使流进入fail()状态，也就是假定输入操作的用户可以从错误中恢复。
//  如果错误真的非常严重，例如发生了磁盘故障，输入操作会使得流进入bad()状态。
...
namespace n2 {
// 如果fill_vector()的调用者想知道是什么原因终止了输入，那么可以检测流是处于fail()还是eof()状态。
// 当然也可以捕获error()抛出的runtime_error异常，但当istream处于bad()状态时，继续获取数据是不可能的。
// 大多数的调用者无须为此烦恼，因为这意味着，几乎在所有情况下，对于bad()状态，我们所能做的只是抛出一个异常。
...
void testN3() {
  ...
  if (!std::cin) {         // 只有输入操作失败，才会跳转到这里
    ...
    if (std::cin.fail()) {  // 流遇到了一些意外情况
      // 处理fail()时所使用的cin.clear()。
      // 当流发生错误时，我们可以进行错误恢复。
      // 为了恢复错误，我们显式地将流从fail()状态转移到其他状态，从而可以继续从中读取字符。
      // clear()就起到这样的作用，执行cin.clear()后，cin的状态就变为good()。

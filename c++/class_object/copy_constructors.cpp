...
// 对于计算机来说，拷贝是指用一份原有的、已经存在的数据创建出一份新的数据，
// 最终的结果是多了一份相同的数据。
...
//   注意是首次赋值，再次赋值不叫初始化。
//   初始化的时候还可以为对象分配其他的资源（打开文件、连接网络、动态分配内存等），
//   或者提前进行一些计算等。
//   说白了，初始化就是调用构造函数。
...
// 初始化对象是指为对象分配内存后第一次向内存中填充数据，这个过程会调用构造函数。
...
// 即使将它们混淆，也不会出现什么错误。
// 但是对于类，它们的区别就非常重要了，
// 因为初始化时会调用构造函数（以拷贝的方式初始化时会调用拷贝构造函数），
// 而赋值时会调用重载过的赋值运算符。

// 如果没有显式地定义拷贝构造函数，
// 那么编译器会自动生成一个默认的拷贝构造函数。
// 这个默认的拷贝构造函数很简单，
// 使用老对象的成员变量对新对象的成员变量进行一一赋值。
...
void func3() {
  ...
  // 理论上讲，运行代码后会调用两次拷贝构造函数，
  // 一次是返回a对象时，另外一次是创建a对象时。

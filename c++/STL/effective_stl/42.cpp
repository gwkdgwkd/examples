...
// 创建一个按照最大速度进行排序的multiset<Widget>容器，
// 一种显而易见的方式是：
...
// 如果使用了less，无论是显式地或是隐式地，
// 都需要确保它与operator<具有相同的意义。
// 如果需要以一种特殊的方式来排序对象，那么最好创建一个特殊的函数子类，
// 它的名字不能是less，这样做其实很简单。

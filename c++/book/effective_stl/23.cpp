...
// 许多STL程序员，当需要提供快速查找功能的数据结构时，立刻会想到关联容器，
// 没有问题，只要合适就行，但它们并不总是合适的，如果查找速度真的很重要，
// 那么考虑哈希容器几乎总是值得的，通过哈希函数几乎能提供常数时间的查找能力。
...
// 标准关联容器通常为平衡的二叉查找树，对输入、删除和查找的混合操作做了优化，
// 总的来说，没办法预测出下一个操作是啥。
// 很多程序使用数据时并不这样混乱，一般为：
// 1.设置阶段：创建数据结构，并插入大量数据，几乎都是插入和删除，很少有查询；
// 2.查找阶段：这个阶段几乎所有的操作都是查找操作，很少或几乎没有插入和删除操作；
// 3.重组阶段：改变数据的内容，或许是删除所有数据，再插入新的数据，类似1，然后到2。
// 对于这样使用数据来说，vector可能比关联容器提供了更好的性能，
// 无论是时间还是空间，但必须是排序的vector才可以。
// 因为只有对排序的容器才能使用查找算法binary_search、lower_bound和equal_range等。

namespace n1 {
// 为什么排序的vector的二分搜索，比通过二叉查找树的二分搜索具有更好的性能呢？
...
//   在排序vector中可能比在标准关联容器中存储同样的数据，要耗费更少的内存，
//   而考虑到页面错误的因素，通过二分搜索法来查找排序vector比查找关联容器要快些。
// 对于排序vector，最不利的地方在于必须保持有序。
// 插入时，新元素之后的所有元素都必须向后移动一个位置，删除也是需要移动。
// 当数据结构的使用方式是，查找操作几乎从不跟插入和删除操作混在一起时，
...
namespace n2 {
// 如果决定用vector来替换map或multimap，那么存储的数据必须是pair<K,V>，
...
// 需要另一个比较函数来执行查找过程。
// 用来做排序的比较函数需要两个pair对象作为参数，但是查找的时候只需要一个键值。

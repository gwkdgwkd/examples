...
namespace n3 {
// copy_if()可以从源序列复制使谓词为true的元素，所以可以看作是一个过滤器。
// 前两个参数定义源序列的输入迭代器，
...
void func2() {
  // 输入流迭代器可以作为copy_if()算法的源，也可以用在其他需要输入迭代器的算法上：
...
// 源序列会被复制到目的序列中。
// copy_backward()的3个参数都必须是可以自增或自减的双向迭代器，
// 这意味着这个算法只能应用到序列容器的序列上。
// 从源序列的反向，将每一个元素依次复制到目的序列的前一个元素之前的位置。
// 在进行这个操作之前，目的序列中的元素必须存在，
// 因此目的序列至少要有和源序列一样多的元素，但也可以有更多。
// copy_backward()算法会返回一个指向最后一个被复制元素的迭代器，
// 在目的序列的新位置，它是一个开始迭代器。

// 相对于普通的从第一个开始复制的copy()算法，copy_backward()提供了哪些优势?
// 在序列重叠时，可以用copy()将元素复制到重叠的目的序列剩下的位置，
// 也就是目的序列第一个元素之前的位置。
// 如果想尝试用copy()算法将元素复制到同一个序列的右边，
// 这个操作不会成功，因为被复制的元素在复制之前会被重写。
// 如果想将它们复制到右边，可以使用copy_backward()，
// 只要目的序列的结束迭代器在源序列的结束迭代器的右边。
// 在想将元素复制到右边时，copy()算法显然不能如愿，因为一些元素在复制之前会被重写。
...
void func1() {  // copy
  ...
  std::copy(std::begin(q), std::end(q),
            std::ostream_iterator<std::string>{std::cout, " "});
  std::cout << std::endl;  // 1 2 3 4 5 6

  std::copy(std::begin(q), std::begin(q) + 6, std::begin(q) + 2);
  std::copy(std::begin(q), std::end(q),
            std::ostream_iterator<std::string>{std::cout, " "});
  std::cout << std::endl;  // 1 2 1 2 1 2 1 2
}

void func2() {  // copy_backward
...
void func1() {
  std::reverse_copy(v.begin(), v.end(),
                    std::ostream_iterator<int>(std::cout, " "));
  std::cout << std::endl;  // 7 6 5 4 3 2 1
}

...
namespace n1 {
// 模板并不是真正的函数或类，它仅仅是编译器用来生成函数或类的一张图纸。
// 模板不会占用内存，最终生成的函数或者类才会占用内存。
// 由模板生成函数或类的过程叫做模板的实例化，
// 相应地，针对某个类型生成的特定版本的函数或类叫做模板的一个实例。
// 实例化是按需进行的，用到哪个类型就生成对应的函数或类，不会提前生成很多代码。
// 也就是说编译器会根据传递给类型参数的实参，也可以是编译器自己推演出来的实参，
// 来生成一个特定版本的函数或类，并且相同类型只生成一次，
// 实例化的过程很简单，就是将所有的类型参数用实参代替。
template <typename T>
void Swap(T &a, T &b) {
  T temp = a;
  a = b;
...
namespace n2 {
// 另外需要注意的是类模板的实例化，通过类模板创建对象时并不会实例化所有的成员函数，
// 只有等到真正调用它们时才会被实例化，如果成员函数永远不会被调用，那就不会被实例化。
// 这说明类的实例化是延迟的、局部的，编译器并不着急生成所有的代码。
// 通过类模板创建对象时，一般只需要实例化成员变量和构造函数。
// 成员变量被实例化后就知道对象的大小了，构造函数被实例化后就能够知道如何初始化了。

...
// 当编译器处理字符串时，可以通过前缀来判断字符串的编码类型，
// 如果目标编码与原编码不同，则编译器会进行转换，
// 比如C++11中的前缀u8表示目标编码为UTF-8的字符，
// 如果代码文件采用的是GBK，编译器按照UTF-8去解析字符串常量，则可能会出现错误。
// 一个字符串从定义到处理再到输出，涉及到编辑器、编译器和输出环境三个因素，
// 正确的处理和显示需要三个因素的共同保障，每一个环节都不能出错。

namespace n1 {
// 在C++98中，为了支持Unicode字符，使用wchar_t类型来表示宽字符，
...
// 多字节字符转换为UTF-16编码
// size_t mbrtoc16(char16_t* pc16, const char* pmb,
//                 size_t max, mbstate_t* ps);
// UTF-16字符转换为多字节字符
// size_t c16rtomb(char* pmb, char16_t c16, mbstate_t* ps);
// 多字节字符转换为UTF-32编码
// size_t mbrtoc32(char32_t* pc32, const char* pmb,
//                 size_t max, mbstate_t* ps);
// UTF-32字符转换为多字节字符
// size_t c32rtomb(char* pmb, char32_t c32, mbstate_t* ps);
// 函数名称中mb表示multi-byte（多字节），
// rto表示convert to（转换为），c16表示char16_t。

// 实际上C++提供了一个类模板codecvt，
// 用于完成Unicode字符串与多字节字符串之间的转换，主要分为4种：
...
void func2() {
  ...
  // 输出结果是GBK的码值，因为你的GBK码值是0xC4E3，好的GBK码值是0xBAC3。
  // 可见，编译器未成功地将GBK编码的你好，
  // 转换为UTF-8的码值你（E4 BD A0）好（E5 A5 BD），
...
void func4() {
  // 使用c16rtomb()完成了将你好啊从UTF-16编码到多字节编码（GBK）的转换：
  ...
  // locale串使用zh_CN.gbk表示目的多字节字符串使用GBK编码。

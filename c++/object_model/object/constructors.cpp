...
// 一个内联函数有静态链接，不会被模块以外看到。如果函数太复杂，不适合做成内联，就会合成出一个explicit non-inline static实体。
class A {
 public:
...
class C {
 public:
  C() {  // 定义了默认构造函数
    // 编译器会初始化对象成员a，由于默认构造函数已经被明确定义出来了，编译没办法合成第二个，
    // 所以编译器会扩展已存在的构造函数，在其中安插一些码，使得用户代码在被执行之前，先调用默认构造函数。
    // a.A::A();  // 安插的代码
    std::cout << "C" << std::endl;
...
void flip(const A &a) {
  ...
  //  &a代表要交给“被调用的某个flip()函数实体”的this指针。
}
void func() {
  B b;      // A
  C c;      // A
...
// 总结：
// 有四种情况，会导致“编译器必须为未声明构造函数的类合成一个默认构造函数”。
...
// 如果程序需要一个“把某指针设为0”的默认构造函数，那么提供它的人应该是程序员。
...
// 而是“默认构造函数和拷贝构造函数在必要的时候才由编译器产生出来”。
...
//  一个对象如果以另一个object作为初值，而后者有一个Virtual Base Class Subobject，那么“bitwise copy semantics”失效。
//  每一个编译器对于虚拟继承的支持承诺，都表示必须让派生对象中的Virtual Base Class Subobject位置在执行期就准备妥当。
//  维护“位置的完整性”是编译器的责任。
//  “bitwise copy semantics”可能会破坏这个位置，所以编译器必须在它自己合成出来的拷贝构造中做出仲裁。
...
class Point3d2 {
 public:
  ...
  Point3d2(const Point3d2 &rhs) {
    ...
    // 那么使用上述函数将会导致那些“被编译器产生的内部成员”的初值被改写。
  }
...
class Z {
 ...
 public:
  ...
  // 那么对于“到底是哪一个member在xfoo()执行时被设立初值”这件事，就可以确保不会发生模棱两可的情况。
};

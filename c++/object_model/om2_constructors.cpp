...
// 如果没有任何用户定义的构造函数，
// 那么会有一个隐式默认构造函数暗中被声明出来，但这个默认构造函数是无用的。
...
void func() {
  ...
  // 如果有多个对象成员都要求初始化操作，
  // 无论是合成的隐式默认构造函数，还是在已经定义了默认构造函数中安插代码，
...
class A {
  // 在这种情况下，
  // 编译器必须合成出一个拷贝构造函数以便调用B成员对象的拷贝构造函数：
...
class D2 : public B {
 public:
  D2(int i) : B(i), b_(4), cb_(3), rb_(rb) {}
...
void func() {
  ...
  // 初始化列表中次序：先所以的基类，然后是所以的成员对象。
  // 无论是基类还是成员对象，当多于一个时，是由成员声明次序决定，
  // 不是由初始化列表中的排列次序决定：
  D1 d1(1);
  // 1
  // 3
...
class B1 {
 public:
  // 构造函数中初始化，先生临时对象，再调用赋值运算符，然后再摧毁临时对象：
  B1(A &a) { a_ = a; }
  A a_;
};
void func1() {

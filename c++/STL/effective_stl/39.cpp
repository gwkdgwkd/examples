...
// 一个判别式（predicate）是一个返回值为bool类型（或者可以隐式地转换为bool类型）的函数。在STL中，判别式有着广泛的用途。
// 标准关联容器的比较函数就是判别式；对于像find_if以及各种与排序有关的算法，判别式往往也被作为参数来传递。

// 一个纯函数是指返回值仅仅依赖于其参数的函数。
// 在C++中，纯函数所能访问的数据应该局限于参数以及常量（在函数生命期内不会被改变，应该被声明为const）。如果一个纯函数需要访问那些
// 可能会在两次调用之间发生变化的数据，那么相同的参数在不同的时刻调用该函数就有可能得到不同的结果，与纯函数的定义相矛盾。

// 判别式类是一个函数子类，它的operator()函数是一个判别式，也就是说，operator()返回true或者flase。
// STL中凡是能接受判别式的地方，就既可以接受一个真正的判别式，也可以接受一个判别式类的对象。

// 接受函数子的STL算法可能会先创建函数子的拷贝，然后存放起来待以后再使用这些拷贝，而且，有些STL算法实现也确实利用了这一特性。
...
// STL中的判别式函数和判别式类的对象可以互相代替，所以判别式函数也应该和判别式类中的operator()函数一样，应该是纯函数：
...
int main() {
  ...
  cout << endl;  // 1 2 4 5
  // 不仅删除了第三个，而且还删除了第六个
  // remove_if中调用了find_if和remove_copy_if，在find_if中加到了3次，然后拷贝给remove_copy_if，调用三次后还返回true。

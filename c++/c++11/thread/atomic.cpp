...
// 在C++11之前，使用第三方API可以实现并行编程，
// 比如pthread多线程库，但是在使用时需要创建互斥锁，以及进行加锁、
// 解锁等操作来保证多线程对临界资源的原子操作，这无疑增加了开发的工作量。
...
void testOperation() {
  // 原子类型可以直接对普通类型进行赋值，
  // 这是因为atomic类模板定义了从atomic到T的类型转换函数，可以隐式转换。
...
// 保证了如果需要更新的地址没有被他人改动过，那么它可以安全的写入。
// 而这也是对于某个数据或者数据结构加锁要保护的内容，
// 保证读写的一致性，不出现dirty data。
...
// 有一个比较特殊的原子类型是atomic_flag，
// 因为atomic_flag与其他原子类型不同，它是无锁（lock_free）的。
// 线程对其访问不需要加锁，而其他的原子类型不一定是无锁的。
// 因为atomic<T>并不能保证类型T是无锁的，
// 另外不同平台的处理器处理方式不同，也不能保证必定无锁。

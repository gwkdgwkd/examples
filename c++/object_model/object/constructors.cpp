...
//   解决方法是把合成的默认构造函数、拷贝构造函数、
//   析构函数、赋值操作符都以inline方式完成。
//   一个内联函数有静态链接，不会被模块以外看到。
//   如果函数太复杂，不适合做成内联，
//   就会合成出一个explicit non-inline static实体。
class A {
 public:
...
class B {
 public:
  ...
  // 被合成的默认构造函数内会调用a的的默认构造函数，这是编译器的责任：
  A a;
...
class D {
 public:
  ...
  // 这一点由编译器完成，它为每一个构造函数安插代码，
  // 这些代码将被安插在explicit user code之前。
  A1 a1;
  A3 a3;
  A2 a2;
};
...
//   类似的道理，如果一个没有任何构造函数的类，
//   派生自一个带有默认构造函数的base class，
...
//   由于缺乏由user声明的构造函数，
//   编译器会详细记录合成一个默认构造的必要信息。
//   为了使多态生效，编译器必须为每一个基类(或其派生类)对象的vptr设定初值，
//   放置适当的virtual table地址。
//   对于类所定义每一个构造函数，编译器会安插一些代码来做这样的事情。
//   对于那些未声明任何构造函数的类，编译器会为它们合成一个默认构造函数，
//   以便正确地初始化每一个对象的vptr。
//   编译器发生：
//   1.一个vtbl被产生出来，内放class的虚函数地址。
//   2.每一个对象中，一个额外的指针成员vptr被编译器合成出来，
//     内含相关的class vtbl的地址。
class A {
 public:
  A() { cout << "A" << endl; };
...
void flip(const A &a) {
  // A.flip()的虚拟引发操作会被重新改写，
  // 以使用a的vptr和vtbl中的flip条目：
  a.flip();
  // 转变为：(*a.vptrp[1])(&a)
...
//   然而，每一种实现法的共通点在于，
//   必须使Virtual Base Class在其每一个派生类对象中的位置，
//   能够于执行期准备妥当。
class Base {
 public:
  Base() { cout << "Base" << endl; }
...
// 无法在编译期决定出pa->Base::i的位置：
void foo(A *pa) { pa->i = 1024; }
// 编译器无法固定住foo中经由pa而存取的Base::i的实际偏移位置，
// 因为pa的真正类型可以改变。
// 编译器必须改变执行存取操作的那些代码，
// 使Base::i可以延迟至执行期才决定下来。
// 对于类所定义的每一个构造函数，
...
class A {
 public:
  ...
  // 可以是多参数形式，从第二个参数起，提供默认值：
  // A(const X &x, int = 0);
  // 大部分情况下，当一个对象以另一个同类对象作为初值时，
  // 会导致拷贝构造函数被调用。
...
// 当对象以同类对象作为初值时，
// 其内部是以所谓的default memberwise initialization手法完成的。
...
// 一个对象可以从两种方式复制得到，一种是被初始化（拷贝构造函数），
// 另一种是被赋值（赋值操作符）。
// 与默认构造函数一样，如果没有声明一个拷贝构造函数，
// 就会有隐含的声明或隐含的定义出现。
...
// 1.当类内含一个对象成员而后者的类声明有一个拷贝构造函数时，
//   不论是被明确声明，还是被编译器合成；
// 2.当类继承自一个基类而后者存在有一个拷贝构造函数时，
//   不论是被明确声明或是被合成而得；
// 3.当类声明了一个或多个虚函数时；
// 4.当类派生自一个继承串链，其中有一个或多个虚基类时。
// 前2种情况中，编译器必须将成员或基类的拷贝构造函数调用操作，
// 安插到被合成的拷贝构造函数中。
// 对于3，需要重新设定virtual Table的指针，
// 如果编译器对于每一个新产生的对象的vptr不能成功而正确地设好其初值，
// 将导致可怕的后果。
// 因此，当编译器导入一个vptr到class中时，
// 该class就不再展现bitwise semantics了。
// 编译器需要合成出一个拷贝构造函数，以求将vptr适当地初始化。
// 对于4，需要处理Virtual Base Class Subobject，
// 一个对象如果以另一个object作为初值，
// 而后者有一个Virtual Base Class Subobject，
// 那么bitwise copy semantics失效。
// 每一个编译器对于虚拟继承的支持承诺，
// 都表示必须让派生对象中的Subobject位置在执行期就准备妥当。
// 维护位置的完整性是编译器的责任。
// bitwise copy semantics可能会破坏这个位置，
// 所以编译器必须在它自己合成出来的拷贝构造中做出仲裁。
...
class B {
 public:
  ...
  ~B() {}
  // 在这种情况下，
  // 编译器必须合成出一个拷贝构造函数以便调用A成员对象的拷贝构造函数：
  // inline B(cosnt B &b) {
  //   str.String::String(b.str);
  //   cnt = b.cnt;
  // }
  // 被合成出来的拷贝构造函数中，
  // 如整数、指针、数组等等的非类对象成员也都会被复制，正如期待的一样。
...
void func3(A a) {
  ...
  // 临时对象先以A的拷贝构造函数正确地设定了初值，
  // 然后再以bitwise方式拷贝到x0这个局部实体中。
  // 函数的声明因而也必须被转化，
  // 形式参数必须从原先的一个对象改变为一个引用：
  // void func3(A & a0);
}
...
A func6() {
  ...
  //   在cfront中优化实施的前提是，
  //   当所有的named return指令句发生于函数的top level。
  //   如果导入a nested local bolck with a return statement，
  //   cfront就会静静地将优化关闭。
  //   许多程序员主张对于这种情况应该以特殊化的构造函数策略取代之。
  // 3.某些程序员真的不喜欢应用程序被优化，
  //   以拷贝方式产生出一个对象时，对称地调用了析构函数。
...
// 默认的拷贝构造函数被视为无用的。
// 它既没有任何对象成员或基类对象带有拷贝构造函数，
// 也没有任何虚函数或虚基类，会导致bitwise copy。
...
// 构造函数中初始化，会产生临时对象，
// 再以一个赋值运算符，然后再摧毁那个临时对象。
// 编译器会一一操作初始化列表，以适当次序在构造函数内安插初始化操作，
// 并且在任何显式的用户代码之前。
// 初始化列表中次序是由类中的成员声明次序决定，
// 不是由初始化列表中的排列次序决定。
...
class Z {
 ...
 public:
  ...
  // 那么对于到底是哪一个member在xfoo()执行时被设立初值这件事，
  // 就可以确保不会发生模棱两可的情况。
};

// 如果一个派生类成员函数被调用，
// 其返回值被当做基类构造函数的一个参数，将会如何：

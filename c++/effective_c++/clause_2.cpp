...
// ASPECT_PATIO也许从未被编译器看见；也许在编译器开始处理源码之前它就被预处理器移走了。于是ASPECT_PATIO有可能没进入符号表内。
// 于是运用此常量但获得一个编译错误时，会带来困惑，提到的是1.653而不是ASPECT_PATIO。特别是在一个别人写的头文件内。
const double AspectRatio = 1.653;  // 解决办法是以一个常量替换上述的宏
// AspectRatio肯定会被编译器看到，当然就会进入记号表。
// 此外对于浮点常量，使用常量可能比使用#define导致小量的码，因为ASPECT_PATIO盲目替换，可能导致目标码出现多份，常量不会这样。
// 以常量替换#define，有两种特殊情况：
// 1 由于常量指针通常被放在头文件内（以便被不同的源码包含），因此有必要将指针（而不只是指针所指之物）声明为const。
// 2 class专属常量。为了将常量的作用域限制于class内，必须让它成为class的一个成员；为了确保只有一份实体，必须让它成为static成员。
class GamePlayer {
 private:
  static const int NumTurns = 5;  // 常量声明式
  int scores[NumTurns];
};
// 通常C++要求对所使用的任何东西提供一个定义式，但如果它是个class专属常量又是static且为整数类型（int、char、bool）,则需特殊处理。
...
// #define无法创建一个class的专属常量，因为#define并不重视作用域。一旦被定义，在其后的编译过程中都有效（除非#undef)。
// 这意味着#define不仅不能够定义class专属常量，也不能提供任何封装型。

// 旧的编译器也许不支持上面的语法，不允许static成员在其声明时获得初值。此外所谓的“in-class初值设定”也只允许对整数常量进行。
...
// 唯一例外的是在class编译期间需要一个class常量值（比如GamePlayer::NumTurns，编译器坚持必须在编译期间知道数组大小），
...
// 使用the enum hack的第二个理由纯粹是为了实用主义，许多代码用了它，看到时必须认识。事实上，它是模板元编程的基础技术。

// 另一个常见的#define误用情况是以它实现宏。宏看起来想函数，但不会招致函数调用带来的额外开销。
#define CALL_WITH_MAX(a, b) f((a) > (b) ? (a) : (b))
// 写这种宏，必须记住宏中的所有实参都要加上小括号，否则产生歧义。就算宏都加上了小括号，还会发送不可思议的事。
...
// #incldue仍然是必须品，而#ifdef/#ifndef也继续扮演控制编译的重要角色。还不到预处理器全面隐退的时候。

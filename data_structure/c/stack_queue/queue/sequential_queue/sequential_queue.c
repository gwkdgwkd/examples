...
// 队列，和栈一样，也是一种对数据的存和取有严格要求的线性存储结构。
// 与栈结构不同的是，队列的两端都开口，要求数据只能从一端进，从另一端出。
// 通常，称进数据的一端为队尾，出数据的一端为队头，
// 数据元素进队列的过程称为入队，出队列的过程称为出队。
// 队列中数据的进出要遵循先进先出的原则，即最先进队列的数据元素，同样要最先出队列。
// 栈和队列不要混淆，栈结构是一端封口，特点是先进后出；
// 而队列的两端全是开口，特点是先进先出。
// 数据从表的一端进，从另一端出，且遵循先进先出原则的线性存储结构就是队列。

// 队列存储结构的实现有以下两种方式：
// 1.顺序队列：在顺序表的基础上实现的队列结构；
// 2.链队列：在链表的基础上实现的队列结构。
// 两者的区别仅是顺序表和链表的区别，即在实际的物理空间中，
// 数据集中存储的队列是顺序队列，分散存储的队列是链队列。
...
// 我们还需要定义两个指针（top和rear）分别用于指向顺序队列中的队头元素和队尾元素。
// 由于顺序队列初始状态没有存储任何元素，因此top指针和rear指针重合，
// 且由于顺序队列底层实现靠的是数组，因此top和rear实际上是两个变量，
// 它的值分别是队头元素和队尾元素所在数组位置的下标。
...
#endif
// 此方法存在的问题：指针top和rear重合位置指向了a[4]而不再是a[0]。
// 也就是说，整个顺序队列在数据不断地进队出队过程中，在顺序表中的位置不断后移。
// 顺序队列整体后移造成的影响是：
// 1.顺序队列之前的数组存储空间将无法再被使用，造成了空间浪费；
// 2.如果顺序表申请的空间不足够大，则直接造成程序中数组a溢出，产生溢出错误。

...
// 数据结构中的查找表，专用于实现各种查找算法，查找表又可分为静态查找表和动态查找表。
...
// 关键字又细分为主关键字和次关键字，若某个关键字可以唯一地识别一个数据元素时，
// 称这个关键字为主关键字，例如学生的学号就具有唯一性，反之，不具有唯一性，称为次关键字。
...
// 查找操作的性能分析主要考虑其时间复杂度，而整个查找过程大部分时间花费在数据的比较上。
// 所以查找算法衡量好坏的依据为：
// 查找成功时，查找的关键字和查找表中的数据元素中进行过比较的个数的平均值，
// 称为平均查找长度（Average Search Length，用ASL表示）。
// 对于具有n个数据元素的查找表，查找成功的平均查找长度的计算公式为：
// ASL=1/n*[1,n](n-i+1)=(n+1)/2
// 1/n是概率，每个元素被查找的概率相同，总和为1。
...
// 对于含有n个数据的表来说，每次查找失败，比较的次数都是n+1。
// 所以查找算法的平均查找长度的计算公式为：3(n+1)/4

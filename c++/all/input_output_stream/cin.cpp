...
// get()是istream类的成员函数，它有多种重载形式，最简单常用的：
//  int get();
// 从输入流中读入一个字符，返回值是该字符的ASCII码，如果到输入末尾，则返回EOF。
// istream类中从输入流（包括文件）中读取数据的成员函数，
// 在把输入数据都读取完后再进行读取，就会返回EOF。
// EOF是在iostream类中定义的一个整型常量，值为-2。
// get()函数不会跳过空格、制表符、回车等特殊字符，所有的字符都能被读入。

// EOF并不是一个字符类型，EOF是一个整型常量，它的值是一个负数，通常是-1。
// 因此，在使用EOF时，需要将它与整型类型进行比较，而不是与字符类型进行比较。
void func1() {
  int c;
  std::cout << EOF << std::endl;  // -1

  // windows下，在输入的空行位置，按ctrl+z可产生EOF
  // linux下，在输入时，按ctrl+d可产生EOF
  while ((c = std::cin.get()) != EOF) {  // 可利用EOF来中止循环
    std::cout.put(c);
  }
}
...
// 哪个条件先满足就按哪个执行，函数会自动在buf中读入数据的结尾添加\0。
// 第二个和第一个的区别在于，第一个是读到\n为止，第二个是读到delim字符为止。
// \n或delim都不会被读入buf，但会被从输入流中取走。
// 这两个函数的返回值就是函数所作用的对象的引用。
// 如果输入流中\n或delim之前的字符个数达到或超过bufSize，
// 就会导致读入出错，虽然本次读入已经完成，但是之后的读入都会失败。
// 从输入流中读入一行，可以用第一个版本，用cin>>str这种写法是不行的，
// 因为此种读法在碰到行中的空格或制表符时就会停止，因此就不能保证str读的是整行。
// 可以用getline()函数的返回值（为false则输入结束）来判断输入是否结束。

void func1() {
  char szBuf[20];
  int n = 120;
  if (!std::cin.getline(szBuf, 6))  // 如果一行字符超过5个，就会出错
...
// 此函数的作用是跳过输入流中的n个字符，或跳过delim及其之后的所有字符，
// 哪个条件先满足就按哪个执行，两个参数都有默认值，
// 因此cin.ignore()就等效于cin.ignore(1, EOF)，即跳过一个字符。
...
// 相当于只是看了一眼下一个字符，因此叫peek。
// cin.peek()不会跳过输入流中的空格、回车符。
// 在输入流已经结束的情况下，cin.peek()返回EOF。
// 在输入数据的格式不同，需要预先判断格式再决定如何输入时，peek()就能起到作用。

// istream还有一个成员函数istsdream &putback(char c)，
// 可以将一个字符插入输入流的最前面。
// 对于下题，也可以在用get()函数读取一个字符并判断是中式格式还是西式格式时，
// 将刚刚读取的字符再用putback()成员函数放回流中，
// 然后再根据判断结果进行不同方式的读入。
std::string Months[12] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
                          "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
int get_time() {
  int c;
  while ((c = std::cin.peek()) != EOF) {  // 取输入流的第一个字符查看
    int year, month, day;
    if (c >= 'A' && c <= 'Z') {  // 美国日期格式
      std::string sMonth;
      ...
      std::cin >> year;
      std::cin.ignore() >> month;  // 用ignore跳过2011.12.3中的.
      std::cin.ignore() >> day;
    }
    std::cin.ignore();  // 跳过行末\n
...
namespace n5 {
// cin是如何判断输入结束（读取结束）的？
// cin可以用来从键盘输入数据，重定向为文件后，cin也可以用来从文件中读入数据。
...
// 1.在Windows中，通过键盘输入时，按Ctrl+Z组合键后再按回车键，就代表输入结束。
// 2.在UNIX/Linux/Mac OS系统中，Ctrl+D代表输入结束。
// 不管是文件末尾，还是Ctrl+Z或者Ctrl+D，它们都是结束标志；
// cin在正常读取时返回true，遇到结束标志时返回false，
// 可以根据cin的返回值来判断是否读取结束。
// 在这种情况下，文件中并不需要包含Ctrl+Z或者Ctrl+D，
// 只要有用空格或回车隔开的若干个正整数即可。
// cin读到文件末尾时，cin>>n就会返回false，从而导致程序结束。

void testN5() {
  int n;
  int maxN = 0;
  // cin>>n的返回值是istream &类型的，而while中的条件表达式应该是bool类型、
  // 整数类型或其他和整数类型兼容的类型，istream &显然和整数类型不兼容，
  // 为什么while(cin>>n)还能成立呢？
  // 这是因为，istream类对强制类型转换运算符bool进行了重载，
  // 这使得cin对象可以被自动转换成bool类型。
  // 所谓自动转换的过程，就是调用cin的operator bool()这个成员函数，
  // 而该成员函数可以返回某个标志值，

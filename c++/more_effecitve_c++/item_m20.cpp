...
// 它返回的是一个指向局部对象result的引用，
// 当operator*退出时result被自动释放。
...
inline const Rational3 operator*(const Rational3& lhs, const Rational3& rhs) {
  ...
  // 你通过这个表达式建立一个临时的Rational对象，
  // 并且这是一个临时对象，函数把它拷贝给函数的返回值。
  return Rational3(lhs.numerator() * rhs.numerator(),
                   lhs.denominator() * rhs.denominator());
}
// 返回constructor argument而不出现局部对象，
// 这种方法还会给你带来很多开销，
// 因为你仍旧必须为在函数内临时对象的构造和释放而付出代价，
// 你仍旧必须为函数返回对象的构造和释放而付出代价。
// 但是你已经获得了好处，C++规则允许编译器优化不出现的临时对象。
// 编译器就会被允许：
// 消除在operator*内的临时变量和operator*返回的临时变量。
// 它们能在为目标c分配的内存里构造return表达式定义的对象。
// 如果你的编译器这样去做，调用operator*的临时对象的开销就是零：
// 没有建立临时对象。
...
// 来消除局部临时对象，是众所周知的和被普遍实现的。
// 它甚至还有一个名字：返回值优化（return value optimization）。
// 但注意，这种优化对普通的赋值运算无效，
// 编译器不能够用拷贝构造函数取代赋值运算动作，

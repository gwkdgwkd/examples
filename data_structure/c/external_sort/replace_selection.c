...
// 2.从内存工作区中选出关键字最小的记录，将其记为MINIMAX记录；
// 3.然后将MINIMAX记录输出到归并段文件中；
// 4.此时内存工作区中还剩余5个记录，若初始文件不为空，则从初始文件中输入下一个记录到内存工作区中；
// 5.从内存工作区中的所有比MINIMAX值大的记录中选出值最小的关键字的记录，作为新的MINIMAX记录；
// 6.重复过程3—5，直至在内存工作区中选不出新的MINIMAX记录为止，由此就得到了一个初始归并段；
// 7.重复2—6，直至内存工作为空，由此就可以得到全部的初始归并段。
...
int main() {
  ...
  printf("初始归并段各为:\n");
  do {
    // 读fi指针指向的文件，并将读的记录赋值给b，整个操作成功与否的结果赋值给k

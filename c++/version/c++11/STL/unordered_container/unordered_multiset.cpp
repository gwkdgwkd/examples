...
// 和unordered_set不同的是，unordered_multiset可以存储多个值相同的元素，
// 且这些元素会存储到哈希表中同一个桶（本质就是链表）上。
// unordered_multiset除了能存储相同值的元素外，它和unordered_set完全相同。

// 实现unordered_multiset容器的模板类并没有定义在以该容器名命名的文件中，
// 而是和unordered_set共用同一个<unordered_set>，并且也位于std命名空间。
...
// Key 确定容器存储元素的类型，则此参数则用于确定各个键值对的键和值的类型，
//     因为它们是完全相同的，因此一定是同一数据类型的数据。
// Hash = hash<Key>
// 指定unordered_multiset容器底层存储各个元素时所使用的哈希函数。
...
// unordered_multiset提供成员方法的方法，无论是种类还是数量，
// 都和unordered_set类模板一样：
// begin()            返回指向容器中第一个元素的正向迭代器。
// end()              返回指向容器中最后一个元素之后位置的正向迭代器。
// cbegin()	          和begin()功能相同，只不过返回的是const的正向迭代器。
// cend()             和end()功能相同，只不过返回的是const的正向迭代器。
...
// swap()	            交换2个unordered_multimap元素，类型必须完全相等。
...
// load_factor()	    返回容器当前的负载因子，所谓负载因子指的是：
...
// 和unordered_set一样，unordered_multiset也没有重载[]，也没有提供at()。
...
void func1() {
  ...
  // 1.调用unordered_multiset模板类的默认构造函数，可以创建空的容器
  ...
  // 即以右值引用的方式，利用临时unordered_multiset中的所有元素给器初始化：

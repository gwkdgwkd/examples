...
// 1.让更多的内存可被使用。
//   让下一次内存分配动作可能成功。
//   一般的策略是，程序一开始执行就分配一大块内存，当new-handler第一次被调用，释放内存给程序用。
// 2.安装另一个new-handler。
//   如果目前这个new-handler无法取得更多可用内存，或许知道另外哪个new-handler有此能力，
...
class NewHandlerSupport {
  ...
  // 拥有实体互异的NewHandlerSupport复件（更明确地说是其static成员变量currentHandler）。
...
// C++标准委员会不想抛弃那些“侦测null”的族群，于是提供另一形式的operator new，分配失败返回null。
// 被称为nothrow形式。
class Widget1 {};
// nothrow new对异常的强制保证性并不高。表达式new (nothrow)发生两件事：
// 第一，nothrow版的operator new被调用，分配内存，如果失败返回null。
// 如果成功，构造函数会被调用。
// 构造函数可以做它想做的任何事，可能又new一些内存，而没人可以强迫它再次使用nothrow new。

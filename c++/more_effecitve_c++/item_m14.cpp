...
// 而且如果一个函数抛出一个不在异常规格范围里的异常，系统在运行时能够检测出这个错误，
// 然后一个特殊函数unexpected将被自动地调用。
// 异常规格既可以做为一个指导性文档同时也是异常使用的强制约束机制，它好像有着很诱人的外表。
// 函数unexpected缺省的行为是调用函数terminate，而terminate缺省的行为是调用函数abort，
// 所以一个违反异常规格的程序其缺省的行为就是halt（停止运行）。
// 在激活的栈中的局部变量没有被释放，因为abort在关闭程序时不进行这样的清除操作。
...
// 几乎不可能为一个模板提供一个有意义的异常规格。因为模板总是采用不同的方法使用类型参数。
...
// 这些异常中最常见的是bad_alloc，当内存分配失败时它被operator new和operator new[]抛出。
// 如果你在函数里使用new操作符，你必须为函数可能遇到bad_alloc异常作好准备。

// 虽然防止抛出unexpected异常是不现实的，
// 但是C++允许你用其它不同的异常类型替换unexpected异常，你能够利用这个特性。
...
// 如果被违反的异常规格包含UnexpectedException异常，那么异常传递将继续下去，好像异常规格总是得到满足。
...
// 而且他们会阻止high-level异常处理器来处理unexpected异常，即使这些异常处理器知道如何去做。

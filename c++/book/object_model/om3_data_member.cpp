...
class B {
  // 事实上并不是空的，它有一个隐晦的1字节，那是被编译器安插进去的一个char，
  // 这使得这个类的两个对象得以在内存中配置独一无二的地址。
};
class D1 : public virtual B {
  // 大小和机器有关，也和编译器有关，大小受到三个因素的影响：
  // 1.语言本身所造成的额外负担
  //   当语言支持虚基类时，就会导致一些额外负担，在派生类中额外负担反映在某种指针上，
  ...
  // 3.Alignment的限制
  //   D1和D2的大小截止目前为9字节，大部分机器上，
  //   结构体的大小会受到alignment的限制，是它们能够更有效率地在内存中存取。
};
...
class DD : public D1, public D2 {
  ...
  // 不管它在类继承体系中出现了多少次，DD的大小由以下几点决定：
  // 1.被大家共享的唯一一个B实体，大小为1字节；
  // 2.基类D1的大小，减去B的大小，结果是8字节，D2的算法相同，加起来是16字节；
  // 3.DD自己的大小，0字节；
  // 4.DD的alignment数量，前三项的和为17，按4字节对齐，补足3字节，结果是20字节。
  // 对empty virtual base class特殊处理的编译器，拿掉了1和额外的3，A的大小是16。
};
...
// 不管是virtual或non virtual base class，不过并没有强调定义其间的排列顺序。
...
// 每一个对象必须有足够的大小以容纳它所有的非静态数据成员，有时候其值比想象的大，因为：
...
// n1中类A的getX和setX函数，操作的都是类A的成员变量x，在C++最早的编译器上，
// 两个函数操作的都是向全局的x，这样的绑定结果几乎遍地都是，但不符合大家的预期，
...
namespace test4 {
// n2，n3这些程序设计风格到今天还存在，从C++2.0（C++11）开始，就已经没有必要了。
// 这个古老的语言规则被称为member rewriting rule，大意是一个内联函数实体，
// 在整个类声明未被完全看见之前，是不会被评估求值的。
// C++标准以member scope resolution rules来精炼这个rewriting rule，
// 其效果是，如果一个内联函数在类声明之后立刻被定义的话，那么就还是对其评估求值。
// 然而成员函数的参数列表不同，参数列表中的名称还是会在第一次遭遇时被适当地决议完成。
...
class A {
 ...
 private:
  ...
  // 此时仍然需要某种防御性程序风格，请始终把nested type声明放在类的起始处。
  length x;
};
}  // namespace test4
}  // namespace n2
...
// 非静态数据成员在对象中的排列顺序和声明顺序一致，任何静态数据成员都不放到对象内存中。
// C++标准要求，在同一个访问段（private、public、protected）中，
// 成员的排序只需符合较晚出现的成员在对象中有较高的地址这一条即可。
// 即各个成员并不一定得连续排序，成员的边界调整可能需要填补一些字节，这些位于成员之间。
// 编译器还可能合成些内部使用的数据成员以支持整个对象模型，如vptr，它会放在什么位置呢？
// 传统上被放在所有明确声明的成员的最后，也有一些会放在对象的最前端。
// C++标准秉承先前所说的对于布局持的放任态度，允许编译器放在任何位置上，
// 甚至那些被程序员声明出来的成员之间。
// C++标准也允许编译器将多个访问段之中的数据成员自由排列，不必在乎他它们的声明中的顺序。
...
// 如果两个类声明了同名的静态成员，那么当它们放在data
// segment时，就会导致名称冲突。
// 编译器的解决方法是暗中对每一个静态数据成员编码（name-mangling)，
// 以获得一个独一无二的程序识别代码，不同的编译器方法不同，不外乎是表格、语法措辞等等。
// 任何name-mangling做法都要两个要点：
// 1.一种算法，推导出独一无二的名称;
// 2.如果编译系统必须和程序员交互，那些独一无二的名称可以轻易被推倒回到原来的名称。
}  // namespace test1
...
class A {
 public:
  int get() {
    ...
    return i;        // 看上去是对i的直接读取
    return this->i;  // 事实上经由this指针完成
  }
  int i;
};

// 欲对非静态数据成员存取，编译器需要把对象的起始地址加上数据成员的偏移量：
// a.i = 0;  // &a + (&A::i - 1);
// 指向数据成员的指针，其offset总是被加上1，这样可以使编译系统区分出以下两种情况：
// 1.一个指向data member的指针，用以指出class的第一个member；
// 2.一个指向data member的指针，没有指出任何member。
// 每一个非静态数据成员的偏移量在编译时期即可获知，因此，存取一个非静态数据成员，
...
// 如果使用对象，就不会有这些问题，其类型是确定的，所以offset也在编译时期就固定了。
...
// 这样设计的好处就是可以把管理x和y的程序代码局部化，还可以表现出类间的紧密关系。
...
void test(B& b1, B& b2) {
  ...
  // b1和b2可能是B也可能是D，之前的设计不支持，这样的弹性，正是面向对象设计的核心。
  // 支持这样的弹性，势必给B类带来空间和存取时间的额外负担：
  // 1.导入一个和B有关的虚函数表，用来存放它所声明的每一个虚函数地址，
  //   这个表的元素数目一般而言是被声明的虚函数的数目，再加上一个或两个slots，
  //   用以支持runtime type identification；
  // 2.在每个对象中导入一个vptr，提供执行期的链接，使每个对象能够找到对应virtual
  // table； 3.加强构造函数，使它能够为vptr设定初值，让它指向类所对应的virtual
  // table，
  ...
  // 因而允许在C程序代码中也能使用，到了C++2.0，开始支持虚拟继承以及抽象基类，
  // 并且由于面向对象典范的兴起，某些编译器开始把vptr放到对象的开头。
  // 把vptr放在对象的前端，对于在多重继承之下，通过指向成员的指针调用虚函数，
  // 会带来一些帮助，否则，不仅从对象起点开始量起的offset必须在执行期备妥，
...
// 把派生类对象指给基类（不管继承有多深）的指针或引用，不需要编译器调整或修改地址。
// 很自然地发生，而且提供了最佳执行效率。
// 把vptr放到起始处，如果基类没有虚函数而派生类有，那么单一继承的自然多态就会被打破。
// 这种情况下，把一个派生类对象转换为其基类类型，就需要编译器的介入，用以调整地址。
...
void test(B2& b2) {
  ...
  // 情况和单继承时相同，因为二者都指向相同的起始地址，只付出地址的指定的成本。
...
// 如果类内有一或多个虚拟基类subobjects，那么将分为两部分：不变局部和共享局部。
...
// 通过这个指针间接存取虚基类的成员，这样的实现模型有两个主要的缺点：
...
//   指向virtual base class table，至于真正的virtual base class指针，
//   就被放在该表格中；
...
// 因为程序代码潜在性地受到专家所谓的一种奇行怪癖，也与特定编译器有关的魔咒影响。

// 单一继承应该不会影响测试的效率，因为成员被连续存储于派生类对象中，
// 并且其offset在编译期就已知，多重继承应该也是相同的，虚拟继承的效率令人失望。
}  // namespace n6
...
// 和一个与派生类对象绑定的成员结合起来，那么因为要加入offset而变得相当复杂。
struct B1 {
  int v1;
};
struct B2 {
...
void func2(D* pd) {
  ...
  // 当bmp被作为func1的第一个参数时，它的值就必须因介入的B1 class的大小而调整，

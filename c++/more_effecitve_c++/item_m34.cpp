...
// 在C中，这个过程是不需要的，因为没有函数重载，但几乎所有C++程序都有函数重名。
// 例如，流运行库就声明了几个版本的operator<<和 operator>>。
// 重载不兼容于绝大部分链接程序，因为链接程序通常无法分辨同名的函数。
// 名变换是对链接程序的妥协；链接程序通常坚持函数名必须独一无二。
// 如果只在C++范围内，名变换不会影响你。
// 如果你你有一个函数叫drawline而编译器将它变换为xyzzy，你总使用名字drawLine，
// 不会注意到背后的obj文件引用的是xyzzy的。
...
// 但如果drawLine是一个C函数，
// obj文件（或者是动态链接库之类的文件）中包含的编译后的drawLine函数仍然叫drawLine；没有名变换动作。
// 当你试图将obj文件链接为程序时，将得到一个错误，因为链接程序在寻找一个叫xyzzy的函数，而没有这样的函数存在。
// 要解决这个问题，你需要一种方法来告诉C++编译器不要在这个函数上进行名变换。
// 你不期望对用其它语言写的函数进行名变换，如C、汇编、Fortran、LISP、Forth或其它。
...
// 使用术语就是，extern 'C'意思是这个函数有C链接，但这个意思表达实在不怎么清晰。
...
// 通过禁止这些C++函数的名变换，你的客户可以使用你选择的自然而直观的名字，
// 而不用使用你的编译生成的变换后的名字。
// 经常，你有一堆函数不想进行名变换，为每一个函数添加extern 'C'是痛苦的。
// 幸好，这没必要，extern 'C'可以对一组函数生效，只要将它们放入一对大括号中：
extern "C" {
...
#endif
// 顺便提一下，没有标准的名变换规则。
// 不同的编译器可以随意使用不同的变换方式，而事实上不同的编译器也是这么做的。
...
// 尤其是，静态的类对象和定义在全局的、命名空间中的或文件体中的类对象的构造函数通常在main被执行前就被调用。
// 这个过程称为静态初始化。
...
// }
// 不要注重于这些名字。
// 函数performStaticInitialization()和performStaticDestruction()通常是更含糊的名字，
// 甚至是内联函数（这时在你的obj文件中将找不到这些函数）。
...
// 然而，这个C++库很可能含有静态对象（即使现在没有，以后可能会有），所以用C++写main()仍然是个好主意。
// 这并不意味着你需要重写你的C代码。
...
// 不是说没救了，只是处理起来比较麻烦一些。
// 编译器生产商们知道这个问题，几乎全都提供了一个额外的体系来启动静态初始化和静态析构的过程。
...
// 两种语言间的函数可以安全地交换指向对象的指针和指向非成员的函数或静态成员函数的指针。
// 自然地，结构和内建类型（如int、char等）的变量也可自由通过。
// 因为C++中的struct的规则兼容了C中的规则，假设“在两类编译器下定义的同一结构将按同样的方式进行处理”是安全的。
// 这样的结构可以在C++和C见安全地来回传递。
// 如果你在C++版本中增加了非虚函数，其内存结构没有改变，
// 所以，只有非虚函数的结构（或类）的对象兼容于它们在C中的孪生版本（其定义只是去掉了这些成员函数的申明）。

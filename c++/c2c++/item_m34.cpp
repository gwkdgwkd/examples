...
// 甚至可以在C++函数上声明extern "C"。
// 这在你用C++写一个库给使用其它语言的客户使用时有用。
// 通过禁止这些C++函数的名变换，
// 客户可以使用选择的自然而直观的名字，而不用使用编译生成的变换后的名字。
// 经常，有一堆函数不想进行名变换，为每一个函数添加extern "C"是痛苦的。
...
// 所以用C++写main()仍然是个好主意。
// 这并不意味着你需要重写C代码，只要将C写的main()改名为realMain()，
...
// 那么，唯一要记住的就是：
// 将new和delete与mallco和free进行严格的隔离。
...
// 所以有基类的结构也不能与C函数交互。
// 就数据结构而言，结论是：
// 1.在C++和C下提供同样的定义并进行编译，这样相互传递数据结构是安全的。
// 2.在C++版本中增加非虚成员函数或许不影响兼容性，但几乎其它的改变都将影响兼容。

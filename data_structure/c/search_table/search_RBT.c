...
// 红黑树（R-B TREE，全称：Red-Black Tree），本身是一棵二叉查找树，又附加了两个要求：
// 1.树中的每个结点增加了一个用于存储颜色的标志域；
// 2.树中没有一条路径比其他任何路径长出两倍，整棵树要接近于平衡的状态。
...
// 1.由于红黑树本身是一棵二叉查找树，所以在插入新的结点时，
//   完全按照二叉查找树插入结点的方法，找到新结点插入的位置；
// 2.将新插入的结点结点初始化，颜色设置为红色后插入到指定位置；
//   将新结点初始化为红色插入后，不会破坏红黑树第5条的性质
...
void RB_Insert_Fixup(RBT_Root* T, RB_TREE* x) {
  ...
  while (x->p->color == RED) {
    ...
    if (x->p == x->p->p->left) {
      ...
      // 如果父结点时祖父结点的右孩子，换汤不换药，
      // 只需将以上代码部分中的left改为right即可，道理是一样的。

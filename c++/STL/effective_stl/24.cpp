...
// map的operator[]函数与众不同，
// 它与vector、deque和string的operator[]函数无关，
// 与用与数组的内置operator[]也没有关系。
// 相反，map::operator[]的设计目的是为了提供添加和更新的功能。
// m[k]=v;
...
// 然后operator[]返回一个指向该新对象的引用，
// 最后这个引用成了被赋值的目标，被用值赋值。
...
// 对效率的考虑得出结论：
// 当向map中添加元素时，要优先选用insert，而不是operator[];
// 从效率和美学的观点考虑，结论是：
// 当更新已经在map中的元素的值时，有优先选择operator[]。
...
int main() {
  ...
  // operator[]的设计目的是为了提供添加和更新的功能，
  // 对于添加操作时，insert比operator[]效率更高。
  // 当操作更新时，形式恰好反过来了。
  ...
  // 所以当调用insert时，必须构造和析构一个该类型的对象，
  // 付出pair构造函数析构函数的代价。
  // 这有导致对Widget的构造和析构动作，因为pair包含了Wiget对象。
  // operator[]不使用pair对象，
  // 所以它不会构造和析构任何pair或Widget。

  // 用MapType::key_type和MapType::mapped_type，
  // 来代替KeyArgType和ValueArgType，

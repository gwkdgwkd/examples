...
// 其实C++代码中的大部分内容都是放在内存中的，
// 例如定义的变量、创建的对象、字符串常量、函数形参、函数体本身、new或malloc()分配的内存等，
// 这些内容都可以用&来获取地址，进而用指针指向它们。
// 除此之外，还有一些我们平时不太留意的临时数据，
// 例如表达式的结果、函数的返回值等，它们可能会放在内存中，也可能会放在寄存器中。
// 一旦它们被放到了寄存器中，就没法用&获取它们的地址了，也就没法用指针指向它们了。
...
void func4() {
  ...
  // 让指针指向相关的（相近的）但不是严格对应的类型的数据，表面上看起来是合理的，但是细思极恐，
  ...
  *p = 19.625;
  ...
  // 2.对于float，程序把最高1位作为符号位，把最低的23位作为尾数位，把中间的8位作为指数位。
}
...
// 总结起来说，给引用添加const限定后，不但可以将引用绑定到临时数据，

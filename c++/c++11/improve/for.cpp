...
namespace n1 {
// C++11标准之前（C++98/03标准），
// 如果要用for循环语句遍历一个数组或者容器，只能套用如下结构：
// for(表达式 1; 表达式 2; 表达式 3){
//   // 循环体
// }

void testN1() {
  for (int i = 0; i < strlen(arc); i++) {  // for循环遍历普通数组
    std::cout << arc[i];
  }
  std::cout << std::endl;

  std::vector<char>::iterator iter;
  for (iter = v.begin(); iter != v.end(); ++iter) {  // for循环遍历vector容器
    std::cout << *iter;
  }
  std::cout << std::endl;
...
namespace n2 {
// 而C++11标准中，除了可以沿用前面介绍的用法外，
// 还为for循环添加了一种全新的语法格式，如下所示：
...
// 需要注意的是，C++11标准中，declaration参数处定义的变量类型可以用auto关键字表示，
// 该关键字可以使编译器自行推导该变量的数据类型。
// expression：
// 表示要遍历的序列，可以为事先定义好的普通数组或者容器，还可以是用{}大括号初始化的序列。

// 同C++98/03中for循环的语法格式相比较，此格式并没有明确限定for循环的遍历范围，
...
void func1() {
  ...
  std::cout << std::endl;
  // 最后还有一个空，新格式的for循环在遍历字符串序列时，不只是遍历到最后一个字符，
  // 还会遍历位于该字符串末尾的'\0'（字符串的结束标志）：
...
void func4() {
  // 在使用新语法格式的for循环遍历某个序列时，如果需要遍历的同时修改序列中元素的值，
  ...
  // declaration参数既可以定义普通形式的变量，
  // 也可以定义引用形式的变量，应该如何选择呢？其实很简单:
  // 如果需要在遍历序列的过程中修改器内部元素的值，就必须定义引用形式的变量；
  // 反之，建议定义const&形式的变量，避免了底层复制变量的过程，
  // 效率更高，也可以定义普通变量。
}
...
// 2.总的来说，基于范围的for循环可以遍历普通数组、string字符串、容器以及初始化列表。
//   除此之外，for循环冒号后还可以放置返回string字符串以及容器对象的函数。
...
void func1() {
  ...
  std::cout << std::endl;  // http://c.biancheng.net/cplus/11/
...
void func2() {
  for (char ch : retStr2()) {  // 遍历函数返回的string字符串
    std::cout << ch;           // retStr2:http://c.biancheng.net/cplus/11/
  }
  std::cout << std::endl;
}
...
void func3() {
  // 不允许修改map、unordered_map、multimap以及
  // unordered_multimap容器存储的键的值；
  // 不允许修改set、unordered_set、multiset以及
  // unordered_multiset容器中存储的元素的值。
  ...
  // 执行结果并不是我们想要的，
  // 因为在for循环遍历arr容器的同时向该容器尾部添加了新的元素（容器扩容），

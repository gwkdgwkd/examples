...
// C++异常处理机制就可以捕获并处理这些错误，然后可以让程序沿着一条不会出错的路径继续执行，或者不得不结束程序，
// 但在结束前可以做一些必要的工作，例如将内存中的数据写入文件、关闭打开的文件、释放分配的内存等。
// C++异常处理机制会涉及try、catch、throw三个关键字。

// 开发程序是一项“烧脑”的工作，程序员不但要经过长期的知识学习和思维训练，还要做到一丝不苟，注意每一个细节和边界。
// 即使这样，也不能防止程序出错。程序的错误大致可以分为三种，分别是语法错误、逻辑错误和运行时错误：
// 1.语法错误在编译和链接阶段就能发现，只有100%符合语法规则的代码才能生成可执行程序。
//   语法错误是最容易发现、最容易定位、最容易排除的错误，程序员最不需要担心的就是这种错误。
// 2.逻辑错误是说我们编写的代码思路有问题，不能够达到最终的目标，这种错误可以通过调试来解决。
// 3.运行时错误是指程序在运行期间发生的错误，例如除数为0、内存分配失败、数组越界、文件不存在等。
//   C++异常（Exception）机制就是为解决运行时错误而引入的。
// 运行时错误如果放任不管，系统就会执行默认的操作，终止程序运行，也就是我们常说的程序崩溃（Crash）。
// C++提供了异常（Exception）机制，让我们能够捕获运行时错误，
...
// 从try的意思可以看出，它只是“检测”语句块有没有异常，如果没有发生异常，它就“检测”不到。
// catch是“抓住”的意思，用来捕获并处理try检测到的异常；
...
//               当函数内部抛出了异常列表中没有的异常时，如果调用的unexpected()函数中抛出了异常，

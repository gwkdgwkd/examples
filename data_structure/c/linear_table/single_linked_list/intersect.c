...
//   原因很简单，数据域中存储元素值相同，并不是2个单链表相交的充分条件，因为2个不相交的链表中很可能存有相同的元素。
...
// 相比第2种方案，此方法的实现逻辑虽然复杂，
// 但优点是，该方法可以找到2个单链表相交的交点，也就是返回True时的temp指针指向的那个节点。
// 此方案的时间复杂度也为O(n)。

// 从另一个角度比较这3种方案，第1种和第3种在判断“2个链表是否相交”的同时，

...
int main() {
  int a[] = {4, 2, 3, 5, 6};
  // 通过在cmp类（结构体）重载的()运算符中自定义排序规则，
  // 并将其实例化后作为priority_queue模板的第3个参数传入，
  // 即可实现为priority_queue容器适配器自定义比较函数。
  std::priority_queue<int, std::vector<int>, cmp<int>> pq(a, a + 5);
  while (!pq.empty()) {
    std::cout << pq.top() << " ";
    pq.pop();
  }
  std::cout << std::endl;  // 2 3 4 5 6

  std::priority_queue<node> pq1;
  pq1.push(node(1, 2));
  pq1.push(node(2, 2));
  pq1.push(node(3, 4));
  pq1.push(node(3, 3));
  ...
  while (!pq1.empty()) {
    std::cout << pq1.top().x << " " << pq1.top().y << std::endl;
    pq1.pop();
  }
  // x y
  // 1 2
  ...
  std::cout << "x y" << std::endl;
  while (!pq2.empty()) {
    std::cout << pq2.top().x << " " << pq2.top().y << std::endl;
    pq2.pop();
  }
  // x y
  // 1 2
  ...
  std::cout << "x y" << std::endl;
  while (!pq3.empty()) {
    std::cout << pq3.top().x << " " << pq3.top().y << std::endl;
    pq3.pop();
  }
  // x y
  // 1 2

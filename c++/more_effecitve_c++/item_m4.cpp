...
// 1.第一种情况是建立数组时；
//   一般来说，没有一种办法能在建立对象数组时给构造函数传递参数，有三种方法能回避开这个限制。

// 2.对于类里没有定义缺省构造函数所造成的第二个问题是它们无法在许多基于模板（template-based）的容器类里使用。
...
// 3.第三个问题是，在设计虚基类时所面临的要提供缺省构造函数还是不提供缺省构造函数的两难决策。
//   不提供缺省构造函数的虚基类，很难与其进行合作。
//   因为几乎所有的派生类在实例化时都必须给虚基类构造函数提供参数。
//   这就要求所有由没有缺省构造函数的虚基类继承下来的派生类都必须知道并理解提供给虚基类构造函数的参数的含义。
//   派生类的作者是不会企盼和喜欢这种规定的。

// 因为这些强加于没有缺省构造函数的类上的种种限制，一些人认为所有的类都应该有缺省构造函数，
...
// 使用这种（没有缺省构造函数的）类的确有一些限制，但是当你使用它时，它也给你提供了一种保证：
// 你能相信这个类被正确地建立和高效地实现。
...
int main() {
  ...
  //   如果你忘了，就会发生内存泄漏。
  // 2.增加了内存分配量，因为正如你需要空间来容纳EquipmentPiece对象一样，你也需要空间来容纳指针。

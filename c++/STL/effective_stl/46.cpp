...
// 对于标题感到惊讶：将函数对象（即可以被伪装成函数的对象）传递给STL算法往往比传递实际的函数更加高效。
...
// 就算C++必须先实例化函数模板和类模板，然后再调用相应的operator()函数，相比之下，C只是进行了一次简单的函数调用。
// 但是C++所有的这些“额外负担”都在编译期间消化殆尽。在运行时，sort算法以内联方式调用它的比较函数，而qsort则通过一个指针来调用它的比较函数。
// 所以最终的结果是，sort算法运行得更快一些。使用包含一百万个double值的vector进行测试，sort比qsort快了670%。

// 除了效率还有一个理由使用函数对象，必须让你的程序正确地通过编译。
...
// }
// 许多编译接受这段代码，但是C++标准却不认同这样的代码。因为理论上可能存在另一个名为average的函数模板，它也只带一个类型参数，有歧义。
// 使用函数对象类代替函数即可：
template <typename FPType>
...
// 所有的C++编译器都应该接受这段修正过的代码。而且，transform内部的Average::operator()调用也符合内联优化的条件，
...
int main() {
  ...
  // 使用greater<double>的sort调用比使用doubleGreater的sort调用快得多。
  // 因为函数内联。如果一个函数对象的operator()函数已经被声明为内联的，那么它的函数体将可以直接被编译器使用，
  // 大多数编译器都乐于在被调用算法的模板实例话过程中将该函数内联进去。最终结果是，sort中不包含函数调用，
  // 编译器就可以对这段不包含函数调用的代码进行优化，而这种优化在正常情况下是很难获得的。
  // 在C/C++中并不能真正地将一个函数作为参数传递给另一个函数。将一个函数作为参数进行传递，则编译器会隐式地将它转换成一个指向该函数的指针。
  // 由于传递的是函数指针，所有在sort内每次doubleGreater被用到的时候，编译器会产生一个间接的函数调用，即通过指针发出的调用。
  // 大多数编译器不会试图对通过函数指针执行的函数调用进行内联优化。可能是编译器厂商觉得这种优化不值得去做。

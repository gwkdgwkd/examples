...
// 所谓完美，即不仅能转发参数的值，还能保证被转发参数的左、右值属性不变。
// C++11引入右值引用语法的同时，还解决了一个C++98/03长期存在的短板，
// 即使用简单的方式即可在函数模板中实现参数的完美转发。

void f(int& t) { std::cout << " lvalue" << std::endl; }
void f(const int& t) { std::cout << " rvalue" << std::endl; }
...
//   因此它永远都是左值，也就是说，传递给f()函数的参数t永远都是左值。
...
// C++98/03中几乎不会用到，但C++11引入了右值引用和移动语义，
// 因此很多场景中是否完美转发，直接决定了该参数使用的是拷贝还是移动。
// 事实上，C++98/03下的C++也可以实现完美转发，只是实现方式比较笨拙。
// C++98/03中只有左值引用，并且可以细分为非const引用和const引用。
...
// 因此如果使用C++98/03，可以采用函数模板重载的方式实现完美转发：
template <typename T>
...
namespace n3 {
// 使用重载的模板函数实现完美转发也是有弊端的，仅适用于有少量参数的情况，
// 否则就需要编写大量的重载函数模板，造成代码的冗余。
// 为了方便用户更快速地实现完美转发，C++11使用右值引用来实现完美转发。
...
namespace n4 {
// 将函数模板的形参类型设置为T&&，可以很好地解决接收左、右值的问题。
// 但除此之外，还需要解决一个问题，即无论传入的形参是左值还是右值，
// 对于函数模板内部来说，形参既有名称又能寻址，因此它都是左值。
// 那么如何才能将函数模板接收到的形参连同其左、右值属性一起传递呢？
// C++11引入了模板函数forword<T>()，只需要调用该函数，就可以解决。
...
// 总的来说，在定义模板函数时应该：
// 1.采用右值引用定义参数类型，既可以接收传入的左值，也可以接收右值；
// 2.使用C++11提供的forword()模板函数修饰需要维持左、右值属性的参数。

...
// 无需用户指明如何转换的称为自动类型转换（隐式类型转换），需要用户显式地指明如何转换的称为强制类型转换。
// 不管是自动类型转换还是强制类型转换，前提必须是编译器知道如何转换，这些规则都是编译器内置的，我们并没有告诉编译器。
// 例如，将小数转换为整数会抹掉小数点后面的数字，将int*转换为float*只是简单地复制指针的值，
// 换句话说，如果编译器不知道转换规则就不能转换，使用强制类型也无用。
// C++允许我们自定义类型转换规则，用户可以将其它类型转换为当前类类型，也可以将当前类类型转换为其它类型。
// 这种自定义的类型转换规则只能以类的成员函数的形式出现，这种转换规则只适用于类。
// 将其它类型转换为当前类类型需要借助转换构造函数（Conversion constructor）。
// 转换构造函数也是一种构造函数，它遵循构造函数的一般规则，但转换构造函数只有一个参数。
// 转换构造函数也是构造函数，它除了可以用来将其它类型转换为当前类类型，还可以用来初始化对象，这是构造函数本来的意义。
...
class Complex {
 public:
  ...
  // 在进行数学运算、赋值、拷贝等操作时，如果遇到类型不兼容、需要将double类型转换为Complex类型时，
  // 编译器会检索当前的类是否定义了转换构造函数，如果没有定义的话就转换失败，如果定义了的话就调用转换构造函数。
...
void func1() {
  ...
  // p1是int*类型，它指向的内存里面保存的是整数，p2是float*类型，将p1赋值给p2后，
  // p2也指向了这块内存，并把这块内存中的数据作为小数处理。
  // 我们知道，整数和小数的存储格式大相径庭，将整数作为小数处理非常荒诞，
...
void func2() {
  ...
  // 以拷贝的方式初始化对象，先调用转换构造函数，将25.5转换为Complex类型（匿名对象），然后再拷贝给a3
...
void func4() {
  // 需要注意的是，为了获得目标类型，编译器会“不择手段”，会综合使用内置的转换规则和用户自定义的转换规则，
  // 并且会进行多级类型转换，例如：
  ...
  // 先将char转换为int，再将int转换为double，最后根据用户自定义规则将double转换为Complex：
...
namespace n2 {
// 转换构造函数能够将其它类型转换为当前类类型，但是不能反过来将当前类类型转换为其它类型。
// 类型转换函数（Type conversion function）解决了这个问题。
// 作用就是将当前类类型转换为其它类型，它只能以成员函数的形式出现，也就是只能出现在类中。
// 类型转换函数的语法格式为： operator type(){ return data;}
// operator是C++关键字，type是要转换的目标类型，data是要返回的type类型的数据。
// 因为要转换的目标类型是type，所以返回值data也必须是type类型。既然已经知道了要返回type类型的数据，
// 所以没有必要再像普通函数一样明确地给出返回值类型。
...
// 4.一个类虽然可以有多个类型转换函数（类似于函数重载），但是如果多个类型转换函数要转换的目标类型
//   本身又可以相互转换（类型相近），那么有时候就会产生二义性。
class A {
 public:
...
void func1() {
  ...
  // 编译器可以调用operator double()将a转换为double类型，也可以调用operator int()将c1转换为int类型，
  // 这两种类型都可以跟12.5进行加法运算，并且从a转换为double与从a转化为int是平级的，没有谁的优先级更高，
  // 所以这个时候编译器就不知道该调用哪个函数了，干脆抛出一个二义性错误，让用户解决。
}
...
// 实践证明，用户对转换构造函数的需求往往更加强烈，这样能增加编码的灵活性，例如：
//  可以将一个字符串字面量或者一个字符数组直接赋值给string类的对象；
//  可以将一个int、double、bool等基本类型的数据直接赋值给Complex类的对象。
// 那么，如果我们想把当前类类型转换为其它类型怎么办呢？
// 很简单，增加一个普通的成员函数即可，例如，string类使用c_str()函数转换为C风格的字符串，
...
void testN3() {
  ...
  // d = 12.5 + c1;  // 产生二义性
  // 进行加法运算时，有两种转换方案：
  // 1.先将12.5转换为Complex类型再运算，这样得到的结果也是Complex类型，再调用类型转换函数就可以赋值给f了。
  // 2.先将c1转换为double类型再运算，这样得到的结果也是double类型，可以直接赋值给f。
  // 很多人认为，既然=左边是double类型，很显然应该选择第二种方案，这样才符合“常理”。
  // 其实不然，编译器不会根据=左边的数据类型来选择转换方案，编译器只关注12.5+c1这个表达式本身，站在这个角度考虑，
  // 上面的两种转换方案都可以，编译器不知道选择哪一种，所以会抛出二义性错误，让用户自己去解决。
...
namespace n4 {
// 一个实用的Complex类能够进行四则运算和关系运算，需要重载+、-、*、/、+=、-=、*=、/=、==、!=这些运算符，
// 不过这里仅仅重载了+、+=、==、!=运算符
class Complex {
 public:
  Complex(double real = 0.0, double imag = 0.0) : m_real(real), m_imag(imag) {}

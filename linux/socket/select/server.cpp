...
// fd_set是自定义的一个数据结构，可看作一个集合，
// 存放可读、可写或异常事件的文件描述符。
// fd_set集合通常有以下四个宏来操作，
// 1.清空fdset中所有文件描述符：
//   void FD_ZERO(fd_set *fdset);
// 2.添加文件描述符fd到集合fdset中：
//   void FD_SET(int fd,fd_set *fdset);
// 3.将文件描述符fd从集合fdset中去除：
//   void FD_CLR(int fd,fd_set *fdset);
// 4.判断文件描述符fd是否在集合fdset中：
//   int FD_ISSET(int fd,fd_set *fdset);

// select工作原理：
// 传入要监听的文件描述符集合（可读、可写或异常）开始监听，
// select处于阻塞状态，当有事件发生或设置的等待时间timeout到了就会返回，
// 返回之前自动去除集合中无事件发生的文件描述符，
// 返回时传出有事件发生的文件描述符集合。
...
// 3.select返回的就绪文件描述符集合，
//   需要用户循环遍历所监听的所有文件描述符是否在该集合中，
//   当监听描述符数量很大时效率较低。

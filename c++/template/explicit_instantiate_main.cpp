...
// 显式实例化也包括声明和定义，定义要放在模板定义（实现）所在的源文件，
// 声明要放在模板声明所在的头文件（当然也可以不写）。
...
// 1.程序员必须要在模板的定义文件（实现文件）中对所有使用到的类型进行实例化。
...
//   只能将模板的声明和定义（实现）全部放到头文件中；
//   C++标准库几乎都是用模板来实现的，这些模板的代码也都位于头文件中。
// 总起来说，如果我们开发的模板只有我们自己使用，那也可以勉强使用显式实例化；

...
// 我们经常使用的cin、cout就属于流对象，其中cin可以获取键盘输入的数据，cout可以将指定数据输出到屏幕上。除此之外，更常见的还有文件I/O流等。
...
// 输入流迭代器用于直接从指定的输入流中读取元素，该类型迭代器本质上就是一个输入迭代器，这意味着假设p是一个输入流迭代器，则其只能进行++p、p++、*p操作，
// 同时输入迭代器之间也只能使用==和!=运算符。
// 输入流迭代器的底层是通过重载++运算符实现的，该运算符内部会调用operator >>读取数据。也就是说，假设iit为输入流迭代器，则只需要执行++iit或者iit++，即可读取一个指定类型的元素。
...
int main() {
  // 创建输入流迭代器的方式有3种，分别为：
  // 1 调用istream_iterator模板类的默认构造函数，可以创建出一个具有结束标志的输入流迭代器。当我们从输入流中不断提取数据时，总有将流中数据全部提取完的那一时刻，
  // 这一时刻就可以用此方式构建的输入流迭代器表示。
  std::istream_iterator<double> eos;
  // 2 还可以创建一个可用来读取数据的输入流迭代器
  std::istream_iterator<double> iit1(std::cin);
  // 创建了一个可从标准输入流cin读取数据的输入流迭代器。通过此方式创建的输入流迭代器，其调用的构造函数中，会自行尝试去指定流中读取一个指定类型的元素。
  ...
  // 只有读取到EOF流结束符时，程序中的iit才会和eos相等。另外，Windows平台上使用Ctrl+Z组合键输入^Z表示EOF流结束符，此结束符需要单独输入，或者输入换行符之后再输入才有效。
  if (iit != eos) {
    value1 = *iit;
  }
  iit++;
  ...
  ostream_iterator<string> out_it4(cout);
  *out_it4 = "http://c.biancheng.net/stl/";
  cout << endl;  // http://c.biancheng.net/stl/
  ostream_iterator<int> out_it5(cout, ",");
  *out_it1 = 1;
  *out_it1 = 2;
  *out_it1 = 3;
  cout << endl;  // 123

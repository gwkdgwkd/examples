...
// 1.begin()和end()参数为容器
//   当将某个具体容器c作为参数分别传给begin()和end()函数时，其中begin()底层会执行c.begin()语句，
//   而end()底层会执行c.end()语句，它们最终会将得到的迭代器作为函数的返回值反馈回来。
//   当作用对象为容器时，end()和begin()函数的语法格式是完全一样的，这里以begin()函数为例，有以下2种格式：
//   1.非const修改的容器作为参数，begin()函数返回的为非const类型的迭代器
//    template <class Container> auto begin(Container& cont)
//   2.传入const修饰的容器，begin()函数返回的为const类型的迭代器
...
//   则通过该函数获得的迭代器也有const修饰（不能用于修改容器中存储的数据）；反之就没有。
// 2.begin()和end()参数为数组

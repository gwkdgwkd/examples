...
// 传统枚举类型在设计上会存在以下几个问题:
// 1.同作用域同名枚举值会报重定义错误，传统C++中枚举常量被暴漏在同一层作用域中，
// 如果同一作用域下有两个不同的枚举类型，但含有同名的枚举常量也是会报编译错误的
// enum Gender1 { Male1, Female };  // error: redeclaration of ‘Female’
// 因为enum则是非强作用域类型，枚举常量可以直接访问，
// 这种访问方式与C++中具名的namespace、class/struct以及union必须通过"名字::成员名"的访问方式大相径庭。
...
// 为了解决以上传统枚举类型的缺陷(非强作用域类型，允许隐式转换为整型，枚举常量占用存储空间以及符号性的不确定)，
// C++11引入了强类型枚举解决了这些问题。
...
// 3.可以指定底层类型。强类型枚举默认的底层类型是int，但也可以显示地指定底层类型。

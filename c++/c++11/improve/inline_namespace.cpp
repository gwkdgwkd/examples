...
// 内联命名空间将子命名空间的名字导入到父命名空间，这样就可以在其它子命名空间使用父命名空间直接调用。
// 一方面是简化使用过程，另一方面可以实现功能函数版本迭代。

// 内联命名空间使用inline关键词前缀:
// 1.内联命名空间兼具普通命名空间的使用特点。
// 2.内联命名空间可以将自身名字导入父命名空间，其它子命名空间可以通过父命名空间进行调用。
// 3.名字访问是命名空间加标识符。
...
namespace n4 {
// 内联命名空间特点就是不需要使用using就可以直接在外层命名空间使用该命名空间内的内容，而且无需使用前缀
...
// 第二次没有使用inline关键字，但由于第一次已经声明了inline，声明的还是内联命名空间，这种情况称为隐式内联。
...
// 由于隐式内联语法的存在，将来出现ver3的时候，只要将唯一的一个inline关键字移动到第一次出现的ver3定义之前就可以了！
...
void testN5() {
  ...
  A2 a2;
  a2.fun1();  // ver2 AA2 fun1
  a2.fun2();  // ver2 AA2 fun2
  B2 b2;
  b2.fun1();  // ver2 BB2 fun1
  C2 c2;
  c2.fun3();  // ver2 CC2 fun3

  // 如果实在是需要原来的类库，可以通用版本前缀加类名的方式
  ver1::A2 a3;
  a3.fun1();  // ver1 AA2 fun1
  ver1::B2 b3;
  b3.fun1();  // ver1 BB2 fun1
}
}  // namespace n5

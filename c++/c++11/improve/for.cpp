...
// expression：表示要遍历的序列，可以为事先定义好的普通数组或者容器，还可以是用{}大括号初始化的序列。

// 同C++98/03中for循环的语法格式相比较，此格式并没有明确限定for循环的遍历范围，这是它们最大的区别，
// 即旧格式的for循环可以指定循环的范围，新的for只会逐个遍历expression参数处指定序列中的每个元素。
...
namespace n3 {
// C++11 for循环使用注意事项:
// 1.首先需要明确的一点是，当使用for循环遍历某个序列时，无论该序列是普通数组、
//   容器还是用{}大括号包裹的初始化列表，遍历序列的变量都表示的是当前序列中的各个元素，不是迭代器。
...
void func1() {
  ...
  //   std::cout << ch;
  // }
  // 原因很简单，此格式的for循环只能遍历有明确范围的一组数据，retStr1()函数返回的是指针变量，
  // 遍历范围并未明确指明，所以编译失败。
}

// 3.当基于范围的for循环遍历的是某函数返回的string对象或者容器时，整个遍历过程中，函数只会执行一次。
std::string retStr2() {
  std::cout << "retStr2:";
  return arc;
}
...
void func3() {
  ...
  // 执行结果并不是我们想要的，因为在for循环遍历arr容器的同时向该容器尾部添加了新的元素（容器扩容），

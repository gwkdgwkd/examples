...
// foo()的调用与template参数毫无关联，所以调用操作必须根据“scope of the template definition”来决议。
// 对于type_dependent的调用，与template参数有关，因为该参数将决定_member的真正类型。
// 所以这一次必须在“scope of the template instantiantion”中决议，有两个foo()函数声明。
...
// 对于template的支持，最困难的莫过于template function的具现。编译器提供了两个策略：
//  一个是编译时期策略，程序代码必须在program text file中准备妥当；
//  另一个是链接时期策略，有一些工具可以引导编译器的具现行为。

// 编译器设计者必须回答三个主要问题：
//  编译器如何找出函数的定义？
//  编译器如何能够只具现出程序用到的成员函数？
//  编译器如何阻止成员定义在多个.o文件中被具现呢？

// 7.2 异常处理

// 7.3 执行期类型识别

// 在2.0之前，除了析构函数外，唯一不能被重载的函数就是转换运算符，因为它们不使用参数。
// 直到引进了const member functions后，转换操作符才能重载成const。
//  operator char*();
//  operator char*() const;

// C++被吹毛求疵的一点就是，缺乏一个保证安全的向下转换操作，只有在“类型真的可以被适当转换”的情况下，才能执行downcast。
// 支持type-safe downcast在对象空间和执行时间上都需要一些额外负担：
//  需要额外的空间以存储类型信息，通常是一个指针，指向某个类型信息节点。
//  需要额外的时间以决定执行期的类型，因为，正如其名，这需要在执行期才能决定。

// C++的RTTI机制提供一个安全的downcast，但只对那些展现“多态”的类型有效。
// 把类相关的RTTI object地址放进virtual table中，通常放在第一个slot中。
// 每一个对象只多花一个指针。这个指针只需被设定一次，它是被编译器静态设定，而不是在执行期有构造函数设定（vptr才这么设定）。

// dynamic_cast运算符可以在执行期决定真正的类型：
//  如果downcast是安全的（基类指针指向一个派生类对象），这个运算符会传回被适当转换的指针。
//  如果downcast不是安全的，这个运算符会传回0。

// references并不是pointers
// 程序中对一个类指针类型执行dynamic_cast运算符，会获得true或false：
//  如果传回真正的地址，表示这个对象的动态类型被确认了，一些与类型有关的操作现在可以施行与其上。
//  如果传会0，表示没有指向任何对象，意味这该以另一种逻辑执行这个动态类型为确定的对象身上。
// dynamic_cast运算符也适用于引用身上。然而对于一个non-type-safecast，其结果不会与指针的情况相同。因为：
//  一个引用不可以像指针那样“把自己设为0便代表了“no object”
//  若将一个引用设为0，会引起一个临时性对象被产生出来，该临时对象的初值为0，这个引用然后被设定为该临时对象的一个别名。
// 因此当dynamic_cast运算符用在引用时，不能够提供等于指针情况下的那一组true和false。会发生下面的事：
//  如果引用真正参考到适当的派生类，downcast会别执行而程序可以继续进行。
//  如果引用并不是某一种派生类，那么由于不能传回0，所以丢出一个bad_cast exception。

// Typeid运算符
// 使用typeid运算符，就有可能以一个引用达到相同的执行期替代路线。
// type_info运算符传回一个const reference，类型为type_info。
// RTTI只适用于多态类，type_info对象也适用于内建类型，以及非多态的使用者自定类型。

// 7.4 效率有了，弹性呢？

// 传统的C++对象模型提供了有效率的执行期支持。这份效率，再加上与C之间的兼容性，造成了C++的广泛接受度。
// 然而，在某些方面，像动态共享函数库、共享内存、以及分布式对象方面，这个对象模型的弹性还是不够。

// 动态共享函数库
// 在目前的C++对象模型中，如果库中的对象布局有所改变，应用程序就必须重新编译。
// 因为类的大小及其每一个直接（或继承而来）的成员的偏移量都在编译时期就已经固定（虚拟继承的成员除外）。
...
int main() {
  ...
  // 换成引用，会具现出一个“Point的float实体”来。
  // 因为引用并不是无物（no object）的代名词。0被视为整数，必须被转换为以下类型的一个对象：Point<float>

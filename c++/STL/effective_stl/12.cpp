...
// 标准C++的世界相当狭小和古旧。在这个纯净的世界中，所有的可执行程序都是静态链接的。不存在内存映像或共享内存。
// 没有窗口系统，没有网络，没有数据库，也没有其他进程。对于STL的线程安全性的第一个期望应该是，它会随不同实现而异。
...
//  对于作用于容器的每个算法，都锁住该容器，直到算法结束（实际上这样做没有意义）。
// 需要手工做同步控制。
...
int main() {
  ...
  vector<int> v1;
  getMutexFor(v1);
  vector<int>::iterator first51(find(v1.begin(), v1.end(), 5));
  if (first51 != v1.end()) {
    *first51 = 0;
  }

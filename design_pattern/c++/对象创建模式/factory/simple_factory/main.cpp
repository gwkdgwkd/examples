...
// 定义一个工厂类，根据不同的参数创建并返回不同的类对象，被创建的对象所属的类一般都具有相同的父类。
// 调用者无需关心产品的具体细节，实现了创建产品的代码（工厂类），与具体的产品类解耦合的效果。
// 简单工厂并不属于GOF的23设计模式之一，不过在实际应用中因为其简单方便理解所以也经常使用。

// 简单工厂模式的结构组成：
// 1.工厂类，工厂模式的核心类，是一个具体的类，非接口抽象类，有一个重要的create()方法，
//   利用if或者switch创建产品并返回，create()方法通常是静态的，所以也称之为静态工厂。

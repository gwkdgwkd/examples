...
// 任何一个类型转换（不论是通过转型操作而进行的显示转换，
// 或通过编译器完成的隐式转换）往往真的令编译器编译出运行期间执行的码。
// 单一对象可能拥有一个以上的地址（例如以Base*指向它时的地址和以Derived*指向它时的地址）。
// C，Java，C#都不可能发生这种事。
// 但C++可能，实际上一旦使用多重继承，这事几乎一直发生着。
// 即使在单一继承中也可能发生。
class Base {};
...
class SpecialWindow : public Window {
 public:
  virtual void onResize() {
    // 调用的不是当前对象上的函数，
    // 而是稍早转型动作建立的一个”*this对象的base class成分“的暂时副本身上的onResize!

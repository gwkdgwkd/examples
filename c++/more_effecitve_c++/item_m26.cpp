...
// 事实确实这样，阻止建立某个类的对象，最容易的方法就是把该类的构造函数声明在类的private域：
...
// 2.全局函数thePrinter被声明为类的友元，让thePrinter避免私有构造函数引起的限制。
// 3.最后thePrinter包含一个静态Printer对象，这意味着只有一个对象被建立。
...
//   与此相反，只有第一次执行函数时，才会建立函数中的静态对象，所以如果没有调用函数，就不会建立对象。
//   不过你得为此付出代价，每次调用函数时都得检查是否需要建立对象。
//   建立C++一个理论支柱是你不需为你不用的东西而付出，在函数里，
//   把类似于Printer这样的对象定义为静态成员就是坚持这样的理论，你应该尽可能坚持这种理论。
//   与一个函数的静态成员相比，把Printer声明为类中的静态成员还有一个缺点，它的初始化时间不确定。
//   我们能够准确地知道函数的静态成员什么时候被初始化：“在第一次执行定义静态成员的函数时”。
//   而没有定义一个类的静态成员被初始化的时间。
//   C++为一个translation unit（编译单元）内的静态成员的初始化顺序提供某种保证，
//   但是对于在不同translation unit中的静态成员的初始化顺序则没有这种保证。
//   在实际使用中，这会给我们带来许多麻烦。
//   当函数的静态成员能够满足我们的需要时，我们就能避免这些麻烦。
// 2.另一个细微之处是内联与函数内静态对象的关系。
//   thePrinter的非成员函数形式，除了第一次执行这个函数时（也就是构造p时），
//   其它时候这就是一个一行函数——它由“return p;”一条语句组成。
//   这个函数最适合做为内联函数使用，然而它不能被声明为内联，为什么呢？
//   为什么你要把对象声明为静态呢？通常是因为你只想要该对象的一个拷贝。
//   现在再考虑“内联”意味着什么呢？
//   从概念上讲，它意味着编译器用函数体替代该对函数的每一个调用，不过非成员函数还不只这些。
//   非成员函数还有其它的含义。它还意味着internal linkage（内部链接）。
//   通常情况下，你不需要理解这种语言上令人迷惑的东西，你只需记住一件事：
//   带有内部链接的函数可能在程序内被复制（也就是说程序的object代码可能包含一个以上的内部链接函数的代码），
//   这种复制也包括函数内的静态对象。
//   结果如何？如果建立一个包含局部静态对象的非成员函数，你可能会使程序的静态对象的拷贝超过一个！
...
// 建立对象的环境
// 这种方法也有一个问题，定义基类和派生类对象会产生多少Pritner对象？答案是两个。
...
Printer3::Printer3() {
  if (numObjects >= 1) {
    throw TooManyObjects();
    // 当需要的对象过多时，会抛出异常，如果你认为这种方式给你的感觉是unreasonablyharsh，
...
// 否则如果有人通过Counted<Printer>*指针删除一个Printer对象，我们就有导致对象行为不正确的风险。
// 在Counted中存在虚函数，几乎肯定影响从Counted继承下来的对象的大小和布局。
...
// 不检测对象的数量就好像限制将被超过，执行完构造函数后也不增加存在对象的数目。
// 所有这些现在都由Counted<Printer>的构造函数来处理，因为Counted<Printer>是Printer的基类，

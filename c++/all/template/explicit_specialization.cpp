...
// 但是却不能比较结构体、对象以及数组的大小，因为并没有针对结构体、类和数组重载>。
// 另外该函数模板虽然可以用于指针，但比较的是地址大小，而不是指向的数据，没意义。
// 模板是一种泛型技术，它能接受的类型是宽泛的、没有限制的，
// 并且对这些类型使用的算法都是一样的（函数体或类体一样）。
// 但是现在希望改变这种游戏规则，让模板能够针对某种具体的类型使用不同的算法，
// 这在C++中这种技术称为模板的显式具体化，函数模板和类模板都可以显式具体化。
...
// 类名后面列出全部类型参数，是为了让编译器确认到底是第几个类型参数被具体化了：

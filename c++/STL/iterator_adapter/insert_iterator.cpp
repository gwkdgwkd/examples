...
//   但前提必须是提供有push_back()成员方法的容器，
//   包括vector、deque和list。
// 2.front_insert_iterator在指定容器的头部插入新元素，
//   但前提必须是提供有push_front()成员方法的容器，
//   包括list、deque和forward_list。
...
// 由于此类型迭代器的底层实现需要调用指定容器的push_back()成员方法，
// 这就意味着，此类型迭代器并不适用于STL标准库中所有的容器，
// 它只适用于包含push_back()成员方法的容器。
...
// 其中，Container用于指定插入的目标容器的类型；
// container用于指定具体的目标容器。
...
// 和back_insert_iterator正好相反，
// front_insert_iterator迭代器的功能是向目标容器的头部插入新元素。
// 由于此类型迭代器的底层实现需要借助目标容器的push_front()成员方法，
// 这意味着，只有包含push_front()成员方法的容器才能使用该类型迭代器。
// C++ STL标准库中，提供有push_front()成员方法的容器，
// 仅有deque、list和forward_list。
...
// 和前2种插入迭代器相比，
// insert_iterator迭代器除了定义时需要多传入一个参数，它们的用法完全相同。
// 除此之外，C++ STL标准库中还提供有inserter()函数，

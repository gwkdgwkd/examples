...
// 然而由于可变模版参数比较抽象，使用起来需要一定的技巧，所以它也是C++11中最难理解和掌握的特性之一。
// 虽然掌握可变模版参数有一定难度，但是它却是C++11中最有意思的一个特性。

// variadic template特性本身是一个很自然的需求，它完善了C++的模板设计手段。
// 原来的模板参数可以使类和函数的参数类型“任意化”，如果再加上“参数个数的任意化”，
// 那么在参数方面的设计手段就基本上齐备了，有了variadic template显然可以让设计出来的函数或是类有更大的复用性。
// 因为有很多处理都是与“处理对象的个数”关系不大的，比如说打屏（printf），比如说比较大小（max，min），
// 比如函数绑定子（bind，function要对应各种可能的函数就要能“任意”参数个数和类型）。
// 如果不能对应任意个参数，那么就总会有人无法重用已有的实现，而不得不再重复地写一个自己需要的处理，
// 而共通库的实现者为了尽可能地让自己写的类（函数）能复用在更多的场景，也不得不重复地写很多的代码或是用诡异的技巧，
// 宏之类的去实现有限个“任意参数”的对应。

// C++03只有固定模板参数;
// C++11加入新的表示法，允许任意个数、任意类别的模板参数，不必在定义时将参数的个数固定，其标准表示：
...
// 模板类能够做到，但是模板类也有它的局限性，c++98规定,模板参数的数量必须是固定的，不能是变长的。
// 这个规定在c++11中被打破了。在C++11中，如果模板参数是可变长的，那么我们可以如下定义:
// template <typename... Elements> class yourclass;
// Elements被称为模版参数包(template parameter pack),可以理解为多个模版参数类型的集合，
// 只要我们能把这个集合拆开，拆开为单个单个的元素，那么就可以正常使用了.

// 可变模板参数是有它的局限性的,没有直接获取包中每一个元素的类型。
// 所以要想明白，运用可变长模板参数的运用场景，可变长模板参数的偏特化和全特化都是不支持的。
// 可变模版参数和普通的模版参数语义是一致的，可以应用于函数和类，然而，函数模版不支持偏特化，
...
void print1(T head, Args... rest) {  // 展开函数
  // 参数包Args...在展开的过程中递归调用自己，每调用一次参数包中的参数就会少一个，直到所有的参数都展开为止
  std::cout << "param " << head << " -> ";
  print1(rest...);
}
...
// 2.逗号表达式展开参数包
// 递归函数展开参数包是一种标准做法，也比较好理解，但也有一个缺点，就是必须要一个重载的递归终止函数，
// 即必须要有一个同名的终止函数来终止递归，这样可能会感觉稍有不便。
// 有没有一种更简单的方式呢？
// 其实还有一种方法可以不通过递归方式来展开参数包，这种方式需要借助逗号表达式和初始化列表。
template <class T>
void printarg(T t) {
...
void print2(Args... args) {
  ...
  // 这种就地展开参数包的方式实现的关键是逗号表达式，逗号表达式会按顺序执行逗号前面的表达式
...
// 可变参数模板类的参数包展开需要通过模板特化和继承方式去展开，展开方式比可变参数模板函数要复杂。
...
// MakeIndexes继承于自身的一个特化的模板类，这个特化的模板类同时也在展开参数包，这个展开过程是通过继承发起的，
// 直到遇到特化的终止条件展开过程才结束。
...
namespace n4 {
// 可变参数模版实现泛化的delegate,C++中没有类似C#的委托，可以借助可变模版参数来实现一个：
...
// 展开参数包的过程是很精妙的，体现了泛化之美、递归之美，正是因为它具有神奇的“魔力”，
// 所以我们可以更泛化的去处理问题，比如用它来消除重复的模版定义，用它来定义一个能接受任意参数的“万能函数”等。
...
// 而printf则使用C语言的函数变长参数特性，通过使用变长函数，printf的实现能够接受任何长度的参数列表。
...
// 那么肯定是需要知道知道栈的地址的，有了int count就意味着我们有了这一块连续内存的首地址。
...
// 而且无法知道可变参数的长度。
// 所以必须事先约定好参数的类型和长度，并且需要传递进函数里面去告诉函数，这样在灵活性上就大打折扣了.

// 变长函数这种实现方式，对于C++这种强调类型的语言来说相当于开了一个“不规范”的后门。
// C++需要引入一种更为“现代化”的变长参数的实现方式，即类型和变量同时能够传递给变长参数的函数，
// 一种好的方式是充分利用C++的函数模板。

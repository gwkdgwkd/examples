...
// traits，又被叫做特性萃取技术，说得简单点就是提取被传进的对象对应的返回类型。
// STL的算法和容器是分离的，两者通过迭代器链接，算法的实现并不知道被传进来什么。
...
namespace n3 {
// traits的作用：
// 1.一方面，在面对不同的输入类时，能找到合适的返回类型；
// 2.另一方面，当类型对应有不同的实现函数的时候，能起到一个提取型别然后分流的作用。
...
template <class T>
struct unknown_class_traits<const T *> {  // 特性萃取器，针对指向常数

...
// 除此之外，还有一些我们平时不太留意的临时数据，例如表达式的结果、
// 函数的返回值等，它们可能会放在内存中，也可能会放在寄存器中。
// 一旦被放到了寄存器中，就没法用&获取它们的地址了，也就没法用指针指向它们了。
...
void func4() {
  ...
  // 但是float*类型的指针不能指向int类型的数据，
  // int*类型的指针也不能指向char类型的数据。
  // 为什么编译器禁止指针指向不同类型的数据是合理的呢？
  // 让指针指向相关的（相近的）但不是严格对应的类型的数据，
  // 表面上看起来是合理的，但是细思极恐，
  // 这样会给程序留下很多意想不到的、难以发现的Bug，
  // 所以编译器禁止这样做是非常合理的。
  // 当然，如果你想通过强制类型转换达到这个目的，
  // 那编译器也会放任不管，给你自由发挥的余地。

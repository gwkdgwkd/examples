...
// 不过C++的这种限制仅仅是语法层面的，通过某种“蹩脚”的方法，我们能够突破访问权限的限制，
// 访问到private、protected属性的成员变量，赋予我们这种“特异功能”的，正是强大而又灵活的指针。
// 在对象的内存模型中，成员变量和对象的开头位置会有一定的距离，将这段距离称为偏移。
// 一旦知道了对象的起始地址，再加上偏移就能够求得成员变量的地址，知道了成员变量的地址和类型，也就能够知道它的值。
...
void func2() {
  A *p = new A(40, 50, 60);
  int a1 = p->m_a;  // 编译器内部会发生类似转换：int a2 = *(int*)((int)p + 0);

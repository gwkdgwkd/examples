...
// 在BASIC甚至是COBOL中，可以创建二维、三维乃至n维数组，FORTRAN只能创最多7维数组。
...
namespace test2 {
// 多维数组在C++中的有用程度和其它语言相同，所以找到一个象样的支持方法是很重要的。
// 常用方法是C++中的标准方法：用一个类来实现所需要的而C++语言中并没有提供的东西。
// 因此，可以定义一个类模板来实现二维数组：
template <class T>
class Array2D {
 public:
  ...
  // 如果能容忍奇怪的语法，可能会学其它语言使用()来索引数组，只需重载operator()：
  T& operator()(int index1, int index2) {}
  const T& operator()(int index1, int index2) const {}
  // 这很容易实现，并很容易推广到任意多维的数组。
  // 缺点是Array2D对象看起来和内嵌数组一点都不象，访问元素(5,6)的操作看起来相函数调用。
};
...
void func1() {
  ...
  // a不是真正的二维数组，它是个10元素的一维数组，其中每个元素又都是一个20元素的数组，
  // 所以data[3][6]是(data[3])[6]，也就是data的第四个元素这个数组的第7个元素，
...
class Array2D {
  ...
  // 其用户编程时就象在使用真正的二维数组一样，对于Array2D类的用户这样做是没有意义的：
  // 为了满足C++的反复无常，必须在语法上兼容于其中的元素是另一个一维数组的一个一维数组。

  // 每个Array1D对象扮演的是一个一维数组，而这个一维数组没有在使用Array2D的程序中出现。
  // 扮演其它对象的对象通常被称为代理类，在这个例子里，Array1D是一个代理类。
...
// 想提供一个单参数构造函数，同时也希望防止编译器不加鉴别地调用这个构造函数，有两个方法：
// 1.容易的方法，利用一个最新编译器的特性，explicit关键字。
//   为了解决隐式类型转换而特别引入的这个特性，它很好理解，构造函数用explicit声明，
//   如果这样做，编译器会拒绝为了隐式类型转换而调用构造函数，显式类型转换依然合法。
namespace test2 {
template <class T>
class Array {
 public:
...
// 2.如果编译器不支持explicit，不得不回到不使用成为隐式类型转换函数的单参数构造函数。
...
void func() {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    // 而且编译器无法把int转换成临时ArraySize对象，然后通过临时对象创建Array<int>对象，
    // 因为这将调用两个用户定义的类型转换，从int到ArraySize，再从ArraySize到Array<int>。
...
// 想区分将operator[]用作左值还是右值，因为对于有引用计数的数据结构，
// 读操作的代价可以远小于写操作的代价，如Item M29解释的，
// 引用计数对象的写操作将导致整个数据结构的拷贝，而读不需要，只要简单地返回一个值。
// 不幸的是在operator[]内部，无法确定怎么被调用的，不可能区分出是做左值还是右值。
class String {
 public:
...
void func() {
  // 对象不是const的，所以对于operator[]，不论是读还是写，
  // 都调用non const版本的operator[]：
  String s1, s2;
  s1[5];  // 读s1
  // char &operator[](int index)
  s2[6] = 'x';  // 写s2
...
// proxy可以得到正确的时机，修改operator[]，返回代理字符的proxy对象而不是字符本身：
...
class String {
 public:
  ...
  const CharProxy operator[](int index) const {
    ...
    // 对*this使用的const_cast，这使得它满足了CharProxy类的构造函数的需要，
    // 它的构造函数只接受一个非const的String类。
    // 类型转换通常是令人不安的，但在此处，operator[]返回的CharProxy对象自己是const的，
  ...
  CharProxy operator[](int index) {
    // 通过operator[]返回的proxy对象，记录了它属于哪个string对象以及所扮演的字符的下标：
...
void func1() {
  ...
  // 表达式s1[2]返回的是一CharProxy对象，没有为这样的对象定义输出流操作，
  // 所以编译器努力地寻找隐式的类型转换以使得operator<<调用成功（见Item M5）。
  // 它们找到一个：在CharProxy类内部声明了一个隐式转换到char的操作。
  ...
  // 这至关重要，因为在CharProxy的赋值操作中，知道被赋值的CharProxy对象是作左值使用的。
  // 因此，知道proxy类扮演的字符是作左值使用的，必须执行必要的操作以实现字符的左值操作：
  s2[6] = 'x';
  // CharProxy operator[](int index)
  // CharProxy& operator=(char c)

  // 调用作用于两个CharProxy对象间的赋值操作，左边一个是作左值，右边一个作右值：
  s1[3] = s2[8];
  // CharProxy operator[](int index)
  // CharProxy operator[](int index)
  // CharProxy& operator=(const CharProxy& rhs)
...
void func2() {
  ...
  // 这是因为右值不只是出现在赋值时，那时proxy对象的行为就和实际的对象不一致了。
  String s1 = "Hello";
...
void func1() {
  ...
  // 其它需要左值的操作中也一样，包括operator*=、operator<<=、operator--等。
  // 如果想让这些操作作用在operator[]上，必须为Arrar<T>::Proxy类定义所有这些函数。
  // 这是一个极大量的工作，可能不愿意去做的，不幸的是，必须要去做这些工作，不能两全。
}
...
void func2() {
  ...
  // 但Rational对象才有numerator()和denominator()，而不是其proxy对象，报错。
  // 要使得proxy对象的行为和它们所扮演的对象一致，必须重载可作用于实际对象的每一个函数。
}
void func() {
  func1();
  func2();
...
void func1() {
  ...
  // String::operator[]返回CharProxy对象，但swap()要求它所参数是char &类型。
  // 一个CharProxy对象可以隐式地转换为一个char，但没有转换为char &的转换函数。
...
// 只调用一个用户自定义的转换函数，于是，很可能在函数调用时，
// 传实际对象是成功的而传proxy对象是失败的。
class TVStation {
 public:
...
void func2() {
  ...
  // 然而，当用proxy类区分operator[]作左右值的带引用计数的数组模板时，就不能这么做了：
  n4::Array<int> a;
  a[4] = 10;
  // watchTV(a[4], 2.5);  // 报错

  // 问题发生在隐式类型转换上，它很难解决，更好的设计应该是声明它的构造函数为explicit，
...
// 同时，proxy类也有缺点：
// 1.作为函数返回值，proxy对象是临时对象（见Item 19），它们必须被构造和析构，

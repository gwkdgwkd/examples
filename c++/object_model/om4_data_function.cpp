...
// 3.将成员函数重新写成一个外部函数，对函数名称进行mangling处理，
//   是它在程序中称为独一无二的名称。
...
// 改为：
// (*ptr->vptr[1])(ptr);
// 1.vptr表示有编译器产生的指针，指向vitual table。
...
// 然而一个inline函数如果别调用太多次的话，
// 会产生大量的扩展码，是程序的大小暴涨。

// 参数带有副作用，或是以一个单一表达式做多重调用，
// 或是在inline函数总有多个局部变量，都会产生临时对象，编译器也许能把它们移除。
// 此外，inline中再有inline，

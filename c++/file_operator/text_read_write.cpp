...
// 举个例子，假设以文本形式将浮点数19.625写入文件，
// 则该文件会直接将19.625这个字符串存储起来。
// 当双击打开此文件，也可以看到19.625。
// 但如果以二进制形式将浮点数19.625写入文件，
// 则存储的不再是19.625这个字符串，而是19.625浮点数对应的二进制数据。
// 以float类型的19.625来说，
// 文件最终存储：0100 0001 1001 1101 0000 0000 0000 0000
...
// fstream或者ifstream类负责实现对文件的读取，
// 它们内部都对>>输出流运算符做了重载；
// 同样，fstream和ofstream类负责实现对文件的写入，
// 它们的内部也都对<<输出流运算符做了重载。

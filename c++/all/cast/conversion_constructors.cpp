...
class Complex {
 public:
  ...
  // 在进行数学运算、赋值、拷贝等操作时，
  // 如果遇到类型不兼容、需要将double类型转换为Complex类型时，
...
namespace n2 {
// 转换构造函数能够将其它类型转换为当前类类型，但不能将当前类类型转换为其它类型。
// 类型转换函数的作用就是将当前类类型转换为其它类型，
...
// 类型转换函数和运算符的重载都用operator，因此也把类型转换函数称为类型转换运算符。
// 关于类型转换函数的说明：
...
void func1() {
  ...
  // 也可以调用operator int()将a转换为int类型，都可以跟12.5进行加法运算，
  // 并且从a转换为double与从a转化为int是平级的，没有谁的优先级更高，
  // 所以编译器就不知道该调用哪个函数了，干脆抛出一个二义性错误，让用户解决。
}
...
// 解决二义性问题的办法是要么只使用转换构造函数，要么只使用类型转换函数。
...
// 增加一个普通的成员函数即可，例如，string的c_str()函数转换为C风格的字符串，
...
void func() {
  ...
  //   再调用类型转换函数就可以赋值给d了；
  // 2.先将c1转换为double再运算，这样得到的结果也是double，可以直接赋值给d。
  // 很多人认为，既然=左边是double类型，很显然应该选择第二种方案才合理。
  // 其实编译器不会根据=左边的数据类型来选择转换方案，编译器只关注表达式本身，
  // 站在这个角度考虑，上面的两种转换方案都可以，编译器不知道选择哪一种，
  // 所以会抛出二义性错误，让用户自己去解决。

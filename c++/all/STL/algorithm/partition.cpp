...
namespace n1 {
// partition()可根据用户自定义的筛选规则，重新排列指定区域内存储的数据，
// 使其分为2组，第一组为符合筛选条件的数据，另一组为不符合筛选条件的数据。
// partition()只会根据条件将数据进行分组，并不关心分组后各个元素的存储位置。
// 如果想在分组之后仍不改变各元素之间的相对位置，可以选用stable_partition()。
...
// pred用于指定筛选规则，其本质就是一个可接收1个参数且返回值类型为bool的函数，
// 可以是普通函数，也可以是一个函数对象。
...
namespace n2 {
// stable_partition()可以保证对指定区域内数据完成分组的同时，不改变相对位置。
// stable_partition()函数语法格式如下：
// BidirectionalIterator stable_partition(BidirectionalIterator first,
//                                        BidirectionalIterator last,
//                                        UnaryPredicate pred);
// 其中，first和last都为双向迭代器，[first,last)用于指定该函数的作用范围；
// pred用于指定筛选规则。
// stable_partition()函数还会返回一个双向迭代器，其指向的是两部分数据的分界位置，
// 更确切地说，指向的是第二组数据中的第1个元素。
void func() {
  std::vector<int> v{1, 2, 3, 4, 5, 6, 7, 8, 9};
...
namespace n3 {
// 需要partition()或stable_partition()分组的功能，但并不想对原序列做任何修改。
// 这种情况下，就可以考虑使用partition_copy()函数，和stable_partition()一样，
// partition_copy()函数也能按照某个筛选规则对指定区域内的数据进行分组，
// 并且分组后不会改变各个元素的相对位置。
// 更重要的是，partition_copy()函数不会对原序列做修改，
// 而是以复制的方式将序列中各个元组分组到其它的指定位置存储。
...
//                     UnaryPredicate pred);
// 其中，各个参数的含义为：
// first、last：都为输入迭代器，[first,last)用于指定该函数处理的数据区域；
// result_true：为输出迭代器，指定某个存储区域，以存储满足筛选条件的数据；
// result_false：为输出迭代器，指定某个存储区域，以存储满足筛选条件的数据；
// pred：用于指定筛选规则，就是接收一个具有1个参数且返回值类型为bool的函数。
...
void func() {
  ...
  std::cout << "b[10]：";
  std::copy(std::begin(b), result.first,
            std::ostream_iterator<int>(std::cout, " "));
  std::cout << std::endl;  // b[10]：2 4 6 8

  std::cout << "c[10]：";
  std::copy(std::begin(c), result.second,
            std::ostream_iterator<int>(std::cout, " "));
...
namespace n4 {
// 对于如何在已分好组的数据中找到分界位置，
// C++11标准库提供了专门解决此问题的函数，即partition_point()函数。
// ForwardIterator partition_point(ForwardIterator first,
//                                 ForwardIterator last,
//                                 UnaryPredicate pred);
// 其中，first和last为正向迭代器，[first,last)用于指定该函数的作用范围；
// pred用于指定数据的筛选规则，其实就是包含1个参数且返回值类型为bool的函数，
// 此函数可以是一个普通函数，也可以是一个函数对象。
// 同时，该函数会返回一个正向迭代器，指向第一个不符合pred筛选规则的元素。
// partition_point()返回了一个指向元素1的迭代器，
// 而该元素为容器中第一个不符合规则的元素，同时其也可以第二组数据中第一个元素。

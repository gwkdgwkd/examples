...
// 想要重载operator new或operator delete，三个最常见的理由：
// 1.用来检测运用上的错误，如果将new的内存delete时不幸失败，会导致内存泄露，
//   如果多次delete则会导致不确定行为；
// 2.为了强化效率，编译器所带的new和delete主要用于一般目的，
//   采取了中庸之道，它对每个场景都适合，但对某些特定的场景没有最佳表现；
// 3 为了收集使用上的统计数据。
...
// 而是返回一个得自malloc且偏移一个int大小的指针，没人能够保证它的安全。

// Boost程序库的Pool就是这样一个分配器，它对于最常见的分配大量小型对象很有帮助。

...
// 2.iostreams,覆盖用户自定义缓冲功能、国际化I/O，以及预先定义好的对象cin，cout，cerr和clog。
// 3.国际化支持，包括多区域能力。
//   像wchar_t和wstring等类型都对促进Unicode有所帮助。
// 4.数值处理，包括复数模板和纯数值数组。
// 5.异常阶层体系，包括base class exception以及derived class logic_error和runtime_error，
//   以及更深继承的各个class。
// 6.C89标准程序库，1989C标准程序库内的每个东西也都被覆盖于C++内。

// TR1详细叙述了14个新组件，统统都放在std命名空间内，更正确地说是在其嵌套命名空间tr1内：

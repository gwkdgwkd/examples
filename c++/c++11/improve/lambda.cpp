...
// 继Python、Java、C#、PHP等众多高级编程语言都支持lambda匿名函数后，
// C++11标准终于引入了lambda。
...
namespace n1 {
// lambda匿名函数中的[外部变量]是指和当前lambda表达式位于同一作用域内的所有局部变量:
...
// [val1,val2,...]：
// 表示以值传递的方式导入val1、val2等外部变量，同时多个变量之间没有先后次序；
void func4() {
  int i = 4;
  float f = 4.4;
  std::string s = "func4";
...
// [&val1,&val2,...]：
// 表示以引用传递的方式导入val1、val2等外部变量，多个变量之间没有前后次序；
void func5() {
  int i = 4;
  float f = 5.5;
  std::string s = "func5";
...
// [=,&val1,...]：
// 表示除val1以引用传递的方式导入外，其它外部变量都以值传递的方式导入。
void func7() {
  int i = 6;
  float f = 7.7;
  std::string s = "func7";

...
// 且矩阵的乘法运算没有交换律，即A*B和B*A是不一样的。
// 计算方法是：用矩阵A的第i行和矩阵B中的每一列j对应的数值做乘法运算，乘积一一相加，
// 所得结果即为矩阵C中第i行第j列的值。
// 在稀疏矩阵做乘法运算时，由于本身矩阵中含有的非0元素少，
// 普通算法会出现很多0*0或者k*0或者0*k（k代表非0元素值）的情况。
// 行逻辑链接的顺序表解决矩阵乘积算法:
// 1.首先，找到矩阵A中第一行的非0元素，分别是A11=3和A14=5；
//   由于行逻辑链接的顺序表中存储的都是非0元素，
//   查找的过程就需要使用记录每行第一个非0元素的首地址的数组来完成。
// 2.用3去和B中对应的第一行中的非0元素相乘，矩阵B中第一行非0元素是B12=2，所以3*2=6，
//   因为6是A11和B12相乘的结果，所以暂时存放在C12中；
//   用5去和B中对应的第4行的非0元素相乘，由于矩阵B中第4行没有非0元素，所以，第一行的计算结束；
// 3.以此类推。
// 现在，解决问题的关键在于，如何知道顺序表中存放的非0元素是哪一行的呢？
// 解决方案：由于使用的是行逻辑链接的顺序表，所以，已经知道了每一个矩阵中的每一行有多少个非0元素，
// 而且第一行的第一个非0元素的位置一定是1。
// 所以，第n行的非0元素的位置范围是：大于或等于第n行第一个元素的位置，
// 小于第n+1行第一个元素的位置（如果是矩阵的最后一行，小于矩阵中非0元素的个数+1）。
// 当稀疏矩阵Amn和稀疏矩阵Bn采用行逻辑链接的顺序表做乘法运算时，
// 在矩阵A的列数（矩阵B的行数）n不是很大的情况下，算法的时间复杂度相当于O(m*p)，比普通算法要快很多。

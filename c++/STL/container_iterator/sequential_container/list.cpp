...
// 这意味着，list容器中的元素可以分散存储在内存空间里，而不是必须存储在一整块连续的内存空间中。
// list容器中各个元素的前后顺序是靠指针来维系的，每个元素都配备了2个指针，分别指向它的前一个元素和后一个元素。
// 其中第一个元素的前向指针总为null，因为它前面没有元素；同样，尾部元素的后向指针也总为null。
// 基于这样的存储结构，list容器具有一些其它容器（array、vector 和 deque）所不具备的优势，
...
// list容器还有一个std::swap(x,y)非成员函数（其中x和y是存储相同类型元素的list容器），
// 它和swap()成员函数的功能完全相同，仅使用语法上有差异。
// C++11新添加的begin()和end()全局函数也同样适用于list容器。
// 即当操作对象为list容器时，其功能分别begin()、end()成员函数相同。

// 和array、vector、deque容器的迭代器相比，list容器迭代器最大的不同在于，
// 其配备的迭代器类型为双向迭代器，而不再是随机访问迭代器。
// 这意味着，假设p1和p2都是双向迭代器，则它们支持使用++p1、p1++、p1--、p1++、*p1、p1==p2以及p1!=p2运算符，
// 但不支持以下操作（其中i为整数）：
...
// 在迭代器pos指定的位置之前插入n个元素 elem，并返回表示第一个新插入元素位置的迭代器。
// iterator insert(pos,first,last)
// 在pos位置之前，插入（array、vector、deque等）中位于[first,last)区域的所有元素，
// 并返回表示第一个新插入元素位置的迭代器。
// iterator insert(pos,initlist)
// 在迭代器pos指定的位置之前，插入初始化列表中所有的元素，并返回表示第一个新插入元素位置的迭代器。

// 对list容器存储的元素执行删除操作，需要借助该容器模板类提供的成员函数。
// 幸运的是，相比其它STL容器模板类，list模板类提供了更多用来实现此操作的成员函数
...
// 恰恰相反，各元素占用的存储空间（又称为节点）是独立的、分散的，它们之间的线性关系通过指针来维持。
...
//     //...
// }
// list容器定义的每个节点中，都包含*prev、*next和myval。
// 其中，prev指针用于指向前一个节点；next指针用于指向后一个节点；myval用于存储当前元素的值。
// 和array、vector这些容器迭代器的实现方式不同，由于list容器的元素并不是连续存储的，所以该容器迭代器中，
// 必须包含一个可以指向list容器的指针，并且该指针还可以借助重载的*、++、--、==、!=等运算符，
// 实现迭代器正确的递增、递减、取值等操作。
// template<tyepname T,...>
...
// SGI STL标准库中，list容器的底层实现为双向循环链表，相比双向链表结构的好处是在构建list容器时，
...
// 使用此方法都可以保证在O(1)时间复杂度内完成对“容器是否为空”的判断；
// 但对于list容器来说，使用size()成员方法判断“容器是否为空”，可能要消耗O(n)的时间复杂度，这个结论适用于很多容器。
// 为什么list容器这么特殊呢？这和list模板类提供了独有的splice()成员方法有关。
// 在实现list容器的过程中我们发现，用户经常需要知道当前list容器中存有多少个元素，
// 于是我们想让size()成员方法的执行效率达到O(1)。
// 为了实现这个目的，我们必须重新设计list，使它总能以最快的效率知道自己含有多少个元素。
// 要想令size()的执行效率达到O(1)，最直接的实现方式是：
// 在list模板类中设置一个专门用于统计存储元素数量的size变量，其位于所有成员方法的外部。
// 与此同时，在每一个可用来为容器添加或删除元素的成员方法中，添加对size变量值的更新操作。
// 由此，size()成员方法只需返回size这个变量即可达到时间复杂度O(1)。
// 不仅如此，由于list容器底层采用的是链式存储结构（也就是链表），该结构最大的特点就是，
// 某一链表中存有元素的节点，无需经过拷贝就可以直接链接到其它链表中，且整个过程只需要消耗O(1)的时间复杂度。
// 考虑到之所以选用list容器，就是看中了其底层存储结构的这一特性。
// 因此，作为list容器设计者的我们，自然也想将splice()方法的时间复杂度设计为O(1)。
// 这里就产生了一个矛盾，即如果将size()设计为O(1)时间复杂度，则由于splice()成员方法会修改list容器存储元素的个数，
// 因此该方法中就需要添加更新size变量的代码（无疑是通过遍历链表来实现），意味着splice()成员方法的执行效率将无法达到O(1)；
// 反之，如果将splice()成员方法的执行效率提高到O(1)，则size()成员方法将无法实现O(1)的时间复杂度。
// 也就是说，list容器中的size()和splice()总有一个要做出让步，即只能实现其中一个方法的执行效率达到O(1)。
// 值得一提的是，不同版本的STL标准库，其底层解决此冲突的抉择是不同的。
// 有些版本选择将splice()成员方法的执行效率达到O(1)，而size()成员方法的执行效率为O(n)。
...
void func7() {
  ...
  // position为迭代器，用于指明插入位置；x为另一个list容器。
  // 此格式的splice()方法的功能是，将x容器中存储的所有元素全部移动当前list容器中position指明的位置处。
  // void splice (iterator position, list& x, iterator i);
  // position为迭代器，用于指明插入位置；x为另一个list容器；i也是一个迭代器，用于指向x容器中某个元素。
  // 此格式的splice()方法的功能是将x容器中i指向的元素移动到当前容器中position指明的位置处。
  // void splice (iterator position, list& x, iterator first, iterator last);
  // position为迭代器，用于指明插入位置；x为另一个list容器；first和last都是迭代器，
  // [fist,last)用于指定x容器中的某个区域。
  // 此格式的splice()方法的功能是将x容器[first, last)范围内所有的元素移动到当前容器position指明的位置处。
...
void func8() {
  // list容器在进行插入（insert()）、接合（splice()）等操作时，都不会造成原有的list迭代器失效，

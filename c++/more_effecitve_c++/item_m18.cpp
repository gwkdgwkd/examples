...
// 有三种方法实现这三种函数：
// 1.使用eager evaluation(热情计算法)，
//   当min，max和avg函数被调用时，我们检测集合内所有的数值，然后返回一个合适的值。
// 2.使用lazy evaluation（懒惰计算法）,
//   只有确实需要函数的返回值时我们才要求函数返回能用来确定准确数值的数据结构。
// 3.使用over-eager evaluation（过度热情计算法），我们随时跟踪目前集合的最小值，
//   最大值和平均值，这样当min,max或avg被调用时，我们可以不用计算就立刻返回正确的数值。
// 如果频繁调用min,max和avg，我们把跟踪集合最小值、最大值和平均值的开销分摊到所有这些函数的调用上，
// 每次函数调用所分摊的开销比eager evaluation或lazy evaluation要小。

// 隐藏在over-eager evaluation后面的思想是如果你认为一个计算需要频繁进行，
// 你就可以设计一个数据结构高效地处理这些计算需求，这样可以降低每次计算需求时的开销。
...
// catching是一种分摊期望的计算开销的方法。
// Prefetching(预提取)是另一种方法。
// 你可以把prefech想象成购买大批商品而获得的折扣。
...
// prefetch在高端应用里也有优点。
// 例如你为dynamic数组实现一个模板，dynamic就是开始时具有一定的尺寸，
...
T& DynArray<T>::operator[](int index) {
  ...
  if (index > 5) {  // 当前最大的索引值
    ...
    // 但是调用new会触发operator new，operator new(和 operator delete)的调用通常开销很大。
    // 因为它们将导致底层操作系统的调用，系统调用的速度一般比进程内函数调用的速度慢。
    // 因此我们应该尽量少使用系统调用。
  }
  // 返回index位置上的数组元素;
}
// 使用Over-eager evaluation方法，其原因我们现在必须增加数组的尺寸以容纳索引i，
// 那么根据位置相关性原则我们可能还会增加数组尺寸以在未来容纳比i大的其它索引。
// 为了避免为扩展而进行第二次（预料中的）内存分配，我们现在增加DynArray的尺寸比能使i合法的尺寸要大，
// 我们希望未来的扩展将被包含在我们提供的范围内。
// template <class T>
// T& DynArray<T>::operator[](int index) {
...
// 即通过over-eager方法分摊预期计算的开销，
// 例如caching和prefething，并不与lazy evaluation的建议相矛盾。
// 当你必须支持某些操作而不总需要其结果时，

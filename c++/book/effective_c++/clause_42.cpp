...
// 从C++的角度来看，在template声明中，class和typename没有不同：
template <class T>
class Widget;
template <typename T>
class Widget;
...
// C::const_iterator不依赖任何template参数的名称，这是所谓的非从属名称。
...
// 1.type不可以出现在base class list内的嵌套从属名称之前，
// 2.也不可以在member initialization list中作为base class修饰符。
// template <typename T>
// class Derived :
...
void workWithIteraror(IterT iter) {
  ...
  typedef typename std::iterator_traits<IterT>::value_type vtype;
  vtype temp1(*iter);
}

// typename相关规则在不同的编译器上有不同的实践。
// 意味着typename和嵌套从属类型名称之间的规则，也许会在移植方面带来麻烦。

// 请记住：
// 1.声明template参数时，前缀关键字class和typename可互换；
// 2.用typename标识嵌套从属类型名称，但不得在基类列表和成员初值列表内使用typename。

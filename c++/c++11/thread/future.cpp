...
// 很轻松解决从异步任务中返回结果。
// 当在一个线程中创建（通过std::async，
// std::packaged_task或者std::promise）了一个异步操作（线程）时，
// 这个异步操作会返回一个future对象给当前的线程，供其访问异步操作的状态，结果等等。

// std::future_errc继承于C++标准异常体系中的logic_error。
// broken_promise 0
// 与该std::future共享状态相关联的std::promise对象在设置值或者异常之前一被销毁。
// future_already_retrieved 1
// 与该std::future对象相关联的共享状态的值已经被获取了，即调用了get函数。
// promise_already_satisfied 2
// std::promise对象已经对共享状态设置了某一值或者异常。
// no_state 3
// 无共享状态。

// std::future_status类型主要用在std::future(或std::shared_future)中
// 的wait_for和wait_until两个函数中的。
// ready 0
// wait_for(或wait_until)因为共享状态的标志变为ready而返回。
// timeout 1
// 超时，即wait_for(或wait_until)因为在指定的时间段（或时刻）
// 内共享状态的标志依然没有变为ready而返回。
// deferred 2
// 共享状态包含了deferred函数。
...
// 一般来说，只有当valid()返回true的时候才调用get()去获取结果，这也是C++推荐的操作。
...
void func0() {
  ...
  // 该std::future对象本身已经不和任何共享状态相关联，
  // 因此该std::future的状态不再是valid的了。
...
// 如果不设置值或者异常，promise对象在析构时会自动地设置一个future_error异
// 常(broken_promise)来设置其自身的准备状态。
...
// 4.拷贝构造函数，被禁用。
// 5.移动构造函数。
// 与std::promise类似，std::packaged_task也禁用了普通的赋值操作运算，
// 只允许move赋值运算。
...
// std::packaged_task::get_future返回一个与packaged_task对
// 象共享状态相关的future对象。
// 返回的future对象可以获得由另外一个线程在该packaged_task对象
// 的共享状态上设置的某个值或者异常。
...
// 1.如果调用包装的对象成功，
//   则返回值（如果被包装的对象有返回值的话）被保存在packaged_task的共享状态中。
...
// 被包装的任务在构造时指定，
// 因此调用operator()的效果由packaged_task对象构造时所指定的可调用对象来决定：
...
void func3() {
  ...
  // std::packaged_task::reset()重置的共享状态，但是保留之前的被包装的任务
  task.reset();
...
// std::async将std::future、std::promise和std::packaged_task三者结合了起来。
...
// 2.launch::deferred：
//   异步任务将会在共享状态被访问时调用，相当与按需调用（即延迟(deferred)调用）。

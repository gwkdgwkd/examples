...
// 如果把关联容器中一个元素的键改变了，这将会打破容器的有序性。

namespace n1 {
void func() {
  // 对于map和multimap尤其简单，因为如果有程序试图改变这些容器中的键，
  // 将不能通过编译，除非使用强制类型转换：
...
// 如果不关心可移植性，而想改变set和multiset中的元素的值，
// 并且STL的实现允许这样做，那就做，只要不修改键就好了。
// 如果重视可移植性，就要确保set和multiset中的元素不能被修改。
// 至少不能未经过强制类型转换就修改。

// 对于set和multiset，如果直接对容器中的元素做了修改，那么要保证该容器仍然是排序的。

void func() {
  // 下面的方法对set和multiset没问题，但对于map和multimap，情况就复杂了。
  // map和multimap中保存的是pair，pair的第一个部分被定义成const，
  // 如果试图强制转换掉const属性，结果将可能会改变键部分。
  // 理论上，一个STL实现可以把这样的值写在一个只读的内存区域，这时要是尝试修改，
  // 最好的结果将是没有效果，如果坚持要遵从C++标准所制定的规则，
  // 那就永远都不要试图修改在map和multimap中作为键的对象。

  // 但对set和multiset是可能的，容器中元素类型是T，而不是const T。
  // 只要愿意，随时可以改变set或multiset中的元素，而无需任何强制类型转换。
  ...
  if (i != se.end()) {
    ...
    // set迭代器解引用后是一个指向该集合中元素的const引用，无法修改。
    // C++在这一点上没有一个统一的说法，不同的实现者以不同的方式做了解释。
    // i->setTitle("Corporate Deity");
    // 标准模棱两可，所有试图修改set和multiset中元素的代码是不可移植的。
    ...
    // 以一种总是可行且安全的方式修改set、multiset、map和multimap中的元素，
    // 可以分为5个简单步骤：
    // 1.找到想修改的容器的元素；
    // 2.为将要被修改的元素做一份拷贝，在map和multimap的情况下，
    //   不要把该拷贝的第一个部分声明为const，毕竟，你想要改变它；
    Employee e(*i);
    // 3.修改该拷贝，是它具有期望它在容器中的值；
    e.setTitle("Corporate Deity");
    // 4.把该元素从容器中删除，通常是通过erase来进行；
    se.erase(i++);
    // 5.把新值插入到容器中，按照容器的排序顺序，
    //   新元素的位置可能与被删除元素的位置相同或紧邻，则使用提示形式的insert，
    //   以便把插入效率从对数时间提高到常数时间，把从第一步得来的迭代器作为提示信息。
    se.insert(i, e);
  }
}
}  // namespace n2

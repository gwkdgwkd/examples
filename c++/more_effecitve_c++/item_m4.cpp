...
// 有时这样的方法是不错的。例如一些行为特性与数字相仿的对象被初始化为空值或不确定的值也是合理的，
// 还有比如链表、哈希表、图等等数据结构也可以被初始化为空容器。
// 但不是所有的对象都属于上述类型，对于很多对象来说，不利用外部数据进行完全的初始化是不合理的。

// 无需任何数据即可建立对象的类可以包含缺省构造函数，而需要数据来建立对象的类则不能包含缺省构造函数。
...
// 第一种情况是建立数组时。一般来说，没有一种办法能在建立对象数组时给构造函数传递参数。有三种方法能回避开这个限制。

// 对于类里没有定义缺省构造函数所造成的第二个问题是它们无法在许多基于模板（template-based）的容器类里使用。
// 因为实例化一个模板时，模板的类型参数应该提供一个缺省构造函数，这是一个常见的要求。这个要求总是来自于模板内部，被建立的模板参数类型数组里。
template <class T>
class Array {
 public:
  Array(int size);
...
// 在多数情况下，通过仔细设计模板可以杜绝对缺省构造函数的需求。例如标准的vector模板。

// 第三个问题是，在设计虚基类时所面临的要提供缺省构造函数还是不提供缺省构造函数的两难决策。
...
// 这样的修改使得其他成员函数变得复杂，因为不再能确保EquipmentPiece对象进行了有意义的初始化。

// 提供无意义的缺省构造函数也会影响类的工作效率。如果成员函数必须测试所有的部分是否都被正确地初始化，那么这些函数的调用者就得为此付出更多的时间。
// 而且还得付出更多的代码，因为这使得可执行文件或库变得更大。

// 如果一个类的构造函数能够确保所有的部分被正确初始化，所有这些弊病都能够避免。
// 缺省构造函数一般不会提供这种保证，所以在它们可能使类变得没有意义时，尽量去避免使用它们。
...
int main() {
  ...
  // 对于使用非堆数组（non-heap arrays）（即不在堆中给数组分配内存）的一种解决方法是在数组定义时提供必要的参数：
  int ID1, ID2, ID3;
  ...
  // 如果你为数组分配raw memory，你就可以避免浪费内存。使用placement new方法在内存中构造EquipmentPiece对象：
  ...
  // 这个技术（和指针数组的主意一样）允许你在没有缺省构造函数的情况下建立一个对象数组。
  // 它没有绕过对构造函数参数的需求，实际上也做不到。如果能做到的话，就不能保证对象被正确初始化。
  // 使用placement new的缺点除了是大多数程序员对它不熟悉外（能使用它就更难了），
  // 还有就是当你不想让它继续存在使用时，必须手动调用数组对象的析构函数，然后调用操作符delete[]来释放raw memory。
  for (int i = 9; i >= 0; --i) {
    bestPieces[i].~EquipmentPiece();
  }
  operator delete[](rawMemory);
  // delete [] bestPieces; // 直接删除一个不是用new操作符来分配的内存指针，其结果没有被定义。

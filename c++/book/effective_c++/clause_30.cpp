...
// inline函数除了免除函数调用成本外，编译器还对代码进行优化。
// 没有免费的午餐，inline函数也不例外，
// 对此函数的每一个调用都以函数本体替换，可能增加目标码大小。
// inline只是对编译器的一个申请，不是强制命令。
// 这项申请可以隐喻提出，也可以明确提出，隐喻方式是将函数定义在class内。
class Person {
 public:
 ...
 private:
  int theAge;
  // friend函数也可以被定义于class内，也是被隐喻声明的inline：
...
// 让好多人认为函数模板一定是inline，这个结论不但无效而且有可能有害。
// inline函数一般被置于头文件内，因为大多数编译器在编译时进行inline，
...
// 如果编译器不知道调用哪个函数，怎么替换？
// 一个表面上看似inline的函数是否真是inline，主要取决于编译器。
// 幸运的是大多数编译器提供了诊断级别，无法inline会给警告信息。

// 有时虽然编译器有意愿inline某个函数，但还是可能为该函数生成函数本体。
// 如果程序要取某个函数的地址，编译器通常必须为此函数生成一个函数本体。
// 毕竟编译没有能力给出一个指向并不存在的函数的指针。
// 编译器通常不对通过函数指针而进行的调用进行inline，
// 调用inline函数有可能被inline，也可能不被inline，取决于调用方式。
// 即使从未使用函数指针，也可能不inline。
// 因为程序员并非唯一要求函数指针的人，
// 有时候编译器会生成构造和析构的outline副本，如此来获得指向函数的指针。

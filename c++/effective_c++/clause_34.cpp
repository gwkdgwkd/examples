...
class Shape {
 ...
 public:
  ...
  // 声明impure virtual函数的目的是让derived class继承该函数的接口和缺省实现。
  ...
  // 声明为non-virtual函数的目的是为了让derived class继承函数的接口及一份强制性实现。
...
class ModelC : public Airplane {};  // 未声明fly函数(忘记写了)，调用默认的fly

// 问题不在于有缺省行为，而在于ModelC在未明确说明“我要”的情况下就继承了该缺省行为。
// 切断“virtual函数接口”和其“缺省实现”之间的连接的伎俩：
class Airplane1 {
 public:
...
void Airplane1::defaultFly(const Airport& destination) {
  // 是non-virtual函数，没有任何一个derived class应该重新定义此函数
...
class ModelC1 : public Airplane1 {
 public:
  // 现在ModelC1不可能意外继承不正确的flay实现了，因为如果不自己实现，编译不通过
  virtual void fly(const Airport& destination);
};

...
// 因此如果你想让这三个operator同时存在并具有你所期望的关系，就必须自己实现它们。
// 同理，operator -, *, /,等等也一样。

// 确保operator的赋值形式（assignment version）（例如operator+=）
// 与一个operator的单独形式（stand-alone）(例如operator+)之间存在正常的关系，
...
// 在这里值得指出的是三个效率方面的问题:
// 1.总的来说operator的赋值形式比其单独形式效率更高，因为单独形式要返回一个新对象，
//   从而在临时对象的构造和释放上有一些开销。
...
//   result += c; // 不用临时对象
//   result += d; // 不用临时对象
//   前者比较容易编写、debug和维护，并且在80％的时间里它的性能是可以被接受的后者具有更高的效率。
//   你可以让客户端开发人员用更容易阅读的单独形式的operator来开发和debug代码，
//   同时保留用效率更高的operator赋值形式替代单独形式的权力。
//   而且根据operator的赋值形式实现其单独形式，这样你能确保当客户端从一种形式切换到另一种形式时，
//   操作的语义可以保持不变。

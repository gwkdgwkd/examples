...
// 为了让Widget重新定义Timer内的virtual函数，
// Widget必须继承自Timer。
// 但public继承并不适当，因为Widget并不是个Timer。
// Widget客户总不该能够对着一个Widget调用onTick吧，
// 因为观念上那并不是Widget接口的一部分。
// 必须以private形式继承Timer：
class Widget1 : private Timer {
 private:
  virtual void onTick() const;
};
// Timer的public onTick函数在Widget内变成private，
// 而我们重新声明定义时仍把它留在那儿。
...
// 相比于比private继承的优点：
// 1.要使Widget拥有derived class，
//   但同时可能想阻止derived class重新定义onTick。
//   如果继承自Timer，这个想法不可能实现，private继承也不可能。
//   但如果WidgetTimer是Widget内部的一个private成员并继承Timer，
//   Widget的derived class将无法取用WidgetTimer，
//   因此无法继承或重新定义它的virtual函数。
//   Java或C#中有阻止derived class重新定义virtual函数的能力，
//   现在可以在C++中模拟它了。
...
//   Widget可以只带一个简单的WidgetTimer声明就可以，
//   不再需要include任何与Timer有关的东西。
//   对于大型系统而言，如此解耦可能是重要的措施。

// private继承主要用于：
// 当一个derived class子类访问base class的protected成分，
// 或为了重新定义一个或多个virtual函数。
// 有一种情况涉及空间最优化，可能会促使你选择private继承而不是继承加复合。
// 只适用于处理的class不带任何数据时。
// 这样的class没有non-static成员变量，
// 没有virtual函数（会有vptr），也没有virtual base class。
// 这种所谓的empty class对象不使用任何空间，
// 因为没有任何隶属对象的数据需要存储。
...
// 现实中的empty class并不是真的empty。
// 虽然从未拥有non-static成员变量，
...
// 当面对并不存在is-a关系的两个class，
// 其中一个需要访问另一个的protected成员，
// 或需要重新定义其一个或多个virtual函数，
...
int main() {
  ...
  // sizeof(HoldsAnInt1)>sizeof(int) 一个Empty成员变量竟然要求内存。
  // 大多数编译器中sizeof(Empty)为１，
  // 因为面对大小为零的独立（非附属）对象，

...
// thread_local关键词只对声明于命名空间作用域的对象、声明于块作用域的对象及静态数据成员允许，它指示对象拥有线程存储期。
// 它能与static或extern结合，以分别指定内部或外部链接（除了静态数据成员始终拥有外部链接），但附加的static不影响存储期。
// 线程存储期: 对象的存储在线程开始时分配，而在线程结束时解分配，每个线程拥有其自身的对象实例。
// 唯有声明为thread_local的对象拥有此存储期。thread_local能与static或extern一同出现，以调整链接。
...
namespace local {
// 本地变量在线程中是持续存在的，不同于普通临时变量的生命周期，它具有static变量一样的初始化特征和生命周期。
// 虽然是局部变量，但是在每个线程的每次for循环中，使用的都是线程中的同一个变量，也印证了thread_local变量会自动static。
void func(const std::string& thread_name) {
  for (int i = 0; i < 3; ++i) {
  ...
  // x++;  // thread_local虽然改变了变量的存储周期，但是并没有改变变量的使用周期或者说作用域
  return;
}
...
// 虽然createA()看上去被调用了多次，实际上只被调用了一次，
...
class B {
 public:
  ...
  ~B() {}
  // b_key是thread_local，虽然其也是static的，但是每个线程中有一个，每次线程中的所有调用共享这个变量。

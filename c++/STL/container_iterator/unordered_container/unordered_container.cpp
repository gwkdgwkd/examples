...
// C++ STL底层采用哈希表实现无序容器时，
// 会将所有数据存储到一整块连续的内存空间中，
// 并且当数据存储位置发生冲突时，解决方法选用的是链地址法（又称开链法）。
// 基于底层实现采用了不同的数据结构，因此和关联式容器相比，
// 无序容器具有以下2个特点：
// 1.无序容器内部存储的键值对是无序的，
//   各键值对的存储位置取决于该键值对中的键，
//   和关联式容器相比，无序容器擅长通过指定键查找对应的值，
//   平均时间复杂度为O(1)；
// 2.对于使用迭代器遍历容器中存储的元素，
//   无序容器的执行效率则不如关联式容器。
...
// 分别为unordered_map、unordered_multimap、
// unordered_set以及unordered_multiset。
// unordered_map存储键值对<key,value>类型的元素，
// 其中各个键值对键的值不允许重复，且该容器中存储的键值对是无序的。
// unordered_multimap和unordered_map唯一的区别在于，
// 该容器允许存储多个键相同的键值对。
// unordered_set不再以键值对的形式存储数据，而是直接存储数据元素本身，
// 元素不能重复，而且是无序的。
// unordered_multiset和unordered_set唯一的区别在于，
// 该容器允许存储值相同的元素。

// C++11标准的STL中，在已提供有4种关联式容器的基础上，
// 又新增了各自的unordered版本，提高了查找指定元素的效率。

...
// lvalue是loactor value的缩写，可意为存储在内存中、有明确存储地址（可寻址）的数据；
// 而rvalue译为read value，指的是那些可以提供数据值的数据，
// 不一定可以寻址，例如存储于寄存器中的数据。

// 很多初学者都感觉右值引用晦涩难懂，其实不然，右值引用只不过是一种新的C++语法。
// 真正理解起来有难度的是基于右值引用引申出的2种C++编程技巧，移动语义和完美转发。

int main() {
  // 在C++中，一个表达式不是左值就是右值。
  // 通常情况下，判断某个表达式是左值还是右值，
  // 最常用的有以下2种方法（只适用于大部分场景）:
  // 1.可位于赋值号（=）左侧的表达式就是左值；
  ...
  // 反之，字面量5、10（通常存储在寄存器中，或者和代码存储在一起），
  // 既无名称，也无法获取其存储地址，所以是右值。

  // C++右值引用:
  // 右值引用可以从字面意思上理解，指的是以引用传递（而非值传递）的方式使用C++右值。
  // 其实C++98/03标准中就有引用，使用&表示。
  // 但非常量左值引用有一个缺陷，即正常情况下只能操作C++中的左值，无法对右值添加引用：
  int num = 10;
  // int &c = 10; // 错误
  int &b1 = num;  // 正确
  // 编译器允许为num左值建立一个引用，但不可以为10这个右值建立引用。
  ...
  // C++11标准中对右值做了更细致的划分：
  // 1.分别称为纯右值Pure value，简称pvalue；
  // 2.将亡值eXpiring value，简称xvalue。
  // 其中纯右值就是C++98/03标准中的右值，而将亡值则指的是和右值引用相关的表达式，
  // 比如某函数返回的T&&类型的表达式。
  // 对于纯右值和将亡值，都属于右值。

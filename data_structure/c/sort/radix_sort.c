...
// 基数排序不同于其他的各类排序，其他的排序或多或少的是通过使用比较和移动记录来实现排序，
// 而基数排序的实现不需要进行对关键字的比较，只需要对关键字进行分配与收集两种操作即可完成。
// 基数排序算法适用于对多个整数或者多个字符串进行升序或降序排序。
// 按照个位-十位-百位的顺序进行基数排序，此种方式是从最低位开始排序，
// 所以被称为最低位优先法，简称LSD法。
// 按照百位-十位-各位的顺序进行排序，称为最高位优先法，简称MSD法。
// 使用该方式进行排序同最低位优先法不同的是：
// 当无序表中的关键字进行分配后，相当于进入了多个子序列，后序的排序工作分别在各个子序列中进行，
// 最低位优先法每次分配与收集都是相对于整个序列表而言的。
...
typedef struct {
  ...
  int next;  // 指针，用于是静态链表，所以每个结点中存储着下一个结点所在数组中的位置下标
} SLCell;
// 静态链表结构
typedef struct {

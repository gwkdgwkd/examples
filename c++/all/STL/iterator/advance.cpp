...
// C++ STL标准库中还提供有一些辅助函数：
// advance(it,n)        it是迭代器，n是整数，将it迭代器前进或后退n个位置
// distance(first,last) first和last都是迭代器，计算first和last之间的距离
...
// 对于prev和next，it至少为双向迭代器。

// advance()函数用于将迭代器前进（或者后退）指定长度的距离，其语法格式如下：
// template <class InputIterator, class Distance>
// void advance(InputIterator& it, Distance n);
// 其中it指的是目标迭代器，n通常为一个整数。
// 如果it为输入迭代器或前向迭代器，则n必须为正数，即表示将it右移（前进）n个位置；
// 如果it为双向迭代器或随机迭代器，则n为正数表示it右移n，n为负数表示it左移n。

// 根据it类型是否为随机访问迭代器，advance()底层采用了不同的实现机制：
// 1.当it为随机访问迭代器时，由于该类型迭代器支持p+n或者p-n运算，
...
// 如果it迭代器的移动位置超出了合理范围，此时使用*it将会导致程序崩溃。

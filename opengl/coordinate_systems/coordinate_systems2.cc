...
int main()
{
    ...
    // -----------------------------
    // 告诉OpenGL我们想要启用深度测试；它默认是关闭的。
    glEnable(GL_DEPTH_TEST); // 可以通过glEnable函数来开启深度测试。
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 因为我们使用了深度测试，我们也想要在每次渲染迭代之前清除深度缓冲（否则前一帧的深度信息仍然保存在缓冲中）。就像清除颜色缓冲一样，
        ...
        // render box
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36); // 使用glDrawArrays来绘制立方体，但这一次总共有36个顶点。
...
// OpenGL存储它的所有深度信息于一个Z缓冲(Z-buffer)中，也被称为深度缓冲(Depth Buffer)。GLFW会自动为你生成这样一个缓冲（就像它也有一个颜色缓冲来存储输出
// 图像的颜色）。深度值存储在每个片段里面（作为片段的z值），当片段想要输出它的颜色时，OpenGL会将它的深度值和z缓冲进行比较，如果当前的片段在其它片段之后，它
// 将会被丢弃，否则将会覆盖。这个过程称为深度测试(Depth Testing)，它是由OpenGL自动完成的。
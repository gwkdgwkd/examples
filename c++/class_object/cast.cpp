...
// 所以能做的事情仅仅是重新解释数据的二进制位，但无法对数据的二进制位做出修正。
// 这就是隐式类型转换和强制类型转换最根本的区别。
// 这里说的修改数据并不是修改原有的数据，
// 而是修改它的副本（先将原有数据拷贝到另外一个地方再修改）。
...
// static_cast是静态转换的意思，也就是在编译期间转换，
// 转换失败的话会抛出一个编译错误。
...
// 总结：
// pa是A*类型的指针，当pa指向A类型的对象时，向下转型失败，pa不能转换为B*或C*。
// 当pa指向D类型的对象时，向下转型成功，pa可以转换为B*或C*。
// 同样都是向下转型，为什么pa指向的对象不同，转换的结果就大相径庭呢？
// pa指向A类对象，根据该对象找到的就是A的类型信息，当程序从这个节点开始向上遍历时，
// 发现A的上方没有要转换的B类型或C类型（实际上没有任何类型了），所以就转换败了。
...
// 造成这种假象的根本原因在于，派生类对象可以用任何基类的指针指向它，这样做始终是安全的。

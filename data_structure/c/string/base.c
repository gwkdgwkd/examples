...
// 串存储结构，也就是存储字符串的数据结构。
// 很明显，字符串之间的逻辑关系也是一对一，用线性表的思维不难想出，串存储结构也有顺序存储和链式存储。
...
// 串的定长顺序存储结构，可以简单地理解为采用"固定长度的顺序存储结构"来存储字符串，
// 因此限定了其底层实现只能使用静态数组。
// 例如，采用定长顺序存储结构存储"data.biancheng.net"，通过目测得知此字符串长度为18，
// 因此我们申请的数组空间长度至少为19（最后一位存储字符串的结束标志'\0'）。

// 串的堆分配存储结构
// 通常，编程语言会将程序占有的内存空间分成多个不同的区域，程序包含的数据会被分门别类并存储到对应的区域。
// 拿C语言来说，程序会将内存分为4个区域，分别为堆区、栈区、数据区和代码区。
...
// 串的块链存储结构
// 实现串的块链存储使用的是无头节点的单链表。
// 当然根据实际需要，你也可以自行决定所用链表的结构（双向链表还是单链表，有无头节点）。
// 链表各节点存储数据个数的多少可参考以下几个因素：
// 1.串的长度和存储空间的大小：
//   若串包含数据量很大，且链表申请的存储空间有限，此时应尽可能的让各节点存储更多的数据，
//   提高空间的利用率（每多一个节点，就要多申请一个指针域的空间）；
//   反之，如果串不是特别长，或者存储空间足够，就需要再结合其他因素综合考虑；
// 2.程序实现的功能：如果实际场景中需要对存储的串做大量的插入或删除操作，则应尽可能减少各节点存储数据的数量；
//   反之，就需要再结合其他因素。

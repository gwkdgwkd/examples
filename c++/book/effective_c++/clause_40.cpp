...
void func() {
  ...
  // checkOut的调用是模棱两可的，即使两个函数中只有一个能用（有一个private）。
  // 这与C++用来解析重载函数调用的规则相符：
  // 在看到是否有个函数可用之前，C++首先确认这个函数对此调用是否为最佳匹配，
  // 找到最佳匹配函数后才检验可取用性。
...
namespace n2 {
// 多重继承是继承一个以上的base class，一般base class没有更高级的base class，
// 否则会导致要命的钻石型多重继承：
class File {};
namespace test1 {
...
class IOFile1 : public InputFile, public OutputFile {};
// 从正确行为的观点来看，public继承应该总是virtual。
// 但为了避免继承得来的成员变量重复，编译必须提供若干幕后戏法，后果是：
// 1.使用virtual继承产生的对象，往往比使用non-virtual继承的对象大；
// 2.访问virtual base class的成员变量时，
//   也比访问non-virtual base class的成员变量速度慢。
// 3.virtual继承的成本还包括其他方面，支持virtual base class初始化规则，
...
class DatabaseID {};
class PersonInfo { // 用来协助以各种格式打印数据库字段
 public:
...
std::shared_ptr<IPerson> makePerson(DatabaseID personIdentifier) {
  return std::make_shared<CPerson>(personIdentifier);
}
...
// 多重继承只是面向对象工具箱里的一个工具而已，和单一继承比，它通常比较复杂，
// 使用上也比较难以理解，所以如果同样的设计，单一继承一定比较受欢迎。

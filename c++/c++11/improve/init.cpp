...
// POD类型，PlainOldData，简单来说，是可以直接使用memcpy复制的对象。
// 通俗的讲，一个类或结构体通过二进制拷贝后还能保持其数据不变，
// 那么它就是一个POD类型。
// 对于普通数组和POD类型，
// C++98/03可以使用初始化列表（initializerList）进行初始化，
// 但是这种初始化方式的适用性非常狭窄，
// 只有这两种数据类型可以使用初始化列表。
...
//   如从long double转double或float。
// 3.从整型数隐式转换为浮点数，并且超出浮点数的表示范围，
//   如float=(unsigned long Long)-1，
//   注意这表示将-1先强制转换为unsigned long long，
//   再隐式转换为float。
// 4.从整型数隐式转换为长度较短的整型数，
//   并且超出其表示范围，如char x=65536。
// 在C++98/03中，类型收窄编译器不会报错，
// 而C++11中可以通过列表初始化来检查及防止类型收窄。

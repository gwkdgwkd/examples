...
// encrypted虽然定义却无任何实参作为初值。
// 这意味着调用的是default构造函数。
// 条款4说通过default构造函数构造初对象，
// 然后对它赋值比直接在构造时指定初值效率差。
...
// 如果这样，不仅能够避免构造和析构非必要对象，
// 还可以避免无意义的default构造行为。

// 如果变量只在循环内使用，
// 那么把它定义于循环外并在每次循环迭代时赋值给它比较好，
// 还是该把它定义于循环内？
// Widget w;
...
// 定义在循环外比较高效，特别当n值很大的时候。
// 否则定义在循环体内或许较好。
// 此外定义在循环体外，造成w的作用域比定义在循环内大，

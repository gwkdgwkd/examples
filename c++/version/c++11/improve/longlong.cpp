...
// C++11标准中只限定了每种类型最少占用多少存储空间，不同的平台可以占用不同的存储空间。

// 早在1995年，就有人提议将long long整形写入C++98标准，但被委员会拒绝了。
// 而后longlong整形被C99标准（C语言标准之一）采纳，并逐渐被很多编译器支持，
// 于是C++标准委员会重新决定将long long整形写入C++11标准中。

// 如同long类型整数需明确标注"L"或者"l"后缀一样，要使用longlong类型的整数，
// 也必须标注对应的后缀：对于有符号longlong整形，后缀用"LL"或者"ll"标识。
...
/*
C++11标准中所有的整形数据类型：
整数类型 	                     等价类型 C++11标准规定占用最少位数 short
short int（有符号短整型） 	         至少16位（2个字节） signed short signed
short int
-----------------------------------------------------------------
unsigned short 	              unsigned short int（无符号短整型）
unsigned short int
--------------------------------------------------------------------------------------
int 	                        int（有符号整形） 至少16位（2个字节） signed
signed int
-----------------------------------------------------------------
unsigned 	                    unsigned int（无符号整形）
unsigned int
---------------------------------------------------------------------------------------
long 	                        long int（有符号长整形）
至少32位（4个字节） long int signed long signed long int
-----------------------------------------------------------------
unsigned long 	              unsigned long int（无符号长整形）
unsigned long int
----------------------------------------------------------------------------------------
long long（C++11） 	           long long int（有符号超长整形）
至少64位（8个字节） long long int（C++11） signed long long（C++11） signed long
long int（C++11）
-----------------------------------------------------------------
...
int main() {
  ...
  // 1.LLONG_MIN：代表当前平台上最小的long long类型整数；
  // 2.LLONG_MAX：代表当前平台上最大的long long类型整数；
  // 3.ULLONG_MAX：代表当前平台上最大的unsigned long long类型整数，最小值为0。

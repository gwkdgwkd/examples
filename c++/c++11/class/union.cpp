...
// 在一个联合体内，可以定义多个不同类型的成员，
// 这些成员将会共享同一块内存空间。
// 老版本的C++为了和C语言保持兼容，
// 对联合体的数据成员的类型进行了很大程度的限制，
...
union T {
  // 因为Student类带有自定义的构造函数，
  // 所以是一个非POD类型的，这导致编译器报错，gcc-5.1.0版本报错。
  // 这种规定只是C++为了兼容C语言而制定，
  // 然而在长期的编程实践中发现，这种规定是没有必要的。
  Student s;
  char name[10];
};
...
// 非受限联合体的赋值注意事项：
// C++11规定，如果非受限联合体内有一个非POD的成员，
// 而该成员拥有自定义的构造函数，
// 那么这个非受限联合体的默认构造函数将被编译器删除；
// 其他的特殊成员函数，
// 例如默认拷贝构造函数、拷贝赋值操作符以及析构函数等，也将被删除。
union U1 {
  std::string s;
  int n;
};

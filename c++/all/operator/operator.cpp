...
// 例如，+号可以对不同类型（int、float等）的数据进行加法操作；
// <<既是位移运算符，又可以配合cout向控制台输出数据，C++本身已经进行了重载。
...
// 可以将operator运算符名称这一部分看做函数名。
// 运算符重载函数除了函数名有特定的格式，其它地方和普通函数并没有区别。
// 虽然运算符重载所实现的功能完全可以用函数替代，
// 但运算符重载使得程序的书写更加人性化，易于阅读。
...
//   上述运算符中，[]是下标运算符，()是函数调用运算符，
//   自增自减运算符的前置和后置形式都可以重载。
//   长度运算符sizeof、条件运算符:?、成员选择符.和域解析运算符::不能被重载。
// 2.重载不能改变运算符的优先级和结合性。
// 3.重载不会改变运算符的用法，有几个操作数、操作数在左还是在右，这些都不变。
//   例如~号右边只有一个操作数，+号总是出现在两个操作数之间，重载后也必须如此。
// 4.运算符重载函数不能有默认的参数，否则就改变了操作数的个数，这显然是错误的。
// 5.运算符重载函数既可以作为类的成员函数，也可以作为全局函数。
//   将运算符重载函数作为类的成员函数时，二元运算符的参数只有一个，
//   一元运算符不需要参数，之所以少一个参数，是因为这个参数是隐含的。
//   将运算符重载函数作为全局函数时，二元操作符就需要两个参数，
//   一元操作符需要一个参数，而且其中必须有一个参数是对象，
//   好让编译器区分这是程序员自定义的运算符，防止修改用于内置类型运算符的性质。
//   如果有两个参数，这可以都是对象，也可以一个是对象，一个是内置类型的数据。
//   另外，将运算符重载函数作为全局函数时，一般都需要在类中将该函数声明为友元，
//   原因很简单，该函数大部分情况下都需要使用类的private成员。
// 6.赋值=、下标[]、调用()和成员访问箭头->运算符只能以成员函数的形式重载。
//   也就是说上文提到这些运算符的左操作数必须是该类类型的参数，换句话说，
//   假设c++编译器允许上面的操作符重载函数是全局的，程序员可以任意定义左操作数的类型，
...
class Complex {
 ...
 public:
  // 程序员如果没有为其定义了赋值操作符重载函数，编译器也会隐式的定义，
  // 这样倘若再定义全局赋值运算符重载函数，将会发生二义性：
  Complex &operator=(const Complex &c) {
    std::cout << "operator=" << std::endl;
    return *this;
  }

  // C++规定，下标运算符[]必须以成员函数的形式进行重载，格式如下：
  ...
  // 因为const对象只能调用const成员函数，如果不提供第二种形式，
  // 那么将无法访问const对象的任何元素。

  // 在C++中，类型的名字（包括类的名字）也是一种运算符，即类型强制转换运算符。
  // 它是单目运算符，也能被重载，但只能重载为成员函数，不能重载为全局函数：
  operator double() {
  ...
  Complex *operator->() {  // 需要用对象调用，而不是指针
    std::cout << "->" << std::endl;
    return this;
  }
  ...
  // 不过，类的成员函数不能对称地处理数据，
  // 必须在（参与运算的）所有类型的内部都重载当前的运算符。
  // 所以C++进行了折中方案，允许以全局函数（友元函数）的形式重载运算符。
  // C++创始人Bjarne Stroustrup也曾考虑过为内置类型定义额外运算符的问题，
  // 但后来还是放弃了这种想法，因为Bjarne Stroustrup不希望改变现有规则：
  // 任何类型（内置类型或用户自定义类型）都不能在其定义完成以后再增加额外的操作。
  // 这里还有另外的一个原因，C内部类型之间的转换已经够肮脏了，决不能再向里面添乱。
  // 而通过成员函数为已存在的类型提供混合运算的方式，从本质上看，
  // 比所采用的全局函数（友元函数）加转换构造函数的方式还要肮脏许多。
  // 采用全局函数能定义这样的运算符，它们的参数具有逻辑的对称性。
  // 与此相对应的，把运算符定义为成员函数能够保证，
  // 在调用时对第一个（最左的）运算对象不出现类型转换，
  // 也就是C++不会对调用成员函数的对象进行类型转换。
  // 总结来说，有一部分运算符重载既可以是成员函数也可以是全局函数，
  // 虽然没有一个必然的、不可抗拒的理由选择成员函数，
  // 但应该优先考虑成员函数，这样更符合运算符重载的初衷；
  Complex &operator+=(const Complex &c) {
  ...
  // 内存管理运算符new、new[]、delete和delete[]也可以进行重载，
  // 既可以是类的成员函数，也可以是全局函数。
  // 无论是作为成员函数还是作为全局函数，new的第一个参数必须是size_t类型。
  // size_t表示的是要分配空间的大小，对于new[]的重载函数而言，
  // size_t则表示所需要分配的所有空间的总和。
  void *operator new(size_t size) {
  ...
  // 如果要使用cout和cin处理自定义类型，就必须以全局函数的形式重载<<和>>，
  // 否则就要修改标准库中的类，这显然不是所期望的。
  // 由于采用了引用的方式进行参数传递，并且也返回了对象的引用，
  // 所以重载后的运算符可以实现连续输出。
...
void func4() {
  // 编译器看到使用new创建自己定义的类的对象时，
  // 它选择成员版本的operator new()而不是全局版本的new()：
  Complex *c1 = new Complex();             // m new
  Complex *c2 = new Complex[2];            // m new[]
  std::cout << c1->real() << std::endl;    // 0
  std::cout << (*c1).imag() << std::endl;  // 0
  delete c1;                               // m delete
  delete[] c2;                             // m delete[]

  // 但是全局版本的new和delete仍为所有其他类型对象使用，
  // 除非它们也有自己的new和delete：
  int *i = new int;     // g new
...
void func() {
  ...
  Complex c9 = c2 - 11.0;
  // 转换为：Complex c9 = c2.operator-(Complex(11.0));
  std::cout << c9 << std::endl;  // 32+21i
  // Complex c10 = 11.0 - c2;
  // 转换为：Complex c10 = (11.0).operator+(c2);
  // 显然是错误的，因为double类型并没有以成员函数的形式重载-，也就是说，
  // 以成员函数的形式重载-，只能计算c2-11.0，不能计算11.0-c2，这是不对称的。

  // 编译器明明可以把11.0先转换成Complex类型再相减呀：
  // Complex c10 = Complex(11.0).operator-(c2);
  // 为什么就是不转换呢？
  // 没错，编译器不会转换，原因在于，C++只会对成员函数的参数进行类型转换，
  // 而不会对调用成员函数的对象进行类型转换，比如：obj.func(params);
  // 编译器不会尝试对obj进行任何类型转换，它有func()成员函数就调用，没有就报错。
  // 而对于实参params，编译器会拼命地将它转换为形参的类型。
}
}  // namespace n2

...
// 数组即可以存储不可再分的数据元素（如数字5、字符'a'），也可以继续存储数组（即n维数组）。
// 以上两种数据存储形式绝不会出现在同一个数组中。
// 有人可能会说，创建一个二维数组来存储{1,{1,2,3}}。
// 在存储上确实可以实现，但无疑会造成存储空间的浪费。
// 对于存储{1,{1,2,3}}这样的数据，更适合用广义表结构来存储。
// 广义表，又称列表，也是一种线性存储结构。
// 同数组类似，广义表中既可以存储不可再分的元素，也可以存储广义表。
// 通常，广义表中存储的单个元素称为原子，而存储的广义表称为子表。

// 以下是广义表存储数据的一些常用形式：
//     A = ()：A表示一个广义表，只不过表是空的。
//     B = (e)：广义表B中只有一个原子e。
//     C = (a,(b,c,d))：广义表C中有两个元素，原子a和子表(b,c,d)。
//     D = (A,B,C)：广义表D中存有3个子表，分别是A、B和C。等同于D = ((),(e),(b,c,d)) 。
//     E = (a,E)：广义表E中有两个元素，原子a和它本身。这是个递归广义表，等同于：E = (a,(a,(a,…)))。
// 注意，A = ()和A = (())是不一样的，
// 前者是空表，而后者是包含一个子表的广义表，只不过这个子表是空表。

// 当广义表不是空表时，称第一个数据（原子或子表）为表头，剩下的数据构成的新广义表为表尾。
// 强调一下，除非广义表为空表，否则广义表一定具有表头和表尾，且广义表的表尾一定是一个广义表。
// 例如在广义表中LS={1,{1,2,3},5}中，表头为原子1，
// 表尾为子表{1,2,3}和原子5构成的广义表，即{{1,2,3},5}。
// 再比如，在广义表LS = {1}中，表头为原子1，但由于广义表中无表尾元素，
// 因此该表的表尾是一个空表，用{}表示。

// 由于广义表中既可存储原子（不可再分的数据元素），也可以存储子表，
// 因此很难使用顺序存储结构表示，通常情况下广义表结构采用链表实现。
// 使用顺序表实现广义表结构，不仅需要操作n维数组，
// 例如{1,{2,{3,4}}}就需要使用三维数组存储，还会造成存储空间的浪费。
...
// 依次遍历广义表C的每个节点，若当前节点为原子（tag值为0），则返回0；
// 若为空表，则返回1；反之，则继续遍历该子表中的数据元素。
// 设置一个初始值为0的整形变量max，每次递归过程返回时，令max与返回值进行比较，并取较大值。
// 这样，当整个广义表递归结束时，max+1就是广义表的深度。
// 其实，每次递归返回的值都是当前所在的子表的深度，原子默认深度为0，空表默认深度为1。

// 对于任意一个非空广义表来说，都是由两部分组成：表头和表尾。
// 反之，只要确定的一个广义表的表头和表尾，那么这个广义表就可以唯一确定下来。
// 复制一个广义表，也是不断的复制表头和表尾的过程。
// 如果表头或者表尾同样是一个广义表，依旧复制其表头和表尾。
...
typedef struct GLNode {
  ...
  // 用到了union共用体，因为同一时间此节点不是原子节点就是子表节点，当表示原子节点时，就使用atom变量；
  // 反之则使用ptr结构体。
  union {
    char atom;  // 原子结点的值域
    struct {
    ...
    // 子表节点中的hp指针用于链接本子表中存储的原子或子表，tp指针用于连接广义表中下一个原子或子表。
  };
} * Glist, GNode;
...
Glist creatGlist(Glist C) {
  ...
  // 开始存放下一个数据元素（b,c,d）,表头为b，表尾为（c,d）
...
// Glist *T，等同于:struct GLNode* *T，此为二级指针，不是一级指针。
// 在主函数中，调用此函数时，传入的是指针T的地址，而不是T。

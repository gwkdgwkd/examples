...
//   但是不能靠返回类型来判断。
// 2.重写override：也叫做覆盖。子类重新定义父类中有相同名称和参数的虚函数。
...
// 这样很多含有final/override变量或者函数名的C++98代码就能够被C++编译器编译通过了。
// 但出于安全考虑，建议在C++11代码中应该尽可能地避免这样的变量名称或将其定义在宏中，
// 以防发生不必要的错误。
int final;
int override;
}  // namespace n4

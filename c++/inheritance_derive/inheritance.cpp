...
//     派生类新增加的成员
// };
// 继承方式包括public、private和protected，此项是可选的，如果不写，那么默认为private。
...
// 3.如果希望基类的成员能够被派生类继承并且毫无障碍地使用，那么这些成员只能声明为public或protected；
//   只有那些不希望在派生类中使用的成员才声明为private。
// 4.如果希望基类的成员既不向外暴露（不能通过对象访问），还能在派生类中使用，那么只能声明为protected。

// 由于private和protected继承方式会改变基类成员在派生类中的访问权限，导致继承关系复杂，所以一般使用public。
...
// 实际上，基类的private成员是能够被继承的，并且会占用派生类对象的内存，它只是在派生类中不可见，导致无法使用罢了。
...
// 因为基类中private成员在派生类中是不可见的，根本不能使用，所以基类中的private成员在派生类中无论如何都不能访问。

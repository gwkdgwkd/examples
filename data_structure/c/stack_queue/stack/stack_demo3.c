...
// 1929年，波兰逻辑学家J・卢卡西维兹提出了一种全新的表示表达式的方法，称为后缀表达式或者逆波兰表达式。
...
// 当用户给定一个后缀表达式时，按照从左到右的顺序依次扫描表达式中的各个运算项和运算符，对它们进行如下处理：
// 1.遇到运算项时，直接入栈；
// 2.遇到运算符时，将位于栈顶的运算项出栈，对于!运算符，取栈顶1个运算项；
//   其它运算符，取栈顶2个运算项，第一个取出的运算项作为该运算符的右运算项，另一个作为左运算项。
//   求此表达式的值并将其入栈。
...
// 后缀表达式的转换
// 对于普通用户来讲，另其输入一个正确的后缀表达式显然是不实现的，只能要求他们输入一个正确的普通表达式。
...
// 1.如果为'0'~'9'的字符，将其添加到postexp数组的末尾；

...
// 2.全局函数CreateC被声明为类的友元，让CreateC避免私有构造函数引起的限制；
...
// 命名空间有些象类，但是它没有public、protected或private，都是public。
namespace NC {
class C {
 public:
  friend C &CreateC();
...
//   与此相反，只有第一次执行函数时，才会建立函数中的局部静态变量，
//   所以如果没有调用函数，就不会建立对象。
//   不过得为此付出代价，每次调用函数时都得检查是否需要建立对象。
//   其次：
//   函数的局部静态变量在函数第一次执行时初始化，类的静态成员的初始化时间不确定。
...
//   为什么要把对象声明为静态呢？通常是因为只想要该对象的一个拷贝。
//   现在再考虑内联意味着什么呢？
//   它意味着编译器用函数体替代该对函数的每一个调用，不过非成员函数还不只这些。
...
//   可能会使程序的静态对象的拷贝超过一个，所以不要建立包含局部静态数据的非成员函数。
}  // namespace n1

namespace n2 {
// 但是可能认为建立函数来返回隐藏对象的引用，这种限制对象的数量的方法是错误的。
// 也许有人认为只需计算对象的数目，一旦对象太多，就抛出异常，这样做也许会更好。
class C {
  // 这种限制建立对象数目的方法有两个较吸引人的优点：
  // 1.它是直观的，每个人都能理解它的用途；
  // 2.很容易推广，可以允许建立对象最多的数量不是1，而是其它大于1的数字。
 public:
  C() {
    if (num_objects_ >= 1) {
...
// 因为程序员心目中的对象的存在含义与编译器不一致。
}  // namespace n2

namespace n3 {
// 通常开发者仅会对允许对象本身存在的情况感兴趣，希望限制这种实例的数量。
// 如果使用n1中示例的方法，就很容易进行这种限制，因为构造函数是private，
// 不存在友元声明的带有私有构造函数的类不能作为基类使用，也不能嵌入到其它对象中。
// 不能从带有私有构造函数的类派生出新类，这个事实导致产生了一种阻止派生类的通用方法，
...
// 类C使用了Counter模板来跟踪对象数量，除了类C的编写者，没有人关心这个事实。
// 它的实现细节最好是private，这就是为什么这里使用private继承的原因。
// 另一种方法是使用public继承，但是必须给Counted类一个虚拟析构函数。
// 否则如果有人通过Counted<C>*指针删除一个类C对象，就有行为不正确的风险。

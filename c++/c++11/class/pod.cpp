...
//   2.是拷贝不变的（trivially copyable）的class，可以使用memcpy，memmove不改变它的语义。
...
//    所有非静态成员都符合标准布局类型
//  好处：
//   1.它对语言间交互很有用，这是因为C++ standard-layout类型和C中struct或union类型有相同的内存布局。
...
// 3.pair(tuple),vector,list,deque,set,map,unordered_set,unordered_map,shared_ptr,
//   unique_ptr都满足is_standard_layout但是不满足is_trivial，因此也不满足is_pod类型。
// 4.unique_ptr具有和普通指针一样大小，大多数时候应该使用它(如unique性能更好等等)，而不是shared_ptr。

...
// C++11移动语义是什么:
// 在C++11标准之前（C++98/03标准中），如果想用其它对象初始化一个同类的新对象，只能借助类中的拷贝构造函数。
...
// 非const右值引用只能操作右值，程序执行结果中产生的临时对象（函数返回值、
// lambda表达式等）既无名称也无法获取其存储地址，所以属于右值。
...
class B {  // has move
 public:
  ...
  B(B &&b) : p(b.p) {  // 浅拷贝
    b.p = nullptr;  // 避免了“同一块对空间被释放多次”情况的发生
...
void func1() {
  ...
  // A construct!
  // 2.执行return A()语句，会调用拷贝构造函数复制一份之前生成的匿名对象，并将其作为getA()函数的返回值，
  // A copy construct!
  ...
  // 由于临时变量的产生、销毁以及发生的拷贝操作本身就是很隐晦的（编译器对这些过程做了专门的优化），
  // 且并不会影响程序的正确性，因此很少进入程序员的视野。
...
void func2() {
  ...
  // 对于程序执行过程中产生的临时对象，往往只用于传递数据（没有其它的用处），并且会很快会被销毁。
  // 因此在使用临时对象初始化新对象时，我们可以将其包含的指针成员指向的内存资源直接移给新对象所有，

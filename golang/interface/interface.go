...
// 先创建一套接口,然后再定义一些满足它们的具体类型。这种方式的结果就是有很多的接口,它们中的每一个仅只有一个实现。
// 这种接口是不必要的抽象;它们也有一个运行时损耗。你可以使用导出机制来限制
// 一个类型的方法或一个结构体的字段是否在包外可见。接口只有当有两个或两个以上的具体
// 类型必须以相同的方式进行处理时才需要。

// 当一个接口只被一个单一的具体类型实现时有一个例外,就是由于它的依赖,这个具体类型
// 不能和这个接口存在在一个相同的包中。这种情况下,一个接口是解耦这两个包的一个好方式。
...
// 个好的标准就是ask only for what you need(只考虑你需要的东西)

// Go语言对面向对象风格的编程支持良好,但这并不意味着你只能使用这一风格。不是任何事物都需要被
// 当做成一个对象;独立的函数有它们自己的用处,未封装的数据类型也是这样。

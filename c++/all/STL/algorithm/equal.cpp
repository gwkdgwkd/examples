...
namespace n1 {
// 可以用和比较字符串类似的方式来比较序列，如果两个序列的长度相同，
// 并且对应元素都相等，equal()算法会返回true，有4个版本的equal()算法，
// 其中两个用==运算符来比较元素，另外两个用提供的作为参数的函数对象来比较元素，
...
void func() {
  ...
  // 3个输入迭代器参数，前两个参数是第一个序列的开始和结束迭代器，
  // 第三个参数是第二个序列的开始 如果第二个序列的元素少于第一个，结果是未定义的。
  ...
  // 4个输入迭代器参数，第一个序列的开始和结束，第二个序列的开始和结束，
  // 如果长度不同，那么结果总是false，推荐使用4参数版本，不会产生未定义行为。
  ...
  // 如果为两个序列提供了开始和结束迭代器，为了使结果为true，序列长度必须相同。
  ...
  // 7.Compare from words1[1] to end with words2 from first to second_to_last:
  // true
...
namespace n2 {
// equal()可以告诉两个序列是否匹配，mismatch()也可以告诉我们两个序列是否匹配，
// 而且如果不匹配，它还能告诉不匹配的位置，mismatch()的4个版本和equal()一样，
// 第二个序列有或没有结束迭代器，有或没有定义比较的额外的函数对象参数。
// mismatch()返回的pair对象包含两个迭代器，first是第一个序列的迭代器，
// second是第二个序列的迭代器，当不匹配时，pair的迭代器指向第一对不匹配的元素；
// 当序列匹配时，pair的成员取决于使用的mismatch()的版本和具体情况。

// iter1和end_iter1表示定义第一个序列的迭代器，
// iter2和end_iter2表示第二个序列的开始和结束迭代器。

// 对于mismatch(iter1，end_iter1，iter2)：
// 返回pair<end_iter1,(iter2+(end_ter1-iter1))>，
// pair的成员second等于iter2加上第一个序列的长度。
// 如果第二个序列比第一个序列短，结果是未定义的。

// 对于mismatch(iterl, end_iter1, iter2, end_iter2)：
// 当第一个比第二个长时，返回pair<end_iter1,(iter2+(end_iter1-iter1))>，
// 所以成员second为iter2加上第一个序列的长度。
// 当第二个比第一个长时，返回pair<(iter1+(end_iter2-iter2)),end_iter2>，
...
// 它的前两个参数定义了第一个序列，第3和第4个参数分别是第二个序列的开始和结束。
// 默认用<运算符来比较元素，但在需要时，也可以使用函数对象作为可选的第5个参数。
// 如果第一个序列的字典序小于第二个，这个算法会返回true，否则返回false。
// 所以，返回false表明第一个序列大于或等于第二个序列。
// 序列是逐个元素比较的，第一对不同的对应元素决定了序列的顺序。
// 如果序列的长度不同，而且短和长的初始元素序列匹配，那么短序列小于长序列。

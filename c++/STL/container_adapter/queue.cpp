...
// 和stack栈容器适配器不同，queue容器适配器有2个开口，
// 其中一个开口专门用来输入数据，另一个专门用来输出数据。
// 这种存储结构最大的特点是，最先进入queue的元素，
// 也可以最先从queue中出来，
// 即用此容器适配器存储数据具有先进先出（简称FIFO）的特点，
// 因此queue又称为队列适配器。
// STL queue容器适配器模拟的就是队列这种存储结构，
// 因此对于任何需要用队列进行处理的序列来说，
// 使用queue容器适配器都是好的选择。
...
int main() {
  ...
  // 在手动指定基础容器的类型时，
  // 其存储的数据类型必须和queue容器适配器存储的元素类型保持一致。
  // 作为queue容器适配器的基础容器，
  // 其必须提供front()、back()、push_back()、pop_front()、
  // empty()和size()这几个成员函数，

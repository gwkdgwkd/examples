...
// AspectRatio肯定会被编译器看到，当然就会进入符号表。
...
namespace test3 {
// 在class编译期间需要一个class常量值，编译器坚持必须在编译期间知道数组大小，
// 可以使用所谓的the enum hack补偿做法，定义如下：
class A {
 private:
  enum { num = 5 };
  int scores[num];
...
// 取const的地址是合法的，但取enum的地址就不合法，而取#define的地址通常也不合法。

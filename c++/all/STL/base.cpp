...
// STL，英文全称standard template library，可译为标准模板库或者泛型库，
// 其包含有大量的模板类和模板函数，是C++提供的一个基础模板的集合，
// 用于完成诸如输入/输出、数学计算等功能。
// STL最初由惠普实验室开发，1998年被定为国际标准，成为C++程序库的重要组成部分。
// STL完全被内置到C++编译器中，无需额外安装，这可能也是STL被广泛使用的原因之一。
// STL位于各个C++头文件中，它并非以二进制代码的形式提供，而是以源代码的形式提供。
// 从根本上说，STL是一些容器、算法和其他一些组件的集合，
// 所有容器和算法都是总结了几十年来算法和数据结构的研究成果，
// 汇集了许多计算机专家学者经验的基础上实现的，因此可以说，
// STL基本上达到了各种存储方法和相关算法的高度优化。

// Alexander Stepanov，STL标准模板库之父，1950年出生于前苏联的莫斯科，
// 他曾在莫斯科大学研究数学，此后一直致力于计算机语言和泛型库研究。
// 1987年，在贝尔实验室工作的Stepanov开始首次采用C++语言进行泛型软件库的研究。
// 当时C++还没有引入模板的编程技术，泛型库只能是通过继承开发，代码非常笨拙。
// 1992年，参加并主持了实验室主任Bill Worley所建立的一个有关算法的研究项目，
// 才使他重新回到了泛型化算法的研究工作上来。
// 项目自建立之后参与者从最初的8人逐渐减少，最后只剩下Stepanov和Meng Lee两个人。
// 经过长时间的努力，最终完成了一个包含有大量数据结构和算法部件的庞大运行库，
// HP版本的C++STL，这便是现在STL的雏形。
...
// 然后又于1994年3月，在圣迭戈会议上向委员会提交了建议书，以期将STL纳入C++标准。
// 最终在1994年的滑铁卢会议上，提案被通过，决定将STL正式纳入C++标准化进程之中，
// 随后STL便被放进了会议的工作文件中，自此，STL终于成为C++家族中的重要一员。
// 此后，随者C++标准的不断改进，STL也在不断地做着相应的演化。
// 1998年ANSI/ISO C++标准正式定案，STL始终是C++标准库不可或缺的重要组成部分。

// 自1998年ANSI/ISO C++标准正式定案，C++ STL规范版本正式通过后，由于是开源的，
// 各个C++编译器厂商在此标准的基础上，实现了满足自己需求的C++ STL泛型库，
...
// 掌握了STL标准，很多功能不用重复的造轮子，直接拿来用即可，总的来说，
// STL是C++标准程序库的重要组成部分，为C++程序员提供了大量的可扩展的程序框架，
// 高度实现了代码的可重用性，并且它是内置的，不需要额外安装，使用非常方便。

// 在计算机程序设计领域，为了避免因数据类型的不同，
// 而被迫重复编写大量相同业务逻辑的代码，人们发展的泛型及泛型编程技术。
// 所以泛型，实质上就是不使用具体数据类型int、double、float等，
// 而是使用一种通用类型来进行程序设计的方法，该方法可以大规模的减少代码量，
// 让程序员可以集中精力用于业务逻辑的实现。
// 值得一提的是，既然泛型并不是真实的数据类型，
// 那么使用泛型编写的代码也就不是真正的程序实体，只能算是一个程序实体的样板。
// 形象的将使用了泛型的代码称为模板，由模板生成实际代码的过程称为模板的具体实现。
// 类型占位符的替换工作，不需要人为操控，可以完全交由计算机来完成，
// 更准确的说，是交由编译器在编译阶段来完成模板的具体实现。
// 总之一句话，泛型也是一种数据类型，只不过它是一种用来代替所有类型的通用类型。
// 在C++中，用以支持泛型的就是STL，它提供了C++泛型设计常用的类模板和函数模板。

// 通常认为，STL是由容器、算法、迭代器、函数对象、适配器、内存分配器这6部分构成，
// 其中后面4部分是为前2部分服务的：
// 1.容器：一些封装数据结构的模板类，例如vector向量容器、list列表容器等；
// 2.算法：STL提供了非常多（约100个）的数据结构算法，都被设计成一个个的模板函数，
//   在std命名空间中定义，其中大部分都在<algorithm>中，少部分位于<numeric>中；
// 3.迭代器：对容器中数据的读和写是通过迭代器完成的，它是容器和算法之间的胶合剂；
// 4.函数对象：如果一个类将()运算符重载为成员函数，这个类就称为函数对象类，
//   这个类的对象就是函数对象，又称仿函数；
// 5.适配器：可以使一个类的接口（模板的参数）适配成用户指定的形式，
//   从而让原本不能在一起工作的两个类工作在一起，
//   值得一提的是，容器、迭代器和函数对象都有适配器；
...
// 学习C++标准库，特别是STL，经常需要考量算法和成员函数的效能，
// 也就是运行效率，又称复杂度，目前最常用的方法称为大O表示法（字母O）。
// 使用大O表示法衡量某个算法的复杂度，
...
//  指数阶  算法运行时间随所操作的元素个数呈m次方增长   O(n^m)，O(n^2)、O(n^3)等
...
// 他认为私藏源代码是一种违反人性的罪恶行为，而如果能与他人分享源代码，
// 便可以让其他人从中学习，并回馈给原始创作者。

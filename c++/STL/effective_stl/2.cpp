...
// 不是针对某种具体的容器，而是把容器的概念泛化，
// 这样当容器从vector变为deque或list时，代码不用改变。
// 这类泛化，出发点好，却几乎总是误入歧途。

// 试图编写对序列容器和关联容器都适用的代码几乎毫无意义。
// 很多成员函数仅当容器为某种类型时才存在：
// 1.只有序列容器才支持push_front或push_back；
// 2.只有关联容器才支持count和lower_bound，等等；
// 3.即使insert和erase这样的基本操作，
//   也会随容器类型的不同而表现出不同的原型和语义。

// 假设想编写对于大多数通用的序列容器，vector、deque和list都适用的代码，
// 那么很显然，程序只能适用它们功能的交集。
// 1.意味着不能使用reserve或capacity，
//   因为deque和list中没有这样的操作；
// 2.有list存在，意味着要放弃operator[]，
//   而且要把操作现在在双向迭代器的能力范围之内；
// 3.意味着要放弃要求随机迭代器的操作，
//   包括sort、stable_sort、partial_sort和nth_elemment。
// 4.为了要支持vector，就不能使用push_front和pop_front。
// 5.对于vector和deque而言，splice和成员函数形式的sort又是被禁用的。

// 这些限制的根源在于，对不同类型的序列容器，
// 使用迭代器、指针和引用无效的规则不同。
// 要想使代码对vector、deque和list都能工作，必须假定，
// 对任何一种容器，
// 使迭代器、指针和引用无效的任何操作将在使用的容器上使他们无效。
// 所以：
// 1.每个insert调用都使所有迭代器、指针和引用无效，
//   因为deque::insert使所有迭代器失效。
...
//   因为vector<bool>并不总是表现的像个vector，
//   实际上并没有存储bool类型的对象。
// 6.不能假定list的常数时间的插入和删除操作，因为vector和deque是线性时间。

// 想要编写对于set和map都适用的代码几乎不可能。
// 面对现实吧，不同的容器是不同的，它们有着非常明显的优缺点。
// 它们并不是被设计来交换使用的，不要尝试这样做。

// 考虑到有时候不可避免的要从一种容器类型转到另一种，
// 可以使用封装技术来实现这种转变。

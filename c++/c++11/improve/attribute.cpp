...
// 随着C++语言的演化和编译器的发展，人们常会发现标准提供的语言能力不能完全满足要求。
// 于是编译器厂商或组织为了满足编译器客户的需求，设计出一系列的语言扩展语法。
// 这些扩展语法并不存在于C++/C标准中，却有可能拥有较多的用户。
// 扩展语法中比较常见的就是属性，属性是对语言中的实体对象（比如函数、变量、类型等）附加一些的额外注解信息，
// 其用来实现一些语言及非语言层面的功能，或是实现优化代码等的一种手段。
...
// 这里的const属性告诉编译器：本函数返回值只依赖于输入，不会改变任何函数外的数据，因此没有任何副作用。
// 在此情况下，编译器可以对area函数进行优化处理，area(3)的值只需要计算一次，
// 编译之后可以将area(3)视为循环中的常量而只使用其计算结果，从而大大提高了程序的执行性能。

// 在GNU对C/C++的扩展中我们可以看到很多不同的attribute属性：
// 常见的如format、noreturn、const和aligned等，具体含义和用法可以参考GNU的在线文档。

// 在Windows平台上，使用另外一种关键字__declspec。
// __declspec是微软用于指定存储类型的扩展属性关键字，用户只要简单地在声明变量时加上：
// c++ __declspec(extended-decl-modifier)
// 即可设定额外的功能。
// 以对齐方式为例，在C++11之前，微软平台可以使用__declspec(align(x))来控制变量的对齐方式，代码如下：
...
// 对于作用于声明的通用属性，既可以写在声明的起始处，也可以写在声明的标识符之后。
...
// 按照C++11的定义，[[attr1]]和[[attr4]]会作用于c1, [[attr1]]和[[attr5]]会作用于c2,
...
// 不会返回和没有返回值的（void）函数的区别：
// 1.没有返回值的void函数在调用完成后，调用者会接着执行函数后的代码；
// 2.而不会返回的函数在被调用完成后，后续代码不会再被执行。
// [[noreturn]]主要用于标识那些不会将控制流返回给原调用函数的函数，
// 典型的例子有：有终止应用程序语句的函数、有无限循环语句的函数、有异常抛出的函数等。
// 通过这个属性，开发人员可以告知编译器某些函数不会将控制流返回给调用函数，这能帮助编译器产生更好的警告信息，
// 同时编译器也可以做更多的诸如死代码消除、免除为函数调用者保存一些特定寄存器等代码优化工作。
...
// 不返回的函数除了是有异常抛出的函数外，还有可能是有终止应用程序语句的函数，或是有无限循环语句的函数等。
// 事实上，在C++11的标准库中，都能看到形如：
[[noreturn]] void abort(void) noexcept;
// abort总是会导致程序运行的停止，甚至连自动变量的析构函数以及本该在atexit()时调用的函数全都不调用就直接退出了。
// 因此声明为[[noreturn]]是有利于编译器优化的。

// 尽量不要对可能会有返回值的函数使用[[noreturn]]:
[[noreturn]] void func2(int i) {  // warning: ‘noreturn’ function does return
  // 当参数i的值为0时，该函数行为不可估计
  if (i < 0)
    throw "negative";
  else if (i > 0)

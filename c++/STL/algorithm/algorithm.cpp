...
// 由于STL标准库使用场景很广，因此很多需要手动编写循环结构实现的功能，
// 用STL算法函数就能完成。
...
// 也就是说，end()方法只需要调用一次就够啦，
// for_each()函数就对这一点进行了优化。
// 通过将end()方法作为参数传入for_each()函数，该方法只执行了1次。
// 当然，这也仅是众多优化中的一处。
// 事实上，STL标准库的开发者对每个算法函数的底层实现代码都多了优化，
// 使它们的执行效率达到最高。
// 难道自己对循环结构进行优化不行吗？
...
// 才知道如何控制指针逐个遍历这些连续空间。
// 另一方面，某些STL函数的底层实现使用了复杂的科学计算方法，
// 并不是普通C++程序员能驾驭的。
// 例如，在实现对某个序列进行排序时，很难编写出比sort()函数更高效的代码。
// 总之，STL开发者比使用者更了解内部的实现细节，
// 他们会充分利用这些知识来对算法进行优化。
// 只有熟悉STL标准库提供的函数，才能在实际编程时想到使用它们。
// 作为一个专业的C++程序员，必须熟悉STL标准库中的每个算法函数，
// 并清楚它们各自的功能。
// C++ STL标准库中包含70多个算法函数，
// 如果考虑到函数的重载，大约有100多个不同的函数模板。

// 其中有些函数名称和STL容器模板类中提供的成员方法名相同。
// 当某个STL容器提供有和算法同名的成员方法时，应该使用哪一个呢？
// 大多数情况下，应该使用STL容器提供的成员方法，
// 而不是同名的STL算法，原因包括：
...
// 要知道，find()函数是通过逐个比对来实现查找的，它以线性时间运行；
// 而由于set容器底层存储结构采用的是红黑树，
// 所以find()成员方法以对数时间运行，而非线性时间。
// 换句话说，对于含有一百万个元素的set容器，
// 如果使用find()成员方法查找目标元素，
// 其最差情况下的比对次数也不会超过40次，平均只需要比对20次就可以查找成功；
// 而使用同名的find()函数查找目标元素，最差情况下要比对一百万次，
// 平均比对50万次才能查找成功。
// 所谓最差情况，指的是当前set容器中未存储有目标元素。
// 虽然有些容器提供的成员方法和某个STL算法同名，
// 但该容器只能使用自带的成员方法，而不适用同名的STL算法。
// 比如，sort()函数根本不能应用到list容器上，
// 因为该类型容器仅支持双向迭代器，
// 而sort()函数的参数类型要求为随机访问迭代器；
// merge()函数和list容器的merge()成员方法之间也存在行为上的不同，
// 即merge()函数是不允许修改源数据的，
// 而list::merge()成员方法就是对源数据做修改。
// 总之，当读者需要在STL算法与容器提供的同名成员方法之间做选择的时候，
// 应优先考虑成员方法。
...
int main() {
  ...
  // 而find()函数底层仅会依据==运算符查找s容器中和abcd相等的元素，
  // 所以会查找失败。

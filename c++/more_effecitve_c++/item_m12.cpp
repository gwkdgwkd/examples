...
// 因此可能会认为用throw抛出一个异常到catch子句中与通过函数调用传递一个参数两者基本相同。
...
// 而是把operator>>内的引用类型变量w指向localWidget，任何对w的操作实际上都施加到localWidget上。
// 这与抛出localWidget异常有很大不同。
// 不论通过传值捕获异常还是通过引用捕获（不能通过指针捕获这个异常，
// 因为类型不匹配）都将进行lcalWidget的拷贝操作，
...
//   那么第一个块中传递出去的还是SpecialWidget异常，即使w的静态类型(static type)是Widget。
//   这是因为重新抛出异常时没有进行拷贝操作。
//   第二个catch块重新抛出的是新异常，类型总是Widget，因为w的静态类型(static type)是Widget。
//   一般来说，你应该用throw来重新抛出当前的异常，因为这样不会改变被传递出去的异常类型，
//   而且更有效率，因为不用生成一个新拷贝。
...
// 当用传值的方式传递函数的参数，我们制造了被传递对象的一个拷贝，并把这个拷贝存储到函数的参数里。
// 同样我们通过传值的方式传递一个异常时，也是这么做的。
// 通过传值捕，会建立两个被抛出对象的拷贝，一个是所有异常都必须建立的临时对象，第二个是把临时对象拷贝进w中。
// catch (Widget w)
...
// 3.catch子句进行异常类型匹配的顺序是它们在源代码中出现的顺序，第一个类型匹配成功的catch将被用来执行。

...
// 毫无疑问，异常规格是一个引人注目的特性。
// 它使得代码更容易理解，因为它明确地描述了函数可以抛出什么样的异常。
// 但它不只是一个有趣的注释，编译器编译时有时能检测到异常规格的不一致。
// 而且如果一个函数抛出一个不在异常规格范围里的异常，
// 系统在运行时能够检测出这个错误，然后特殊函数unexpected将被自动调用。
// 异常规格既可以做为指导性文档同时也是异常使用的强制约束机制，它有着很诱人的外表。
// unexpected缺省的行为是调用terminate，而terminate缺省的行为是调用abort，
// 所以一个违反异常规格的程序其缺省的行为就是halt（停止运行）。
// 在激活的栈中的局部变量没有被释放，因为abort在关闭程序时不进行这样的清除操作。
// 对异常规格的触犯变成了一场并不应该发生的灾难。
// 不幸的是，很容易就能够编写出导致发生这种灾难的函数。
...
// opertor&可能会抛出一个异常，这样就违反了异常规格，使得程序控制跳转到unexpected。
// 几乎不可能为一个模板提供有意义的异常规格，因为模板总是采用不同的方法使用类型参数。
...
void CallBack::makeCallBack(int eventXLocation, int eventYLocation) const
    throw() {
  // 调用func要冒违反异常规格的风险，因为无法知道func会抛出什么类型的异常：
...
// 如果在函数里使用new操作符，必须为函数可能遇到bad_alloc异常作好准备。

// 虽然防止抛出unexpected异常是不现实的，
// 但是C++允许用其它不同的异常类型替换unexpected异常，能够利用这个特性。
// 所有的unexpected异常对象被替换为这种类型对象：
class UnexpectedException {
 public:
  void what() const { std::cout << "UnexpectedException" << std::endl; }
};
// 如果一个unexpected异常被抛出，这个函数被调用：
void convertUnexpected() { throw UnexpectedException(); }
// 当这么做了以后，一个unexpected异常将触发调用convertUnexpected函数。
...
// 如果这么做，应该在所有异常规格里包含bad_exception或它的基类，标准类exception。
// 将不必再担心如果遇到unexpected异常会导致程序运行终止。
...
// 在把它们加入到函数之前，应考虑它们所带来的行为是否就是所希望的行为。

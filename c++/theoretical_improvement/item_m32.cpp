...
// 作为软件开发人员，也许知道得不多，但知道万物都会变化。
// 没必要知道什么将发生变化，这么变化又怎么发生，
// 以及什么时候发生，在哪里发生，但知道：万物都会变化。

// 好的软件能够适应变化，它提供新的特性，
// 适应到新的平台，满足新的需求，处理新的输入。
// 软件的灵活性、健壮性、可靠性不是来自于意外。
// 它是程序员们在满足了现在的需求并关注了将来的可能后设计和实现出来的。
// 这样的软件（接受小改变的软件）是那些在未来时态下开发程序的人写出来的。

// 要在未来时态下开发程序，就必须接受事物会发生变化，并为此作了准备。
// 这是应该考虑的：
// 1.新的函数将被加入到函数库中，新的重载将发生，
//   于是要注意那些含糊的函数调用行为的结果；
// 2.新的类将会加入继承层次，现在的派生类将会是以后的基类，并已为此作好准备；
// 3.将会编制新的应用软件，函数将在新的运行环境下被调用，
//   它们应该被写得在新平台上运行正确；
// 4.程序的维护人员通常不是原来编写它们的人，
//   因此应该被设计得易于被别人理解、维护和扩充。
...
// 2.如果一个类需要其实例全部创建在堆中，
//   不要只是对用户说了这么一句，用Item M27的方法来强迫这一点。
// 3.如果拷贝构造和赋值对一个类是没有意义的，
...
// 要承认用户可能犯错误，
// 所以要将你的类设计得可以防止、检测或修正这些错误。

// 努力于可移植的代码，写可移植的代码并不比不可移植的代码难太多，
// 只有在性能极其重要时采用不可移植的结构才是可取的。
// 即使是为特定的硬件设计的程序也经常被移植，
// 因为这些平台在几年内就会有一个数量级的性能提升。
// 可移植的代码使得你在更换平台是比较容易，
// 扩大你的用户基础，吹嘘支持开放平台。
// 这也使得赌错了操作系统时比较容易补救。
...
// 未来时态的考虑完全不同，不是问一个类现在正被怎么使用，
// 而是问这个类是被设计为怎么去使用的。
// 未来时态的考虑认为：
// 如果一个类被设计为作一个基类使用，即使现在还没有被这么使用，
// 它就应该有一个虚析构函数。
// 这样的类在现在和将来都行为正确，并且当新类从它们派生时并不影响其它库用户。
// 至少，它们没有任何影响，直到其析构函数被使用。
// 如果需要对类的额外修改，其它用户将受影响。

// 有一个商业类库，在C++标准运行库申明string以前，
// 包含了一个没有虚析构函数的string类。
// 其生产商解释：
// 我们没有使用虚析构函数，因为我们不想这个string类有vtbl。
// 我们甚至不期望想有一个string*，所以这不成为问题，
// 我们很清楚这么做将造成的困难。
// 这是进行时态的考虑还是未来时态的考虑？
// 当然，vbtl有个技术上的问题。
// 大部分string类的实现都是对象内部只有一个char*指针，
// 所以增加一个vptr造成每个string类的大小翻倍。
// 所以可以理解为什么不肯实现它，
...
// 不过，这仍然是个合法的设计。
// 的确，ISO/ANSI标准委员会似乎是这么想的：
// 标准string类型有一个非虚的析构函数。
// 更有问题的是生产商的注释：
// 我们甚至不期望有一个string*，所以这不成为问题。
// 这可能是正确的，
// 但他们的这个string类是提供给数以千记的开发人员使用的类库的一部分。
// 有如此多的开发人员，每个人对C++的掌握程度都不同，
// 每个人做事的方法也都不同。
// 这些人都了解string没有虚析构函数的后果吗？
// 生产商确信他们的客户知道没有虚析构函数时，
// 用string*指针delete对象时可能工作不正确，
// 在string的指针和引用上使用RTTI操作可能得到不正确的信息吗？
// 这个类是易于正确使用而不容易用错的吗？
// 这个生产商应该提供明确的文档以指出他的string类没有被设计得可被继承的，
// 但如果程序员没注意到这个警告或未能读到这个文档时会发生什么？
// 一个可选方法是用C++自己来阻止继承。
// Item M26描述了怎么限制对象只生成于堆中，
// 以及用auto_ptr对象来操作堆中的对象。

...
//  其中之一如果单一继承（SI）是好的，多重继承一定更好。
//  令一派则主张，单一继承是好的，但多重继承不值得拥有。

// 当MI进入设计框架，程序有可能从一个以上的base class继承相同的名称，会导致较多的歧义。
...
// 多重继承是继承一个以上的base class，一般base class没有有更高级的base class，否则会导致要命的“钻石型多重继承”：
class File {};
class InputFile : public File {};
class OutputFile : public File {};
class IOFile : public InputFile, public OutputFile {};
// 任何时候如果你有一个继承体系而其中某个base class和某个derived class之间有一条以上的相同路线，就必须面对这样一个问题：
// 是否打算让base class内的成员变量经由每一条路径被复制？C++并没有倾斜立场，两个方案（复制和不复制）都支持。
...
// 但为了避免继承得来的成员变量重复，编译必须提供若干幕后戏法，后果是：
//  使用virtual继承的class错产生的对象往往比使用non-virtual继承的对象体积大；
//  访问virtual base class的成员变量时，也比访问non-virtual base class的成员变量速度慢。
// virtual继承的成本还包括其他方面，支持virtual base class初始化”规则比起non-virual base的情况远为复杂且不直观。
...
// 建议：
// 第一、非必要不使用virtual base。平时请使用non-virtual继承。
// 第二、如果必须使用virtual base class，尽可能避免在其中放置数据。这样就不需担心初始化和赋值所带来的诡异事情了。
// Java和.NET的Interface值得注意，许多方面兼容C++的virtual base class，而且也不允许含有任何数据。
...
// 多重继承一个通情达理的应用场景：
// 将”public继承自某接口“和”private继承自某实现“结合在一起：
class CPerson : public IPerson, private PersonInfo {
  // CPerson和PersonInfo的关系是，PersonInfo刚好有若干函数可以帮助CPerson比较容易实现出来。
  // 它们的关系是is-implemented-in-terms-of，这种关系有两种技术实现：复合和private继承。
  // 通常复合比较受欢迎，但如果需要重新定义virtual函数，那么继承是必要的。
  // 需要重新定义valueDelimOpen和valueDelimClose，复合不合适，所以使用private继承。
...
// 和单一继承比，它通常比较复杂，使用上也比较难以理解，所以如果同样的设计，单一继承一定比较受欢迎。
...
int main() {
  ...
  // checkOut的调用是模棱两可的，即使两个函数中只有一个可用的（有一个private）。
  // 这与C++用来解析重载函数调用的规则相符：
  // 在看到是否有个函数可用之前，C++首先确认这个函数对此调用是最佳匹配。找到最佳匹配函数后才检验可取用性。

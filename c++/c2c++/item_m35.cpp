...
// 必须先知道标准C++运行库的两个特性。
// 第一，在运行库中的几乎任何东西都是模板。
// 在本书中，谈到过运行库中的string类，实际上没有这样的类。
...
// 但end指针在推广到其它容器类型时比NULL指针好。
// 迭代器就是被设计为操作STL的容器的类指针对象。
...
// 2.STL的类提供如同begin()和end()这样的函数，
//   这些函数返回类型定义在此类内的迭代器对象。
// 3.STL的算法函数使用迭代器对象操作这些类，
//   STL的迭代器类似于指针，这就是STL的全部。
// 它没有庞大的继承层次，也没有虚函数，
// 只是影响类模板和函数模板，及它们所遵守的规则。
// 这导致了另外一个发现：STL是可扩充的。
// 可以在STL家族中加入自己的容器，算法和迭代器。
// 只要遵守STL的规则，STL中的容器将可以使用自定义的算法，
// 自定义容器上也可使用STL的算法。
// 当然，模板不会成为标准C++运行库的一部分，
// 但它们将用同样的规则来编译，并可被重用。

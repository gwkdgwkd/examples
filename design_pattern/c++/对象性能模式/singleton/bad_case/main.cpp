...
// singleton.num in main : 100
// 从输出内容中，可以看出singleton出现了两个实例：
// 1.一个实例在main函数中；
// 2.另一个实例在插件hello.so中。

// 原因分析：
// 究其原因，是由于插件的动态链接引起的。
// hello.so在动态链接过程中，没有发现example1中有singleton::instance这个实例，
// 事实上该实例已经存在了，那么就会自己创建一个实例，这样就导致了两个实例。
// 通过工具nm查看example1的符号表（symboltable），
// 看看其中是否包含singleton::instance符号（dynamic）：
...
// -D用于查看动态符号dynamicsymbol，会发现singleton::pInstance在静态表中存在，
// 在动态表中不存在，这也就是说，动态连接器（dynamiclinker）在加载hello.so的时候，
// 无法找到singleton::pInstance的唯一实例，只能构造一个新的实例，
// 该实例在hello.so中是唯一的，但是不能保证在example1和hello.so中唯一。

// 解决方法：
// 解决问题的关键在于如何暴露example1中的singleton::pInstance。
// 好在，GNUmake有一个链接选项-rdynamic，可以解决这个问题。
// nm -C example1 | grep singleton
...
// 0000000000400f54 W singleton::singleton()
// 0000000000400f54 W singleton::singleton()
// 静态符号没有什么变化，但是动态符号却显示了更多的内容，
// 其中包括我们想要的singleton::pInstance，也就是singleton的唯一实例。
// singleton.num in main : 100
// singleton.num in hello.so : 100
// singleton.num in hello.so after ++ : 101
// singleton.num in main : 101

...
// 1.隐式类型转换，无需用户指明如何转换的称为自动类型转换；
// 2.显式类型转换，需要用户显式地指明如何转换的称为强制类型转换。
// 隐式类型转换利用的是编译器内置的转换规则，
// 或者用户自定义的转换构造函数以及类型转换函数（已知的转换规则），
// 例如从int到double、从派生类到基类、
// 从type *到void *、从double到Complex等。
// type*是一个具体类型的指针，例如int*、double*、Student*等，
// 它们都可以直接赋值给void*指针。
// 而反过来是不行的，必须使用强制类型转换才能将void*转换为type*，
// 例如，malloc()分配内存后返回的就是一个void *指针，
// 必须进行强制类型转换后才能赋值给指针变量。
// 当隐式转换不能完成类型转换工作时，就必须使用强制类型转换了。
// 强制类型转换的语法也很简单，只需要在表达式的前面增加新类型的名称，
// 格式为：(new_type) expression

// 类型转换的本质：
// 数据是放在内存中的，变量（以及指针、引用）是给这块内存起的名字，
// 有了变量就可以找到并使用这份数据。
// 内存中的数据有多种解释方式，使用之前必须要确定。
// 这种确定数据的解释方式的工作就是由数据类型（Data Type）来完成的。
// 数据类型用来说明数据的类型，确定解释方式，让计算机和程序员不会产生歧义。
// 所谓数据类型转换，就是对数据所占用的二进制位做出重新解释。
// 如果有必要，在重新解释的同时还会修改数据，改变它的二进制位。

// 隐式类型转换和显式类型转换最根本的区别是：
// 1.隐式类型转换除了会重新解释数据的二进制位，
//   还会利用已知的转换规则对数据进行恰当地调整；
// 2.而显式类型转换只能简单粗暴地重新解释二进制位，不能对数据进行任何调整。
// 其实，能不能对数据进行调整是显而易见地事情，有转换规则就可以调整，
// 没有转换规则就不能调整，当进行数据类型转换时，编译器是知道有没有转换规则的。
// 站在这个角度考虑，强制类型转换的语法就是多此一举，
// 编译器完全可以自行判断是否需要调整数据。
// 例如从int*转换到float*，加不加强制类型转换的语法都不能对数据进行调整。
// C/C++之所以增加强制类型转换的语法，是为了提醒程序员存在风险，一定要小心。
// 说得通俗一点，现在的类型转换存在风险，你自己一定要知道。
...
namespace n1 {
// 对于隐式类型转换，编译器可以根据已知的规则来决定是否需要修改二进制位；
// 而对于强制类型转换，由于没有对应的转换规则，
// 所以能做的事情仅仅是重新解释数据的二进制位，但无法对二进制位做出修正。
// 这就是隐式类型转换和强制类型转换最根本的区别。
// 这里说的修改数据并不是修改原有的数据，而是修改它的副本。
// 修改数据的二进制位非常重要，它能把转换后的数据调整到正确的值，
// 所以这种修改时常会发生，例如：
// 1.整数和浮点数在内存中的存储形式大相径庭，将浮点数f赋值给整数i时，
//   不能原样拷贝f的二进制位，也不能截取部分二进制位，
//   必须先将f的二进制位读取出来，然后直接截掉小数部分，
//   把剩下的整数部分再转换成二进制形式，拷贝到i所在的内存中。
// 2.short一般占用两个字节，int一般占用四个字节，
//   将short类型的s赋值给int类型的i时，如果仅仅是将s的二进制位拷贝给i，
//   那么i最后的两个字节会原样保留，这样会导致赋值结束后i的值并不等于s的值，
//   所以这样做是错误的。正确的做法是，
//   先给s添加16个二进制位（两个字节）并全部置为0，然后再拷贝给i所在的内存。
// 3.当存在多重继承时，如果把派生类指针pd赋值给基类指针pb，
//   就必须考虑基类子对象在派生类对象中的偏移，
//   偏移不为0时就要调整pd的值，让它加上或减去偏移量，
//   这样赋值后才能让pb恰好指向基类子对象。
// 4.Complex占用16个字节，double占用8个字节，
//   将double类型的数据赋值给Complex类型的变量（对象）时，
//   必须调用转换构造函数，否则剩下的8个字节就不知道如何填充了。
// 以上这些都是隐式类型转换，它对数据的调整都是有益的，让程序更安全稳健地运行。
...
// C语言之所以增加强制类型转换的语法，就是为了强调风险。
// 但是，这种强调风险的方式还是比较粗放，粒度比较大，
// 它并没有表明存在什么风险，风险程度如何。
...
namespace n3 {
// static_cast关键字：
// static_cast只能用于良性转换，转换风险较低，一般不会发生意外，例如：
...
// static_cast是静态转换的意思，也就是在编译期间转换，失败的话会编译错误。
...
// 需要注意的是，static_cast不能用于无关类型之间的转换，
// 因为这些转换都是有风险的，例如：
...
void func2() {
  ...
  // 不能将整数转换为指针类型：
  // float *fp2 = static_cast<float *>(0X2DF9);
...
// 使用const_cast进行强制类型转换可以突破C/C++的常数限制，
// 修改常数的值，因此有一定的危险性；
...
void func2() {
  ...
  // 而是将pd的值直接赋给了pb，最终导致pb也指向了n。
  // 因为pb指向的不是一个对象，所以get_a()得不到m_a的值，
  // pb->func()也得不到func()函数的正确地址。
}

// 向下转型是有风险的，dynamic_cast会借助RTTI信息进行检测，
// 确定安全的才能转换成功，否则就转换失败。
// 当使用dynamic_cast对指针进行类型转换时，程序会先找到该指针指向的对象，
// 再根据对象找到当前类（指针指向的对象所属的类）的类型信息，
// 并从此节点开始沿着继承链向上遍历，如果找到了要转化的目标类型，
// 那么说明这种转换是安全的，就能够转换成功，如果没有找到要转换的目标类型，
// 那么说明这种转换存在较大的风险，就不能转换。
...
// 总结：
// pa是A*类型的指针：
// 1.当pa指向A类型的对象时，向下转型失败，pa不能转换为B*或C*；
// 2.当pa指向D类型的对象时，向下转型成功，pa可以转换为B*或C*。
// 同样都是向下转型，为什么pa指向的对象不同，转换的结果就大相径庭呢？
// pa指向A类对象，根据该对象找到的就是A，当程序从这个节点开始向上遍历时，
// 发现A的上方没有要转换的B类型或C类型（没有任何类型），所以就转换败了。
// pa指向D类对象，根据该对象找到的就是D，程序从这个节点向上遍历的过程中，
// 发现了C类型和B类型，所以就转换成功了。
// 从表面上看起来dynamic_cast确实能够向下转型，
// 成功地将pa从A类型指针转换成了B和C类型指针。
// 但是从本质上讲，dynamic_cast还是只允许向上转型，因为它只会向上遍历继承链。
// pa指向的是D类型的，pa、pb、pc都是D的基类的指针，所以它们都可以指向D对象，

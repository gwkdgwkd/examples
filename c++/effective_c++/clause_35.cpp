...
class GameCharacter1 {  // 再明白不过的设计，但从某个角度说，却反而成了弱点
 public:
  // 返回人物的健康指数，derived class可重新定义。未被pure virtual，暗示有缺省算法
...
class GameCharacter2 {
 public:
  // 通过public non-virtual成员函数间接调用private virtual函数，称为non-virtual interface(NVI)手法。
  ...
  // NVI手法涉及在derived class内重新定义private virtual函数。重新定义了若干个derived class并不能调用的函数，这里并不矛盾。
  // derived class重新定义virtual函数，从而赋予它们“如何实现机能”的控制能力，但base class保利“函数何时被调用”的权利。

  // 在NVI手法下，其实没必要让virtual函数一定是private。
  // 某些class继承体系要求derived class在virtual函数的实现内必须调用其base class的对应兄弟，为了调用合法，virtual必须为protected。
...
// 2 某已知人物之健康指数计算函数可在运行期间变更。GameCharacter3可提供一个成员函数，用来替换当前的健康指数计算函数。
int loseHealthQuickly1(const GameCharacter3&) {
  cout << "loseHealthQuickly1" << endl;
};
// 换句话说，“健康指数计算函数不再是GameCharacter3继承体系内的成员函数”，这意味着这些函数并未访问对象的内部成分。
// 如果人物的健康可纯粹根据该人物public接口得来的信息加以计算，这就没问题，但如果需要non-public信息进行计算，就有问题了。
// 一般而言，唯一能够解决“需要以non-member函数访问class的non-public成分”的办法就是：弱化class的封装。
// 可声明哪个non-member函数为friend，或是为其实现的某一部分public访问函数。
// 运用函数指针替换virtual函数，其优点（像是“每个对象可各自拥有自己的健康计算函数”和“可在运行期改变计算函数”）是否可以弥补
...
class GameCharacter4 {  // 这个做法是常见的Strategy设计模式的简单应用。
 public:
  ...
  // 不使用函数指针，改用一个类型为function对象，可以解决指针的一些约束：可以持有任何可调用物（函数指针、函数对象或成员函数指针），只要其签名一致就可以。
...
// 这个解法的吸引力在于，熟悉标准Strategy模式的人很容易辨认它，而且提供“将一个既有的健康算法纳入使用”的可能性：
...
// 3 以function成员变量替换virtual函数，因而允许使用任何可调用物搭配一个兼容的签名式。这也是Strategy设计模式的某种形式。

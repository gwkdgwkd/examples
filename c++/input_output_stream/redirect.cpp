...
// cout和cerr、clog的一个区别是，
// cout允许被重定向，而cerr和clog都不支持。
// 值得一提的是，cin也允许被重定向。
// 在默认情况下，cin只能接收从键盘输入的数据，cout也只能将数据输出到屏幕上。
// 但通过重定向，cin可以将指定文件作为输入源，即接收文件中早已准备好的数据，
// 同样cout可以将原本要输出到屏幕上的数据转而写到指定文件中。
// C++中，实现重定向的常用方式有3种。

// 1.freopen()函数实现重定向：
// freopen()定义在<stdio.h>头文件中，是C语言标准库中的函数，
// 专门用于重定向输入流（scanf()、gets()）和输出流（printf()、puts()）。
...
// streambuf是C++标准库中用于表示缓冲区的类，
// 该类的指针对象用于代指某个具体的流缓冲区。
// 其中，第一种语法格式仅是返回一个指向当前流缓冲区的指针；
// 第二种语法格式用于将sb指向的缓冲区设置为当前流的新缓冲区，
// 并返回一个指向旧缓冲区的对象。
void func2() {

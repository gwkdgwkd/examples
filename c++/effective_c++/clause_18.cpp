...
// 没使用对象管理资源时，为避免资源泄露，要求必须释放资源。给客户两个犯错的机会：
// 没有删除指针，或删除同一指针超过一次。
...
// 会自动使用它的每个指针专属的删除器，因而消除潜在的客户错误：所谓的“cross-DLL problem”。
...
// shared_ptr是原始指针的两倍大，以动态分配内存作为计算或删除器的专属数据，以virtual形式调用删除器。
// 只要定义一个预处理符合就可以关闭多线程支持。
// 总之，它比原始指针大且慢，而且使用辅助动态内存。
// 在许多应用中这些额外成本并不显著，然而其“降低客户错误”的成效确是显著的。

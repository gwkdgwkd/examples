...
namespace n1 {
// 异常必须显式地抛出才能被检测和捕获到，如果没有显式的抛出即使有异常也检测不到。
// 在C++中，使用throw关键字来显式地抛出异常，用法为：throw exceptionData;
...
namespace n2 {
// throw关键字除了可以用在函数体中抛出异常，还可以用在函数头和函数体之间，
// 指明当前函数能够抛出的异常类型，这称为异常规范（Exception specification），
// 有些教程也称为异常指示符或异常列表，如果抛出异常列表中没有的异常，
// try将无法捕获，只能终止程序。

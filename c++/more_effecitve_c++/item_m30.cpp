...
// 然而，使用这些array对象并不直接了当。根据C和C++中的语法习惯，我们应该能够使用[]来索引数组：cout << data1[3][6];
// 但是没有这个操作符，我们不能重载。
...
// 其中每一个元素又都是一个20元素的数组，所以表达式data[3][6]实际上是(data[3])[6]，也就是data的第四个元素这个数组的第7个元素。
// 简而言之，第一个[]返回的是一个数组，第二个[]从这个返回的数组中再去取一个元素。
// 我们可以通过重载Array2D类的operator[]来玩同样的把戏。Array2D的operator[]返回一个新类Array1D的对象。
// 再重载Array1D的operator[]来返回所需要的二维数组中的元素。
// Array2D的用户并不需要知道Array1D类的存在。这个背后的“一维数组”对象从概念上来说，并不是为Array2D类的用户而存在的。
// 其用户编程时就象他们在使用真正的二维数组一样。对于Array2D类的用户这样做是没有意义的：
// 为了满足C++的反复无常，这些对象必须在语法上兼容于其中的元素是另一个一维数组的一个一维数组。
// 每个Array1D对象扮演的是一个一维数组，而这个一维数组没有在使用Array2D的程序中出现。扮演其它对象的对象通常被称为代理类。
// 在这个例子里，Array1D是一个代理类。它的实例扮演的是一个在概念上不存在的一维数组。
// 术语代理对象（proxy object）和代理类（proxy classs）还不是很通用；这样的对象有时被叫做surrogate。
...
// 这个名词来自于编译器，左值出现在赋值运算的左边，右值出现在赋值运算的右边。通常，将一个对象做左值使用意味着它可能被修改，做右值用意味着它不能够被修改。
// 我们想区分将operator[]用作左值还是右值，因为，对于有引用计数的数据结构，读操作的代价可以远小于写操作的代价。
// 引用计数对象的写操作将导致整个数据结构的拷贝，而读不需要，只要简单地返回一个值。
// 不幸的是，在operator[]内部，没有办法确定它是怎么被调用的，不可能区分出它是做左值还是右值。
// 可以基于const属性重载operator[]，这样就可以区分读还是写了。”换句话说，你建议我们这么解决问题：
// class String {
// public:
//  const char& operator[](int index) const; // for reads
//  char& operator[](int index); // for writes
// };
// 这不能工作。编译器根据调用成员函数的对象的const属性来选择此成员函数的const和非const版本，而不考虑调用时的环境。

// 也许不可能在operator[]内部区分左值还是右值操作，但我们仍然能区别对待读操作和写操作，如果我们将判断读还是写的行为推迟到我们知道operator[]的结果被怎么使用之后的话。
// 我们所需要的是有一个方法将读或写的判断推迟到operator[]返回之后。
// proxy类可以让我们得到我们所需要的时机，因为我们可以修改operator[]让它返回一个（代理字符的）proxy对象而不是字符本身。
// 我们可以等着看这个proxy怎么被使用。如果是读它，我们可以断定operator[]的调用是读。如果它被写，我们必须将operator[]的调用处理为写。
// 在proxy类上只能做三件事：
//  1 创建它，也就是指定它扮演哪个字符。
//  2 将它作为赋值操作的目标，在这种情况下可以将赋值真正作用在它扮演的字符上。这样被使用时，proxy类扮演的是左值。
...
const String::CharProxy String::operator[](int index) const {
  // operator[]的const版本返回一个const的proxy对象。因为CharProxy::operator=是个非const的成员函数，这样的proxy对象不能作赋值的目标使用。
  // 因此，不管是从operator[]的const版本返回的proxy对象，还是它所扮演的字符都不能作左值使用。很方便啊，它正好是我们想要的const版本的operator[]的行为。
  return CharProxy(const_cast<String&>(*this), index);
  // 同样要注意在const的operator[]返回而创建CharProxy对象时，对*this使用的const_cast。
  // 这使得它满足了CharProxy类的构造函数的需要，它的构造函数只接受一个非const的String类。
  // 类型转换通常是领人不安的，但在此处，operator[]返回的CharProxy对象自己是const的，所以不用担心String内部的字符可能被通过proxy类被修改。
}
...
// 再看CahrProxy的赋值操作的实现，这是我们必须处理proxy对象所扮演的字符作赋值的目标（即左值）使用的地方。
String::CharProxy& String::CharProxy::operator=(const CharProxy& rhs) {
  // 我们将写操作的实现移入CharProxy的赋值操作中，于是可以避免非const的operator[]的调用只是作右值时所多付出的写操作的代价。
  // 随便提一句，这个函数需要访问string的私有数据成员value。这是前面将CharProxy申明为string的友元的原因。
...
String::CharProxy& String::CharProxy::operator=(char c) {
  ...
  // 作为一个资深的软件工程师， 你当然应该消除这两个赋值操作中的代码重复，应该将它们放入一个私有成员函数中供二者调用。
  if (theString.value->isShared()) {
    theString.value = new StringValue(theString.value->data);
  }
...
// 我们很喜欢proxy对象对其所扮演的对象的无缝替代，但这很难实现。这是因为，右值不只是出现在赋值运算的情况下，那时，proxy对象的行为就和实际的对象不一致了。
// 如果String::operator[]返回一个CharProxy而不是char&，下面的代码将不能编译：
// char* p = &s1[1];  // error!
// 表达式s1[1]返回一个CharProxy，于是“=”的右边是一个 CharProxy*。没有从CharProxy*到char *的转换函数，所以p的初始化过程编译失败了。
...
// 最后一种proxy对象不能无缝替换实际对象的情况是隐式类型转换。当proxy对象隐式转换为它所扮演的实际对象时，一个用户自定义的转换函数被调用了。
...
// 由于问题发生在隐式类型转换上，它很难解决。实际上，更好的设计应该是申明它的构造函数为explicit，以使得第一次的调用watchTV()的行为都编译失败。

// Proxy类可以完成一些其它方法很难甚至不可能实现的行为。多维数组是一个例子，左/右值的区分是第二个，限制隐式类型转换是第三个。
// 同时，proxy类也有缺点。作为函数返回值，proxy对象是临时对象，它们必须被构造和析构。这不是免费的，虽然此付出能从具备了区分读写的能力上得到更多的补偿。
// Proxy对象的存在增加了软件的复杂度，因为额外增加的类使得事情更难设计、实现、理解和维护。
// 最后，从一个处理实际对象的类改换到处理proxy对象的类经常改变了类的语义，因为proxy对象通常表现出的行为与实际对象有些微妙的区别。
// 有时，这使得在设计系统时无法选择使用proxy对象，但很多情况下很少有操作需要将proxy对象暴露给用户。
// 例如，很少有用户取上面的二维数组例子中的Array1D对象的地址，也不怎么有可能将下标索引的对象作参数传给一个期望其它类型的函数。
// 在很多情况下，proxy对象可以完美替代实际对象。当它们可以工作时，通常也是没有其它方法可采用的情况。
...
int main() {
  ...
  // intArray[5] += 5;  // error!
  // ++intArray[5];     // error!
  // 当operator[]作最简单的赋值操作的目标时，是成功的，但当它出现operator+=和operator++的左侧时，失败了。
  ...
  // 这是一个极大量的工作，你可能不愿意去做的。不幸的是，你要么去做这些工作，要么没有这些操作，不能两全。
  ...
  // 因此，你的编译器发出了抱怨。要使得proxy对象的行为和它们所扮演的对象一致，你必须重载可作用于实际对象的每一个函数。

  String s = "+C+";
  // swap(s[0], s[1]);
  // String::operator[]返回一个CharProxy对象，但swap()函数要求它所参数是char &类型。
  // 一个CharProxy对象可以印式地转换为一个char，但没有转换为char&的转换函数。
  // 而它可能转换成的char并不能成为swap的char&参数，因为这个char是一个临时对象（它是operator char()的返回值），

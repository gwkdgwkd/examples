...
// 既然类似，完全可以利用泛型技术，将它们设计成适用所有容器的通用算法，
// 从而将容器和算法分离开，但实现此目的需要有一个类似中介的装置，
// 它除了要具有对容器进行遍历读写数据的能力之外，
// 还要能对外隐藏容器的内部差异，从而以统一的界面向算法传送数据。
// 简单来讲，迭代器和C++的指针非常类似，它可以是需要的任意类型，
// 通过迭代器可以指向容器中的某个元素，如果需要，还可以对该元素进行读/写操作。

// STL标准库为每一种标准容器定义了五种迭代器类型，不同容器的迭代器也不同，
// 其功能强弱也有所不同，迭代器的功能强弱，决定了该容器是否支持STL中的某种算法。
// 常用的迭代器按功能强弱分为：
// 1.输入迭代器和输出迭代器
//   比较特殊，它们不是把数组或容器当做操作对象，而是把输入流/输出流作为操作对象。
// 2.前向迭代器（forward iterator）
//   假设p是一个前向迭代器，则p支持++p，p++，*p操作，还可以被复制或赋值，
//   可以用==和!=运算符进行比较，此外，两个前向迭代器可以互相赋值。
// 3.双向迭代器（bidirectional iterator）
//   双向迭代器具有前向迭代器的全部功能，除此之外，则还可以进行--p或者p--操作。
// 4.随机访问迭代器（random access iterator）
//   随机访问迭代器具有双向迭代器的全部功能，除此之外，还支持以下操作：
//   p+=i：使得p往后移动i个元素
//   p-=i：使得p往前移动i个元素
//   p+i：返回p后面第i个元素的迭代器
//   p-i：返回p前面第i个元素的迭代器
//   p[i]：返回后面第i个元素的引用
//   两个随机访问迭代器还可以用<、>、<=、>=进行比较，p2-p1也是有定义的，
//   表示p2所指向元素和p1所指向元素的序号之差，即p2和p1之间的元素个数减1。
...
// 以上4种定义迭代器的方式，并不是每个容器都适用，有一部分容器支持以上4种方式，
// 比如array、deque、vector，而有些容器只支持其中部分的定义方式，

...
// 对参数的说明：
//  ptr为内存区块的指针，它可以是数组、变量、结构体等。fread()中的ptr用来存放读取到的数据，fwrite()中的ptr用来存放要写入的数据。
//  size：表示每个数据块的字节数。
//  count：表示要读写的数据块的块数。
//  fp：表示文件指针。
//  理论上，每次读写size*count个字节的数据。
// size_t是在stdio.h和stdlib.h头文件中使用typedef定义的数据类型，表示无符号整数，也即非负数，常用来表示数量。
...
int main() {
  ...
  if (0 == flag) {
    ...
    FILE *fp;
    if ((fp = fopen("demo", "rb+")) == NULL) {  // 以二进制方式打开

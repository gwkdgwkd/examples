...
// set容器具有以下几个特性：
// 1.不再以键值对的方式存储数据，
//   因为set容器专门用于存储键和值相等的键值对，
//   因此该容器中真正存储的是各个键值对的值（value）；
// 2.set容器在存储数据时，
//   会根据各元素值的大小对存储的元素进行排序（默认做升序排序）；
...
//                  增加了const属性，不能用于修改容器内存储的元素值。
// find(val)        在multiset容器中查找值为val的元素，
//                  如果成功找到，则返回指向该元素的双向迭代器；
//                  反之，则返回和end()方法一样的迭代器。
//                  另外，如果multiset容器用const限定，
//                  则该方法返回的是const类型的双向迭代器。
// lower_bound(val)	返回一个指向multiset中第一个大于或等于val的元素的双向迭代器。
//                  如果multiset容器用const限定，
//                  则该方法返回的是const类型的双向迭代器。
...
// max_size()       返回multiset容器所能容纳元素的最大个数，
//                  不同的操作系统，其返回值亦不相同。
// insert()	        向multiset容器中插入元素。
// erase()	        删除multiset容器中存储的指定元素。
// swap()	          交换2个multiset容器中存储的所有元素。
//                  这意味着，操作的2个multiset容器的类型必须相同。
// clear()	        清空multiset容器中所有的元素，即令multiset容器的size()为0。
// emplace()        在当前multiset容器中的指定位置直接构造新元素。
//                  其效果和insert()一样，但效率更高。
...
// 虽然multiset容器和set容器拥有的成员方法完全相同，
// 但由于multiset容器允许存储多个值相同的元素，
// 因此诸如count()、find()、lower_bound()、upper_bound()、

...
// 但这并不是一直有理，因为将“被共享的数据”抽取出来放在基类中，是一种正当的设计。
...
// 纯虚析构函数一定得定义它。
// 因为每一个派生类析构函数会被编译器扩展，以静态调用的方式调用其“每一个虚基类”以及“上一层基类”的析构函数。
// 因此，只要缺乏任何一个基类析构函数的定义，就会导致链接失败。
Abstract_base::~Abstract_base() {}
// 编译器不能压抑纯虚析构函数的调用，因为C++语言的一个保证前提：继承体系中每一个对象的析构函数都会被调用。
// 所以编译器不能压抑这个调用操作。
...
// 当编译器遇到下面的定义，无用的默认构造函数和析构函数都会别调用。
...
// 一个“临时性的定义”可以在程序中发生多次，那些实例会别链接器折叠起来，只留单独一个实体，
// 被放在程序data segment中一个“特别保留给未初始化的全局对象使用”的空间，这块空间被称为BSS（Block Started by Symbol）。
// C++并不支持“临时性的定义”，这是因为类构造行为的隐含应用之故。
// 虽然可以判断一个对象是Plain Ol' Data，但似乎没有必要搞得那么复杂。
// 因此，global在C++中被视为完全定义（它会阻止第二个或更多个定义）。
// C和C++的一个差异就在于，BSS data segment在C++中相对地不重要。
// C++的所有全局对象都被当做“初始化过的数据”来对待。
Point foobar() {
...
// 一般而言，如果代码中有需要许多函数都需要以传值方式传回一个局部对象，那么提供一个拷贝构造函数就比较合理。
// 甚至即使默认的按位拷贝语意已经足够。
// 因为它的出现会触发NRV优化。
// NRV优化后将不再需要调用拷贝构造函数，因为运算结果已经被直接放置与“被传回的对象”体内了。
...
// 构造中的对象就可以严格而正确地变成”构造过程中所幻化出来的每一个类“的对象。
...
// 何时需要供应参数给一个基类构造函数？
// 这种情况下在“类的构造函数的member initialization list”中调用该类的虚拟函数，仍然安全么？

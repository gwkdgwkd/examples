...
// 这里的const属性告诉编译器：本函数返回值只依赖于输入，
// 不会改变任何函数外的数据，因此没有任何副作用。
...
// 即可设定额外的功能。
// 以对齐方式为例，在C++11之前，
// 微软平台可以使用__declspec(align(x))来控制变量的对齐方式，代码如下：
...
// 这样设计得好处是：
// 既不会消除语言添加或者重载关键字的能力，又不会占用用户空间的关键字的名字空间。
...
// 不返回的函数除了是有异常抛出的函数外，
// 还有可能是有终止应用程序语句的函数，或是有无限循环语句的函数等。
// 事实上，在C++11的标准库中，都能看到形如：
[[noreturn]] void abort(void) noexcept;
// abort总是会导致程序运行的停止，
// 甚至连自动变量的析构函数以及本该在atexit()时调用的函数全都不调用就直接退出了。

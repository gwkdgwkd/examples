...
// 全局对象
// C++保证，一定会在main()函数中第一用到全局变量之前，把全局对象构造出来，
// 而在main()结束之前把全局变量摧毁掉。
...
// nm命令（导出对象文件的符合表格）输出到munch程序，munch搜索以__sti或__std开头的名称，
// 然后把函数名称加到sti函数和std函数的跳离表格中。
// 接下来它把这个表格写到一个小的program text文件中，然后CC命令被重新激活，将这个内含表格的文件加以编译。
// 使用静态初始化的对象有些缺点：
// 1.如果exception handing被支持，那些对象将不能够被放置于try区段之内。
//   这对于被静态调用的构造函数可能是特别无法接受的，
//   因为任何的throw操作将必然触发exception handing library默认的terminate()函数。
// 2.另一个缺点是为了控制需要跨越模块做静态初始化对象的相依顺序而扯出来的复杂度。
Widget global(1);
Widget g;
...
// 除了将获得的指针所指的地址传回外，还要在arena所值得地址上执行Point2w的构造函数。
// 然而有一个轻微的不良行为：
// 在原已存在的一个对象上构造新的对象，而该现有的对象有一个析构函数，这个析构函数并不会被调用。
// 如何知道所指的区域是否需要先析构呢？这个问题在语言层面上并没有解答。
// 一个习惯是，令执行new的这一端也要负责执行析构的责任。
// C++标准说，arena要不指向相同的类，要不就是一块新鲜内存，足够容纳该类型的对象。

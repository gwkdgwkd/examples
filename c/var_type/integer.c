...
// 整数是编程中常用的一种数据，C语言通常使用int来定义整数（int是integer的简写。
// 在现代操作系统中，int一般占用4个字节（Byte）的内存，共计32位（Bit）。如果不考虑正负数，当所有的位都为1时它的值最大，为232-1=4,294,967,295 ≈ 43亿，
// 这是一个很大的数，实际开发中很少用到，而诸如1、99、12098等较小的数使用频率反而较高。
// 让整数占用更少的内存可以在int前边加short，让整数占用更多的内存可以在int前边加long。
...
// 目前我们使用较多的PC系统为Win XP、Win 7、Win 8、Win 10、Mac OS、Linux，在这些系统中，short和int的长度都是固定的，分别为2和4，大家可以放心使用，只有long的
// 长度在Win64和类Unix系统下会有所不同，使用时要注意移植性。
...
// 在C语言中也是一样，short、int、long都可以带上正负号。如果不带正负号，默认就是正数。
// C语言规定，把内存的最高位作为符号位。在符号位中，用0表示正数，用1表示负数。
// short、int和long类型默认都是带符号位的，符号位以外的内存才是数值位。如果只考虑正数，那么各种类型能表示的数值范围（取值范围）就比原来小了一半。
// 如果不希望设置符号位，可以在数据类型前面加上unsigned关键字。不能再表示负数了，但正数的取值范围更大了。
...
// 在内存中，整数一律采用补码的形式来存储。这意味着，当读取整数时还要采用逆向的转换，也就是将补码转换为原码。将补码转换为原码也很简单：先减去1，再将数值位取反即可。
...
//  假设6和18都是short类型的，现在我们要计算6-18 的结果，根据运算规则，它等价于6+(-18)。
//  如果采用原码计算，那么运算过程为：
//  6 - 18 = 6 + (-18)
...
//  = -12
//  这样一来，计算结果就正确了。
//  然而，这样还不算万事大吉，我们不妨将减数和被减数交换一下位置，也就是计算18-6的结果：
//  18 - 6 = 18 + (-6)
...
//  按照反码来计算，是不是小数减去大数正确，大数减去小数就不对了，始终相差1呢？
//  5 - 13 的运算过程为：
//  5 - 13 = 5 + (-13)
...
//  相差的这个1要进行纠正，但是又不能影响小数减去大数，怎么办呢？于是人们又绞尽脑汁设计出了补码，给反码打了一个“补丁”，终于把相差的1给纠正过来了。
//  6 - 18 = 6 + (-18)
...
//  = 8
// 小数减去大数，结果为负数，之前（负数从反码转换为补码要加1）加上的1，后来（负数从补码转换为反码要减1）还要减去，正好抵消掉，所以不会受影响。
// 而大数减去小数，结果为正数，之前（负数从反码转换为补码要加1）加上的1，后来（正数的补码和反码相同，从补码转换为反码不用减1）就没有再减去，
// 不能抵消掉，这就相当于给计算结果多加了一个1。
// 补码这种天才般的设计，一举达成了前面的两个目标，简化了硬件电路。

// short、int、long的长度分别是2、4、4或者8，只能存储有限的数值，当数值过大或者过小时，超出的部分会被直接截掉，数值就不能正确存储了，将这种现象称为溢出（Overflow）。
// 计算无符号数（unsigned 类型）的取值范围（或者说最大值和最小值）很容易，将内存中的所有位（Bit）都置为1就是最大值，都置为0就是最小值。
// 有符号数以补码的形式存储，计算取值范围也要从补码入手。以char类型为例：
//  按照传统的由补码计算原码的方法，那么1000 0000是无法计算的，因为计算反码时要减去1，1000 0000 需要向高位借位，而高位是符号位，不能借出去，所以这就很矛盾。
//  是不是该把1000 0000作为无效的补码直接丢弃呢？然而，作为无效值就不如作为特殊值，这样还能多存储一个数字。计算机规定，1000 0000这个特殊的补码就表示-128。
//  为什么偏偏是-128而不是其它的数字呢？
//   首先，-128使得char类型的取值范围保持连贯，中间没有“空隙”。
//   其次，我们再按照“传统”的方法计算一下-128的补码：
//    -128的数值位的原码是1000 0000，共八位，而char的数值位只有七位，所以最高位的1会覆盖符号位，数值位剩下000 0000。最终，-128的原码为1000 0000。
//    接着很容易计算出反码，为1111 1111。
//    反码转换为补码时，数值位要加上1，变为1000 0000，而char的数值位只有七位，所以最高位的1会再次覆盖符号位，数值位剩下000 0000。最终求得的-128的补码是1000 0000。
//  -128从原码转换到补码的过程中，符号位被1覆盖了两次，而负数的符号位本来就是1，被1覆盖多少次也不会影响到数字的符号。
//  虽然从1000 0000这个补码推算不出-128，但是从-128却能推算出1000 0000这个补码，这么多么的奇妙，-128这个特殊值选得恰到好处。
//  负数在存储之前要先转换为补码，“从-128推算出补码1000 0000”这一点非常重要，这意味着-128能够正确地转换为补码，或者说能够正确的存储。
// 在char的取值范围内只有一个零值，没有+0和-0的区别，并且多存储了一个特殊值，就是-128，这也是采用补码的另外两个小小的优势。
// 如果直接采用原码存储，那么0000 0000和1000 0000将分别表示+0和-0，这样在取值范围内就存在两个相同的值，多此一举。
// 另外，虽然最大值没有变，仍然是127，但是最小值却变了，只能存储到-127，不能存储-128了，因为-128的原码为1000 0000，这个位置已经被-0占用了。
...
int main() {
  ...
  int b2 = 0xffffffff;
  printf("a=%u, b=%d\n", a2, b2);  // a=0, b=-1
  // 变量a为unsigned int类型，长度为4个字节，能表示的最大值为0xFFFFFFFF，而0x100000000=0xFFFFFFFF+1，占用33位，已超出a所能表示的最大值，
  // 所以发生了溢出，导致最高位的1被截去，剩下的32位都是0。
  // a被存储到内存后就变成了0，printf从内存中读取到的也是0。
  // 变量b是int类型的有符号数，在内存中以补码的形式存储。0xffffffff的数值位的原码为1111 1111 …… 1111 1111，共32位，而int类型的数值位只有31
  // 位，所以最高位的1会覆盖符号位，数值位只留下31个1。

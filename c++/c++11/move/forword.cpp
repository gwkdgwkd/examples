...
// 1.参数t为非引用类型，这意味着在调用时，实参将值传递给形参的过程就需要额外进行一次拷贝操作；
// 2.无论调用function()函数模板时传递给参数t的是左值还是右值，对于函数内部的参数t来说，它有自己的名称，
//   也可以获取它的存储地址，因此它永远都是左值，也就是说，传递给otherdef()函数的参数t永远都是左值。
...
namespace n2 {
// 完美转发这样严苛的参数传递机制，很常用吗？C++98/03标准中几乎不会用到，
// 但C++11标准为C++引入了右值引用和移动语义，因此很多场景中是否实现完美转发，
...
// 但对于函数模板中使用右值引用语法定义的参数来说，它不再遵守这一规定，既可以接收右值，也可以接收左值,
// 此时的右值引用又被称为“万能引用”。
...
// C++11标准引入了一个模板函数forword<T>()，只需要调用该函数，就可以很方便地解决此问题。
...
// 总的来说，在定义模板函数时应该：
// 1.采用右值引用的语法格式定义参数类型，由此该函数既可以接收外界传入的左值，也可以接收右值；
// 2.使用C++11标准库提供的forword()模板函数修饰被调用函数中需要维持左、右值属性的参数。

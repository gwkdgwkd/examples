...
// 换句话说，作用域能够彼此包含，被包含（或者说被嵌套）的作用域称为内层作用域（inner scope），
// 包含着别的作用域的作用域称为外层作用域（outer scope）。
// 一旦在外层作用域中声明（或者定义）了某个名字，那么它所嵌套着的所有内层作用域中都能访问这个名字。
...
// 换句话说，一旦内层作用域有同名的函数，不管有几个，编译器都不会再到外层作用域中查找，
// 编译器仅把内层作用域中的这些同名函数作为一组候选函数，这组候选函数就是一组重载函数。
// 只有一个作用域内的同名函数才有重载关系，不同作用域内的同名函数会造成遮蔽，使得外层函数无效。
...
void testN1() {
  ...
  // 编译器继续到B类的作用域（外层作用域）中查找，仍然没有找到，再继续到A类的作用域中查找，
  // 结果就发现了func这个名字，于是查找结束，编译器决定调用A类作用域中的func()函数。
  // 这个过程叫做名字查找，也就是在作用域链中寻找与所用名字最匹配的声明（或定义）的过程。
  C obj;
  obj.func();                           // A::func
...
class C2 : public B {
 ...
 private:
  // 当基类A、B的成员变量被遮蔽时，仍然会留在派生类对象的内存中，C类新增的成员变量始终排在基类A、B的后面。
  int m_b;  // 遮蔽A类的成员变量
  int m_c;  // 遮蔽B类的成员变量
  int m_d;  // 新增成员变量
};

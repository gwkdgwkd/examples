...
// 基数排序不同于之前所介绍的各类排序，前边介绍到的排序方法或多或少的是通过使用比较和移动记录来实现排序，而基数排序的实现不需要进行对关键字的比较，
// 只需要对关键字进行“分配”与“收集”两种操作即可完成。
// 按照个位-十位-百位的顺序进行基数排序，此种方式是从最低位开始排序，所以被称为最低位优先法（简称“LSD法”）。
// 按照百位-十位-各位的顺序进行排序，称为最高位优先法（简称“MSD法”）。使用该方式进行排序同最低位优先法不同的是：当无序表中的关键字进行分配后，
// 相当于进入了多个子序列，后序的排序工作分别在各个子序列中进行（最低位优先法每次分配与收集都是相对于整个序列表而言的）。
...
// 基数，例如关键字是数字，无疑由0~9组成，基数就是10；如果关键字是字符串（字母组成），基数就是26
#define RADIX 10
...
void Distribute(SLCell *r, int i, ArrType f, ArrType e) {
  ...
  for (int p = r[0].next; p; p = r[p].next) {
    // 取出每个关键字的第i位，由于采用的是最低位优先法，所以，例如，第1位指的就是每个关键字的个位
    int j = r[p].keys[i];

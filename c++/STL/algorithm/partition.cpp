...
// partition()函数只会根据筛选条件将数据进行分组，
// 并不关心分组后各个元素具体的存储位置。
// 如果想在分组之后仍不改变各元素之间的相对位置，
// 可以选用stable_partition()函数。
// partition()、stable_partition()、
// partition_copy()和partition_point()定义于<algorithm>头文件中。
...
// 其组合[first,last)用于指定该函数的作用范围；
// pred用于指定筛选规则。
// 所谓筛选规则，其本质就是一个可接收1个参数且返回值类型为bool的函数，
// 可以是普通函数，也可以是一个函数对象。
// 同时，partition()函数还会返回一个正向迭代器，
// 其指向的是两部分数据的分界位置，
...
// stable_partition()函数可以保证对指定区域内数据完成分组的同时，
// 不改变各组内元素的相对位置。
// stable_partition()函数语法格式如下：
// BidirectionalIterator stable_partition(BidirectionalIterator first,
//                                        BidirectionalIterator last,
//                                        UnaryPredicate pred);
// 其中，first和last都为双向迭代器，
// 其组合[first,last)用于指定该函数的作用范围；
// pred用于指定筛选规则。
// 同时，stable_partition()函数还会返回一个双向迭代器，
// 其指向的是两部分数据的分界位置，
// 更确切地说，指向的是第二组数据中的第1个元素。
void func2() {
  std::vector<int> v{1, 2, 3, 4, 5, 6, 7, 8, 9};
...
// 需要类似partition()或者stable_partition()函数分组的功能，
// 但并不想对原序列做任何修改。
// 这种情况下，就可以考虑使用partition_copy()函数。
// 和stable_partition()一样，
...
//                                 UnaryPredicate pred);
// 其中，first和last为正向迭代器，
// [first,last)用于指定该函数的作用范围；
// pred用于指定数据的筛选规则。
// 所谓筛选规则，其实就是包含1个参数且返回值类型为bool的函数，
// 此函数可以是一个普通函数，也可以是一个函数对象。
// 同时，该函数会返回一个正向迭代器，
// 该迭代器指向的是[first,last]范围内第一个不符合pred筛选规则的元素。

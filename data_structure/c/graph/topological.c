...
// 重连通图其实就是没有关节点的连通图。
// 在重连通图中，只删除一个顶点及其相关联的边，肯定不会破坏其连通性。如果一直删除顶点，直到删除K个顶点及其关联的边后，图的连通性才遭到破坏，则称此重连通图的连通度为K。

// 判断重连通图的方法
// 对于任意一个连通图来说，都可以通过深度优先搜索算法获得一棵深度优先生成树。图中有，但是遍历时没有用到的边，简称“回边”。
...
// 左图中顶点之间的关系被称为“偏序”关系；右图中顶点之间的关系被称为”全序“关系。
...
// 用来表示某种活动间的优先关系的有向图简称为“AOV网”。
// 如果顶点之间只是具有偏序关系，那么拓扑排序的结果肯定不唯一；如果顶点之间是全序关系，那么拓扑排序得到的序列唯一。
// 在编写程序解决拓扑排序的问题时，大致思路为：首先通过邻接表将AOV网进行存储，由于拓扑排序的整个过程中，都是以顶点的入度为依据进行排序，所以需要根据建立的邻接表统计出各顶点的入度。
// 在得到各顶点的入度后，首先找到入度为0的顶点作为拓扑排序的起始点，然后查找以该顶点为起始点的所有顶点，如果入度为1，说明如果删除前一个顶点后，该顶点的入度为0，为拓扑排序的下一个对象。

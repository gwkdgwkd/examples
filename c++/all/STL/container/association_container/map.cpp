...
// 其中，各个键值对的键和值可以是任意数据类型，包括基本数据类型、结构体和类。
// 通常情况下，map容器中存储的各个键值对都选用string字符串作为键的类型。
// 在使用map存储多个键值对时，会自动根据各键值对的键的大小，按照规则进行排序。
// 默认情况下，map容器选用std::less<T>排序规则，进行升序排序。
// 当然，也可以手动指定map容器的排序规则，既可以选用STL中提供的其它排序规则，
// 比如std::greater<T>，也可以自定义排序规则。
// 需要注意的是，使用map容器存储的各个键值对，键的值既不能重复也不能被修改。
// map容器中存储的各个键值对不仅键的值独一无二，键的类型也会用const修饰，
// 这意味着只要键值对被存储到map容器中，其键的值将不能再做任何修改。
// map存储的都是pair类型的键值对元素，更确切的说，都是pair<const K,T>。
...
// > class map;
// map容器模板有4个参数，其中后2个参数都设有默认值，大多数场景中，
// 只需要前2个参数，有些场景可能会用到第3个参数，但最后一个参数几乎不会用到。

// map容器包含的成员方法：
// begin()          返回指向容器中排序后的第一个键值对的双向迭代器。
// end()            返回指向排序后的最后一个元素所在位置后一个位置的双向迭代器。
// rbegin()         返回指向排序后的最后一个元素的反向双向迭代器。
// rend()           返回指向排序后第一个元素所在位置前一个位置的反向双向迭代器。
// cbegin()         和begin()功能相同，只不过增加了const属性，不能修改键值对。
// cend()           和end()功能相同，只不过增加了const属性，不能修改键值对。
// crbegin()        和rbegin()功能相同，只不过增加了const属性，不能修改键值对。
// crend()          和end()功能相同，只不过增加了const属性，不能修改键值对。
// find(key)        查找键为key的键值对，若找到则返回指向该键值对的双向迭代器，
//                  反之，则返回和end()方法一样的迭代器。
// lower_bound(key) 返回一个指向map中第一个大于或等于key的键值对的双向迭代器。
// upper_bound(key) 返回一个指向map中第一个大于key的键值对的迭代器。
// equal_range(key) 该方法返回一个pair对象（包含2个双向迭代器），
//                  其中pair.first和lower_bound()方法的返回值等价，
//                  pair.second和upper_bound()方法的返回值等价。
//                  也就是说将返回一个范围，该范围中包含的键为key的键值对。
// empty()          若容器为空，则返回true，否则返回false。
// size()           返回当前map容器中存有键值对的个数。
// max_size()       返回map所能容纳键值对的最大数，不同的操作系统返回不相同。
// operator[]       map重载了[]运算符，通过键直接获取对应的值，像数组一样。
// at(key)          找到map中key键对应的值，如果找不到抛出out_of_range异常。
// insert()         向map容器中插入键值对。
// erase()          删除map容器指定位置、指定键（key）值或者指定区域内的键值对。
// swap()           交换2个map容器中存储的键值对，2个map键值对的类型必须相同。
// clear()          清空map容器中所有的键值对，即使map容器的size()为0。
// emplace()        在map中的指定位置处构造新键值对，和插入一样，但效率更高。
// emplace_hint()   在本质上和emplace()在map容器中构造新键值对的方式是一样的，
//                  不同之处在于，必须为该方法提供一个指示键值对生成位置的迭代器，
//                  并作为该方法的第一个参数。
// count(key)       在map容器中，查找键为key的键值对的个数并返回，map最大返回1。

// 对于begin、end、rbegin、rend、find、lower_bound、upper_bound，
// 如果map容器用const限定，则方法返回的是const类型的双向迭代器。
...
void func1() {
  ...
  // 3.可以利用先前已创建好的map容器，再创建一个新的map容器：
  ...
  // 无论是调用拷贝构造还是调用移动构造，都必须保证这2个容器的类型完全一致。

  // 4.map类模板还支持取已建map中指定区域内的键值对，创建并初始化新的map容器：
  ...
  //   默认情况下，map容器调用std::less<T>规则，升序排序。
...
void func2() {
  ...
  // map提供了2种方法可直接获取map容器指定键对应的值，还有两个其他的方法：

  // 1.map类模板中对[]运算符进行了重载，类似于借助数组下标可以直接访问数组元素
  ...
  //   和前一种方法相比，at()也需要根据指定的键，才能从容器中找到该键对应的值；
  //   不同之处在于，如果查找失败不会增加键值对，而是抛出out_of_range异常。
  ...
  // 3.除了可以直接获取指定键对应的值之外，还可以借助find()间接实现此目的，
  //   和以上2种方式不同的是，该方法返回的是一个迭代器，即如果查找成功，
  //   该迭代器指向查找到的键值对，反之，则指向map容器最后一个键值对之后的位置，
  //   和end()成功方法返回的迭代器一样。
  ...
  // 4.如果以上方法都不适用，还可以遍历整个map，找到包含指定键的键值对
  ...
  // 获取键值对对应的值，更推荐使用at()成员方法，因为该方法既简单又安全。
}

void func3() {
  // 除了使用[]向map中添加新键值对外，map还提供有insert()成员方法，
  // 该方法专门用来向map容器中插入新的键值对。
  // 这里所谓的插入，指的是insert()可以将新的键值对插入到map的指定位置，
  ...
  //   即无论是局部键值对变量还是全局键值对变量，都采用普通引用传递的方式；
  ...
  //   其中val为要插入的键值对变量。
  //   注意，和第1种不同，insert()方法返回的是迭代器，而不再是pair对象：
  ...
  //   其中first和last都是迭代器，<first,last>可以表示某map中的指定区域。
...
void func4() {
  // map模板类中提供有operator[]和insert()，这2个成员方法具有相同的功能，
  // 它们既可以向map中添加新的键值对元素，也可以修改map已存储键值对的值。
  // operator[]和insert()的功能发生了重叠，谁的执行效率更高呢？
  // 1.向map容器中增添元素，insert()效率更高：
  //   使用operator[]添加新键值对元素的流程是，先构造一个有默认值的键值对，
  //   key是期望值，value是默认值，然后再为其value赋值。
  //   那么，为什么不直接构造一个要添加的键值对元素呢？
  //   insert就是这样做的，它省略了创建临时value对象的过程以及析构该对象的过程，
  //   同时还省略了调用重载的赋值运算符。
  //   由此可见，同样是完成添加新键值对，insert()比operator[]的执行效率更高。
  std::map<int, A> m;
  A a(2);  // A(int)
  m.insert({1, a});
  // copy
  ...
  //   insert()在进行更新操作之前，需要有一个pair类型元素做参数。
  ...
  //   因此，对于更新已经存储在map容器中键值对的值，应优先使用operator[]。
}

void func5() {
  // map还提供了emplace()和emplace_hint()，也可以向map中插入新的键值对。
  // 和insert()相比，emplace()和emplace_hint()的使用要简单很多，只有一种格式。
  // 1.emplace()方法的语法格式如下：
  //   template <class... Args>
  ...
  // 向map中插入时，emplace()和emplace_hint()执行效率更高，不用移动键值对。
  // 原因很简单，它们向map容器插入键值对时，底层的实现方式不同：
  // 使用insert()向map容器中插入键值对的过程是，先创建该键值对，
  // 然后再将该键值对复制或者移动到map容器中的指定位置；
  // 使用emplace()或emplace_hint()是直接在map容器中的指定位置构造该键值对。
  std::map<int, A> m1;
  m1.insert({1, A(2)});
  // A(int)
  // move
...
void func6() {
  // map还提供有lower_bound和upper_bound，它们的功能类似，唯一的区别在于：
  ...
  // lower_bound(key)和upper_bound(key)更多用于multimap，map中很少用。
  ...
  // 和lower_bound(key)、upper_bound(key)一样，该方法也更常用于multimap，
  // 因为map中各键值对的键的值都是唯一的，因此map调用此方法，最多返回1个键值对。
}

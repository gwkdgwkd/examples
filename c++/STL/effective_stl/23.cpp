...
// 许多STL程序员，当需要一个可提供快速查找功能的数据结构时，立刻会想到关联容器。
// 没有问题，只要合适就行。
...
// 很多程序使用数据时并不这样混乱，一般为：
// 1.设置阶段：
//   创建数据结构，并插入大量数据。这个阶段，几乎所有的操作都是插入和删除，很少或几乎没有查找操作。
// 2.查找阶段：
//   这个阶段几乎所有的操作都是查找操作，很少或几乎没有插入和删除操作。
// 3.重组阶段：
//   改变数据的内容，或许是删除所有数据，再插入新的数据。行为上，与阶段1类似，这个阶段结束后，有回到阶段2。
// 对于这样使用数据来说，vector可能比关联容器提供了更好的性能（无论是时间还是空间）。
// 但必须是排序的vector才可以，
// 因为只有对排序的容器才能够正确地使用查找算法binary_search、lower_bound和equal_range等。

// 为什么通过（排序的）vector执行的二分搜索，比通过二叉查找树执行的二分搜索具有更好的性能呢？
// 1.大小的问题，关联容器存储一个对象所伴随的空间开销至少是3个指针；
//   vector中存储对象，不会有任何额外开销，
// 2.但vector末尾可能会有空闲的空间，不过平均到每个对象上，这些开销通常是微不足道的，
//   而且如果有必要，可以用swap技巧去除。
...
// 所以查找的比较函数必须带一个与键同类型的对象（将被查找的值）和一个pair对象（存储在vector中的对象）作为参数，
// 两个参数的类型不相同。

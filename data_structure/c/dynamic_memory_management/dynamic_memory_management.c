...
// 动态内存管理机制，主要包含两方面内容，用户申请内存空间时，系统如何分配；
// 用户使用内存空间完成后，系统如何及时回收。
// 另外，内存不断分配与回收的过程，会产生诸多内存碎片，
// 但通过利用数据结构，内存碎片化的问题能够得到有效的解决。
// 其实，无论是那种操作系统，其内存管理机制都使用了大量数据结构的知识，大大提高内存管理的效率。

// 数据结构之动态内存管理机制
// 对于计算机中的内存来说，称已经分配给用户的的内存区统称为占用块；
// 还未分配出去的内存区统称为空闲块或者可利用空间块。
...
// 分配存储空间的方式。
// 由于链表中各结点的大小不一，在用户申请内存空间时，
// 就需要从可利用空间表中找出一个合适的结点，有三种查找的方法：
...
// 该方法适用于申请内存大小范围较广的系统。
// 最差拟合法，由于每次都是从存储空间最大的结点中分配给用户空间，所以链表中的结点大小不会起伏太大。
// 依次适用于申请分配内存空间较窄的系统。
...
// head域中包含有4部分：llink和rlink分别表示指向当前内存块结点的直接前驱和直接后继。
//                   tag值用于标记当前内存块的状态，是占用块（用1表示）还是空闲块（用0表示）。
//                   size用于记录该内存块的存储大小。
// foot域中包含有2部分：uplink是指针域，用于指向内存块本身，通过uplink就可以获取该内存块所在内存的首地址。
//                   tag同head域中的tag相同，都是记录内存块状态的。
// 注意：head域和foot域在本节中都假设只占用当前存储块的1个存储单位的空间，
...
// 对于当前空闲块p，p-1就是相邻的低地址处的空闲块的foot域，如果foot域中的tag值为0，表明其为空闲块；
...
// 对于初始容量为2m的一整块存储空间来说，形成的链表就有可能有m+1个，为了更好的对这些链表进行管理，
// 系统将这m+1个链表的表头存储在数组中，就类似于邻接表的结构。
// #define m 16  // 设定m的初始值
...
//  如果大小为2^k的链表为NULL，就需要依次查看比2^k大的链表，找到后从链表中删除，

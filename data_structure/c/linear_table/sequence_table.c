...
// 线性表，全名为线性存储结构，最简单的一种存储结构，专门用于存储逻辑关系为一对一的数据。
// 使用线性表存储数据的方式可以这样理解，即把所有数据用一根线儿串起来，再存储到物理空间中。
// 使用线性表存储的数据，如同向数组中存储数据那样，要求数据类型必须一致，
// 也就是说，线性表存储的数据，要么全部都是整形，要么全部都是字符串。

// 线性表，基于数据在实际物理空间中的存储状态，又可细分为：
// 1.将数据依次存储在连续的整块物理空间中，这种存储结构称为顺序存储结构，简称顺序表；
// 2.将数据分散的存储在物理空间中，通过一根线保存着它们之间的逻辑关系，
//   这种存储结构称为链式存储结构，简称链表。

// 对于一对一逻辑关系的数据，用某一元素的左侧（前边）或右侧（后边）不专业，专业术语是：
// 1.某一元素的左侧相邻元素称为直接前驱，位于此元素左侧的所有元素都统称为前驱元素；
// 2.某一元素的右侧相邻元素称为直接后继，位于此元素右侧的所有元素都统称为后继元素。

// 顺序表和链表由于存储结构上的差异，导致它们具有不同的特点，适用于不同的场景。
// 顺序表存储数据，需预先申请一整块足够大的存储空间，
// 然后将数据按照次序逐一存储，数据之间紧密贴合，不留一丝空隙。
// 链表的存储方式与顺序表截然相反，什么时候存储数据，什么时候才申请存储空间，
// 数据之间的逻辑关系依靠每个数据元素携带的指针维持。
// 顺序表和链表有以下几种不同：
// 1.开辟空间的方式
//   顺序表存储数据实行的是一次开辟，永久使用，即存储数据之前先开辟好足够的存储空间，
...
//   链表在存储数据时，每次只申请一个节点的空间，且空间的位置是随机的。
//   这种申请存储空间的方式会产生很多空间碎片，一定程序上造成了空间浪费。
//   不仅如此，由于链表中每个数据元素都必须携带至少一个指针，
//   因此，链表对所申请空间的利用率也没有顺序表高。
// 3.时间复杂度，解决不同类型的问题，顺序表和链表对应的时间复杂度也不同，
//   根据顺序表和链表在存储结构上的差异，问题类型主要分为以下2类：
//   顺序表适合问题中主要涉及访问元素的操作，元素的插入、删除和移动操作极少；
//   因为顺序表中存储的元素可以使用数组下标直接访问，无需遍历整个表，
//   因此使用顺序表访问元素的时间复杂度为O(1)；
//   顺序表中，插入、删除和移动数据可能会牵涉到元素的整体移动，因此时间复杂度至少为O(n);
//   链表适合问题中主要涉及元素的插入、删除和移动，访问元素的需求很少；
//   因为在链表中访问数据元素，需要从表头依次遍历，直到找到指定节点，时间复杂度为O(n);
//   链表中数据元素之间的逻辑关系靠的是节点之间的指针，当需要在链表中某处插入或删除节点时，
//   只需改变相应节点的指针指向即可，无需大量移动元素，
//   因此链表中插入、删除或移动数据所耗费的时间复杂度为O(1)；
// 综上所述，不同类型的场景，选择合适的存储结构会使解决问题效率成倍数地提高。

// 存储结构和存取结构，完全不是一码事。
// 所谓存储结构，指的是数据在内存中真实的存储状态，可分为即顺序存储结构和链式存储结构。
// 而存取结构，指的是存取数据的方式，可分为顺序存取结构和随机存取结构。
// 线性表的顺序存储结构是随机存取结构，而不是顺序存取结构；
// 线性表的链式存储结构，又可以称为顺序存取结构，而不是随机存取结构。
// 顺序存储结构最大的特点是，我们可以随机存或者取数据。
// 总之，线性表的顺序存储结构，又可以称为随机存取结构；
// 而线性表的链式存储结构（栈和队列），又可以称为顺序存取结构。

// 顺序表，全名顺序存储结构，是线性表的一种，顺序表存储数据时，
// 会提前申请足够大的物理空间，然后将数据依次存储起来，存储时做到数据元素之间不留一丝缝隙。
// 使用顺序表存储数据之前，除了要申请足够大小的物理空间之外，
// 为了方便后期使用表中的数据，顺序表还需要实时记录以下2项数据：
// 1.顺序表申请的存储容量；
...
table addTable(table t, int elem, int add) {
  ...
  // 插入时首先判断顺序表是否有多余的存储空间提供给插入的元素，如果没有，需要申请：
  if (t.length == t.size) {

...
// &v[0]的方法对于vector是适用的，但对于string却是不可靠的。
// 因为：
// 1.string中的数据不一定存储在连续的内存中；
// 2.string的内部表示不一定是以空字符结尾的。
// string中有成员函数c_str，该函数返回一个指向字符串的值的指针，
// 而且该指针可用于C：
void doSomething(const char* pString) { std::cout << pString << std::endl; }

// 上面两种情况，要传入的指针都是指向const的指针，被用来读取，
// 而不是改写这些数据的API，到此为止，这是最安全的方式。
// 对于string，这是唯一能做的，
...
// 对于vector，多了一点灵活性，
// 通常是可以修改元素的值，但不能试图改变元素的个数。
// 比如，不能试图在vector的未使用的容量中创建新元素，
// 不然内部将会变得不一致，size就按个产生不正确的结果。
// 如果容量和大小相等时，真正可怕的事情就会发生。
// 有些vector对数据有额外的限制，C API改变数据时，
// 也要保证这些额外的限制还能被满足，比如要保持有序。

// 想用C API中的元素初始化一个vector，
// 那么可以利用vector和数组的内容布局兼容性，
// 想API传入该矢量中元素的存储区域：

...
// C风格的类型转换:
// 一来过于粗鲁，能允许你在任何类型之间进行转换。不过如果要进行更精确的类型转换，这会是一个优点。
//  把一个指向const对象的指针转换成指向非const对象的指针，把一个指向基类的指针转换成指向子类的指针（即完全改变对象类型）。
//  传统的C风格的类型转换不对上述两种转换进行区分。（这一点也不令人惊讶，因为C风格的类型转换是为C语言设计的，而不是为C++语言设计的）。
// 二来C风格的类型转换在程序语句中难以识别。在语法上，类型转换由圆括号和标识符组成，而这些可以用在C++中的任何地方。
//  人工阅读很可能忽略了类型转换的语句，而利用象grep的工具程序也不能从语句构成上区分出它们来。
// 还有第三个理由：也许让类型转换符不美观和键入麻烦是一件好事。

// C++通过引进四个新的类型转换操作符克服了C风格类型转换的缺点，这四个操作符是static_cast,const_cast,dynamic_cast和reinterpret_cast。
// 这样的类型转换不论是对人工还是对程序都很容易识别。

// static_cast 在功能上基本上与C风格的类型转换一样强大，含义也一样。它也有功能上限制:
//  不能用static_cast象用C风格的类型转换一样把struct转换成int类型;
//  不能把double类型转换成指针类型；
...
// 这个含义被编译器所约束。如果你试图使用const_cast来完成修改constness或者volatileness属性之外的事情，你的类型转换将被拒绝。
...
// 这就是说，你能用dynamic_cast把指向基类的指针或引用转换成指向其派生类或其兄弟类的指针或引用，而且你能知道转换是否成功。
...
// 使用reinterpret_cast这个操作符的类型转换，转换结果几乎都是执行期定义（implementation-defined）。因此，使用reinterpret_casts的代码很难移植。
...
// 所以你应该避免转换函数指针类型，除非你处于着背水一战和尖刀架喉的危急时刻。一把锋利的刀。一把非常锋利的刀。

// 如果你使用的编译器缺乏对新的类型转换方式的支持，你可以用传统的类型转换方法代替static_cast, const_cast以及reinterpret_cast。
// 也可以用下面的宏替换来模拟新的类型转换语法：
#define static_cast(TYPE, EXPR)((TYPE)(EXPR))
#define const_cast(TYPE, EXPR)((TYPE)(EXPR))
#define reinterpret_cast(TYPE, EXPR)((TYPE)(EXPR))
// 没有一个容易的方法来模拟dynamic_cast的操作，但是很多函数库提供了函数，安全地在派生类与基类之间进行类型转换。
// 如果你没有这些函数而你有必须进行这样的类型转换，你也可以回到C风格的类型转换方法上，但是这样的话你将不能获知类型转换是否失败。
...
int main() {
  ...
  Widget *pw1;
  // 正确，传递给update函数一个指针是指向变量类型为SpecialWidget的pw的指针，如果pw确实指向一个对象。否则传递过去的将使空指针
  update(dynamic_cast<SpecialWidget *>(pw1));
  // 正确，传递给updateViaRef函数SpecialWidget pw引用，如果pw确实指向了某个对象。否则将抛出异常

...
// 和unordered_set容器不同的是，unordered_multiset容器可以同时存储多个值相同的元素，
...
// 而是和unordered_set容器共用同一个<unordered_set>头文件，并且也位于std命名空间。
...
// Key 确定容器存储元素的类型，如果读者将unordered_multiset看做是存储键和值相同的键值对的容器，
...
// unordered_multimap容器的成员方法(unordered_multiset供的方法，无论是种类还是数量，都和unordered_set类模板一样):
...
//                    所谓负载因子，指的是的当前容器中存储元素的数量（size()）和使用桶数（bucket_count()）的比值，
...
// 和unordered_set容器一样，unordered_multiset模板类也没有重载[]运算符，没有提供at()成员方法。

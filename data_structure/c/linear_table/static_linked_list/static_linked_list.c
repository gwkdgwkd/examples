...
// 数据之间"一对一"的逻辑关系通过一个整形变量（称为"游标"，和指针功能类似）维持（和链表类似）。
...
//  这就造成了静态链表中需要使用另一条链表（通常称为"备用链表"）来记录空间存储空间的位置，以便后期分配给新添加元素使用。
...
//  将存有目标元素的节点从数据链表中摘除；
//  将摘除节点添加到备用链表，以便下次再用；
// 提示：若问题中涉及大量删除元素的操作，建议读者在建立静态链表之初创建一个带有头节点的静态链表，
// 方便实现删除链表中第一个数据元素的操作。
...
// 静态链表查找元素
// 静态链表查找指定元素，由于我们只知道静态链表第一个元素所在数组中的位置，

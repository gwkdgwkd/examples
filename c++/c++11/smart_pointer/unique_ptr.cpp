...
// 作为智能指针的一种，unique_ptr指针自然也具备在适当时机自动释放堆内存空间的能力。
...
// unique_ptr是以模板类的形式提供的，unique_ptr<T>定义在<memory>头文件，并位于std命名空间中。
...
//                通过->运算符可以获取其内部的指定成员。
// operator =() 	重载了=赋值号，从而可以将nullptr或者一个右值unique_ptr指针
//                直接赋值给当前同类型的unique_ptr指针。
// operator []() 	重载了[]运算符，当unique_ptr指针指向一个数组时，
//                可以直接通过[]获取指定下标位置处的数据。
// get() 	        获取当前unique_ptr指针内部包含的普通指针。
// get_deleter() 	获取当前unique_ptr指针释放堆内存空间所用的规则。
// operator bool()unique_ptr指针可直接作为if语句的判断条件，以判断该指针是否为空，
//                如果为空，则为false；反之为true。

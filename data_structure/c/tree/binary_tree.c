...
//   对于一个二叉树来说，除了度为0的叶子结点和度为2的结点，
//   剩下的就是度为1的结点（设为n1），那么总结点n=n0+n1+n2。
//   同时，对于每一个结点来说都是由其父结点分支表示的，假设树中分枝数为B，那么总结点数n=B+1。
//   而分枝数是可以通过n1和n2表示的，即B=n1+2*n2，所以，n用另外一种方式表示为n=n1+2*n2+1。
//   两种方式得到的n值组成一个方程组，就可以得出n0=n2+1。
...
// 若节点i有左右孩子，则其左孩子为2*i，右孩子为2*i+1，此性质可用于还原数组中存储的完全二叉树。
...
// 2.节点存储的数据（data）；
// 3.指向右孩子节点的指针（Rchild）。
// 在某些实际场景中，可能会做查找某节点的父节点的操作，这时可以在节点结构中再添加一个指针域，
...
// 任意一棵普通树都可以转化为二叉树，而森林是由多棵普通树构成的，
// 因此自然也可以转化为二叉树，其转化方法是：
// 1.首先将森林中所有的普通树各自转化为二叉树；
// 2.将森林中第一棵树的树根作为整个森林的树根，
//   其他树的根节点看作是第一棵树根节点的兄弟节点，采用孩子兄弟表示法将所有树进行连接。

...
void func2() {
  ...
  // 访问单个元素方法一：通过容器名[]的方式直接访问和使用容器中的元素，这和C++标准数组访问元素的方式相同：
  ...
  a.front() += 5;
  a.back() += 6;
  // 访问单个元素方法三：通过调用data函数可以得到指向容器首个元素的指针，通过该指针，可以获得容器中的各个元素：
  int* data = a.data();
  *(data + 3) += 9;
  for (int i = 0; i < a.size(); ++i) {
    std::cout << *(data + i) << " ";
  ...
  // array容器之所以提供empty()，对于其他元素可变或者元素可删除的容器（例如vector、deque等）来说，
...
// 除此之外，C++11标准库还新增加了begin()和end()这2个函数，和array容器包含的begin()和end()成员函数不同的是，
...
void func3() {
  ...
  // 访问单个元素方法四：array容器还提供了get<n>模板函数，它是一个辅助函数，能够获取到容器的第n个元素。
  // 需要注意的是，该模板函数中，参数的实参必须是一个在编译时可以确定的常量表达式，所以它不能是一个循环变量。
...
//  at()成员函数，可以有效防止越界操纵数组的情况；
//  fill()函数可以实现数组的快速初始化；
//  swap()函数可以轻松实现两个相同数组（类型相同，大小相同）中元素的互换。
// 总之，array 容器就是普通数组的“升级版”，使用普通数组能实现的，使用array容器都可以实现，
...
void func4() {
  ...
  // 当两个array容器满足大小相同并且保存元素的类型相同时，两个array容器可以直接直接做赋值操作，
  // 即将一个容器中的元素赋值给另一个容器：
  addr1 = addr2;
  ...
  // 逐个取两容器中的元素进行大小比较（根据ASCII码表），直到遇到两个不相同的元素，那个元素的值大，则该容器就大。
}

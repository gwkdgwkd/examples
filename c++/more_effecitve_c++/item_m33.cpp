...
void func1() {
  ...
  *pAnimal1 = *pAnimal2;
  ...
  // 这里有两个问题:
  // 1.最后一行的赋值运算调用的是Animal类的，虽然相关对象的类型是Lizard。
...
class Lizard1 : public Animal1 {
 public:
  ...
  // 基于C++语言最近作出的修改，我们可以修改返回值的类型（于是每个都返回正确的类的引用），
  // 但C++的规则强迫我们申明相同的参数类型。
  ...
  virtual Lizard1& operator=(const Animal1& rhs) {
    ...
    // 如果rhs不是Lizard类型，函数传递出dynamic_cast转换失败时抛的bad_cast类型的异常。
    // 即使不在乎有异常，这个函数看起来也是没必要的复杂和昂贵，dynamic_cast必要引用一个type_info结构；
...
class Lizard2 : public Animal1 {
 public:
  ...
  // 我们可以处理这种情况而无需增加复杂度或花费dynamic_cast，
...
// 更重要的是，它要求使用Lizard和Chicken的用户必须在每次赋值操作时都准备好捕获bad_cast异常的准备。
// 如果他们没有这么做的话，那么不清楚我们得到的好处是否超过最初的方案。
// 指出了这个关于虚赋值操作的令人非常不满意的状态后，
// 在最开始的地方重新整理以试图找到一个方法来阻止用户写出有问题的赋值语句是有必要的。
...
// 于是，Lizard对象可以赋值给Lizard对象，Chicken对象可以赋值给Chicken对象，但部分或混合类型赋值被禁止：
class Animal2 {
 public:
  Animal2() { a = 0; }
  Animal2(int i) : a(i) {}
...
// 允许Animal对象间的赋值而阻止Lizard和Chicken对象通过Animal的指针进行部分赋值的两难问题仍然存在。
// 程序该怎么办？最容易的事情是排除Animal对象间赋值的需求，其最容易的实现方法是将Animal设计为抽象类。
// 作为抽象类，Animal不能被实例化，所以也就没有了Animal对象间赋值的需求了。
// 当然，这导致了一个新问题，因为我们最初的设计表明Animal对象是必须的。有一个很容易的解决方法：
// 不用将Animal设为抽象类，我们创一个新类叫AbstractAnimal，
...
// 肯定，这些代码需要重新编译，
// 但这是为获得“确保了编译通过的赋值语句的行为是正确的而行为可能不正确的赋值语句不能编译通过”所付出的很小的代价。
// 要使得这一切工作，AbstractAnimal类必须是抽象类，它必须至少有一个纯虚函数。
// 大部分情况下，带一个这样的函数是没问题的，但在极少见的情况下，
// 你会发现需要创一个如AbstractAnimal这样的类，没有哪个成员函数是自然的纯虚函数。
// 此时，传统方法是将析构函数申明为纯虚函数；这也是上面所采用的。
// 为了支持多态，基类总需要虚析构函数，将它再多设为纯虚的唯一麻烦就是必须在类的定义之外实现它。
...
// 如果你有两个实体类C1和C2并且你喜欢C2公有继承自C1，你应该将两个类的继承层次改为三个类的继承层次，
// 通过创造一个新的抽象类A并将C1和C2都从它继承。
// 这种修改的重要价值是强迫你确定抽象类A。

// 这一切导致了一些令人不安的思考。
// 毕竟，每个类都完成了某些类型的抽象，我们不应该在此继承体系中创造两个类来针对每个原型吗？
// 一个是抽象类来表示其抽象部分(to embody the abstract part of the abstraction)，
// 一个是实体类来表示对象生成部分(to embody the object-generation part of the abstraction)？
// 不应该，如果你这么做了，将使得继承体系中有太多的类。
// 这样的继承体系是难以理解的，难以维护的，编译的代价很昂贵，这不是面向对象设计的目的。
// 其目的是：确认有用的抽象，并强迫它们（并且只有它们）放入如抽象类这样的实体。
// 但怎么确认有用的抽象？谁知道什么抽象在将来被证明有用？谁能预知他将来要从什么进行继承？
// 在一个原型第一次被需要时，
// 我们无法证明同时创造一个抽象类（为了这个原型）和一个实体类（为了原型对应的对象）是正确的，
// 但第二次需要时，我们就能够这么做是正确的。
// 我描述过的修改简单地实现了这个过程，并且在这么做的过程中强迫设计着和程序员明确表达那些有用的抽象，
// 即使他们不知道那些有用的原型，这也碰巧使得构建正确的赋值行为很容易。

// 假设你处理的只有一种传输协议，也只有一种包。
// 也许你听说了其它协议和数据包类型的存在，但还从未支持它们，也没有任何计划以在未来支持它们。
// 你会为数据包（for the concept that a packet represents）既设计一个抽象类吗，
// 又设计一个你实际使用的实体类？
// 如果你这么做了，你可以在以后增加新的数据包而不用改变基类。
// 这使得你增加新的数据包类型时程序不用重新编译。
// 但这种设计需要两个类，而你现在只需要一个（针对于你现在使用的特殊数据包类型）。
...
// “当发现需求从一个实体类派生出另外一个实体类时”，这也不是唯一需要引入抽象类的地方。
// 不管怎么说啦，需要通过公有继承将两个实体类联系起来，通常表示需要一个新的抽象类。
// 这种情况是如此常见，所以引起了我们的深思。
// 第三方的C++类库越来越多，当发现你需要从类库中的实体类派生出一个新的实体类，
// 而这个库你只有只读权时，你要怎么做？
// 你不能修改类库以加入一个新的抽象类，所以你的选择将很有限、很无趣：
// 1.从已存在的实体类派生出你的实体类，并容忍我们在本Item开始时说到的赋值问题。
...
// 但对于你能控制的代码，遵守它可以提高程序的可靠性、健壮性、可读性、可扩展性。

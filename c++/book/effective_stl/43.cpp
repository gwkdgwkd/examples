...
//   这意味着每次循环时，函数list::end()都要被调用，但并不需要每次都调用它，
//   因为并没有改变链表，这个函数调用一次就够了：

...
// 首先可以明确的是，map和multimap容器只能采用“先删除，再添加”的方式修改某个键值对的键。
// 原因很简单，C++ STL标准中明确规定，map和multimap容器用于存储类型为pair<const K, V> 的键值对。
// 显然，只要目标键值对存储在当前容器中，键的值就无法被修改。
// 直接修改map或multimap容器中某个键值对的键是行不通的。
// 但对于set或者multiset容器来说，却是可行的。
// 和map、multimap不同，C++ STL标准中并没有用const限定set和multiset容器中存储元素的类型。
// 换句话说，对于set<T>或者multiset<T>类型的容器，其存储元素的类型是T而不是const T。
// 总之，set和multiset容器的元素类型没有用const修饰。
// 所以从语法的角度分析，我们可以直接修改容器中元素的值，但一定不要修改元素的键。
...
void func1() {
  ...
  // 但借助const_cast，我们可以直接修改map和multimap容器中元素的键。
  ...
  // 不要试图修改map或者multimap容器中键值对的键，这违反了C++ STL标准的规定。
}
...
void func2() {
  ...
  // set容器中每个元素也可以看做是键和值相等的键值对，但对于这里的s容器来说，
  // 其实每个student对象的id才是真正的键，其它信息（name和age）只不过是和id绑定在一起而已。
  // 因此，在不破坏s容器中元素的有序性的前提下（即不修改每个学生的id），学生的其它信息是应该允许修改的，
  // 但有一个前提，即s容器中存储的各个student对象不能被const修饰（这也是set容器中的元素类型不能被const修饰的原因）。
  ...
  // 在已创建好的s容器的基础上，如下代码尝试修改s容器中某个学生的name名字，也是无法通过编译
  ...
  // 这意味着，C++ STL标准不允许用户借助迭代器来直接修改set或者multiset容器中的元素。

  // 那么，如何才能正确修改set或multiset容器中的元素呢？
  // 最直接的方式就是借助const_cast运算符，该运算符可以去掉指针或者引用的const限定符。
  ...
  // 虽然使用const_cast能直接修改set或者multiset容器中的元素，但一定不要修改元素的键！

...
void func() {
  ...
  *pb1 = *pb2;
  ...
  //   在赋值后，d1的Base成员有了来自于d2的值，但其Derived成员部分没被改变。
...
// 程序该怎么办？
// 最容易的是不让Base对象间赋值，其最容易的实现方法是将Base设计为抽象类。
...
AbstractBase::~AbstractBase() {
  ...
  // 1.当前类是抽象类；
  // 2.任何从此类派生的实体类必须将此函数申明为一个普通的虚函数，不能带=0。
...
namespace n5 {
// 前面通过基类指针进行赋值的问题，是基于假设实体类（如Base）有数据成员。
// 如果它们没有数据成员，那么就不会有问题，从一个无数据的实体类派生新的实体类是安全的。

// 无数据而可以成为实体类的基类会两种可能，在将来，可能有数据成员，或者它仍然没有：
// 1.如果将来可能有数据成员，现在做的只是推迟问题的发生直到数据成员被加入，在用短利换长痛。
// 2.如果这个基类真的不会有数据成员，那么它现在就该是抽象类，没有数据的实体类有什么用处？
...
// 2.试图在类库的继承树的更高处找到一个完成了所需的大部分功能的抽象类，从它进行继承，
//   当然，可能没有合适的类，即使有，可能不得不重复很多已经在实体类中实现了的东西；
// 3.用包容试图继承的类的方法来实现新类。

...
// C++编译器们必须实现语言的每一个特性。
// 这些实现的细节当然是由编译器来决定的，并且不同的编译器有不同的方法实现语言的特性。
// 在多数情况下，你不用关心这些事情。
// 然而有些特性的实现对对象大小和其成员函数执行速度有很大的影响，所以对于这些特性有一个基本的了解，
...
// 非虚函数（包括构造函数）就象普通的C函数那样被实现，在性能上没有特殊的考虑。
class C2 : public C1 {
 public:
  C2();                        // 非虚函数
  virtual ~C2();               // 重定义函数
...
// 虚函数表：C2::~C2|C1::~C1|C2::f2|C1::f2|C1::f3|C2::f5
...
// 大多数程序和程序库由多个object（目标）文件连接而成，但是每个object文件之间是独立的。
...
// 通常启发式算法是这样的：
// 要在一个object文件中生成一个类的vtbl，要求该object文件包含该类的第一个非内联、
// 非纯虚拟函数（non-inline non-pure virual function）定义（也就是类的实现体）。
// 因此上述C1类的vtbl将被放置到包含C1::~C1定义的object文件里（不是内联的函数），
// C2类的vtbl被放置到包含C1::~C2定义的object文件里（不是内联函数）。
// 实际当中，这种启发式算法效果很好。
// 但是如果你过分喜欢声明虚函数为内联函数，如果在类中的所有虚函数都内声明为内联函数，
// 启发式算法就会失败，大多数基于启发式算法的编译器会在每个使用它的object文件中生成一个类的vtbl。
...
// 只有用某种方法指出每个对象对应的vtbl时，它们才能使用，这是vptr的工作，它来建立这种联系。
...
// 仅仅看这段代码，不会知道调用的是哪个f1函数――C1::f1或C2::f1，因为pC1可以指向C1对象也可以指向C2对象。
// 尽管如此编译器仍然得为在makeACall的f1函数的调用生成代码，它必须确保无论pC1指向什么对象，函数的调用必须正确。
// 编译器生成的代码会做如下这些事情：
// 1.通过对象的vptr找到类的vtbl。
//   这是一个简单的操作，因为编译器知道在对象内哪里能找到vptr（毕竟是由编译器放置的它们）。
...
// 3.调用第二步找到的的指针所指向的函数。
// 类似(*pC1->vptr[i])(pC1);
// 调用被vtbl中第i个单元指向的函数，而pC1->vptr指向的是vtbl；pC1被做为this指针传递给函数。
// 这几乎与调用非虚函数效率一样，在大多数计算机上它多执行了很少的一些指令。
// 调用虚函数所需的代价基本上与通过函数指针调用函数一样，虚函数本身通常不是性能的瓶颈。

// 虚函数所需的第三个代价：你实际上放弃了使用内联函数。
// 当通过对象调用虚函数时，它可以被内联，但是大多数虚函数是通过对象的指针或引用被调用的，这种调用不能被内联。
// 因为这种调用是标准的调用方式，所以虚函数实际上不能被内联。
// 在实际运行中，虚函数所需的代价与内联函数有关。
// 实际上虚函数不能是内联的，这是因为“内联”是指“在编译期间用被调用的函数体本身来代替函数调用的指令”，
...
// 基类的数据成员被复制到每一个继承类对象里，继承类与基类间的每条路径都有一个拷贝。
// 程序员一般不会希望发生这种复制，而把基类定义为虚基类则可以消除这种复制。
// 然而虚基类本身会引起它们自己的代价，因为虚基类的实现经常使用指向虚基类的指针做为避免复制的手段，
// 一个或者更多的指针被存储在对象里。
// 恐怖的多继承菱形：
class A {};
class B : virtual A {};
...
//  vptr
// 虽然存在四个类，但是只有三个vptr。
// 只要编译器喜欢，当然可以生成四个vptr，但是三个已经足够了（它发现B和D能够共享一个vptr），
...
// 每个类我们只需要信息的一个拷贝，我们需要一种方法从任何包含虚函数的对象里获得合适的信息。
...
//  RTTI           no               yes              no
// 一些人看到这个表格以后，会很吃惊，他们宣布“我还是应该使用C”。
...
// 再有，你必须自己人工跟踪对象类型，这意味着对象会携带它们自己的类型标签（type tag）。
// 因此你不会得到更小的对象。

// 理解虚函数、多继承、虚基类、RTTI所需的代价是重要的，但是如果你需要这些功能，
// 不管采取什么样的方法你都得为此付出代价，理解这点也同样重要。
// 有时你确实有一些合理的原因要绕过编译器生成的服务。
// 例如隐藏的vptr和指向虚基类的指针会使得在数据库中存储C++对象或跨进程移动它们变得困难，

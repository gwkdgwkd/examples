...
// 一旦TMP程序结束执行，其输出，
// 也就是从template具现出来的若干C++源码，便会一如往常地被编译。

// TMP有两个优点：
// 1.它让某些事情更容易。
//   如果没有它，那些事情将是困难的，甚至不可能的。
// 2.由于TMP执行于C++编译期，因此可将工作从运行期转移到编译期。
//   这导致的一个结果是，某些错误原本通常在运行期才能检测到，
//   现在在编译期就能找出来。
...
// TMP的递归甚至不是正常种类，因为TMP循环并不涉及递归函数调用，
// 而是涉及递归模板具现化。
template <unsigned n>
struct Factorial {
...
// TMP是一个在相对短时间之前才意外发现的语言，
// 其编程方式还多少需要依赖经验。
// 尽管如此，将工作从运行期移往编译期所带来的效率改善还是令人深刻的。
// 而TMP对难以或甚至不可能运行期实现出来的行为的表现能力也很吸引人。
// TMP或许永远不会成为主流，但对某些程序员，
// 特别是程序库开发人员，几乎确定会成为他们的主要粮食。

// 请记住：
// TMP，模板元编程可将工作由运行期移往编译期，
// 因而得以实现早期错误侦测和更高的执行效率。
// TMP可被用来生成基于政策选择组合的客户定制代码，

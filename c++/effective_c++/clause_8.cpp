...
// 如果析构函数中发生了异常，可以：
//  结束程序，通常用abort完成。
//  吞下异常，但这是个坏主意，压制了某些动作失败的重要信息。
//  把产生异常的操作放到普通函数中，提供给使用者，由使用者决定对异常的处理。这么做不会带来负担，而是给使用者一个处理错误的机会。

// 请记住：
// 析构函数决定不要吐出异常。如果一个析构函数调用的函数可能抛出异常，析构函数应该捕获任何异常，然后吞下它们（不传播）或结束程序。

...
// 串存储结构，也就是存储字符串的数据结构，很明显，字符串之间的逻辑关系也是一对一，
// 用线性表的思维不难想出，串存储结构也有顺序存储和链式存储。
// 提到字符串，常做的操作就是串之间的匹配，2种串的模式匹配算法，BF算法和KMP算法。

// 严格意义上讲，串存储结构也是一种线性存储结构，因为字符串中的字符之间也具有一对一的逻辑关系。
// 只不过，与之前所学的线性存储结构不同，串结构只用于存储字符类型的数据。
// 数据结构中，根据串中存储字符的数量及特点，对一些特殊的串进行了命名，比如说：
// 1.空串，存储0个字符的串，例如S = ""（双引号紧挨着）；
// 2.空格串，只包含空格字符的串，例如S = "     "（双引号包含5个空格）；
// 3.子串和主串，假设有两个串a和b，如果a中可以找到几个连续字符组成的串与b完全相同，
//   则称a是b的主串，b是a的子串，例如，若a="shujujiegou"，b="shuju"，
//   由于a中也包含"shuju"，因此串a和串b是主串和子串的关系。

// 存储一个字符串，数据结构包含以下3种具体存储结构：
// 1.定长顺序存储，实际上就是用普通数组（又称静态数组）存储，
//   例如C语言使用普通数据存储字符串的代码为char a[20] = "data.biancheng.net"；
// 2.堆分配存储，用动态数组存储字符串；
// 3.块链存储，用链表存储字符串。

// 串的定长顺序存储结构
// 顺序存储结构（顺序表）的底层实现用的是数组，根据创建方式的不同，
// 数组又可分为静态数组和动态数组，因此顺序存储结构的具体实现其实有两种方式。
// 串的定长顺序存储结构，可以简单地理解为采用固定长度的顺序存储结构来存储字符串，
// 因此限定了其底层实现只能使用静态数组。
// 例如，采用定长顺序存储结构存储"data.biancheng.net"，通过目测得知此字符串长度为18，
// 因此申请的数组空间长度至少为19（最后一位存储字符串的结束标志'\0'）。

// 串的堆分配存储结构
// 通常，编程语言会将程序占有的内存空间分成多个不同的区域，
// 程序包含的数据会被分门别类并存储到对应的区域，拿C语言来说，
// 程序会将内存分为4个区域，分别为堆区、栈区、数据区和代码区。
// 与其他区域不同，堆区的内存空间需要程序员手动使用malloc函数申请，
// 并且在不用后要手动通过free函数将其释放。
// C语言中使用malloc函数最多的场景是给数组分配空间，这类数组称为动态数组。
// 动态数组相比普通数组（静态数组）的优势是长度可变，
// 换句话说，根据需要动态数组可额外申请更多的堆空间（使用relloc函数）。

// 串的块链存储结构
// 实现串的块链存储使用的是无头节点的单链表，当然根据实际需要，
// 也可以自行决定所用链表的结构（双向链表还是单链表，有无头节点）。
// 链表各节点存储数据个数的多少可参考以下几个因素：
// 1.串的长度和存储空间的大小，若串包含数据量很大，且链表申请的存储空间有限，
//   此时应尽可能的让各节点存储更多的数据，提高空间的利用率，
//   每多一个节点，就要多申请一个指针域的空间，反之，如果串不是特别长，
//   或者存储空间足够，就需要再结合其他因素综合考虑；
// 2.程序实现的功能，如果实际场景中需要对存储的串做大量的插入或删除操作，
//   则应尽可能减少各节点存储数据的数量，反之，就需要再结合其他因素。

...
// 分别为unordered_map、unordered_multimap、unordered_set以及unordered_multiset。
// unordered_map存储键值对<key,value>类型的元素，
// 其中各个键值对键的值不允许重复，且该容器中存储的键值对是无序的。
// unordered_multimap和unordered_map唯一的区别在于，该容器允许存储多个键相同的键值对。
// unordered_set不再以键值对的形式存储数据，而是直接存储数据元素本身，元素不能重复，而且是无序的。
// unordered_multiset和unordered_set唯一的区别在于，该容器允许存储值相同的元素。

// C++11标准的STL中，在已提供有4种关联式容器的基础上，
// 又新增了各自的“unordered”版本，提高了查找指定元素的效率。

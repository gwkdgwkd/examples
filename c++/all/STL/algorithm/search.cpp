...
// 可以借助search()函数，search()函数定义在<algorithm>头文件中，
...
// 查找[first1,last1)内，和[first2,last2)满足pred规则的第一个子序列：
// ForwardIterator search(ForwardIterator first1,
//                        ForwardIterator last1,
//                        ForwardIterator first2,
//                        ForwardIterator last2,
...
// 实际上，第一种语法格式包含一个默认的pred参数，该参数指定的是一种相等规则，
...
//     while (*it1 == *it2) {  // 或者 while (pred(*it1,*it2))
//     对应第二种语法格式
...
void func() {
  ...
  int a2[] = {2, 4, 6};
  it = search(v.begin(), v.end(), a2, a2 + 3, mycomp());
  if (it != v.end()) {
...
//       函数定义的形式可以是普通函数，也可以是函数对象。
// 实际上，第一种语法格式包含默认的pred参数，该参数指定的是一种相等规则，
...
bool mycomp(int i, int j) { return (i % j == 0); }
...
void func() {
  ...
  std::vector<int>::iterator iter =
      std::search_n(v.begin(), v.end(), 3, 2, mycomp);
  if (iter != v.end()) {

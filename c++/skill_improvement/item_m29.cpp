...
void func2() {
  ...
  *p = 'x';
  ...
  // 这个问题不仅局限于指针，
  // 如果保存了一个String的非const operator[]的返回值的引用，
  // 那么也会产生一样的问题：
  r = 'z';
...
//   如果你有带引用计数的String类，试一下上面的例子，看是否存在。
//   即使不能确定你操作的是否是带引用计数的String类，
//   也无论如何应该试一下这个例子。
//   由于封装，可能使用了一个这样的类型而不自知。
// 2.不是所以的实现都忽略这个问题，稍微好些的方法是明确说明它的存在。
//   通常是将它写入文档，或多或少地说明：
//   别这么做。如果你这么做了，结果为未定义。
//   无论以哪种方式这么做了（有意地或无意地），并抱怨其结果时，
//   他们辩解道：好了，我们告诉过你别这么做的。
...
class String {
 ...
 private:
  struct StringValue {
    ...
    bool shareable;  // add
    char* data;
    StringValue(const char* initValue) : refCount(1), shareable(true) {  // add
      data = new char[strlen(initValue) + 1];
      strcpy(data, initValue);
    }
    ~StringValue() { delete[] data; }
...
class RCObject {
 public:
  ...
  // 同样的，一定数目的String对象在赋值之前指向前v2，
  // 在赋值后，sv2的引用计数同样没有改变。
  // 当RCObject在赋值过程中被涉及时，指向它的对象的数目没有受影响，
  // 因此RCObject::operator=不应该改变引用计数值。
  // 上面的实现是正确的，虽然违反直觉但是正确的。
  RCObject& operator=(const RCObject& rhs) { return *this; }

  // RCObject::removeReference的代码不但负责减少对象的refCount值，
  // 还负责当refCount值降到0时析构对象。
  // 后者是通过delete this来实现的，如Item M27中解释的，
  // 这只当知道*this是一个堆对象时才安全。
  ...
  // 限制了创建被引用计数对象的权力，
  // 当交出这个权力时，必须明确其附带条件是满足创建对象的限制条件。
...
//   用调用isShared函数代替了直接检查refCount的值，
...
// 将引用计数功能移入一个新类，增加了智能指针来自动处理引用计数，
// 但用户的一行代码都不需要修改。
// 当然，改变了String类的定义，所以用户需要重新编译和链接，
...
// 让CountHolder包含一个指针指向Widget。
// 然后用等价的智能指针RCIPter模板替代RCPtr模板，它知道CountHolder类的存在。
// 名字中的i表示间接indirect。
...
// StringValue、RCObject和RCPtr。
// 确实，这个更复杂的设计确保在值可共享时的更高的效率；
...
// 如果你不放心谁应该去执行删除动作，那么引用计数正是这种放下担子的技巧。
// 很多程序员只因为这个原因就使用引用计数。

...
// 因此，我们需要的不是为class SmartPtr写一个构造函数，而是为它写一个构造模板（member function template，简称member template）：
template <typename T>
class SmartPtr1 {
 public:
  ...
  // explicit故意没写。因为原始指针之间的转换是隐式的，所以让智能指针模仿这种行为也属合理。
  // 这个泛化copy构造函数提供的比我们需要的更多：
  //  希望根据一个SmartPtr<Bottom>创建一个SmartPtr<Top>，而不希望根据SmartPtr<Top>创建一个SmartPtr<Bottom>，对public继承是矛盾的。
  //  也不希望根据SmartPtr<doublie>创建一个SmartPtr<int>，因为现实中没有将int*转换为double*的隐式转换行为。
...
// shared_prt支持所有“来自兼容的内置指针、shared_ptr、auto_ptr和weak_ptr”的构造行为，以及上述各物的赋值操作。
...
// 所以想要控制copy构造的方方面面，必须同时声明泛化copy构造函数和正常的copy构造函数。相同的规则也适用于赋值操作。

// 请记住：
// 使用member function template（成员函数模板）生成“可接受所有兼容类型”的函数。
// 如果声明member template用于“泛化copy构造”或“泛化assignment操作”，还是需要声明正常的copy构造函数和copy assignmet操作符。
...
int main() {
  ...
  // 带有base-derived关系的两个类型分别具现化某个template，产生出来的两个具现体并不带有base-derived关系。
  // 所以SmartPtr<Top>和SmartPtr<Middle>为完全不同的class，它们之间的关系并不比vector<float>和Widget更密切。

...
// nth_element()  调整指定范围内元素的存储位置，实现位于位置n的元素正好是全排序情况下的第n个元素，
//                并且按照全排序规则排在位置n之前的元素都在该位置之前，按照全排序规则排在位置n之后的元素都在该位置之后。
...
// 怎样选择最合适（效率最高）的排序函数呢？这里为大家总结了以下几点：
// 1.如果需要对所有元素进行排序，则选择sort()或者stable_sort()函数；
// 2.如果需要保持排序后各元素的相对位置不发生改变，就只能选择stable_sort()函数，而另外3个排序函数都无法保证这一点；
// 3.如果需要对最大（或最小）的n个元素进行排序，则优先选择partial_sort()函数；
// 4.如果只需要找到最大或最小的n个元素，但不要求对这n个元素进行排序，则优先选择nth_element()函数。
// 除此之外，很多读者都关心这些排序函数的性能。
// 总的来说，函数功能越复杂，做的工作越多，它的性能就越低（主要体现在时间复杂度上）。
...
//                     反之，时间复杂度为O(N*log2(N)2)，其中N为指定区域[first,last)中last和first的距离。
// partial_sort()      函数实现排序的平均时间复杂度为N*log(M)，其中N指的是[first,last)范围的长度，
//                     M指的是[first, middle)范围的长度。
// partial_sort_copy() 函数实现排序的平均时间复杂度为N*log(min(N,M))，其中N指的是[first,last)范围的长度，
...
// 和为关联式容器设定排序规则不同，给sort()函数指定排序规则时，需要为其传入一个函数名或者函数对象。
void func1() {
...
// 换句话说，stable_sort()和sort()具有相同的使用场景，就连语法格式也是相同的，
// 只不过前者在功能上除了可以实现排序，还可以保证不改变相等元素的相对位置。
// stable_sort()函数的用法也有2种，其语法格式和sort()函数完全相同（仅函数名不同）：
...
// 具体来说，partial_sort()会将[first,last)范围内最小（或最大）的middle-first个元素移动到[first, middle)区域中，
// 并对这部分元素做升序（或降序）排序。
// 需要注意的是，partial_sort()函数受到底层实现方式的限制，它仅适用于普通数组和部分类型的容器。
// 换句话说，只有普通数组和具备以下条件的容器，才能使用partial_sort()函数：
// 1.容器支持的迭代器类型必须为随机访问迭代器。这意味着，partial_sort()函数只适用于array、vector、deque这3个容器。
// 2.当选用默认的升序排序规则时，容器中存储的元素类型必须支持<小于运算符；
//   同样，如果选用标准库提供的其它排序规则，元素类型也必须支持该规则底层实现所用的比较运算符；
// 3.partial_sort()函数在实现过程中，需要交换某些元素的存储位置。
//   因此，如果容器中存储的是自定义的类对象，则该类的内部必须提供移动构造函数和移动赋值运算符。
void func3() {
...
// partial_sort_copy()功能和partial_sort()类似，唯一的区别是前者不会对原有数据做任何变动，
...
// partial_sort_copy()函数会将[first,last)范围内最小（或最大）的result_last-result_first个元素复制
// 到[result_first,result_last)区域中，并对该区域的元素做升序（或降序）排序。
// 值得一提的是，[first,last] 中的这2个迭代器类型仅限定为输入迭代器，这意味着相比partial_sort()函数，
// partial_sort_copy()函数放宽了对存储原有数据的容器类型的限制。
// 换句话说，partial_sort_copy()函数还支持对list容器或者forward_list容器中存储的元素进行“部分排序”，
// 而partial_sort()函数不行。
// 但是，介于result_first和result_last仍为随机访问迭代器，
// 因此[result_first, result_last)指定的区域仍仅限于普通数组和部分类型的容器，这和partial_sort()函数对容器的要求是一样的。
void func4() {
...
// 简单的理解nth_element()函数的功能，当采用默认的升序排序规则（std::less<T>）时，
// 该函数可以从某个序列中找到第n小的元素K，并将K移动到序列中第n的位置处。不仅如此，
// 整个序列经过nth_element()函数处理后，所有位于K之前的元素都比K小，所有位于K之后的元素都比K大。
// 也可以将nth_element()函数的排序规则自定义为降序排序，此时该函数会找到第n大的元素K并将其移动到第n的位置处，
...
//  comp：用于自定义排序规则。
// 注意，鉴于nth_element()函数中各个参数的类型，其只能对普通数组或者部分容器进行排序。
// 换句话说，只有普通数组和符合以下全部条件的容器，才能使用使用nth_element()函数：
// 1.容器支持的迭代器类型必须为随机访问迭代器。这意味着，nth_element()函数只适用于array、vector、deque这3个容器。
// 2.当选用默认的升序排序规则时，容器中存储的元素类型必须支持<小于运算符；
//   同样，如果选用标准库提供的其它排序规则，元素类型也必须支持该规则底层实现所用的比较运算符；
...
// 其中，first和last都为正向迭代器（这意味着该函数适用于大部分容器），[first, last)用于指定要检测的序列；
// comp用于指定自定义的排序规则。
...
// 和is_sorted()函数相比，is_sorted_until()函数不仅能检测出某个序列是否有序，还会返回一个正向迭代器，
...
// 函数对象可以理解为伪装成函数的对象，根据以往的认知，函数对象的执行效率应该不如普通函数。
// 但事实恰恰相反，即便将普通函数定义为更高效的内联函数，其执行效率也无法和函数对象相比。
// 通过在4个不同的STL平台上，对包含100万个double类型数据的vector容器进行排序，
// 最差情况下使用函数对象的执行效率要比普通内联函数高50%，最好情况下则高160%。
// 那么，是什么原因导致了它们执行效率上的差异呢？
// 以mycomp6()函数对象为例，其mycomp6::operator()也是一个内联函数，
// 编译器在对sort()函数进行实例化时会将该函数直接展开，这也就意味着，展开后的sort()函数内部不包含任何函数调用。
// 而如果使用mycomp5作为参数来调用sort()函数，情形则大不相同。
// 要知道，C++并不能真正地将一个函数作为参数传递给另一个函数，换句话说，如果我们试图将一个函数作为参数进行传递，
...
// 可能会问，程序中comp()函数也是内联函数，为什么C++不像函数对象那样去处理呢？具体原因我们无从得知，
// 事实上也没必要关心，也许是编译器开发者觉得这种优化不值得去做。
// 除了效率上的优势之外，相比普通函数，以函数对象的方式自定义规则还有很多隐藏的优势。
// 例如在某些特殊情况下，以普通函数的形式编写的代码看似非常合理，但就是无法通过编译，
// 这也许是由于STL标准库的原因，也许是编译器缺陷所至，甚至两者都有可能。
// 而使用函数对象的方式，则可以有效避开这些“坑”，而且还大大提升的代码的执行效率。
// 总之，以函数对象的方式为STL算法自定义规则，具有效率在内的诸多优势。
// 当调用带有comp参数的STL算法时，除非调用STL标准库自带的比较函数，否则应优先以函数对象的方式自定义规则。

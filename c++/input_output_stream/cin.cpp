...
// 第一个版本从输入流中读取bufSize-1个字符到缓冲区buf，或遇到\n为止（哪个条件先满足就按哪个执行）。函数会自动在buf中读入数据的结尾添加\0。
...
// 从输入流中读入一行，可以用第一个版本。用cin>>str这种写法是不行的，因为此种读法在碰到行中的空格或制表符时就会停止，因此就不能保证str中读入的是整行。
...
// cin.peek()不会跳过输入流中的空格、回车符。在输入流已经结束的情况下，cin.peek()返回EOF。
...
// istream还有一个成员函数istsdream &putback(char c)，可以将一个字符插入输入流的最前面。对于上面的例题，也可以在用get()函数读取一个字符并判断是中式
// 格式还是西式格式时，将刚刚读取的字符再用putback()成员函数放回流中，然后再根据判断结果进行不同方式的读入。

// cin是如何判断输入结束（读取结束）的？
// cin可以用来从键盘输入数据；将标准输入重定向为文件后，cin也可以用来从文件中读入数据。在输入数据的多少不确定，且没有结束标志的情况下，该如何判断输入数据已经读完了呢？
...
// 不管是文件末尾，还是Ctrl+Z或者Ctrl+D，它们都是结束标志；cin在正常读取时返回true，遇到结束标志时返回false，我们可以根据cin的返回值来判断是否读取结束。
// 在这种情况下，文件中并不需要包含Ctrl+Z或者Ctrl+D，只要有用空格或回车隔开的若干个正整数即可。cin读到文件末尾时，cin>>n就会返回false，从而导致程序结束。
...
int main() {
  ...
  if (x == 1) {
    ...
    char szBuf1[MAX_LINE_LEN + 10];
    freopen("test.txt", "r", stdin);  // 将标准输入重定向为test.txt
    while (cin.getline(szBuf1, MAX_LINE_LEN + 5)) {
      cout << szBuf1 << endl;
    }
  } else if (x == 6) {
    ...
    int n3;
    int maxN = 0;
    // cin>>n的返回值的确是istream &类型的，而while语句中的条件表达式的返回值应该是bool类型、整数类型或其他和整数类型兼容的类型，
    // istream &显然和整数类型不兼容，为什么while(cin>>n)还能成立呢？
    // 这是因为，istream类对强制类型转换运算符bool进行了重载，这使得cin对象可以被自动转换成bool类型。所谓自动转换的过程，就是调用cin的operator bool()这个成员函数，
    // 而该成员函数可以返回某个标志值，该标志值在cin没有读到输入结尾时为true，读到输入结尾后变为false。对该标志值的设置，在operator <<()成员函数中进行。
    // 程序本该输入整数，如果输入了一个字母，则程序就会结束。因为，应该读入整数时却读入了字母也算读入出错。
    while (cin >> n3) {  // 输入没有结束，cin就返回true，条件就为真；
      if (maxN < n3) maxN = n3;
    }
    // 1
    // 2

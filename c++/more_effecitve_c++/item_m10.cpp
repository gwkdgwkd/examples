...
BookEntry::BookEntry(const string& name, const string& address,
  ...
  // 构造函数把指针theImage和theAudioClip初始化为空，然后如果其对应的构造函数参数不是空，就让这些指针指向真实的对象。
  if (imageFileName != "") {
...
// 一个异常被抛出，可以是因为operator new不能给AudioClip分配足够的内存，也可以因为AudioClip的构造函数自己抛出一个异常。
// 不论什么原因，如果在BookEntry构造函数内抛出异常，这个异常将传递到建立BookEntry对象的地方。
// 现在假设建立theAudioClip对象建立时，一个异常被抛出（而且传递程序控制权到BookEntry构造函数的外面），那么谁来负责删除theImage已经指向的对象呢？
// 答案显然应该是由BookEntry来做，但是这个想当然的答案是错的。~BookEntry()根本不会被调用，永远不会。
// C++仅仅能删除被完全构造的对象（fully contructed objects）, 只有一个对象的构造函数完全运行完毕，这个对象才被完全地构造。

// 如果你试图采取主动手段处理异常情况，即当异常发生时调用delete：
...
// 你会发现在BookEntry构造函数里为Image 分配的内存仍旧被丢失了，这是因为如果new操作没有成功完成，程序不会对pb进行赋值操作。
// 如果BookEntry的构造函数抛出一个异常，pb将是一个空值，所以在catch块中删除它除了让你自己感觉良好以外没有任何作用。
// 用灵巧指针（smart pointer）类auto_ptr<BookEntry>代替raw BookEntry*也不会也什么作用，因为new操作成功完成前，也没有对pb进行赋值操作。
// C++拒绝为没有完成构造操作的对象调用析构函数是有一些原因的，而不是故意为你制造困难。原因是：在很多情况下这么做是没有意义的，甚至是有害的。
...
// 所以如果BookEntry构造函数体开始执行，对象的theName,theAddress和thePhones数据成员已经被完全构造好了。
// 这些数据可以被看做是完全构造的对象，所以它们将被自动释放，不用你介入操作。
// 当然如果这些对象的构造函数调用可能会抛出异常的函数，那么那些构造函数必须自己去考虑捕获异常并在继续传递这些异常之前完成必需的清除操作。

// BookEntry构造函数的catch块中的语句与在BookEntry的析构函数的语句几乎一样。
// 这里的代码重复是绝对不可容忍的，所以最好的方法是把通用代码移入一个私有helper function中，让构造函数与析构函数都调用它。
class BookEntry1 {
 public:
  BookEntry1(const string& name, const string& address = "",
             const string& imageFileName = "",
             const string& audioClipFileName = "");
  ~BookEntry1();
...
void BookEntry1::cleanup() {
  delete theImage;
  delete theAudioClip;
}
BookEntry1::BookEntry1(const string& name, const string& address,
                       const string& imageFileName,
                       const string& audioClipFileName)
    : theName(name), theAddress(address), theImage(0), theAudioClip(0) {
  try {
...
BookEntry2::BookEntry2(const string& name, const string& address,
    ...
    : theName(name),
      ...
      theAudioClip(audioClipFileName != "" ? new AudioClip(audioClipFileName)
                                           : 0) {}
// 这样做导致我们原先一直想避免的问题重新出现：如果theAudioClip初始化时一个异常被抛出，theImage所指的对象不会被释放。
// 而且我们不能通过在构造函数中增加try和catch语句来解决问题，因为try和catch是语句，而成员初始化表仅允许有表达式（这也
// 是为什么我们必须在theImage和theAudioClip的初始化中使用?:以代替if-then-else的原因）。

// 无论如何，在异常传递之前完成清除工作的唯一的方法就是捕获这些异常，所以如果我们不能在成员初始化表中放入try和catch语句，我们把它们移到其它地方。
...
BookEntry3::BookEntry3(const string& name, const string& address,
                       const string& imageFileName,
                       const string& audioClipFileName)
    : theName(name),
      theAddress(address),
...
Image* BookEntry3::initImage(const string& imageFileName) {
  if (imageFileName != "") {
...
AudioClip* BookEntry3::initAudioClip(const string& audioClipFileName) {
  try {
    if (audioClipFileName != "") {
...
// 更好的解法决方法是采用条款M9的建议，把theImage和theAudioClip指向的对象做为一个资源，被一些局部对象管理。
class BookEntry4 {
 public:
  BookEntry4(const string& name, const string& address = "",
             const string& imageFileName = "",
             const string& audioClipFileName = "");
  ~BookEntry4();
...
// 这样做使得BookEntry的构造函数即使在存在异常的情况下也能做到不泄漏资源，而且让我们能够使用成员初始化表来初始化theImage和theAudioClip：
...
// 而且因为theImage和theAudioClip现在是包含在BookEntry中的对象，当BookEntry被删除时它们能被自动地删除。因此不需要手工删除它们所指向的对象。
// 可以这样简化BookEntry的析构函数：
BookEntry4::~BookEntry4() {}

// 如果你用对应的智能指针对象替代指针成员变量，就可以防止构造函数在存在异常时发生资源泄漏，你也不用手工在析构函数中释放资源，
// 并且你还能象以前使用非const指针一样使用const指针，给其赋值。

// 在对象构造中，处理各种抛出异常的可能，是一个棘手的问题，但是auto_ptr(或者类似于 auto_ptr 的类)能化繁为简。

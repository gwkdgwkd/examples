...
void func1() {  // 输出单独字符
  ...
  // 除了使用cout.put()函数输出一个字符外，
  // 还可以用putchar()函数输出一个字符。
  // 该函数是C语言中使用的，在<stdio.h>头文件中定义，
  // C++保留了这个函数，在<iostream>头文件中定义。
  putchar('n');
  putchar('\n');
  // n
}
...
void func2() {  // 输出字符串
  ...
  // write()成员方法专用于向输出流缓冲区中添加指定的字符串，
  // 4表示要输出的前4个字符：
  std::cout.write(str, 4) << std::endl;  // http

  // 该函数会返回一个ostream类的引用对象，所以可以连续调用：
...
// 2.通过偏移量间接指定下一个字符的存储位置：
//   ostream& seekp (streamoff off, ios_base::seekdir way);
//   其中，各个参数的含义如下：
//   pos：用于接收一个正整数；
//   off：用于指定相对于way位置的偏移量，其本质也是接收一个整数，
//        可以是正数（代表正偏移）或者负数（代表负偏移）；
//   way：用于指定偏移位置，即从哪里计算偏移量，它可以接收下面3个值：
//      ios::beg 从文件头开始计算偏移量
//      ios::end 从文件末尾开始计算偏移量
//      ios::cur 从当前位置开始计算偏移量
// seekp()方法会返回一个引用形式的ostream类对象，
// 这意味着seekp()方法可以这样使用：
...
namespace n3 {
// cout格式化输出:
// 当使用printf()函数输出数据时，可以通过设定一些合理的格式控制符，
// 来达到以指定格式输出数据的目的。
// C++通常使用cout输出数据，和printf()函数相比，
// cout实现格式化输出数据的方式更加多样化。
// 一方面，cout作为ostream类的对象，
// 该类中提供有一些成员方法，可实现对输出数据的格式化；
// 另一方面，为了方面用户格式化输出数据，
// C++标准库专门提供了一个<iomanip>头文件，
// 该头文件中包含有大量的格式控制符（严格意义上称为流操纵算子），使用更加方便。
// ostream类中包含一些可实现格式化输出的成员方法，
// 这些成员方法都是从ios基类（以及ios_base类）中继承来的，
// cout（以及cerr、clog）也能调用:
//  flags(fmtfl) 	    当前格式状态全部替换为fmtfl。
//                    注意，fmtfl可以表示一种格式，也可以表示多种格式。
//  precision(n) 	    设置输出浮点数的精度为n。
//  width(w) 	        指定输出宽度为w个字符。
//  fill(c) 	        在指定输出宽度的情况下，
//                    输出的宽度不足时用字符c填充（默认用空格填充）。
//  setf(fmtfl, mask) 在当前格式的基础上，追加fmtfl格式，并删除mask格式。
//                    其中，mask参数可以省略。
//  unsetf(mask) 	    在当前格式的基础上，删除mask格式。
// flags()函数的fmtfl参数、setf()函数中的fmtfl和mask以及unsetf()函数mask，
// 可以选择这些值：
...
// 值得一提的是，当调用unsetf()或者1个参数的setf()函数时，
// 为了提高编写代码的效率，可以给mask参数传递如下3个组合格式：
...
// 当cout采用此方式(成员函数的方式)进行格式化输出时，
// 其后不能立即输出数据，只在下条语句生效。

// <iomanip>头文件中定义的一些常用的格式控制符，
// 它们都可用于格式化输出(带*的是默认选项):
// 　*dec 	          以十进制形式输出整数
// 　hex 	            以十六进制形式输出整数
// 　oct 	            以八进制形式输出整数
// 　fixed 	          以普通小数形式输出浮点数
...
// 　showpos 	        在非负数值中显示+
// 　*noshowpos 	    在非负数值中不显示+
// 　uppercase 	      十六进制数中使用A~E
//                    若输出前缀，则前缀输出-1X，科学计数法中输出E
// 　*nouppercase 	  十六进制数中使用a~e
//                    若输出前缀，则前缀输出-1x，科学计数法中输出e
// 　internal 	      数值的符号（正负号）在指定宽度内左对齐，
//                    数值右对齐，中间由填充字符填充
// 注意，如果两个相互矛盾的标志同时被设置，
// 如先设置setiosflags(ios::fixed)，
// 然后又设置setiosflags(ios::scientific)，
// 那么结果可能就是两个标志都不起作用。
// 因此，在设置了某标志，又要设置其他与之矛盾的标志时，
// 就应该用resetiosflags清除原先的标志。
...
void func5() {
  // 除非知道当前没有设置基标志，
  // 否则ios::setf(_IFlags)不应和ios::dec、
  // ios::oct或ios::hex的标志值一起使用。

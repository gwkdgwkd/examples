...
// 一般而言，类的数据成员应该被初始化，并且只在构造函数或在类的其他成员函数中指定初值。
// 其他任何操作都将破坏封装性，不易维护。
// 不应该在抽象的基类总声明数据成员，这是比较强而有力的论点（把接口和实现分类）。
...
// 纯虚析构函数一定得定义它。
// 因为每一个派生类析构函数会被编译器扩展，以静态调用的方式调用其每一个虚基类以及上一层基类的析构函数。
...
// 2.声明一个函数为const，然后发现派生类又必须修改某一个数据成员，可以考虑不再用const了。
...
// 用C++编译上面的代码，编译器会声明一个无用的默认构造函数、一个无用额析构函数、
// 一个无用的拷贝构造函数，以及一个无用的拷贝赋值操作符。
// 但实际上编译器会分析这个声明，并为它提上Plain Ol' Data标签。
// 当编译器遇到下面的定义，无用的默认构造函数和析构函数都会被调用。
// 构造是在程序起始出被调用而析构是在exit处被调用（exit有系统产生，放在main结束之前）。
...
Point foobar() {
  ...
  *heap = local;  // 如果local被适当的初始化化过，一切就没有问题
  ...
  return local;  // 观念上，会调用拷贝构造函数，不过实际上只是简单的位拷贝操作，因为是POD
}
...
void mumble() {
  ...
  // 上面比下面会稍微快一些。
  // 因为activation record被放进程序堆栈时，初始化列表中的常量就可以被直接放进l1内存中了。
  Point1 l2;
  // l2._x = 1.0;
  // l2._y = 1.0;
  // l2._z = 1.0;
...
// 那么Explicit initialization list的效率会比inline构造好的多，特别是对全局对象而言。
...
// 因为它的出现会触发NRV优化。
// NRV优化后将不再需要调用拷贝构造函数，因为运算结果已经被直接放置与被传回的对象体内了。
...
// 构造函数内有大量的隐藏代码，因为编译器会扩充每一个构造函数，扩充程度视类的继承体系而定。
// 一般而言编译器所做的扩充大约如下：
...
// 4.在那之前，所有上一层的基类构造函数必须被调用，
//   以基类的声明顺序为顺序（与member initialization list中的顺序没有关联）：
...
class Line {
 ...
 public:
  ...
  // 所以Line的implicit copy constructor、copy operator和destructor都将有实际功能。
};
// Line a;
...
// 然后，当Point3d和Vertex同为Vertex3d的subobject时，它们对Point2构造函数的调用一定不可以发生，
// 取而代之的是，作为一个最底层的类，Vertex3d来负责完成被共享的Point subobject的构造。
...
// 如果虚函数调用操作限制在构造和析构中直接调用，那么将每一个调用操作以静态方式决定，千万不要用到虚拟机制。
// 其他情况，使用虚拟机制。
// 上面的限制操作，需要通过vptr来实现。
...
// 1.在任何操作之前。
// 2.在基类构造函数调用操作之后，
//   但是在程序员定义的代码或是member initialization list中所列的members初始化操作之前。
// 3.在每一件事情发生之后。
// 答案是2，另外两种没有什么价值。
// 令每一个基类构造函数设定其对象的vptr，使它指向相关的virtual table之后，
// 构造中的对象就可以严格而正确地变成构造过程中所幻化出来的每一个类的对象。
// 也就是说，PVertex对象会先形成一个Point2对象、Point3d对象、一个Vertex对象、一个Vertex3d对象，
// 然后才成为一个PVertex对象。
...
// 何时需要供应参数给一个基类构造函数？
// 这种情况下在类的构造函数的member initialization list中调用该类的虚拟函数，仍然安全么？
...
class Point3 {
 ...
 protected:
  ...
  // 与拷贝构造函数一样，不会。
  // 已经有了bitwise copy语意，所以implicit赋值操作符被视为毫无用处，也根本不会被合成出来。
};
void func3() {
  Point3 a3, b3;
  a3 = b3;
...
// 如果类没有定义析构函数，那么只有在类内带成员对象（或是类自己的基类）拥有析构函数的情况下，
// 编译器才会自动合成出一个来。
// 否则，析构函数会别视为不需要，也就不需要被合成（当然更不需要被调用）。
// 应该因为需要而非感觉来提供析构函数，更不要因为你不确定是否需要一个析构函数，于是就提供它。

...
// 4.双向迭代器；
// 5.随机访问迭代器。
// 这5种迭代器是STL标准库提供的最基础的迭代器，
// 很多场景中遍历容器的需求，它们并不适合。

// 所谓迭代器适配器，其本质也是一个模板类，比较特殊的是，
// 该模板类是借助以上5种基础迭代器实现的。
// 换句话说，迭代器适配器模板类的内部实现，
// 是通过对以上5种基础迭代器拥有的成员方法进行整合、修改，
// 甚至为了实现某些功能还会添加一些新的成员方法。
// 由此，将基础迭代器改头换面，就变成了迭代器适配器。
...
// STL迭代器适配器种类:
// 1.反向迭代器（reverse_iterator） 又称逆向迭代器
//   其内部重新定义了递增运算符（++）和递减运算符（--），
//   专门用来实现对容器的逆序遍历。
// 2.安插型迭代器（inserter或者insert_iterator）
//   通常用于在容器的任何位置添加新的元素，需要注意的是，
//   此类迭代器不能被运用到元素个数固定的容器（比如array）上。
...
//   此类型迭代器是C++11标准中新添加的，
//   可以将某个范围的类对象移动到目标范围，而不需要通过拷贝去移动。
...
void func2() {
  ...
  // 就可以获得逆序遍历容器所需要的反向迭代器，
  // 从而轻松实现逆序输出容器中存储的所有数据。
}

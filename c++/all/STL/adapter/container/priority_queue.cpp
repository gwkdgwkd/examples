...
// 即使用此容器适配器存储元素只能从队尾进，从队头出，
// 且每次只能访问priority_queue中位于队头的元素。
// 但是，priority_queue容器适配器中元素的存和取，
// 遵循的并不是先入先出原则，而是First in，Largest out原则。
...
// priority_queue容器适配器为了保证每次从队头移除的都是当前优先级最高的元素，
// 每当有新元素，都会根据既定的排序规则找到优先级最高的元素，并将其移动到队头；
// 同样，当priority_queue从队头移除出一个元素之后，
// 它也会再找到当前优先级最高的元素，并将其移动到队头。
...
//  typename Container：
//  指定priority_queue底层使用的基础容器，默认使用vector容器。
//  typename Compare：指定容器中评定元素优先级所遵循的排序规则，
//                    默认使用std::less<T>按照元素值从大到小进行排序，
//                    还可以使用std::greater<T>按照元素值从小到大排序，
//                    但更多情况下是使用自定义的排序规则。
//  其中，std::less<T>和std::greater<T>都是函数对象，定义在<function>中。

// priority_queue容器适配器模板位于<queue>中，并在std命名空间里，成员函数：
// empty() 	               如果为空的话，返回true，反之，返回false。
// size() 	               返回priority_queue中存储元素的个数。
// top() 	                 返回priority_queue中第一个元素的引用形式。
// push(const T& obj) 	   根据既定的排序规则，将obj的副本存到适当的位置。
// push(T&& obj) 	         根据既定的排序规则，将obj移动到适当的位置。
// emplace(Args&&... args) 参数表示构造一个存储类型的元素所需要的数据。
//                         此函数的功能是根据既定的排序规则，
//                         在容器适配器适当的位置直接生成该新元素。
// pop()                   移除priority_queue容器适配器中第一个元素。
// swap(priority_queue<T>& other) 将两个容器适配器中的元素进行互换，
//                         需要注意的是，进行互换的2个priority_queue容器适配器中，
...
// 这里又说使用堆结构存储数据，它们之间不冲突吗？显然，它们之间是不冲突的。
// 首先，vector和deque是用来存储元素的容器，而堆是一种数据结构，
// 其本身无法存储数据，只能依附于某个存储介质，辅助其组织数据存储的先后次序。
// 其次，priority_queue底层采用vector或者deque作为基础容器，这毋庸置疑。
// 但由于vector或deque容器并没有提供：
// 实现priority_queue容器适配器First in，Largest out特性的功能，
// 因此STL选择使用堆来重新组织vector或deque容器中存储的数据，从而实现该特性。
// 虽然不使用堆结构，通过编写算法调整vector或者deque容器中存储元素的次序，
// 也能使其具备First in，Largest out的特性，但执行效率通常没有使用堆结构高。

// 堆是完全二叉树的基础上，要求树中所有的父节点和子节点之间，都要满足排序规则：
// 1.如果排序规则为从大到小排序，则表示堆的完全二叉树中，
//   每个父节点的值都要不小于子节点的值，这种堆通常称为大顶堆；
// 2.如果排序规则为从小到大排序，则表示堆的完全二叉树中，
//   每个父节点的值都要不大于子节点的值，这种堆通常称为小顶堆；
// 无论是通过大顶堆或者小顶堆，总可以筛选出最大或最小的那个元素（优先级最大），
// 并将其移至序列的开头，此功能也正是priority_queue容器适配器所需要的。

// 为了验证priority_queue底层确实采用堆存储结构实现的，
// 可以尝试用堆结合基础容器vector或deque实现priority_queue。
// STL已经为封装好了可以使用堆存储结构的方法，它们都位于<algorithm>中。
// 常用的几个和堆存储结构相关的方法：
// make_heap(first,last,comp) 	  选择位于[first,last)区域内的数据，
//                                并根据comp排序规则建立堆，默认是建立大顶堆
//                                其中fist和last可以是指针或者迭代器。 。
// push_heap(first,last,comp) 	  当向数组或容器中添加数据之后，
//                                此数据可能会破坏堆结构，该函数的功能是重建堆。
// pop_heap(first,last,comp)      将序列头部的元素（优先级最高）移动序列尾部，
//                                并使[first,last-1]区域内的元素满足堆存储结构。
// sort_heap(first,last,comp) 	  对[first,last)区域内的元素进行堆排序，
//                                将其变成一个有序序列。
...
int main() {
  ...
  // 2.可以使用普通数组或其它容器中指定范围内的数据，对容器适配器进行初始化
  int a1[]{4, 1, 3, 2};
  std::priority_queue<int> pq2(a1, a1 + 4);
  std::array<int, 4> a2{4, 1, 3, 2};
  std::priority_queue<int> pq3(a2.begin(), a2.end());
  // 以上2种方式必须保证：
  // 数组或容器中存储的元素类型和priority_queue指定的存储类型相同。
  // 另外，用来初始化的数组或容器中的数据不需要有序，priority_queue会自动排序。
  ...
  // 使用vector模拟了priority_queue容器适配器部分成员函数的底层实现：

...
// accumulate就是这样的算法，它和inner_product、adjacent_difference和partial_sum三个数值算法一样，位于<numeric>中。
...
class PointAverage1 : public binary_function<Point, Point, Point> {
 public:
...
int main() {
  ...
  string::size_type lengthSum = accumulate(
      ss.begin(), ss.end(), static_cast<string::size_type>(0), stringLengthSum);
  // 计算区间中数值的乘积更加容易：
  vector<float> vf;

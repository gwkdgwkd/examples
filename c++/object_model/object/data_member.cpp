...
// 这个古老的语言规则被称为“member rewriting rule”，大意是“一个内联函数实体，
// 在整个类声明未被完全看见之前，是不会被评估求值的”。
...
//   z += pt.z;
// }
// 看上去是对于x，y，z的直接存取，事实上是经由一个“implicit class object”（由this指针表达）完成。
// 事实上这个函数的参数是：
...
// 指向数据成员的指针，其offset总是被加上1，这样可以使编译系统区分出“一个指向数据成员的指针，
// 用以指出类的第一个成员”和“一个指向数据成员的指针，没有指出任何成员”两种情况。
...
// 多重继承
// 单一继承提供了一种“自然多态”形式，是关于类体系中的基类和派生类之间的转换。
...
// 其复杂度在于派生类和其上一个基类乃至于上上一个基类之间的“非自然”关系。
class Point2d_4 {
 public:
  void print() {
...
// 对一个多重派生类对象，将地址指定给“最左端（第一个）基类的指针”，情况和单一继承时相同，因为二者都指向相同的起始地址。
// 付出的成本只有地址的指定操作而已。
// 至于第二个或后继的基类的地址指定操作，则需要将地址修改：加上（或减去，如果downcast的话）介于中间的基类subobjcet大小。

// 虚拟继承
// 多重继承的一个语意上的副作用就是，它必须支持某种形式的”share subobject继承“。
...
//  把经由拷贝操作取得得所有的nested virtual base class指针，放到派生类对象中。
//  这就解决了“固定存取时间”的问题，虽然付出了一些空间上的代价。
// 至于第一个问题，一般有两个解决方法：
//  a microsoft编译器引入所谓的virtual base class table。
...
// 因为程序代码潜在性地受到专家所谓的“一种奇行怪癖...与也特定编译器有关”的魔咒影响。
...
// 在多重继承下，如果要将第二个（或后继）base class的指针和一个“与派生类对象绑定”的成员结合起来，

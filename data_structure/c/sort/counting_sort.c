...
// 通过统计序列中各个元素出现的次数，完成对整个序列的升序或降序排序，称为计数排序算法。

// 使用计数排序算法完成升序排序的过程为：
// 1.找到序列中的最大值，用max表示，对于{4,2,2,8,3,3,1}序列来说，最大值是8；
// 2.创建一个长度为max+1元素初值全部为0的数组，为数组中[1,max]区域内的各个空间建立索引：
...
// 缺点：
// 1.基本上只有在排序后立刻输出时有作用，这是因为，如果把输入放在一个数组中，
//   计数排序是无法改变这个数组的，因为它需要另外一个数组来统计各个数字的出现个数；
// 2.计数排序只能排序非负整数，因为数组的索引是非负整数，所以排序的数字也只能是非负整数。

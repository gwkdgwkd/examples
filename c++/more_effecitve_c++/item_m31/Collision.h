...
// 无名命名空间中的东西是当前编译单元（其实就是当前文件）私有的，很象被申明为文件范围内static的函数一样。
...
// 它也使用了Item E26中讲的技巧来限制CollisionMap对象的个数为1，因为我们的系统中只有一个映射表。
// 更复杂的游戏需要多张映射表是可以想象到的。
// 最后，它允许我们简化在映射表中增加对称性的碰撞（也就是说，
// 类型T1的对象撞击T2的对象和T2的对象撞击T1的对象，其效果是相同的）的过程，

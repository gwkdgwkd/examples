...
// 但由于unordered_map容器底层采用的是哈希表存储结构，
// 该结构本身不具有对数据的排序功能，
...
// 当无序容器中存储键值对的键为自定义类型时，
// 默认的哈希函数hash以及比较函数equal_to将不再适用，
// 只能自己设计适用该类型的哈希函数和比较函数，
// 并显式传递给Hash参数和Pred参数。
...
// at(key)	          返回容器中存储的键key对应的值，
//                    如果key不存在，则会抛出out_of_range异常。
...
// swap()	            交换2个unordered_map容器存储的键值对，
//                    前提是必须保证这2个容器的类型完全相等。
...
//                    至少容纳count个元（<最大负载因子）所需的数量，
//                    并重新整理容器。
// hash_function()    返回当前容器使用的哈希函数对象。

// C++无序容器的底层实现机制
// C++ STL标准库中，不仅是unordered_map容器，
// 所有无序容器的底层实现都采用的是哈希表存储结构。
// 更准确地说，是用链地址法（又称开链法）解决数据存储位置发生冲突的哈希表。
// 当使用无序容器存储键值对时，会先申请一整块连续的存储空间，
// 但此空间并不用来直接存储键值对，而是存储各个链表的头指针，
// 各键值对真正的存储位置是各个链表的节点。
// STL标准库通常选用vector容器存储各个链表的头指针。
// 在C++ STL标准库中，将各个链表称为桶（bucket），
// 每个桶都有自己的编号（从0开始）。
// 当有新键值对存储到无序容器中时，整个存储过程分为如下几步：
// 1.将该键值对中键的值带入设计好的哈希函数，
//   会得到一个哈希值（一个整数，用H表示）；
// 2.将H和无序容器拥有桶的数量n做整除运算（即H%n），
//   该结果即表示应将此键值对存储到的桶的编号；
...
// 无序容器中，负载因子的计算方法为：
// 负载因子=容器存储的总键值对/桶数，
// 默认情况下，无序容器的最大负载因子为1.0。
// 如果操作无序容器过程中，使得最大复杂因子超过了默认值，
// 则容器会自动增加桶数，并重新进行哈希，以此来减小负载因子的值。
// 需要注意的是，此过程会导致容器迭代器失效，
// 但指向单个键值对的引用或者指针仍然有效。
// 这也就解释了，为什么在操作无序容器过程中，
// 键值对的存储顺序有时会莫名的发生变动。
// C++ STL标准库为了方便用户更好地管控无序容器底层使用的哈希表存储结构，
// 各个无序容器的模板类中都提供了以下成员方法：
// bucket_count()     返回当前容器底层存储键值对时，使用桶的数量。
// max_bucket_count() 返回当前系统中，
//                    unordered_map容器底层最多可以使用多少个桶。
// bucket_size(n)     返回第n个桶中存储键值对的数量。
// bucket(key)        返回以key为键的键值对所在桶的编号。
// load_factor()      返回unordered_map容器中当前的负载因子。
...
//                    反之，如果n的值小于当前容器使用的桶数，
//                    则调用此方法可能没有任何作用。
// reserve(n)         将容器使用的桶数（bucket_count()返回值）设置为，
//                    最适合存储n个元素的桶数。
// hash_function()    返回当前容器使用的哈希函数对象。
...
void func1() {
  // 创建unordered_map容器：
  // 1.通过调用unordered_map模板类的默认构造函数，
  //   可以创建空的unordered_map容器：
  ...
  //   将现有unordered_map容器中存储的键值对，
  //   复制给新建unordered_map容器：
  ...
  // 即以右值引用的方式将临时unordered_map容器中存储的所有键值对，
  // 全部复制给新建容器。
  ...
  //   在现有unordered_map容器中选择部分区域内的键值对，
  //   为新建unordered_map容器初始化：
...
void func2() {
  ...
  //   不同之处在于，如果在当前容器中查找失败，
  //   该方法不会向容器中添加新的键值对，而是直接抛出out_of_range异常。
  ...
  //   该迭代器的指向分以下2种情况：
  //   当find()方法成功找到以指定元素作为键的键值对时，
  //   其返回的迭代器就指向该键值对；
  //   当find()方法查找失败时，返回的迭代器和end()方法返回的迭代器一样，
  //   指向容器中最后一个键值对之后的位置。
  auto it = um.find(2);
...
void func4() {
  ...
  // 该容器就会适当增加桶的数量（通常是翻一倍），
  // 并自动执行rehash()成员方法，
  // 重新调整各个键值对的存储位置（此过程又称重哈希），
  // 此过程很可能导致之前创建的迭代器失效。
  // 所谓迭代器失效，针对的是那些用于表示容器内某个范围的迭代器，
  // 由于重哈希会重新调整每个键值对的存储位置，
...
void func5() {
  ...
  //   当insert()将val成功添加到容器中时，
  //   返回的迭代器指向新添加的键值对，bool值为True；
  //   当insert()添加键值对失败时，
  ...
  //   其中first和last都为迭代器，
  //   [first,last)表示复制其它unordered_map容器中键值对的区域。

...
// 但对于multiset和multimap呢？
// 可以包含重复的值，因此即使容器认为两个等值的对象不等价，又有什么关系呢？
// 可以把这两个值都保存起来，这正是multiset和multimap应该做的，没问题。
// 除非你的比较函数对相等的值总是返回false，
// 否则会破坏所有的标准关联容器，不管它们是否允许储存重复的值。
// 从技术上来说，用于对关联容器排序的比较函数，
// 必须为它们所比较的对象定义一个严格的弱序化。
// 但是在严格的弱序化的要求中，有一个要求就是：
// 任何一个定义了严格的弱序化的函数必须对相同值的两个拷贝返回false。

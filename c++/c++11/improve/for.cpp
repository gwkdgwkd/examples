...
//   需要注意的是，C++11标准中，
//   declaration参数处定义的变量类型可以用auto关键字表示，
//   该关键字可以使编译器自行推导该变量的数据类型。
// 2.expression：
//   表示要遍历的序列，可以为事先定义好的普通数组或者容器，
//   还可以是用{}大括号初始化的序列。

// 同C++98/03中for循环的语法格式相比较，
// 此格式并没有明确限定for循环的遍历范围，
...
void func4() {
  ...
  // 也可以定义引用形式的变量，应该如何选择呢？
  // 其实很简单：
  // 1.如果需要在遍历序列的过程中修改器内部元素的值，
  //   就必须定义引用形式的变量；
  // 2.反之，建议定义const&形式的变量，
  //   避免了底层复制变量的过程，效率更高，也可以定义普通变量。
}
...
// 2.总的来说，基于范围的for循环可以遍历普通数组、
//   string字符串、容器以及初始化列表。
//   除此之外，
//   for循环冒号后还可以放置返回string字符串以及容器对象的函数。
...
void func1() {
  ...
  // 注意，基于范围的for不支持遍历函数返回的以指针形式表示的数组，
  // 比如：
  // for (char ch : retStr1()) {
  //   std::cout << ch;
  // }

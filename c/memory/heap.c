...
// 在进程的地址空间中，代码区、常量区、
// 全局数据区的内存在程序启动时就已经分配好了，
// 它们大小固定，不能由程序员分配和释放，
// 只能等到程序运行结束由操作系统回收，这称为静态内存分配。
// 栈区和堆区的内存在程序运行期间可以根据实际需求来分配和释放，
// 不用在程序刚启动时就备足所有内存，这称为动态内存分配。
// 使用静态内存的优点是速度快，省去了向操作系统申请内存的时间，
// 缺点是不灵活，缺乏表现力，例如不能控制数据的作用范围，不能使用较大的内存。
// 而使用动态内存可以让程序对内存的管理更加灵活和高效，
// 需要内存就立即分配，而且需要多少就分配多少，从几个字节到几个GB不等；
// 不需要时就立即回收，再分配给其他程序使用。

// 栈和堆的区别
// 栈区和堆区的管理模式有所不同：
// 栈区内存由系统分配和释放，不受程序员控制；
// 堆区内存完全由程序员掌控，想分配多少就分配多少，
// 想什么时候释放就什么时候释放，非常灵活。
// 程序启动时会为栈区分配一块大小适当的内存，对于一般的函数调用这已经足够了，
// 函数进栈出栈只是ebp、esp寄存器指向的变换，
// 或者是向已有的内存中写入数据，不涉及内存的分配和释放。
// 当函数中有较大的局部数组时，比如1024*10个元素，
// 编译器就会在函数代码中插入针对栈的动态内存分配函数，
// 这样函数被调用时才分配内存，不调用就不分配。
...
// 几点注意：
// 1.每个内存分配函数必须有相应的free函数，
//   释放后不能再次使用被释放的内存。
// 2.在分配内存时最好不要直接用数字指定内存空间的大小，这样不利于程序的移植。
//   因为在不同的操作系统中，同一数据类型的长度可能不一样。
//   为了解决这个问题，C语言提供了一个判断数据类型长度的操作符，就是sizeof。
// 3.free(p)并不能改变指针p的值，p依然指向以前的内存，
//   为了防止再次使用该内存，建议将p的值手动置为NULL。
// sizeof是一个单目操作符，不是函数，用以获取数据类型的长度时必须加括号，
// 例如sizeof(int)、sizeof(char)等。

// 相对于栈而言，堆这片内存面临着一个稍微复杂的行为模式：
// 在任意时刻，程序可能发出请求，要么申请一段内存，
// 要么释放一段已经申请过的内存，而且申请的大小从几个字节到几个GB都有可能，
...
// free()函数的用处在于实时地回收内存，如果程序很简单，
// 程序结束之前也不会使用过多的内存，
// 不会降低系统的性能，那么也可以不用写free()函数。
// 当程序结束后，操作系统会释放内存。
// 在程序中使用malloc()分配内存时都对应地写出一个free()是一个良好的编程习惯。
// 这不但体现在处理大型程序时的必要性，
// 并能在一定程度上体现程序优美的风格和健壮性。
...
int main() {
  ...
  if (0 == flag) {
    ...
    // 第一次分配的100字节的内存没有指针指向它了，
    // 而且也不知道这块内存的地址，所以就再也无法找回了，也没法释放了，
    // 这块内存就成了垃圾内存，虽然毫无用处，但依然占用资源，
    // 唯一的办法就是等程序运行结束后由操作系统回收。

...
// 2.是拷贝不变的的class，可以使用memcpy，memmove不改变它的语义。
...
void func() {
  ...
  std::cout << std::is_trivial<D>::value << std::endl;  // false
  std::cout << std::is_trivial<E>::value << std::endl;  // false
  std::cout << std::is_trivial<F>::value << std::endl;  // false
  std::cout << std::is_trivial<G>::value << std::endl;  // false
  std::cout << std::is_trivial<H>::value << std::endl;  // false
  std::cout << std::is_trivial<I>::value << std::endl;  // false
  std::cout << std::is_trivial<J>::value << std::endl;  // true
}
}  // namespace n1

namespace n2 {
// 标准布局的standard_layout：
// 1.所有非静态成员有相同的访问权限；
...
class Base {};
class F : virtual Base {};  // 有虚继承，也违反定义5么？
...
void func() {
  ...
  std::cout << std::is_standard_layout<A>::value << std::endl;   // false
  std::cout << std::is_standard_layout<B3>::value << std::endl;  // false
  std::cout << std::is_standard_layout<C2>::value << std::endl;  // false
  std::cout << std::is_standard_layout<D>::value << std::endl;   // false
  std::cout << std::is_standard_layout<E>::value << std::endl;   // false
  std::cout << std::is_standard_layout<F>::value << std::endl;   // false
  std::cout << std::is_standard_layout<G>::value << std::endl;   // false
}
}  // namespace n2
...
namespace n4 {  // 其他的描述
// POD是C++中一个比较重要的概念，用来描述一个类型的属性。
// POD类型，包括class、union和struct等，一般具有以下几种特征：
...
void func() {
  std::cout.setf(std::ios::boolalpha);
  std::cout << std::is_pod<A1>::value << std::endl;  // true
  std::cout << std::is_pod<A2>::value << std::endl;  // false
  std::cout << std::is_pod<B1>::value << std::endl;  // true
  std::cout << std::is_pod<B2>::value << std::endl;  // true
  std::cout << std::is_pod<B3>::value << std::endl;  // true
}
}  // namespace n4

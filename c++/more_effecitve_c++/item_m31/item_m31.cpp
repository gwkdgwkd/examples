...
//   }
// }
// 当你调用processCollision()时，你知道object1和object2正好相撞，
// 并且你知道发生的结果将取决于object1和object2的真实类型，但你并不知道其真实类型；
// 你所知道的就只有它们是GameObject对象。
// 如果碰撞的处理过程只取决于object1的动态类型，你可以将processCollision()设为虚函数，
...
// 这个名字来自于object-oriented programming community，在那里虚函数调用的术语是“message dispatch”，
// 而基两个参数的虚调用是通过“double dispatch”实现的，
// 推而广之，在多个参数上的虚函数叫“multiple dispatch”。
// 有几个方法可以考虑：
// 但没有哪个是没有缺点的，这不该奇怪。
// C++没有直接提供“double dispatch”，所以你必须自己完成编译器在实现虚函数时所做的工作。
...
// 在这种刺眼的体系下，我们首先是发现otherObject的真实类型，然后测试所有的可能：
...
// 粗一看，它象依据参数的顺序进行循环调用，
// 也就是开始的otherObject变成了调用成员函数的对象，而*this成了它的参数。
// 但再仔细看一下啦，它不是循环调用。
// 你知道的，编译器根据参数的静态类型决定调那一组函数中的哪一个。
// 在这儿，有四个不同的collide函数可以被调用，但根据*this的静态类型来选中其中一个。
// 现在的静态类型是什么？
// 因为是在SpaceShip的成员函数中，所以*this肯定是SpaceShip类型。
// 调用的将是接受SpaceShip参数的collide函数，而不是带GameOjbect类型参数的collide函数。
...
// 修改现存类经常是你做不到的。
// 比如，你不是在写整个游戏，只是在完成程序框架下的一个支撑库，你可能无权修改GameObject类或从其经常的框架类。
...
// 使用vtbl，编译器避免了使用if...then...else链，并能在所有调用虚函数的地方生成同样的代码：
...
// 因为这张表包含的是指向成员函数的指针，所以在增加新的GameObject类型时仍然需要修改类的定义，
// 这还是意味着所有人都必须重新编译，即使他们根本不关心这个新的类型。
// 如果映射表中包含的指针指向非成员函数，那么就没有重编译问题了。
// 而且，转到非成员的碰撞处理函数将让我们发现一个一直被忽略的设计上的问题，
// 就是，应该在哪个类里处理不同类型的对象间的碰撞在前面的设计中，如果对象1和对象2碰撞，
// 而正巧对象1是processCollision的左边的参数，碰撞将在对象1的类中处理；
...
// 最终达到了我们的目的:
// 1.如果增加了新的GaemObject的子类，现存类不需要重新编译（除非它们用到了新类）。
...
// 我们期望调用void shipAsteroid(GameObject& spaceShip, GameObject& asteroid);
// 它不会被调用的，实际上，抛了一个UnknownCollision的异常。
// 因为lookup在根据类型名“MilitaryShip”和“Asteroid”在collisionMap中查找函数时没有找到。
// 虽然MilitaryShip可以被转换为一个SpaceShip，但lookup却不知道这一点。
// 而且，没有没有一个简单的办法来告诉它。
// 如果你需要实现二重调度，并且需要这儿的向上类型映射，
// 你只能采用我们前面讨论的二次虚函数调用的方法（同时也意味着增加新类的时候，所有人都必须重新编译）。
...
// 但它使得容易提供数据给基于map的实现，如果我们认为这种实现最接近我们的需要的话。
void func3() {
...
// 例如，对于类Spaceship，type_info::name的一个实现返回“class SpaceShip”。

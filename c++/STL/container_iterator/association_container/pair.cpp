...
// 考虑到“键值对”并不是普通类型数据，C++ STL标准库提供了pair类模板，
// 其专门用来将2个普通元素first和second（基本数据类型、结构体、类）创建成一个新元素<first,second>。
// 通过其构成的元素格式不难看出，使用pair类模板来创建“键值对”形式的元素，再合适不过。
...
//  template<class U, class V> pair(const pair<U,V>& pr);拷贝（复制）构造函数

...
int main() {
  ...
  // 2 在创建反向迭代器的时候，我们可以直接将一个基础迭代器作为参数传递给新建的反向迭代器
  std::vector<int> myvector{1, 2, 3, 4, 5};
  std::reverse_iterator<std::vector<int>::iterator> my_reiter2(myvector.end());
  // 反向迭代器是通过操纵内部的基础迭代器实现逆向遍历的，但是反向迭代器的指向和底层基础迭代器的指向并不相同。my_reiter2其内部的基础迭代器指向的是myvector容
  // 器中元素5之后的位置，但是my_reiter指向的却是元素5。
  // 反向迭代器的指向和其底层基础迭代器的指向具有这样的关系，即反向迭代器的指向总是距离基础迭代器偏左1个位置；反之，基础迭代器的指向总是距离反向迭代器偏右1个位置处。
  // 其中，begin和end表示基础迭代器，r(begin)和r(end)分别表示有begin和end获得的反向迭代器。
  cout << *my_reiter2 << endl;                         // 5
  cout << myvector.end() - my_reiter2.base() << endl;  // 0
  // 3 reverse_iterator模板类还提供了一个复制（拷贝）构造函数，可以实现直接将一个反向迭代器复制给新建的反向迭代器
  std::reverse_iterator<std::vector<int>::iterator> my_reiter31(
      myvector.rbegin());
  std::reverse_iterator<std::vector<int>::iterator> my_reiter32 =
      myvector.rbegin();

  std::vector<int> myvector1{1, 2, 3, 4, 5, 6, 7, 8};
  std::reverse_iterator<std::vector<int>::iterator> my_reiter(
      myvector1.rbegin());           // 指向8
  ...
  std::reverse_iterator<std::vector<int>::iterator> begin(myvector1.begin());
  std::reverse_iterator<std::vector<int>::iterator> end(myvector1.end());

...
// mp3支持的样本格式为S16P(平面格式)，先存完所有的左声道数据，再存右声道数据。
// 双声道，立体声，单帧的样本为1152个，采样率为44100。
...
int main() {
  ...
  // 准备一个结构体来接受重采样的每一帧的音源数据，每帧的样本大小为1152
  AVFrame* frame;
  ...
  // 给保存pcm文件数据分配空间
  ret = av_samples_alloc_array_and_samples(&input_data, &input_linesize, 2,
                                           1152, AV_SAMPLE_FMT_S16, 0);
  if (ret < 0) {
    cout << "av_samples_alloc_array_and_samples input failed" << endl;
    return -1;
  }
  // 缓存重采样数据的空间分配
  ret = av_samples_alloc_array_and_samples(&output_data, &output_linesize, 2,
                                           1152, AV_SAMPLE_FMT_S16P, 0);
  if (ret < 0) {
  ...
  while (!feof(finput)) {
    ...
    frame->data[0] = output_data[0];  // 左声道数据
    frame->data[1] = output_data[1];  // 右声道数据
    ...
    // [libmp3lame @ 0x55fa1cd36dc0] 5 frames left in the queue on closing
    while (ret >= 0) {

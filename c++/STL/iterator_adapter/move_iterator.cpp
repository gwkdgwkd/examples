...
void func2() {
  ...
  // 那么，如果不想采用复制的方式，而就是想把容器中存储的元素全部移动到其他容器中，该怎么办呢？
  // 没错，就是采用移动迭代器。
  ...
  // 即便通过移动迭代器将容器中某区域的元素移动到了其他容器中，该区域内仍可能残留有之前存储的元素，
  // 但这些元素是不能再被使用的，否则极有可能使程序产生各种其他错误。
}

void func3() {
  // 和其他迭代器适配器一样，move_iterator模板类中也提供有base()成员方法，
  // 通过该方法，我们可以获取到当前移动迭代器底层所使用的基础迭代器
  std::vector<std::string> v1{"nihao", "wohao", "tahao"};

...
// mutex（互斥量）的种类:
// 1.std::mutex，最基本的mutex类，提供了独占所有权的特性，即不支持递归地对std::mutex对象上锁。
...
// func1函数和func2函数都获取了同一个锁，而bar函数又会调用func1函数。
...
// 当需要获取两个互斥量时，避免死锁的方法，关键是如何在线程之间，
// 以一定的顺序获取锁。一些情况下，这种方式相对简单。

...
//   拷贝（复制）构造函数。
// 4.在C++11标准中，在引入右值引用的基础上，
//   pair类模板中又增添了如下2个构造函数：
...
int main() {
  ...
  // 调用移动构造函数：
  // make_pair()函数，它也是<utility>头文件提供的，
  // 其功能是生成一个pair对象。
  // 因此，当将make_pair()函数的返回值是一个临时对象，
  // 作为参数传递给pair()构造函数时，
  ...
  // 对于进行比较的2个pair对象，其对应的键和值的类型比较相同，
  // 否则将没有可比性，同时编译器提示没有相匹配的运算符，
  // 即找不到合适的重载运算符。

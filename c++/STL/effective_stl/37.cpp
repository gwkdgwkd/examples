...
// accumulate就是这样的算法，它和inner_product、
// adjacent_difference和partial_sum三个数值算法一样，位于<numeric>中。
...
// for_each是另一个可被用来统计区间的算法，
// 而且它不受accumulate的那些限制。
// 与accumulate一样，for_each也带两个参数：
// 一个是区间，一个是函数（通常是函数对象）。
// 区间中的每个元素都要调用这个函数，
// 但是，传给for_each的这个函数只接收一个实参（即当前的区间元素）。
...
// for_each的函数允许有副作用，而accumulate的函数参数却不允许呢？
// 这是一个深层次的问题。

int main() {
  // accumulate有两种形式：
  // 1.第一种形式有两个迭代器和一个初始值，
  //   返回该初始值加上迭代器标识的区间中的值的总和：
  std::list<double> ld;
  double sum = accumulate(ld.begin(), ld.end(), 0.0);  // 初始值为0.0，不能是0
  // accumulate只要求输入迭代器，
  // 甚至可以使用istream_iterator和istreambuf_iterator:
  std::cout << "sum : "
  ...
  // 上面的代码可以工作，但是标准中描述，
  // 传递给accumulate的函数不允许有副作用。
  // 而修改numPoints、xSum、ySum的值会带来副作用，
  // 所以从技术上说，很多专家不允许这么做。

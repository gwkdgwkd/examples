...
// 不能提前到编译期发现，也不具有强制性，也谈不上改善编译信息的可读性。
// 既然是运行期检查，对性能肯定是有影响的，
// 所以经常在发行版本中，assert都会被关掉。

// #error可看作是预编译期断言（甚至都算不上断言），
// 仅仅能在预编译时显示一个错误信息，
// 可以配合#ifdef/ifndef参与预编译的条件检查，
// 由于它无法获得编译信息，当然，也就做不了更进一步分析了。

// 在stastic_assert提交到C++0x标准之前，为了弥补assert和#error的不足，
// 出现了一些第三方解决方案，可以作编译期的静态检查：
// BOOST_STATIC_ASSERT和LOKI_STATIC_CHECK，
...
// 2.常量表达式：
//   static_assert的断言表达式的结果必须是在编译时期可以计算的表达式，
//   即必须是常量表达式：
static_assert(sizeof(int) == 4, "64-bit code generation is not supported.");
//   该static_assert用来确保编译仅在32位的平台上进行，
//   不支持64位的平台，该语句可放在文件的开头处，
//   这样可以尽早检查，以节省失败情况下耗费的编译时间。
// 3.模板参数：
//   编译器在遇到一个static_assert语句时，
//   通常立刻将其第一个参数作为常量表达式进行演算。
//   但如果该常量表达式依赖于某些模板参数，
//   则延迟到模板实例化时再进行演算，这就让检查模板参数也成为了可能：

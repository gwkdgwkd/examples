...
// 和unordered_map一样，unordered_multimap也以键值对的形式存储数据，
// 且底层也采用哈希表结构存储各个键值对，两者唯一的不同之处在于，
// unordered_multimap可以存储多个键相等的键值对，而unordered_map不行。
...
// 以上5个参数中，必须显式给前2个参数传值，且除极个别的情况外，最多只用前4个参数。
...
//                  而不适用于自定义的结构体或者类。
// Pred = equal_to<Key> unordered_multimap容器可以存储多个键相等的键值对，
//                      而判断是否相等的规则，由此参数指定。
...
// 这种情况下，需要自定义适用的哈希函数和比较函数，并分别显式传递给Hash和Pred。

// unordered_multimap容器的成员方法：
// begin()	          返回指向容器中第一个键值对的正向迭代器。
// end()	            返回指向容器中最后一个键值对之后位置的正向迭代器。
// cbegin()	          和begin()功能相同，只不过const属性，不能修改键值对。
// cend()	            和end()功能相同，只不过增加了const，不能修改键值对。
// empty()	          若容器为空，则返回true，否则false。
// size()	            返回当前容器中存有键值对的个数。
// max_size()       	返回容器所能容纳键值对的最大个数，不同系统返回值不相同。
// find(key)	        查找key，如果找到则返回指向该键值对的正向迭代器；
...
// swap()	            交换unordered_multimap的键值对，容器的类型完全相等。
// bucket_count()	    返回当前容器底层存储键值对时，使用桶的数量。
// max_bucket_count()	返回系统中，unordered_multimap底层最多可以使用多少桶。
// bucket_size(n)	    返回第n个桶中存储键值对的数量。
// bucket(key)	      返回以key为键的键值对所在桶的编号。
// load_factor()	    返回unordered_multimap中当前的负载因子，负载因子是：
//                    容器中存储键值对的size()和使用桶数bucket_count()的比值，
...
//                    至少容纳count个（不超过最大负载因子）所需的数量，并重新整理容器。
// hash_function()  	返回当前容器使用的哈希函数对象。

// 和unordered_map相比，unordered_multimap的类模板中没有重载[]运算符，
// 也没有提供at()成员方法，除此之外它们完全一致。
// 没有提供[]运算符和at()成员方法，意味着unordered_multimap无法通过键获取值，
...
// 除了可以用容器模板类中的swap()成员方法外，STL还提供了swap()非成员函数。

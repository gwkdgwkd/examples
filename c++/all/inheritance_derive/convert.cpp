...
// 这种转换只有在基类和派生类之间才有意义，并且只能将派生类赋值给基类，包括：
// 1.将派生类对象赋值给基类对象；
// 2.将派生类指针赋值给基类指针；
// 3.将派生类引用赋值给基类引用。
...
namespace n2 {
// 将派生类指针赋值给基类指针时，通过基类指针只能使用派生类的成员变量，
// 但不能使用派生类的成员函数。
// 通过指针来访问成员变量，指针指向哪个对象就使用哪个对象的数据；
// 通过指针的类型来访问成员函数，指针属于哪个类的类型就使用那个类的函数。
// 通过引用或对象实现向上转型时，也是一样的，
// 使用基类成员函数，成员函数中引用派生类成员变量。
...
void func1() {
  ...
  // 只有pa、pb、pd三个指针的值相等，pc的值比它们都大。
  // 赋值就是将一个变量的值交给另外一个变量，这种想法虽然没错，
  // 但是赋值以前编译器可能会对现有的值进行处理。
  // 例如将double类型的值赋给int类型的变量，编译器会直接抹掉小数部分，
  // 导致赋值运算符两边变量的值不相等。
  // 将派生类的指针赋值给基类的指针时也是类似的道理，编译器也可能会在赋值前进行处理。
  // pd的内存模型：   (pa pb pd)            pc
  //                    B(A(m_a)   m_b)   C(m_c)   m_d
  // 对象的指针必须要指向对象的起始位置。
  // 对于A类和B类来说，它们的子对象的起始地址和D类对象一样，
  // 所以将pd赋值给pa、pb时不需要做任何调整，直接传递现有的值即可；
  // 而C类子对象距离D类对象的开头有一定的偏移，将pd赋值给pc时要加上这个偏移，
  // 这样pc才能指向C类子对象的起始位置。
  // 也就是说，执行pc = pd;时编译器对pd的值进行了调整，才导致pc、pd的值不同。

...
// #define命令虽然能解决问题，但也带来了不小的副作用，导致宏名过多，代码松散，看起来总不舒服。C语言提供了枚举（Enum）类型，能列出所有可能值，并给它们取一个名字。
...
// 定义枚举和枚举变量
// 枚举值默认从0开始，往后逐个加 1（递增）；
// enum week1 { Mon, Tues, Wed, Thurs, Fri, Sat, Sun };
// 可以给每个名字都指定一个值：
// enum week2 { Mon = 1, Tues = 2, Wed = 3, Thurs = 4, Fri = 5, Sat = 6, Sun = 7 };
...
// 需要注意的两点是：
// 1) 枚举列表中的Mon、Tues、Wed这些标识符的作用范围是全局的（严格来说是main()函数内部），不能再定义与它们名字相同的变量。
// 2) Mon、Tues、Wed等都是常量，不能对它们赋值，只能将它们的值赋给其他的变量。
// 枚举和宏其实非常类似：宏在预处理阶段将名字替换成对应的值，枚举在编译阶段将名字替换成对应的值。我们可以将枚举理解为编译阶段的宏。
// case关键字后面必须是一个整数，或者是结果为整数的表达式，但不能包含任何变量，正是由于Mon、Tues、Wed这些名字最终会被替换成一个整数，所以它们才能放在case后面。

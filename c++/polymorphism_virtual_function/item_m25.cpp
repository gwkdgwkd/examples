...
// 仅当你还没拥有一个对象但是你又确切地知道想要的对象的类型时，
// 你才会调用构造函数，那么虚拟构造函数又从何谈起呢？
...
// 为了能够回到正常的语法上来，
// 我们必须把operator<<移出TextBlock和Graphic类，
...
// 具有虚拟行为的非成员函数很简单。
// 你编写一个虚拟函数来完成工作，然后再写一个非虚拟函数，
// 它什么也不做只是调用这个虚拟函数。
// 为了避免这个句法花招引起函数调用开销，你当然可以内联这个非虚拟函数。

// 你知道如何根据它们的一个参数让非成员函数虚拟化，
// 你可能想知道是否可能让它们根据一个以上的参数虚拟化呢？

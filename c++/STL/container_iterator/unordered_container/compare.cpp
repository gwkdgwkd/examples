...
// 在此存储结构中，哈希函数的功能是根据各个键值对中键的值，
// 计算出一个哈希值（本质就是一个整数），
// 哈希表可以根据该值判断出该键值对具体的存储位置。
// 简单地理解哈希函数，它可以接收一个元素，
// 并通过内部对该元素做再加工，最终会得出一个整形值并反馈回来。
// 需要注意的是，哈希函数只是一个称谓，
...
// 和哈希函数一样，无论创建哪种无序容器，
// 都需要为其指定一种可比较容器中各个元素是否相等的规则。
// 默认情况下无序容器使用的std::equal_to<key>比较规则，
// 其本质也是一个函数对象类，底层实现如下：
// template <class T>
// class equal_to {
//  public:
...
// 则该容器可以使用默认的std::equal_to<key>比较规则；
// 反之，就不可以使用。
...
void test() {
  ...
  // 因为该容器使用的是默认的std::equal_to<key>比较规则，
  // 但此规则并不适用于该容器。
...
// 创建一个可存储Person类对象的unordered_set容器，
// Person为自定义的类型，
...
// Person类对象，不支持直接使用==运算符做比较，
// 这种情况下，有以下2种方式可以解决此问题：
// 1.在Person 类中重载==运算符，

...
// C++的发明人Bjarne Stroustrup也说过，他在C++中引入引用的直接目的是为了让代码的书写更加漂亮，
...
void func2() {
  ...
  // int &const r3 = i;  // 没有const引用，本来就不能改变指向，加上const是多此一举
}
...
namespace n2 {
void func1() {
  // 变量是要占用内存的，但是通过&r获取到的却不是引用变量的地址，而是被引用变量的地址：
  int a = 99;
  int &r = a;
...
void func2() {
  ...
  // 2.r与num地址是一样的。
  // 使用&r取地址时，编译器会对代码进行隐式的转换，使得代码输出的是r的内容（a的地址），
  // 而不是r的地址，这就是为什么获取不到引用变量的地址的原因。
  // 也就是说，不是变量r不占用内存，而是编译器不让获取它的地址。
  // 其实引用只是对指针进行了简单的封装，它的底层依然是通过指针实现的，
  // 引用占用的内存和指针占用的内存长度一样，在32位环境下是4个字节，在64位环境下是8个字节，
  // 之所以不能获取引用的地址，是因为编译器进行了内部转换，转换称指针。
  // 当引用作为形参时，也会有类似的转换。
}

...
// 数据之间的关系有3种，分别是"一对一"、"一对多"和"多对多"，
// 前两种关系的数据可分别用线性表和树结构存储，"多对多"逻辑关系数据的结构用图存储。
...
// 5.路径和回路:无论是无向图还是有向图，从一个顶点到另一顶点途径的所有顶点组成的序列（包含这两个顶点），称为一条路径。
...
// 存储方式的不同，指的是：
// 在使用二维数组存储图中顶点之间的关系时，如果顶点之间存在边或弧，在相应位置用1表示，反之用0表示；
// 如果使用二维数组存储网中顶点之间的关系，顶点之间如果有边或者弧的存在，在数组的相应位置存储其权值，反之用0表示。

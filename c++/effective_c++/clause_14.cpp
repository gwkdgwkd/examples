...
//  对底层资源使用“引用计数法”：保有资源直到最后一个使用者被销毁。
//  复制底部资源：对资源进行复制，也就是进行“深度拷贝”。
//  转移底部资源的拥有权：某些罕见场合下希望确保永远只有一个RAII对象指向一个未加工资源，即使复制也依然如此。

// copying函数（包括copy构造函数和copy assignment操作符）有可能被编译器自动创建出来，除非编译做了你想要的事，否则就需要自己编写。

...
// 在某些场景中，可能需要逐个读取文件中存储的字符，或者逐个将字符存储到文件中。
// 这种情况下，就可以调用get()和put()成员方法实现。
// fstream和ofstream类继承自ostream类，
// 因此fstream和ofstream类对象都可以调用put()方法。
// 当fstream和ofstream文件流对象调用put()方法时，
// 该方法的功能就变成了向指定文件中写入单个字符。
// put()方法的语法格式如下：
// ostream& put (char c);
// 其中，c用于指定要写入文件的字符。
...
// 等刷新该缓冲区（缓冲区满、关闭文件、调用flush()等，都会导致缓冲区刷新）时，
// 才会将缓冲区中存储的所有字符一股脑儿全写入文件。

// 和put()成员方法的功能相对的是get()方法，
// 其定义在istream类中，借助cin.get()可以读取用户输入的字符。
// 在此基础上，fstream和ifstream类继承自istream类，
...
// int get();
// 返回值就是读取到的字符，只不过返回的是它的ASCII码，
// 如果碰到输入的末尾，则返回值为EOF。
// istream& get(char& c);
// 需要传递一个字符变量，get()方法会自行将读取到的字符赋值给这个变量。
// 注意，和put()方法一样，操作系统在接收到get()方法的请求后，
// 哪怕只读取一个字符，也会一次性从文件中将很多数据（通常至少是512个字节，

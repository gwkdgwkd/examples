...
// 它们能够应用于更多的上下文环境中，因此应当尽可能地使函数对象可以配接。
...
void func() {
  ...
  // 找到第一个不满足条件的，编译失败：
  // find_if(widgetPtrs.begin(), widgetPtrs.end(), std::not1(isInteresting));
...
// unary_function和binary_function operator()的返回类型，都是最后一个参数。
template <typename T>
...
// 虽然operator()的参数都是const Widget&，但传给binary_function的却是Widget。
// 对于unary_function和binary_function，一般传的非指针类型需要去掉const和引用部分：
...
void func2() {
  ...
  std::list<Widget>::reverse_iterator it = find_if(
...
// 传给unary_function和binary_function的类型与operator()的参数和返回类型完全相同。
struct PtrWidgetNameCompare
    : public std::binary_function<const Widget*, const Widget*, bool> {
  bool operator()(const Widget* lhs, const Widget* rhs) const {}
};

// STL函数对象是C++函数的抽象和建摸形式，而每个C++函数只有一组确定的参数和一个返回类型。
// 所以STL总是假设每个函数子类只有一个operator()成员函数，并且其参数和返回类型，

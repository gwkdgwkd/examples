...
int ciCharCompare(char c1, char c2) {
  ...
  // 当char有符合时，确保它的值可以用unsigned char来表达的唯一办法是，在调用tolower之前将它强制转换成一个unsigned char。
...
int ciStringCompareImpl(const string& s1, const string& s2) {
  ...
  // 如果直接用ciCharCompare作为判别式，那么C++将会把它的返回类型转换成bool。0->false,-1 1->true，和期望相反。
  if (p.first == s1.end()) {
...
int ciStringCompare1(const string& s1, const string& s2) {
...
// 第二种实现方式将产生一个很常用的STL判别式。想这样的函数可以别用作关联容器的比较函数。
// 修改ciCharCompare，使它成为一个具有判别式接口的字符比较函数，然后把执行字符串比较的工作交给STL中名字第二长的算法：
...
int ciStringCompare3(const string& s1, const string& s2) {
  // return stricmp(s1.c_str(), s2.c_str());  // 也许叫别的名字
  // 有人认为这是取巧，但strcmp/strcmpi通常是被优化过的，它们在长字符串的处理上一般要比通用的算法mismatch和lexicographical_compare快得多。
}
...
int main() {
  ...
  if (ciStringCompare1(s1, s2) == 0) {
    cout << "xiangdeng" << endl;
  } else {
    cout << "buxiangdeng" << endl;
  }
  if (ciStringCompare1(s1, s3) == 0) {
    cout << "xiangdeng" << endl;
  } else {
    cout << "buxiangdeng" << endl;
  }

  if (ciStringCompare2(s1, s2) == 0) {
    cout << "xiangdeng" << endl;
  } else {
    cout << "buxiangdeng" << endl;
  }
  if (ciStringCompare2(s1, s3) == 0) {
    cout << "xiangdeng" << endl;
  } else {
    cout << "buxiangdeng" << endl;
  }

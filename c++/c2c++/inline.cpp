...
// CPU在执行主调函数代码时如果遇到了被调函数，
// 主调函数就会暂停，CPU转而执行被调函数的代码；
// 被调函数执行完毕后再返回到主调函数，主调函数根据刚才的状态继续往下执行。
// 一个C/C++程序的执行过程可以认为是多个函数之间的相互调用过程，
// 它们形成了一个或简单或复杂的调用链条，这个链条的起点是main()，终点也是main()。
// 当main()调用完了所有函数，会return一个值来结束自己的生命，从而结束整个程序。
// 函数调用是有时间和空间开销的。
// 程序执行函数前需要做些准备工作，将实参、局部变量、
// 返回地址以及若干寄存器都压入栈中，然后才执行函数体中的代码；
...
// 宏定义是一项细思极密的工作，一不小心就会踩坑，
// 而且不一定在编译和运行时发现，给程序埋下隐患。
...
// 更为严格地说，内联函数不应该有声明，应该将函数定义放在本应该出现函数声明的地方，
// 这是一种良好的编程风格。
// 在多文件编程中，通常将函数的定义放在源文件中，将函数的声明放在头文件中，
// 希望调用函数时，引入对应的头文件即可，鼓励这种将函数定义和函数声明分开的做法。
...
// 链接器在将多个目标文件（.o或.obj文件）合并成一个可执行文件时找不到func()的定义，
// 所以会产生链接错误。

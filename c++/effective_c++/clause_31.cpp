...
像Person这个使用pimpl idiom的class，往往被称为Handle class。并不会改变Person做的事，只会改变它做事的方法。
#include "Person.h"
...
就像Handle classes的客户一样，除非Interface class的接口被修改否则其客户不需要重新编译。
...
shared_ptr<Person> create(const string& name, const Date& birthday, const Address& addr) {
  return shared_ptr<Person>(new RealPerson(name, birthday, addr));
}
更现实的Person::create实现会根据额外参数、读自文件或数据库的数据、环境变量来创建不同类型的derived class对象。

RealPerson示范实现Interface class的两个常见机制之一（另一个涉及多重继承，参考条款40）：
...
//   声明today和clearAppointments函数无需定义Date。一旦任何人调用那些函数，调用之前一定要找到Date的定义。
//   何必费心声明一个没人调用的函数呢？并非没人调用，而是并非每个人都调用。
//   如果能将“提供class定义（通过#include完成）”的义务从“函数声明所在”的头文件移到“内含函数调用”的客户文件，
//   便可将“并非正真必要的类型定义”与客户端的编译依赖性去除掉。
// 3 为声明和定义提供不同的头文件。
//   需要两个头文件，一个用于声明，一个用于定义。如果有个声明被改来，两个文件都得改。
//   程序库客户应该总是#include一个声明文件而非前置声明若干函数。
//   #include “datefwd.h"  // 参考C++标准程序库文件的<iosfwd>。包含各种组件的声明，定义分布在<sstream>,<streambuf>,<fstream>和<iostream>。
//   Date today();
//   void clearAppointments(Date d);
//   iosfwd深具启发意义的另一个原因，分外彰显“本条款适用于template也适用于non-template”
//   C++也提供export关键字，允许将template声明和定义分割于不同的文件内。不幸的是支持这个关键在的编译器目前非常少。

// Handle class和Interface class解除来接口和实现之间的耦合关系，从而降低文件间的编译依赖。
// 在Handle class身上，成员函数必须通过implementation pointer取得对象数据。那会为每一次访问增加一层间接性。
...
// 至于Interface class，由于每个函数都是virtual，所以必须为每次函数调用付出一个间接跳跃成本。
//  此外Interface class派生的对象必须包含一个vptr，这个指针可能会增加存放对象所需的内存数量。

// 最后，不论Handle class还是Interface class，一旦脱离inline函数都无法有太大作为。
// 条款30解释了函数本体为了被inline必须置于头文件内，但Handle class和Interface class这是特别被设计出来用来隐藏实现细节的，如函数本体。

...
// 2.抽象数据类型模型（ADT），该模型所谓“抽象”是和一组表达式（public接口）一起提供，而其运算定义仍然隐而未明。
// 3.面向对象模型，在此模型中有一些彼此相关的类型，通过一个抽象base class（用以提供公共接口）被封装起来。
...
// 转型（cast）其实是一种编译器指令。大部分情况下它并不改变一个指针所含的真正地址，
// 它只影响“被指出的内存的大小和其内容”的解释方式。
// 一个指针或一个引用之所以支持多态，是因为它们并不引发内存中任何“与类型有关的内存委托操作”。
// 改变的只是指向内存的“大小和内容解释方式”而已。
...
// 1.速度快是因为所有的函数引发的操作都在编译时期解析完成，对象构建起来时不需要设置virtual机制；

...
int main() {
  ...
  // ws2_32.dll 支持的最高版本为2.2，建议使用的版本也是2.2。
  // 综上所述：
  ...
  //   AF是“Address Family”的简写，INET是“Inetnet”的简写。
  //   AF_INET表示IPv4地址，例如127.0.0.1，代表本机；
  //   AF_INET6表示IPv6地址，例如1030::C9B4:FF12:48AA:1A2B。
  //   你也可以使用PF前缀，PF是“Protocol Family”的简写，它和AF是一样的。
  ...
  //   但0~1023的端口一般由系统分配给特定的服务程序，例如Web服务的端口号为80，FTP服务的端口号为21，
  //   所以我们的程序要尽量在1024~65536之间分配端口号。
  //   端口号需要用htons()函数转换。
  ...
  // socket()函数的第一个参数已经指明了地址类型，为什么在sockaddr_in结构体中还要再说明一次呢，这不是啰嗦吗？
  // 应该是历史原因吧，后面的接口总要兼容前面的代码。

  // 为什么使用sockaddr_in而不使用sockaddr？
  // bind()第二个参数的类型为sockaddr，而代码中却使用sockaddr_in，然后再强制转换为sockaddr，这是为什么呢？
  //   struct sockaddr {
  //     sa_family_t sin_family;  // 地址族（Address Family），也就是地址类型
  //     char sa_data[14];        // IP地址和端口号
  //   };
  // sockaddr和sockaddr_in的长度相同，都是16字节，只是将IP地址和端口号合并到一起，用一个成员sa_data表示。
  // 要想给a_data赋值，必须同时指明IP地址和端口号，例如”127.0.0.1:80“，遗憾的是，
  // 没有相关函数将这个字符串转换成需要的形式，也就很难给sockaddr类型的变量赋值，所以使用sockaddr_in来代替。
  ...
  // 所谓被动监听，是指当没有客户端请求时，套接字处于“睡眠”状态，
  ...
  // ssize_t在size_t前面加了一个s，代表signed，
  // 即ssize_t是typedef声明的signed int类型。

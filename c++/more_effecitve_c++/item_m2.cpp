...
// 3.static_cast不能从表达式中去除const属性，
//   因为另一个新的类型转换操作符const_cast有这样的功能。

// const_cast用于类型转换掉表达式的const或volatileness属性。
// 通过使用const_cast，你向人们和编译器强调，
// 你通过类型转换想做的只是改变一些东西的const或者volatile属性。
// 这个含义被编译器所约束。
// 如果你试图使用const_cast来完成修改const或者volatile属性之外的事情，
// 你的类型转换将被拒绝。
...
// 第二种特殊的类型转换符是dynamic_cast，
// 它被用于安全地沿着类的继承关系向下进行类型转换。
// 这就是说，你能用dynamic_cast把指向基类的指针或引用转换成，
// 指向其派生类或其兄弟类的指针或引用，而且你能知道转换是否成功。
// 失败的转换将返回空指针（当对指针进行类型转换时），
// 或者抛出异常（当对引用进行类型转换时）。
// dynamic_cast在帮助你浏览继承层次上是有限制的。
// 它不能被用于缺乏虚函数的类型上，也不能用它来转换掉const。
// 如你想在没有继承关系的类型中进行转换，你可能想到static_cast。
// 如果是为了去除const，你总得用const_cast。
void updateViaRef(SpecialWidget &rsw) {}

// 使用reinterpret_cast这个操作符的类型转换，
// 转换结果几乎都是执行期定义（implementation-defined）。
...
int doSomething() {}
// 转换函数指针的代码是不可移植的，
// C++不保证所有的函数指针都被用一样的方法表示，
// 在一些情况下这样的转换会产生不正确的结果，
// 所以你应该避免转换函数指针类型，
// 除非你处于着背水一战和尖刀架喉的危急时刻。
// 一把锋利的刀，一把非常锋利的刀。

// 如果你使用的编译器缺乏对新的类型转换方式的支持，
// 你可以用传统的类型转换方法代替static_cast，
// const_cast以及reinterpret_cast。
...
// 如果你没有这些函数而你有必须进行这样的类型转换，
// 你也可以回到C风格的类型转换方法上，
// 但是这样的话你将不能获知类型转换是否失败。
// 当然，你也可以定义一个宏来模拟dynamic_cast的功能，
// 就象模拟其它的类型转换一样：
#define dynamic_cast(TYPE, EXPR) (TYPE)(EXPR)
// 请记住，这个模拟并不能完全实现dynamic_cast的功能，
// 它没有办法知道转换是否失败。
...
int main() {
  ...
  // 错误!const SpecialWidget*类型变量，
  // 不能传给处理SpecialWidget*类型变量的函数：
  // update(&csw);

...
// 树的结点：使用树结构存储的每一个数据元素都被称为结点。
// 根结点：如果一个结点没有父结点，那么这个每个非空树有且只有一个的结点就是整棵树的根结点。
...
// 有序树和无序树
// 如果树中结点的子树从左到右看，谁在左边，谁在右边，是有规定的，则为有序树，否则是无序树。
...
// 树的表示方法
// 1.用树型图表示；
// 2.以嵌套的集合的形式表示的，集合之间绝不能相交，即图中任意两个圈不能相交；
// 3.凹入表示法；
// 4.最常用的表示方法是使用广义表的方式：(A,(B(E(K,L),F),C(G),D(H(M),I,J)))
...
// 其存储过程是：从树的根节点开始，使用顺序表依次存储树中各个节点，
// 需要注意的是，与双亲表示法不同，孩子表示法会给各个节点配备一个链表，
// 用于存储各节点的孩子节点位于顺序表中的位置。
// 如果节点没有孩子节点（叶子节点），则该节点的链表为空链表。
// 还可以将双亲表示法和孩子表示法合二为一，既能快速找到指定节点的父节点，
// 又能快速找到指定节点的孩子节点。

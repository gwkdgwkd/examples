...
// ASPECT_PATIO也许从未被编译器看见；
// 也许在编译器开始处理源码之前它就被预处理器移走了。
// 于是ASPECT_PATIO有可能没进入符号表内。
// 于是运用此常量但获得一个编译错误时，会带来困惑，
// 提到的是1.653而不是ASPECT_PATIO。
// 特别是在一个别人写的头文件内。
...
// 2.class专属常量，为了将常量的作用域限制于class内，
//   必须让它成为class的一个成员；
...
// const int GamePlayer::NumTurns = 6;

...
// 在函数生命期内不会被改变，应该被声明为const。
// 如果一个纯函数需要访问那些可能会在两次调用之间发生变化的数据，
// 那么相同的参数在不同的时刻调用该函数就有可能得到不同的结果，
// 与纯函数的定义相矛盾。

// 判别式类是一个函数子类，它的operator()函数是一个判别式，
// 也就是说，operator()返回true或者false。
// STL中凡是能接受判别式的地方，就既可以接受一个真正的判别式，
// 也可以接受一个判别式类的对象。

// 接受函数子的STL算法可能会先创建函数子的拷贝，
// 然后存放起来待以后再使用这些拷贝，
...
// 即使是const成员函数，它也可以访问mutable数据成员、
// 非cosnt对象、以及非const的全局对象。

...
// C++允许我们自定义类型转换规则，用户可以将其它类型转换为当前类类型，
// 也可以将当前类类型转换为其它类型。
...
class Complex {
 public:
  ...
  // 转换构造函数就是包含了一个参数的构造函数，
  // 恰好能够和其他两个普通的构造函数融合在一起。
...
void func1() {
  ...
  // 整个过程中，并没有告诉编译器如何去做，编译器使用内置的规则完成数据类型的转换。
  int a = 6;
  a = 7.5 + a;
  std::cout << a << std::endl;
...
void testN3() {
  ...
  //   可以直接赋值给f。
  // 很多人认为，既然=左边是double类型，很显然应该选择第二种方案，这样才符合常理。

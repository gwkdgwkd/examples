...
// 简单的理解，在申请堆内存空间的同时，会为其配备一个整形值（初值为1），每当有新对象使用此堆内存时，该整形值+1；
// 反之，每当使用此堆内存的对象被释放时，该整形值减1。
// 当堆空间对应的整形值为0时，即表明不再有对象使用它，该堆空间就会被释放掉。

// C++11 shared_ptr智能指针:
// 每种智能指针都是以类模板的方式实现的，shared_ptr也不例外。
// shared_ptr<T>（其中T表示指针指向的具体数据类型）的定义位于<memory>头文件，并位于std命名空间中。
// 值得一提的是，和unique_ptr、weak_ptr不同之处在于，多个shared_ptr智能指针可以共同使用同一块堆内存。
// 并且，由于该类型智能指针在实现上采用的是引用计数机制，即便有一个shared_ptr指针放弃了堆内存的“使用权”（引用计数减1），
// 也不会影响其他指向同一堆内存的shared_ptr指针（只有引用计数为0时，堆内存才会被自动释放）。
...
void func5() {
  ...
  // 对于std::move(p1)来说，该函数会强制将p1转换成对应的右值，因此初始化p2和p3调用的是移动构造函数。
...
namespace deletor {
// 在初始化shared_ptr智能指针时，还可以自定义所指堆内存的释放规则，引用计数为0时，优先调用自定义的释放规则。

...
//  stderr 	标准错误文件，一般指显示器；perror()等函数默认向stderr输出数据（后续会讲到）。
//  stdprn 	标准打印文件，一般指打印机。
// 标准输入文件stdin（表示键盘）、标准输出文件stdout（表示显示器）、标准错误文件stderr（表示显示器）是由系统打开的，可直接使用。
// 文件是数据源的一种，除了文件，还有数据库、网络、键盘等；数据传递到内存也就是保存到C语言的变量（例如整数、字符串、数组、缓冲区等）。
// 把数据在数据源和程序（内存）之间传递的过程叫做数据流(Data Stream)。
// 相应的，数据从数据源到程序（内存）的过程叫做输入流(Input Stream)，从程序（内存）到数据源的过程叫做输出流(Output Stream)。

// 使用<stdio.h>头文件中的fopen()函数即可打开文件，它的用法为：FILE *fopen(char *filename, char *mode);

// fopen()函数的返回值
// fopen()会获取文件信息，包括文件名、文件状态、当前读写位置等，并将这些信息保存到一个FILE类型的结构体变量中，然后将该变量的地址返回。
// 打开文件出错时，fopen()将返回一个空指针，也就是 NULL，我们可以利用这一点来判断文件是否打开成功：if((fp=fopen("D:\\demo.txt","rb") == NULL)
...
// 关闭文件
// 文件一旦使用完毕，应该用fclose()函数把文件关闭，以释放相关资源，避免数据丢失。fclose()的用法为：int fclose(FILE *fp);
...
// 如果新建一个mp4文件，给它写入一串字符，然后再用文本编辑器打开，你一样可以读得懂。
...
// C语言程序将\n作为换行符，类UNIX/Linux系统在处理文本文件时也将\n作为换行符，所以程序中的数据会原封不动地写入文本文件中，反之亦然。
// 但是Windows系统却不同，它将\r\n作为文本文件的换行符。
// 在Windows中，如果以文本方式打开文件，当读取文件时，程序会将文件中所有的\r\n转换成一个字符\n。也就是说，如果文本文件中有连续的两个字符是\r\n，则程序会丢弃前面的\r，只读入\n。
// 当写入文件时，程序会将\n转换成\r\n写入。也就是说，如果要写入的内容中有字符\n，则在写入该字符前，程序会自动先写入一个\r。
// 因此，如果用文本方式打开二进制文件进行读写，读写的内容就可能和文件的内容有出入。
// 总起来说，对于Windows平台，为了保险起见，最好用"t"来打开文本文件，用"b"来打开二进制文件。对于Linux平台，使用"r"还是"b"都无所谓，既然默认是"r"，那什么都不写就行了。

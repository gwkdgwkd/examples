...
// 二叉树是一种非线性结构，采用任何遍历二叉树的方法，都可以得到树中所有结点的一个线性序列。
// 在这个序列中，除第一个结点外，每个结点都有自己的直接前趋；
// 除最后一个结点外，每个结点都有一个直接后继。
// 如果算法中多次涉及到对二叉树的遍历，普通的二叉树就需要使用栈结构做重复性的操作。
// 线索二叉树不需要如此，在遍历的同时，
// 使用二叉树中空闲的内存空间记录某些结点的前趋和后继元素的位置（不是全部）。
// 这样在算法后期需要遍历二叉树时，就可以利用保存的结点信息，提高了遍历的效率。
// 使用这种方法构建的二叉树，即为线索二叉树。

// 如果在二叉树中想保存每个结点前趋和后继所在的位置信息，
// 最直接的想法就是改变结点的结构，即添加两个指针域，分别指向该结点的前趋和后继。
...
// 实际上就是两个布尔类型的变量：
// 1.LTag为0时，表示lchild指针域指向的是该结点的左孩子，为1时，表示指向的是该结点的直接前趋结点；
// 2.RTag为0时，表示rchild指针域指向的是该结点的右孩子，为1时，表示指向的是该结点的直接后继结点。
...
// 线索链表中的线索，指的是链表中指向结点前趋和后继的指针。
// 二叉树经过某种遍历方法转化为线索二叉树的过程称为线索化。

// 对二叉树进行线索化
// 将二叉树转化为线索二叉树，实质上是在遍历二叉树的过程中，
// 将二叉链表中的空指针改为指向直接前趋或者直接后继的线索。
// 线索化的过程即为在遍历的过程中修改空指针的过程，在遍历过程中，如果当前结点没有左孩子，
// 需要将该结点的lchild指针指向遍历过程中的前一个结点，所以在遍历过程中，
// 设置一个指针（名为pre），时刻指向当前访问结点的前一个结点。
// 中序对二叉树进行线索化
BiThrTree pre = NULL;
...
// 当在遍历过程，解决的办法是：寻找先序、中序、后序遍历的规律，找到下一个结点。
// 1.在先序遍历过程中，如果结点因为有右孩子导致无法找到其后继结点，
//   如果结点有左孩子，则后继结点是其左孩子，否则，就一定是右孩子；
...
//   c.如果该结点是父结点的左孩子，且父结点有右子树，
//     后继结点为父结点的右子树在后序遍历列出的第一个结点。
...
// 逆向遍历线索二叉树的过程即从头结点的右指针指向的结点出发，
// 逐个寻找直接前趋结点，结束标志同正向遍历一样：
// 中序逆方向遍历线索二叉树

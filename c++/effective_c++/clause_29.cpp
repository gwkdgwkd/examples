...
// 一般而言应该会提供可实现的最强烈保证。
// nothrow很好，但是很难在C part of C++领域中，
// 完全没有调用任何一个可能抛出异常的函数。
// 任何使用动态内存的东西（例如所有STL容器）如果无法找到足够内存，
// 通常会抛出bad_alloc异常。
// 可能的话请提供nothrow保证，但对大部分函数而言，
// 抉择往往落在基本保证和强烈保证之间。
...
class PrettyMenu {
 public:
  ...
  // 原则很简单：
  // 1.为打算修改的对象进行复制。
  // 2.在副本上做一切必要的修改，若有任何修改动作抛出异常，
  //   原对象仍保持未改变状态。
  // 3.待所有改变都成功后，
  //   再将修改过的那个副本和对象在一个不抛出异常的操作中交换。
  // 实现上通常是将所有隶属对象的数据从原对象放进另一个对象，
  // 然后赋予原对象一个指针，指向哪个所谓的实现对象，
  // 这种手法常别称为pimpl idiom(条款31)。
...
// 现实中会发现，可以提供强烈保证，
// 但效率和复杂度带来的成本会使你退而求其次，只提基本保证。
// 任何人都不该因此责怪你，对许多函数而言，
// 异常安全性的基本保证是一个绝对通情达理的选择。

// 如果具备异常安全性：
// 首先是以对象管理资源，可阻止资源泄露。
// 然后挑选三个异常安全保证中的某一个实施。
// 应该挑选现实可实现的最强烈等级，
// 只有当调用了传统代码，才别无选择地设为无任何保证。

// 请记住：
// 1.异常安全函数即使发生异常也不会泄露资源或允许任何数据结果被破坏。
// 2.这样的函数区分为三种可能的保证：基本型、强烈型、不抛异常型。
// 3.强烈保证往往能够以copy-and-swap实现出来，
//   但强烈保证并非对所有函数都可实现或具备现实意义。
// 4.函数提供的异常安全保证通常最高只等于，
//   其所调用的各个函数的异常安全保证中的最弱者。

...
//   这会影响到构造函数和析构函数以及内存分配函数和释放函数（operator new，operator new[]，
//   operator delete和operator delete[]）的设计。
...
// 4.什么是新type的”合法值“？
//   对class的成员变量而言，通常只有某些数值集是有效的，这些数值决定了必须维护的约束条件，
//   也就决定了成员函数（特别是构造函数、赋值操作符和所谓setter函数）必须进行的错误检查工作。
...
// 6.你的新type需要什么样的转换？
//   有很多类型，彼此之间该有转换行为么？如果希望允许类型T1能被隐式转换为类型T2，
//   就必须在T1内写一个类型转换函数或者在T2内写一个non-explicit-one-argument(可被单一实参调用)的构造函数。
...
//   也帮助决定哪个class或function应该是friend，以及将它们嵌套于另一个之内是否合理。
// 10.什么是新type的”未声明接口“？
...
// 然后如果能够设计出至少像C++内置类型一样好的用户自定义class，一切汗水便都值得。

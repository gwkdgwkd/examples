...
// ifstream和fstream的read()方法实际上继承自istream类，
// 其功能正好和write()方法相反，即从文件中读取count个字节的数据。
// 该方法的语法格式：istream& read(char* buffer, int count);
...
// 另外，在使用read()方法的同时，如果想知道一共成功读取了多少个字节，
// 可以在read()后调用文件流对象的gcount()成员方法，
// 其返回值就是最近一次read()方法成功读取的字节数。
// 读到文件尾时，未必能读取count个字节。

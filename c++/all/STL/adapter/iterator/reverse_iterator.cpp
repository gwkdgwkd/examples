...
// 常用来对容器进行反向遍历，即从容器的最后一个元素开始，一直遍历到第一个元素。
// 反向迭代器底层可以选用双向迭代器或者随机访问迭代器作为其基础迭代器。
// 不仅如此，通过对++（递增）和--（递减）运算符进行重载，使得：
// 1.当反向迭代器++时，底层的基础迭代器则执行--，意味着反向迭代器在反向遍历容器；
// 2.当反向迭代器--时，底层的基础迭代器则执行++，意味着反向迭代器在正向遍历容器。

// 反向迭代器的模板类定义如下：
// template <class Iterator> class reverse_iterator;
// Iterator模板参数指的是模板类中所用的基础迭代器，只能选择双向迭或随机迭代器。

...
//   当用户引用被智能指针所指的对象，会发生什么事情呢？
//   可以自行决定。
//   例如可以用智能指针实现条款M17提到的lazy fetching方法。

namespace n1 {
// 智能指针从模板中生成，因为要与内建指针类似，
...
void func1() {
  // 使用智能指针与使用dump pointer没有很大的差别，
  // 这表明了封装是非常有效的。
  // 智能指针的用户可以象使用dumb pointer一样使用智能指针，
  // 有时这种替代会更透明化。
...
void func2() {
  ...
  // 绕过了类指针（pointer-like）对象所提供的智能特性：
  int* p = sp1;
}
...
void func2() {
  ...
  // 这些类之间没有继承关系，
  // 不可能要求编译器把一种对象转换成（完全不同的）另一种类型的对象。
}
...
// 这种方法有两个缺点：
// 1.必须人为地特化（specialize）SmartPtr类，
//   所以加入隐式类型转换操作符也就破坏了模板的通用性。
...
//   它们不能把指向T的智能指针转换为指向cT的间接基类的智能指针，
//   除非只要一步就能完成。
}  // namespace test2
...
void func2() {
  ...
  // 因此show的调用具有二义性，
  // 因为从SmartPtr<DD>到SmartPtr<D1>的类型转换，
...
// 使用这种设计方法，
// 指向non-const-T对象的智能指针包含一个指向const-T的dumb指针，
...
// 无论如何你也不可能设计出一种通用目的的智能指针，能够替代dumb指针。
// 达到同样的代码效果，使用智能指针更方便。

...
// 句柄类，也就是handle class，是C++中的一种重要的面向对象方法，
// 其作用有两条：
...
// 由于系统存在诸多的包含依赖关系，
// 因此编译的过程中会先去检测外部包含文件是否改动，
// 而外部包含文件如果仍然存在外部包含，那编译器就会继续检测，
// 直到检测不到为止，这个过程往往非常的漫长，
// 而编译却是系统开发中最普遍的行为之一，
...
// 这么做就达到了增加编译效率的目的，
// 比如说三个类A、B、C，三个类存在这样的依赖关系：
// A包含B，B包含C，如果按照传统的方法，
// 如果C中改动了一个数据成员，那么A和B全部都需要重新编译，
// 那么编译的代价就是对A、B、C三个文件全部重新编译，
// 那么如果改用句柄类，那么C的改动，就引起不了A和B的重新编译，
// 因为A和B只包含了C的前置声明，而没有包含C的类定义，
// 因此重编译的代价仅仅是一个文件而已。

// 这种方式的另一个好处是实现了类数据成员和函数接口的完全分离，
// 增加了系统的安全性，很多人有个误解，
// 认为类的数据成员属于类声明的部分，而不是类实现，这是错的，
// 实际上一个数据成员建立，会在无形中调用构造方法、设置内存等等，
// 只不过这些东西都是编译器做的，程序员看不到而已。
// 在一些完全的面向对象语言里（比如JAVA），类的实现和类的声明是完全分离的，
// 虽然用户有时候会把类的数据成员和数据方法定义到一块，
// 但是编译器却并不会直接在类的主体中直接定义实体，
// 而仅仅是定义了一个地址而已，而在实现部分，才会分配实体的内存。
// 而在C++中如果想这么做，就需要句柄类的帮忙，
// 也就是把一个类拆分成两个类，一个负责数据成员，一个负责数据方法，
// 数据成员类只提供基本的数据和基本的数据操作函数，
// 而数据方法则提供真正的用户接口。

// 通过Person内含有的指针成员pimpl pointer to implementation，
// 完成接口（Person）与实现（PersonImpl）分离，
// 一个只负责提供接口，另一个负责实现该接口。
// PersonImpl的修改不需要Person重新编译。
// 这个分离的关键在于以声明的依存性替换定义的依存性。

// pImpl方法，它是微软的Herb Sutter提出来的，
// 该方法是为了尽量减小接口和实现之间的耦合，

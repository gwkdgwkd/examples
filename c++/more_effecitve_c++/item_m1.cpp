...
// 1.因此如果你使用一个变量并让它指向一个对象，但是该变量在某些时候也可能不指向任何对象，
// 2.这时你应该把变量声明为指针，因为这样你可以赋空值给该变量。
// 3.相反，如果变量肯定指向一个对象，例如你的设计不允许变量为空，这时你就可以把变量声明为引用。
...
// 总的来说，在以下情况下你应该使用指针：
// 1.你考虑到存在不指向任何对象的可能（在这种情况下，你能够设置指针为空）；
// 2.你需要能够在不同的时刻指向不同的对象（在这种情况下，你能改变指针的指向）。

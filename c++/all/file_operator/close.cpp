...
// 那么，调用close()方法关闭已打开的文件，
// 就可以理解为是切断文件流对象和文件之间的关联。
// 注意，close()方法的功能仅是切断文件流与文件之间的关联，
// 该文件流并未被销毁，其后续还可用于关联其它的文件。
// close()方法的用法很简单，其语法格式如下：
// void close();
// 该方法既不需要传递任何参数，也没有返回值。
// 即便不调用close()方法，也能成功向文件中写入字符串。
// 因为当文件流对象的生命周期结束时，会自动调用其析构函数，该函数在销毁对象之前，
// 会先调用close()方法切断它与任何文件的关联，最后才销毁它。
// 强烈建议使用open()方法打开的文件，一定要手动调用close()方法关闭，
// 这样可以避免程序发生一些奇葩的错误！
...
// 既然文件流对象自行销毁时会隐式调用close()方法，
// 是不是就不用显式调用close()方法了呢？当然不是。
// 在实际进行文件操作的过程中，对于打开的文件，
// 要及时调用close()方法将其关闭，否则很可能会导致读写文件失败。
// 对于已经打开的文件，如果不及时关闭，一旦程序出现异常，
// 则很可能会导致之前读写文件的所有操作失效。

// 在很多实际场景中，即便已经对文件执行了写操作，但后续还可能会执行其他的写操作。
// 对于这种情况，可能并不想频繁地打开/关闭文件，
// 可以使用flush()方法及时刷新输出流缓冲区，也能起到防止写入文件失败的作用。
// 总之，C++中使用open()打开的文件，
// 在读写操作执行完毕后，应及时调用close()方法关闭文件，

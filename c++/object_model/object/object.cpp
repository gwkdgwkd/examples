...
// 在C语音中，“数据”和“处理数据的操作（函数）”是分开声明的，
// 也就是说，语言本身并没有支持“数据和函数”之间的关联性。
// 把这种程序方法称为程序性的，有一组“分布在各个以功能为导向的函数中”的算法所驱动，
// 它们处理的是共同的外部数据。
// 从软件工程的眼光来看，一个ADT或class hierarchy的数据封装比在C程序中程序性地使用全局数据好。
...
// 4.至于每一个拥有零个或一个定义的inline function则会在其每一个使用者（模块）身上产生一个函数实体。
// 封装性并未带给程序任何空间或执行期的不良反应。
// C++在布局以及存取时间上主要的额外负担是由virtual引起，包括：
// 1.virtual function机制，用以支持一个有效率的执行期绑定。
// 2.virtual base class，用以实现多次出现在继承体系中的base class，有一个单一而被共享的实体。
...
//   b.每一个对象被添加一个指针，指向相关的virtual table，通常这个指针被称为vptr。
//     vptr的设置和重置都由每一个class的构造、析构和拷贝运算符自动完成。
//     每一个class所关联的type_info object（用以支持RTTI）也经由virtual table被指出来，
//     通常放在表格的第一个slot处。
...
//   int (*pq)();  // 一个声明
// 3.如果C++并不需要支持C原有的struct，那么class的观念由关键词class来支持。
...
// C struct在C++中的一个合理用途，是当要传递一个复杂的对象的全部或部分到某个C函数中时，
// struct声明可以将数据封装起来，并保证拥有与C兼容的空间布局，这项保证只在组合的情况下存在。
// 如果是继承而不是组合，编译器会决定是否应该有额外的数据成员被安插到base struct subobject中。
...
// 2.抽象数据类型模型（ADT），该模型所谓“抽象”是和一组表达式（public接口）一起提供，
//   而其运算定义仍然隐而未明。
...
// 指向不同类型的指针间的差异，既不在其指针表示法不同，也不在其内容（地址）不同，
// 而是在其所寻址出来的object类型不同。
// 也就是说，指针类型会教导编译器如何解释某个特定地址中的内存内容及其大小。
// 转型（cast）其实是一种编译器指令。
// 大部分情况下它并不改变一个指针所含的真正地址，它只影响被指出的内存的大小和其内容的解释方式。
// 一个指针或一个引用之所以支持多态，是因为它们并不引发内存中任何与类型有关的内存委托操作。
...
// 当一个基类对象被直接初始化为一个派生类对象时，就会别切割，以塞入较小的base type内存中，
...
// 2.空间紧凑则是因为每一个对象不需要负担传统上为了支持virtual机制而需要的额外负荷；

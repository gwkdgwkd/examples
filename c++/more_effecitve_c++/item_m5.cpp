...
// C++编译器能够在两种数据类型之间进行隐式转换（implicit conversions），它继承了C语言的转换方法，例如允许把char隐式转换为int和从short隐式转换为double。
// 因此当你把一个short值传递给准备接受double参数值的函数时，依然可以成功运行。
// C中许多这种可怕的转换可能会导致数据的丢失，它们在C++中依然存在，包括int到short的转换和double到char的转换。
...
// 当你在不需要使用转换函数时，这些的函数缺却会被调用运行。结果，这些不正确的程序会做出一些令人恼火的事情，而你又很难判断出原因。
...
class Rational1 {
 public:
  Rational1(int numerator = 0, int denominator = 1) {}  // 转换int到有理数类
  operator double() const {}  // 转换Rational类成double类型
};

// 解决方法是用不使用语法关键字的等同的函数来替代转换运算符。
// 例如为了把Rational对象转换为double，用asDouble函数代替operator double函数：
class Rational2 {
 public:
  Rational2(int numerator = 0, int denominator = 1) {}  // 转换int到有理数类
  double asDouble() const {}
};
// 在多数情况下，这种显式转换函数的使用虽然不方便，但是函数被悄悄调用的情况不再会发生，这点损失是值得的。
// 一般来说，越有经验的C++程序员就越喜欢避开类型转换运算符。
// 库函数中的string类型没有包括隐式地从string转换成C风格的char*的功能，而是定义了一个成员函数c_str用来完成这个转换。

// 通过单参数构造函数进行隐式类型转换更难消除。而且在很多情况下这些函数所导致的问题要甚于隐式类型转换运算符。
template <class T>
class Array {
 public:
  Array(int lowBound, int highBound);  // 不能做为类型转换函数
...
// 想给调用者提供一个单参数构造函数。同时你也希望防止编译器不加鉴别地调用这个构造函数。
// 有两个方法：一是容易的方法，二是当你的编译器不支持容易的方法时所必须使用的方法。

// 容易的方法是利用一个最新编译器的特性，explicit关键字。为了解决隐式类型转换而特别引入的这个特性，它的使用方法很好理解。
...
int main() {
  ...
  // cout << r;  // 打印操作将失败，因为没有合适的的 operator<<被调用
  Rational1 r1(1, 2);
  // 当编译器调用operator<<时，会发现没有这样的函数存在，但是它会试图找到一个合适的隐式类型转换顺序以使得函数调用正常运行。
  // 类型转换顺序的规则定义是复杂的，但是在现在这种情况下，编译器会发现它们能调用Rational::operator double函数来把r转换为double类型。
  cout << r1;  // 打印的结果是一个浮点数，而不是一个有理数。隐式类型转换的缺点。
  ...
  Array1<int> a(10);  // 正确, explicit构造函数在建立对象时能正常使用
  Array1<int> b(10);  // 也正确
  for (int i = 0; i < 10; ++i) {
  ...
  // 编译器意识到它能从int参数转换成一个临时ArraySize对象，ArraySize对象只是Array<int>构造函数所需要的，这样编译器进行了转换。
  Array2<int> a2(10);
  // 事实上你仍旧能够安心地构造Array对象，不过这样做能够使你避免类型转换:
  for (int i = 0; i < 10; ++i) {
    // 为了调用operator＝＝函数，编译器要求Array<int>对象在”==”右侧,但是不存在一个参数为int的单参数构造函数。
    // 而且编译器无法把int转换成一个临时ArraySize对象然后通过这个临时对象建立必须的Array<int>对象，因为这将调用两个用户定义（user-defined）的类型转换，
    // 一个从int到ArraySize，一个从ArraySize到Array<int>。这种转换顺序被禁止的，所以当试图进行比较时编译器肯定会产生错误。
    // if (a2 == b2[i]) {}  // 现在是一个错误
  }

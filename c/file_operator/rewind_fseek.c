...
// rewind()用来将位置指针移动到文件开头，它的原型为：
// void rewind(FILE *fp);
// fseek()用来将位置指针移动到任意位置，它的原型为：
// int fseek(FILE *fp, long offset, int origin);
// 参数说明：
// 1.fp为文件指针，也就是被移动的文件。
// 2.offset为偏移量，也就是要移动的字节数。
//   之所以为long类型，是希望移动的范围更大，能处理的文件更大。
//   offset为正时，向后移动；
//   offset为负时，向前移动。
...
//   当前位置 SEEK_CUR 1
//   文件末尾 SEEK_END 2
// 值得说明的是，fseek()一般用于二进制文件，
// 在文本文件中由于要进行转换，计算的位置有时会出错。
// 在移动位置指针之后，就可以用任何一种读写函数进行读写了。
// 由于是二进制文件，因此常用fread()和fwrite()读写。

...
// 从C++11开始，C++从语言层面开始支持并行编程，
// 内容包括了管理线程、保护共享数据、线程间的同步操作、低级原子操作等各种类。
...
void testOperation() {
  ...
  std::atomic_int ai3;
  ai3 = 10;
  // 不支持拷贝构造，因为原子读和原子写都是独立原子操作，无法保证2个独立的操作加在一起仍然保证原子性：
  // ai3 = ai1;
...
// load函数可以使用memory_order_seq_cst、memory_order_acquire、

...
// lambda源自希腊字母表中第11位的λ，在计算机领域，它被用来表示匿名函数。
// 简单地理解就是没有名称的函数，又常被称为lambda函数或者lambda表达式。
// 继Python、Java、C#、PHP等众多高级语言都支持后，C++11终于引入了lambda。
...
//   在方括号内部，可以注明当前lambda函数的函数体中可以使用哪些外部变量。
...
//   默认情况下，对于以值传递方式引入的外部变量，不允许在lambda表达式内部修改，
//   可以理解为这部分变量都是const常量，而如果想修改，就必须使用mutable关键字。
//   注意，对于以值传递方式引入的外部变量，是拷贝，并不会修改真正的外部变量。
// 4.noexcept/throw()
//   可以省略，如果使用，在之前的()小括号将不能省略，参数个数可以为0，默认情况下，
//   lambda函数的函数体中可以抛出任何类型的异常，而标注noexcept关键字，
//   则表示函数体内不会抛出任何异常，使用throw()可以指定lambda函数内部可以抛出的异常类型。
//   如果lambda函数标有noexcept而函数体内抛出了异常，又或者使用throw()限定了异常类型，
//   而函数体内抛出了非指定类型的异常，这些异常无法使用try-catch捕获，会导致程序执行失败。
// 5.->返回值类型
//   指明lambda匿名函数的返回值类型，如果lambda函数体内只有一个return语句，
//   或者该函数返回void，则编译器可以自行推断出返回值类型，此情况下可以直接省略->返回值类型。
//   如果使用，在之前的()小括号将不能省略，参数个数可以为0。
// 6.函数体
//   和普通函数一样，lambda匿名函数包含的内部代码都放置在函数体中。
//   该函数体内除了可以使用指定传递进来的参数之外，
//   还可以使用指定的外部变量以及全局范围内的所有全局变量。
//   外部变量会受到以值传递还是以引用传递方式引入的影响，而全局变量则不会。
//   换句话说，在lambda表达式内可以使用任意一个全局变量，还可以直接修改它们。

// 最简单的lambda匿名函数：[]{}
// 显然，此lambda匿名函数未引入任何外部变量，也没有传递任何参数，
// 没有指定mutable、noexcept等关键字，没有返回值和函数体。
// 所以，这是一个没有任何功能的lambda匿名函数。

namespace n1 {
// lambda匿名函数中的[外部变量]是指：和当前lambda表达式位于同一作用域内的所有局部变量。
...
// [val1,val2,...]，表示以传值的方式导入val1、val2，多个变量间没有先后次序：
void func4() {
  int i = 4;
  float f = 4.4;
  std::string s = "func4";
...
// [&val1,&val2,...]，表示以引用的方式导入val1、val2，多个变量间没有先后次序：
void func5() {
  int i = 4;
  float f = 5.5;
  std::string s = "func5";
...
// [=,&val1,...]，表示除val1以引用传递的方式导入外，其它外部变量都以传值方式导入：
void func7() {
  int i = 6;
  float f = 7.7;
  std::string s = "func7";
...
// 注意，单个外部变量不允许以相同的传递方式导入多次，这是非法的：
void func8() {
  int i = 8;
  // [=, i] {}();
  // [&, &i] {}();
...
void func2() {
  // 虽然lambda匿名函数没有函数名称，但仍可以为其手动设置一个名称，比如：
  auto display1 = [](int a, int b) -> void {
    std::cout << a << " " << b << std::endl;
  };
  // 使用auto为lambda匿名函数设定了一个函数名，由此即可在作用域内调用该函数：
...
namespace n3 {
void func1() {
  // 如果不使用noexcept或者throw()，则lambda函数体中允许发生任何类型的异常。
  try {
    []() { throw 10; }();
  } catch (int i) {
...
void func2() {  // 反例1
  // except匿名函数指定了函数体中不发生任何异常，但却发生了整形异常：
...
void func3() {  // 反例2
  // except匿名函数指定函数体可能发生字符异常，但却发生了整形异常：
  ...
  // 由于指定异常类型和真正发生的异常类型不匹配，无法捕获，最终程序运行崩溃。
}
}  // namespace n3

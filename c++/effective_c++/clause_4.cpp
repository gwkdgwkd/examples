...
ABEntry::ABEntry(const string& name, const string& address,
                 const list<PhoneNumber>& phones) {
  // 下面都不是被初始化，而是被赋值。
  // 初始化发生时间更早，这些成员default构造函数被自动调用之时（比进入ABEntry构造函数本体的时间更早）。
  theName = name;
  theAddress = address;
  thePhones = phones;
...
// 想要default构造一个成员变量，也可以使用初始化列表，指定(nothing)作为初始化实参即可。
ABEntry::ABEntry(const string& name, const string& address,
                 const list<PhoneNumber>& phones)
...
// 有多个构造函数，会导致重复代码，可以在初始化列表中遗漏那些（赋值和初始化一样好）的成员变量，改用赋值操作，并将赋值放到一个函数内（private）。

// C++有着十分固定的“成员初始化次序”：
//  base class更早于derived class被初始化；
//  class的成员变量总是以其声明次序被初始化。即使在成员初始化列表中以不同的次序（合法的），也不会有任何影响。所以初始化列表最好和声明顺序一致。

// 不同编译单元内定义的non-local static对象的初始化次序：
//  static对象，从被构造出来直到程序结束为止，因此stack和heap对象都被排除。包括global对象、定义namespace内的对象、class内、函数内以及文件作用
//  内被声明为static的对象。函数内的static对象被称为local static对象，其他static对象称为non-local static对象。
//  程序结束时static对象会被自动销毁，析构函数会在main()结束时被自动调用。
//  所谓编译单元是指产出单一目标文件的源码。基本上是单一源码文件加上包含的头文件。
// 如果某个编译单元内的某个non-local static对象的初始化使用了另一个编译单元内的某个non-local static对象，用到的对象可能尚未被初始化。
// 因为C++对“定义于不同编译单元内的non-local static对象”的初始化次序并无明确定义。因为决定次序非常困难，根本无解。
// 消除这个问题需要做的是，将每个non-local static对象搬到自己的专属函数内（在函数内被声明为static），这些函数返回一个引用指向所包含的对象。
// 然后用户调用这些函数，而不直接接触这些对象。换句话说，non-local static对象被local static对象替换了（单例模式常见的实现手法）。
// C++保证，函数内的local static对象会在“该函数被调用期间”“首次遇上该对象的定义式”时被初始化。
// 使用函数调用代替访问non-local static对象，就可以保证获取已经初始化的对象。
// 更棒的是，如果从未调用non-local static对象的“仿真函数”，就绝不会引发构造和析构成本。真正的non-local static对象可没这等便宜。

// 请记住：
// 为内置类型对象进行手工初始化，因为C++不保证初始化它们。
// 构造函数最好使用成员初始化列表，而不要在构造函数本体内使用赋值操作。初始化列表的成员变量，其排列次序应该和在class中的声明次序相同。
// 为免除“跨编译单元的初始化次序”问题，应该用local static对象替换non-local static对象。

...
//   它不会调用构造函数。
// 3.如果你想定制自己的在堆对象被建立时的内存分配过程，
//   你应该写你自己的operator new函数，
//   然后使用new操作符，new操作符会调用你定制的operator new。
// 4.如果你想在一块已经获得指针的内存里建立一个对象，
//   应该用placement new。
...
// 如果你用placement new在内存中建立对象，
// 你应该避免在该内存中用delete操作符。
...
// 这就允许你控制数组的内存分配，就象你能控制单个对象内存分配一样。
// 对于数组，在数组里的每一个对象的构造函数都必须被调用。
// 同样当delete操作符用于数组时，它为每个数组元素调用析构函数，
// 然后调用operator delete来释放内存。
// 就象你能替换或重载operator delete一样，
// 你也替换或重载operator delete[]。

// new和delete操作符是内置的，其行为不受你的控制，
// 凡是它们调用的内存分配和释放函数则可以控制。
// 当你想定制new和delete操作符的行为时，请记住你不能真的做到这一点。
// 你只能改变它们为完成它们的功能所采取的方法，
// 而它们所完成的功能则被语言固定下来，不能改变。
...
int main() {
  ...
  // 一般不会直接调用operator new，
  // 但是一旦这么做，就可以象调用其它函数一样调用它：
  void *rawMemory = operator new(sizeof(string));
  // 操作符operator new将返回一个指针，
  // 指向一块足够容纳一个string类型对象的内存。
  ...
  // 分配内存，没有调用构造函数：
  A *buffer = (A *)operator new(50 * sizeof(A));
  // 释放内存,没有调用析构函数：
  ...
  // 结果不确定! 共享内存来自mallocShared, 而不是operator new：
  delete pw;
  // 正确。析构pw指向的Widget，但是没有释放包含Widget的内存：
  pw->~Widget();
  // 正确。释放pw指向的共享内存但是没有调用析构函数：
  freeShared(pw);

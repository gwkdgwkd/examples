...
// unordered_set可直译为无序set，即unordered_set和set很像，
// 唯一的区别就在于set会自行对存储的数据进行排序，而unordered_set不会。
// 总的来说，unordered_set容器具有以下几个特性：
// 1.不再以键值对的形式存储数据，而是直接存储数据的值；
// 2.容器内部存储的各个元素的值都互不相等，且不能被修改。
// 3.不会对内部存储的数据进行排序
// 对于unordered_set不以键值对的形式存储数据，即存储的都是键和值相等的键值对，
// 为了节省存储空间，该类容器在实际存储时选择只存储每个键值对的值。

// 实现unordered_set的模板类定义在<unordered_set>，并位于std命名空间中。
...
// > class unordered_set;
// 以上4个参数中，只有第一个参数没有默认值，至少需要手动传递1个参数。
// 在99%的实际场景中最多只需要使用前3个参数，最后一个参数保持默认值即可。
// Key 确定容器存储元素的类型，用于确定各个键值对的键和值的类型。
...
// cbegin()	          和begin()功能相同，只不过返回的是const类型正向迭代器。
// cend()	            和end()功能相同，只不过返回的是const类型正向迭代器。
// empty()            若容器为空，则返回true，否则false。
// size()             返回当前容器中存有元素的个数。
// max_size()         返回容器所能容纳元素的最大个数，不同的系统返回值不相同。
// find(key)	        查找以值为key的元素，如果找到则返回该元素的正向迭代器；
...
// swap()	            交换2个unordered_map的元素，容器类型必须完全相等。
// bucket_count()     返回当前容器底层存储元素时，使用桶的数量。
// max_bucket_count() 返回当前系统中，unordered_map底层最多可以使用多少桶。
// bucket_size(n)     返回第n个桶中存储元素的数量。
// bucket(key)        返回值为key的元素所在桶的编号。
// load_factor()      返回unordered_map容器中当前的负载因子，负载因子是：
...
void func1() {
  ...
  // 1.过调用unordered_set模板类的默认构造函数，可以创建空的unordered_set，
  ...
  // C++11还向unordered_set模板类增加了移动构造函数，即以右值引用的方式，

...
// 区间成员函数是指这样一类成员函数，像STL算法一样，
// 使用两个迭代器参数来确定该成员操作执行的区间。
class Widget {};
// assign是一个使用及其方便，却被许多程序员所忽略的成员函数。
// 几乎所有通过利用插入迭代器的方式来限定目标区间的copy调用，
// 比如inserter、back_inserter或front_inserter，
// 其实都可以（也应该）被替换为对区间成员函数的调用。

// 通过使用区间成员函数，通常可以少写一些代码。
// 使用区间成员函数通常会得到意图清晰和更加直接的代码。

// 对于标准序列容器，为了同样的结果，使用单元素的成员函数比使用区间成员函数比，
// 需要更多地调用内存分配子，更频繁地拷贝对象，而且或者做冗余操作。
// 相对于区间版本的insert，单元素版本的insert总共在三个方面影响效率：
// 1.不必要的函数调用，调用了多次insert函数，使用区间的成员函数，
//   只调用了一次，使用内联可以避免，但实际实现中不一定是内联；
// 2.元素移动的多，每插入一次都要移动后面所以的元素，使用区间的成员函数，
//   只需要每元素只需要移动一次，移到最终的位置；
// 3.使用单元素成员函数，可能会导致多次分配内存。
// 这些问题对string、deque同样适用。
// list使用区间成员函数也有效率的优势，重复调用的论断继续生效。
// 还有新问题，对某些节点的next和prev指针的重复的、多余的赋值操作。

// 对于标准序列容器，在单元素的插入和区间的插入之间做选择，
// 所依据的不只是程序风格的考虑，对于关联容器，效率问题更加难以说清楚。

// 哪些函数支持区间？
// 1.区间创建，所有标准容器都提供了下面的构造函数：
//   container::container(InputIterator Begin,InputIterator end);
// 2.区间插入，所有标准序列容器都提供了如下形式的inert：
//   void container::insert(iterator position,InputIterator Begin,
//                          InputIterator end);
...
int main() {
  ...
  // 把循环替换为copy，效率和显示循环几乎是相同的，只是写的代码少了：

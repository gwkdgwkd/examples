...
// 对于set和multiset，如果直接对容器中的元素做了修改，
// 那么要保证该容器仍然是排序的。
...
int main() {
  ...
  // 下面的方法对set和multiset没有任何问题，
  // 但对于map和multimap，情况就复杂了。
  // map和multimap中保存的是pair，pair的第一个部分被定义成const，
  // 如果试图强制转换掉const属性，结果将可能会改变键部分。
  // 理论上，一个STL实现可以把这样的值写在一个只读的内存区域，
  // 这时要是尝试修改，最好的结果将是没有效果。
  // 如果你坚持要遵从C++标准所制定的规则，
  // 那就永远都不要试图修改在map和multimap中作为键的对象。

  // 但对set和multiset是可能的。
  // 对于set和multiset类型的对象，容器中元素类型是T，而不是const T。
  ...
  if (i != se.end()) {
    ...
    // 以一种总是可行且安全的方式修改set、multiset、
    // map和multimap中的元素，可以分为5个简单步骤：
    // 1.找到想修改的容器的元素。
    // 2.为将要被修改的元素做一份拷贝。
    //   在map和multimap的情况下，
    ...
    se.erase(i++);
    // 5.把新值插入到容器中。
    //   按照容器的排序顺序，新元素的位置可能与被删除元素的位置相同或紧邻，
    //   则使用提示形式的insert，以便把插入效率从对数时间提高到常数时间。
    //   把你从第一步得来的迭代器作为提示信息。
    se.insert(i, e);
  }

...
// 只有当字符串被修改的时候才创建各自的拷贝，这种实现方式称为写时复制策略。
...
void func2() {
  ...
  // 并返回该字符串的const char*指针，在C中，使用strlen函数获取字符串的长度：
  ...
  // string的末尾没有'\0'，所以length()返回的是字符串的真实长度，而不是长度+1。
}
...
namespace n3 {
void func() {
  // 使用string.find()方法查找字符串，如果找到，
  // 则返回该子字符串首次出现时其首字符的索引，否则，返回string::npos。

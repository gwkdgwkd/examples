...
// 不过可以明确地说：
// 学了C语言就相当于学了C++的一半，从C语言转向C++时，不需要再从头开始，
// 接着C语言往下学就可以，所以强烈建议先学C语言再学C++。

// C++和C语言虽然是两门独立的语言，但是它们却有着扯也扯不清的关系。
// 早期并没有C++这个名字，而是叫做带类的C。
// 带类的C是作为C语言的一个扩展和补充出现的，
...
// Linux下的GCC编译器、Mac下的Clang编译器（Xcode默认编译器，雄心勃勃，立志超越GCC），
...
// 在C++中，通过类名就可以创建对象，这个过程叫做类的实例化，
// 因此也称对象是类的一个实例（Instance）。
...
// 所以使用这些语言编写程序也被称为面向对象编程，这些语言也被称为面向对象的编程语言。
// C语言因为不支持类和对象的概念，被称为面向过程的编程语言。
// 在C语言中，我们会把重复使用或具有某项功能的代码封装成一个函数，
// 将拥有相关功能的多个函数放在一个源文件，再提供一个对应的头文件，这就是一个模块。
// 使用模块时，引入对应的头文件就可以。
// 而在C++中，多了一层封装，就是类（Class）。
// 类由一组相关联的函数、变量组成，你可以将一个类或多个类放在一个源文件，
// 使用时引入对应的类就可以。
// 不要小看类（Class）这一层封装，它有很多特性，
// 极大地方便了中大型程序的开发，它让C++成为面向对象的语言。
// 面向对象编程在代码执行效率上绝对没有任何优势，
// 它的主要目的是方便程序员组织和管理代码，快速梳理编程思路，带来编程思想上的革新。
// 面向对象编程是针对开发中大规模的程序而提出来的，目的是提高软件开发的效率。
// 不要把面向对象和面向过程对立起来，面向对象和面向过程不是矛盾的，
// 而是各有用途、互为补充的。
// 如果你希望开发一个贪吃蛇游戏，类和对象或许是多余的，几个函数就可以搞定；
// 但如果开发一款大型游戏，那你绝对离不开面向对象。

// C++和C语言类似，也要经过编译和链接后才能运行。
// VS、Dev C++、VC 6.0、Code::Blocks、C-Free、GCC、Xcode等常见IDE或编译器，
// 它们可以运行C程序和C++程序。
// C/C++代码生成可执行文件的过程：
...
// GCC中还有一个g++命令，它专门用来编译C++程序，广大C++开发人员也都使用这个命令。
// g++命令和gcc命令的用法如出一辙：
//  g++ main.cpp
...
// C++不但在C语言的基础上进行了很多扩展，而且也对C语言部分做了细节上的改进，
// 变量的定义位置就是其中之一。

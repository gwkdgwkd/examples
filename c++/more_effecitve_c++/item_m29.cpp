...
// String的拷贝构造函数很高效，
// 新生成的String对象与被拷贝的对象共享相同的StringValue对象：
#ifdef SHAREABLE
...
// 只要引用计数值不是0，也就是至少有一个String对象使用这个值，
// 这个值就不可以被销毁。
...
// 这个函数实现了C++传统意义上的下标索引，根本不会说不。
// 如果你想加上参数检查，这是非常容易的。
const char& String::operator[](int index) const { return value->data[index]; }
// 当我们修改一个String对象的值时，
// 必须小心防止修改了与它共享相同StringValue对象的其它String对象的值。
// 不幸的是，C++编译器没有办法告诉我们operator[]是用作读的还是写的，
// 所以我们必须保守地假设所有调用非const operator[]的行为都是为了写操作。
// 为了安全地实现非const的operator[]，
// 我们必须确保没有其它String对象在共享这个可能被修改的StringValue对象。
// 简而言之，当我们返回StringValue对象中的一个字符的引用时，
...
// 在计算机科学中已经有了悠久而著名的历史了，
// 尤其是在操作系统中：进程共享内存页直到它们想在自己的页拷贝中修改数据为止。
// 这个技巧如此常用，以至于有一个名字：写时拷贝，
// 它是提高效率的一个更通用方法Lazy原则的特例。
// 大部分情况下，写时拷贝可以同时保证效率和正确性，
// 只有一个挥之不去的问题，见代码。
// 至少有三种方法来应付这个问题:
// 1.忽略它，假装它不存在。
//   这是实现带引用计数的String类的类库中令人痛苦的常见问题。
// 2.稍微好些的方法是明确说明它的存在。
//   通常是将它写入文档，或多或少地说明别这么做。
// 3.排除这个问题。
//   它不难实现，但它将降低一个值共享于对象间的次数。
//   它的本质是这样的：
//   在每个StringValue对象中增加一个标志以指出它是否为可共享的。
//   在最初（对象可共享时）将标志打开，
//   在非const的operator[]被调用时将它关闭。
...
// 这样就它允许了：
// string的可共享标志在调用可能修改string的函数时被重设为true。

// 创建一个带引用计数的String类并不困难，但需要注意一些细节。
// 认识到我们需要一个地方来存储这个计数值是很重要的。
// 这个地方不能在String对象内部，
// 因为需要的是每个String值一个引用计数值，
// 而不是每个String对象一个引用计数。
// 这意味着String值和引用计数间是一一对应的关系，
// 所以我们将创建一个类来保存引用计数及其跟踪的值。
// 我们叫这个类StringValue，又因为它唯一的用处就是帮助我们实现String类，
// 所以我们将它嵌套在String类的私有区内。
// 另外，为了便于Sting的所有成员函数读取其数据区，
// 我们将StringValue声明为struct。
// 需要知道的是：将一个struct内嵌在类的私有区内，
// 能便于这个类的所有成员访问这个结构，
...
// 这是因为我们正在构造新的值对象，
// 而这个新的值对象总是未被共享的，只被它的构造者引用。
// 再一次，构造者负责将refCount设为正确的值。
RCObject::RCObject(const RCObject&) : refCount(0), shareable(true) {}
// 赋值运算看起来完全出乎意料：
// 它没有做任何事情，这个函数不太可能被调用的。
// RCObject是基于引用计数来共享的值对象的基类，
// 它不该被从一个赋给另外一个，
...
void RCObject::removeReference() {
  // 只当我们知道*this是一个堆对象时才安全。
  // 要让这个类正确，必须确保RCObject只能被构建在堆中。
  // 实现这一点的常用方法见Item M27，但我们这次采用一个特别的方法。
  if (--refCount == 0) delete this;
  // 我们用习惯方法来解决。
  // RCObject被设计为只作被引用计数的值对象的基类使用，
  // 而这些值对象应该只通过灵巧指针RCPtr引用。
  // 此外，值对象应该只能由值会共享的对象来实例化；
  // 它们不能被按通常的方法使用。
  // 在例子中，值对象的类是StringValue，
  // 我们通过将它声明为String的私有而限制其使用。
  // 只有String可以创建StringValue对象，
  // 所以String类的作者应该确保这些值对象都是通过new操作产成的。
...
// 这个版本的StringValue和前面的几乎一样，
// 唯一改变的就是StringValue的成员函数不再处理refCount字段。
// RCObject现在接管了这个工作。

// 自动的引用计数处理
// RCObject类给了我们一个存储引用计数的地方，
// 并提供了成员函数供我们操作引用计数，
// 但调用这些函数的动作还必须被手工加入其它类中。
// 仍要在String的拷贝构造函数和赋值操作符时，
// 调用StringValue的addReference和removeReference函数，这很笨拙。
// 我们想将这些调用也移入一个可重用的类中，
// 以使得String这样的类的作者不用再担心引用计数的任何细节。
template <class T>
class RCPtr {
  // 供引用计数对象使用的灵巧指针模板
 public:
...
class String2 {
  ...
  // 因为编译器为String自动生成的拷贝构造函数，
  // 将自动调用其RCPtr成员的拷贝构造函数，
...
// 1.代码有很多的减少，
//   因为RCPtr完成了大量以前在String内部完成的处理引用计数的担子。
// 2.剩下的代码几乎没有变化：灵巧指针无缝替换了内建指针。
//   实际上，唯一的变化是在operator[]里，
//   我们用调用isShared函数代替了直接检查refCount的值，
//   并用灵巧指针RCPtr对象消除了写时拷贝时手工维护引用计数值的工作。
// 这当然全都很漂亮。谁能反对减少代码？谁能反对成功的封装？
// 然而，这个全新的String类本身对用户的冲击远胜过它的实现细节，
// 这才是真正的闪光点。
// 如果没有什么消息是好消息的话，这本身就是最好的消息。
// String的接口没有改变！我们增加了引用计数，
// 我们增加了标记某个String的值为不可共享的能力，
// 我们将引用计数功能移入一个新类，我们增加了灵巧指针来自动处理引用计数，
// 但用户的一行代码都不需要修改。
// 当然，我们改变了String类的定义，所以用户需要重新编译和链接，
// 但他们在自己代码上的投资受到了完全的保护。
// 你看到了吗？封装确实是个很好的东西。
...
// 然后用等价的灵巧指针RCIPter模板替代RCPtr模板，
// 它知道CountHolder类的存在。
// 在这种情况下，我们需要增加一个类RCWidget以供用户使用，
// 而所有的事情都和String/StringValue的例子一样，
...
// RCIPtr与RCPtr只两处不同:
// 1.RCPtr对象直接指向值对象，
//   而RCIptr对象通过中间层的CountHolder对象指向值对象。
// 2.RCIPtr重载了operator->和operator*，
//   当有对被指向的对象的非const的操作时，写时拷贝自动被执行。
class Widget {
 public:
  Widget(int size);
...
//   即使这样，如果不是多个对象共享相同的值，
//   引用计数仍然帮不了你任何东西。
// 只有一个方法来确认这些条件是否满足，而这个方法不是猜测或依赖直觉，
// 这个方法是使用profiler或其它工具来分析。
// 使用这种方法，你可以发现是否创建和销毁值的行为是性能瓶颈，
// 并能得出对象/值的比例。
// 只有当你手里有了这些数据，你才能得出是否从引用计数上得到的好处超过其缺点。
// 即使上面的条件满足了，使用引用计数仍然可能是不合适的。
// 有些数据结构（如有向图）将导致自我引用或环状结构。
// 这样的数据结构可能导致孤立的自引用对象，
// 它没有被别人使用，而其引用计数又绝不会降到零。
...
int main() {
  ...
  *p = 'x';           // 有不受欢迎的结果：修改了s3和s4
  ...
  // 并且，这个问题不局限于指针，它同样存在于：
  // 保存了一个String的非const operator[]的返回值的引用的情况下：
  char& c1 = p[1];
  cout << c1 << endl;  // l
  p[1] = 'a';
  cout << c1 << endl;  // a

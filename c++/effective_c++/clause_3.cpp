...
// 只要某值保持不变是事实，就应该说出来，因为说出来可以获得编译器的帮助，确保这条约束不被违反。
// const多才多艺，可以：
//  在class外部修饰全局或命名空间中的常量，或修饰文件、函数、或区块作用域中被声明为static的对象；
//  修饰class内部的static和non-static成员变量；
//  面对指针，也可以指出指针本身、指针所指物，或两者都（不）是const。
char greeting[] = "Hello";
char* p1 = greeting;
...
// const最具威力的用法是面对函数声明的应用。在一个函数内，const可以和函数返回值、各参数、函数自身（成员函数）产生关联。
// 令函数返回一个常量值，往往可以降低因客户错误而造成的意外，而不至于放弃安全性和高效型。
class Rational {};
const Rational operator*(const Rational& lhs, const Rational& rhs){};
Rational operator/(const Rational& lhs, const Rational& rhs){};

// const 参数，没有什么特别新颖的概念，与local const对象一样，除非需要改动，否则请将它们声明为const。
// 多打了6个字符，却可以省下恼人的错误，比如把==写成=的错误。

// 将const用于成员函数的目的，是为了确认该成员函数可作用于const对象身上。
// 这一类成员函数之所以重要，基于两个理由：
//  1 使class接口比较容易理解。可以得知哪个函数可以改动对象内容而哪个函数不行。
//  2 使操作const对象成为可能。这对编写高效代码是个关键。改善C++程序效率的一个根本办法是以pass by reference-to-const方式传递对象，
...
//    textLength和lengthIsValid不是bitwise constness，可以被修改。编译器坚持bitwise constness，解决方法为：
//    利用C++的一个与const相关的关键字：mutable(可变的)。mutable释放掉none-static成员变量的bitwise constness约束。
class TextBlock2 {
 public:
...
// 在const和non-const成员函数中避免重复。不能用const成员函数调用non-const成员函数，这是一种错误行为。
...
int main() {
  ...
  // 错误是对"const版的operator[]返回"的const char&进行赋值导致的。
  // non-const operator[]的返回类型是个char的引用，不是char。如果operator[]只是返回一个char,下面的句子就无法通过编译：
  tb[0] = 'x';
  // 因为，如果函数返回类型是个内置类型，那么改动函数返回值从来就不合法。就算合法，改动的也是副本，不是tb.text[0]自身。

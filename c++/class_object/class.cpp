...
//  使用class时，类中的成员默认都是private属性的；
//  而使用struct时，结构体中的成员默认都是public属性的。
//  class继承默认是private继承，而struct继承默认是public继承。
//  class可以使用模板，而struct不能。
// C++没有抛弃C语言中的struct关键字，其意义就在于给C语言程序开发人员有一个归属感，
// 并且能让C++编译器兼容以前用C语言开发出来的项目。
// 在编写C++代码时，强烈建议使用class来定义类，而使用struct来定义结构体，这样做语义更加明确。
...
// 类只是一个模板，编译后不占用内存空间，所以在定义类时不能对成员变量进行初始化，因为没有地方存储数据。
// 只有在创建对象以后才会给成员变量分配内存，这个时候就可以赋值了。
// 类可以理解为一种新的数据类型，与char、int、float等不同的是，类是一种复杂数据类型，
// 可以包含基本类型，而且还有很多基本类型中没有的特性。
...
// 根据C++软件设计规范，项目中的成员变量以及只在类内部使用的成员函数（只被成员函数调用的成员函数）都建议声明为private，
...
// 如果不考虑成员变量对齐问题的话，对象所占用的存储空间的大小等于各成员变量所占用的存储空间的大小之和。
...
class A {
  ...
  // Java、C#程序员注意，C++中访问控制只能修饰类的成员，不能修饰类，C++中的类没有共有私有之分。
  // 在类的内部，无论成员被声明为public、protected还是private，都是可以互相访问的，没有访问权限的限制。
  // 在类的外部，只能通过对象访问成员，并且只能访问public属性的成员，不能访问private、protected属性的成员。
 public:
  // 类的成员变量和普通变量一样，也有数据类型和名称，占用固定长度的内存。
  // 但是，在定义类的时候不能对成员变量赋值（C++11可以了），因为类只是一种数据类型或者说是一种模板，
 ...
 public:
  ...
  // 当然，在类体内部定义的函数也可以加inline关键字，但这是多余的，因为类体内部定义的函数默认就是内联函数。
  // 内联函数一般不是我们所期望的，它会将函数调用处用函数体替代，所以建议在类体内部对成员函数作声明，
  // 而在类体外部进行定义，这是良好的编程习惯，实际开发中也是这样做的。
  ...
  // 在类体外定义inline函数的方式，必须将类的定义和成员函数的定义都放在同一个头文件中（或者同一个源文件中），
  // 否则编译时无法进行嵌入（将函数代码的嵌入到函数调用出）。
  // 虽然C++支持将内联函数定义在类的外部，但强烈建议将函数定义在类的内部，这样它会自动成为内联函数，
  // 何必费力不讨好地将它定义在类的外部呢，这样并没有任何优势。

  void testThis() {
    // 成员函数最终被编译成与对象无关的全局函数，如果函数体中没有成员变量，就很简单，不用做任何处理，直接调用即可。
...
void func1() {
  ...
  A a1;
  class A a2;  // 创建对象时，class可写可不写，但出于习惯通常会省略掉class关键字
...
namespace n2 {
// 构造函数的名字和类名相同，没有返回值，不需要用户显式调用（也不能调用），而是在创建对象时自动执行。
...
// 析构函数（Destructor）也是一种特殊的成员函数，没有返回值，不需要程序员显式调用（也没法显式调用），
...
void func() {
  A a1(5);
  // 一旦用户自己定义了构造函数，不管有几个，也不管形参如何，编译器都不再自动默认构造函数生成。
  // A a2;  // 调用默认构造函数
}
}  // namespace test3
...
namespace n3 {
// C++中的new和delete分别用来分配和释放内存，它们与C语言中malloc()、free()最大的一个不同之处在于：
// 用new分配内存时会调用构造函数，用delete释放内存时会调用析构函数。
// 构造函数和析构函数对于类来说是不可或缺的，所以在C++中鼓励使用new和delete。
// 析构函数的执行时机:
// 1.析构函数在对象被销毁时调用，而对象的销毁时机与它所在的内存区域有关。
// 2.在所有函数之外创建的对象是全局对象，它和全局变量类似，位于内存分区中的全局数据区，
//   程序在结束执行时会调用这些对象的析构函数。
// 3.在函数内部创建的对象是局部对象，它和局部变量类似，位于栈区，函数执行结束时会调用这些对象的析构函数。
// 4.new创建的对象位于堆区，通过delete删除时才会调用析构函数；如果没有delete，析构函数就不会被执行。
...
// 成员对象析构函数的执行次序和构造函数的执行次序相反，即先构造的后析构，这是C++处理此类次序问题的一般规律。
void testN5() {
  ABA aba(5, 6, 8.7);
  // A(int)
  // B(int)
...
// 并且这个赋值的过程是编译器自动完成的，不需要用户干预，用户也不能显式地给this赋值。
...
// 静态成员既可以通过对象访问，又可以通过类访问，而typedef定义的类型只能通过类来访问。
// 一个类就是一个作用域的事实能够很好的解释为什么在类的外部定义成员函数时必须同时提供类名和函数名。
// 在类的外部，类内部成员的名字是不可见的。
class A {
 public:
  typedef int INT;

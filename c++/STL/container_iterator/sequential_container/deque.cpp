...
// deque容器底层实现原理：
// 和vector容器采用连续的线性空间不同，
// deque容器存储数据的空间是由一段一段等长的连续空间构成，
// 各段空间之间并不一定是连续的，可以位于在内存的不同区域。
// 为了管理这些连续空间，deque容器用数组map存储着各个连续空间的首地址。
// 也就是说，map数组中存储的都是指针，
// 指向那些真正用来存储数据的各个连续空间通过建立map数组，
// deque容器申请的这些分段的连续空间就能实现整体连续的效果。
// 换句话说，当deque容器需要在头部或尾部增加存储空间时，
// 它会申请一段新的连续空间，同时在map数组的开头或结尾添加指向该空间的指针，
// 由此该空间就串接到了deque容器的头部或尾部。
// 如果map数组满了怎么办？
...
//   return *this;
// }
// deque容器除了维护先前讲过的map数组，
// 还需要维护start、finish这2个deque迭代器。
// 以下为deque容器的定义：
// template<class _Ty,
...
void func1() {
  ...
  // 5.通过拷贝其他类型容器中指定区域内的元素（也可以是普通数组），
  //   可以创建一个新容器：
  int a[] = {1, 2, 3, 4, 5};
...
void func2() {
  ...
  // 全局的begin()和end()函数，当操作对象为容器时，
  // 它们的功能是上面的begin()/end()成员函数一样：
  ...
  // 前者返回的是const类型的正向迭代器，
  // 这就意味着，由cbegin()和cend()成员函数返回的迭代器，
  // 可以用来遍历容器内的元素，也可以访问元素，
  // 但是不能对所存储的元素进行修改。
  auto first = d.cbegin();
  auto end = d.cend();
  // 常量迭代器不能用来修改容器中的元素值
...
void func3() {
  ...
  // 另外，结合deque模板类中和迭代器相关的成员函数，
  // 可以实现遍历deque容器中指定区域元素的方法：
  print(d);  // 10 4 3 20 [4]
...
void func6() {
  ...
  // emplace系列函数都只调用了构造函数，
  // 没有调用移动构造函数，提高了代码的运行效率：

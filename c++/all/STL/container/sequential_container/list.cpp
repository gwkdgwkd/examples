...
// 这意味着，list中的元素是分散存储的，而不是必须存储在一整块连续的内存空间中。
// list容器中各个元素的前后顺序是靠指针来维系的，每个元素都配备了2个指针，
// 分别指向它的前一个元素和后一个元素，其中第一个元素的前向指针总为null，
// 因为它前面没有元素，同样，尾部元素的后向指针也总为null。
// 基于这样的存储结构，list具有其它容器（array、vector和deque）所不具备的优势，
...
// end() 	         返回指向容器中最后一个元素的下一个位置的双向迭代器
// rbegin()        返回指向最后一个元素的反向双向迭代器
// rend() 	       返回指向第一个元素所在位置前一个位置的反向双向迭代器
// cbegin()        和begin()功能相同，只不过增加了const属性，不能修改元素
// cend() 	       和end()功能相同，只不过增加了const属性，不能用于修改元素
// crbegin()       和rbegin()功能相同，只不过增加了const属性，不能用于修改元素
// crend() 	       和rend()功能相同，只不过增加了const属性，不能用于修改元素
// empty() 	       判断容器中是否有元素，若无元素则返回true，反之返回false
// size() 	       返回当前容器实际包含的元素个数
// max_size()      返回容器所能包含元素的最大值，通常是2^32-1，很少会用这个函数
// front() 	       返回第一个元素的引用
// back() 	       返回最后一个元素的引用
// assign()        用新元素替换容器中原有内容
// emplace_front() 在容器头部生成一个元素，和push_front()功能相同，但效率更高
// push_front()    在容器头部插入一个元素
// pop_front() 	   删除容器头部的一个元素
// emplace_back()  在容器尾部直接生成一个元素，和push_back()功能相同，但效率更高
// push_back() 	   在容器尾部插入一个元素
// pop_back() 	   删除容器尾部的一个元素
// emplace() 	     在容器中的指定位置插入元素，和insert()功能相同，但效率更高
// insert()  	     在容器中的指定位置插入元素
// erase() 	       删除容器中一个或某区域内的元素
// swap() 	       交换两个容器中的元素，必须保证两个容器中的元素类型相同
// resize()        调整容器的大小
// clear() 	       删除容器存储的所有元素
...
// merge()         合并两个已排好序的list，并且合并之后的list依然是有序的
...
// 和array、vector、deque的迭代器相比，list迭代器最大的不同在于，
// 其配备的迭代器类型为双向迭代器，而不再是随机访问迭代器。
// 这意味着，支持使用++p1、p1++、p1--、p1++、*p1、p1==p2以及p1!=p2运算符，
// 但不支持以下操作（其中i为整数）：
// 1.p1[i]：不能通过下标访问list容器中指定位置处的元素；
// 2.p1-=i、p1+=i、p1+i 、p1-i：双向迭代器不支持使用-=、+=、+、-运算符；
// 3.p1<p2、p1>p2、p1<=p2、p1>=p2：双向迭代器不支持使用<、>、<=、>=比较运算符。
...
// list容器底层存储结构：
// list的底层是用双向链表实现的，甚至一些STL版本中（SGI STL），
// list的底层实现使用的是双向循环链表。
// 使用链表存储数据，并不会将它们存储到一整块连续的内存空间中。
// 恰恰相反，各元素占用的存储空间（又称为节点）是独立的、分散的，
// 它们之间的线性关系通过指针来维持，双向链表的各个节点中存储的不仅仅是元素的值，
// 还应包含2个指针，分别指向前一个元素和后一个元素。
// template<typename T,...>
// struct __List_node{
//     //...
//     __list_node<T>* prev;  指针用于指向前一个节点
//     __list_node<T>* next;  指针用于指向后一个节点
//     T myval;               用于存储当前元素的值
//     //...
// }
// 和array、vector迭代器的实现方式不同，由于list的元素并不是连续存储的，
// 所以该容器迭代器中，必须包含一个可以指向list容器的指针，
// 并且该指针还可以借助重载的*、++、--、==、!=等运算符，
// 实现迭代器正确的递增、递减、取值等操作。
// template<tyepname T,...>
...
// 并分别指向链表中头部的空白节点和尾部的空白节点（也就是说包含2个空白节点）。
...
// 建议使用empty()成员方法，理由很简单，无论是哪种容器，
// 只要其模板类中提供了empty()成员方法，都可以保证O(1)时间复杂度；
// 但对于list容器来说，使用size()成员方法判断容器是否为空，
// 可能要消耗O(n)的时间复杂度，这个结论适用于很多容器。
// 为什么list容器这么特殊呢，这和list模板类提供了独有的splice()成员方法有关。
// 在实现list容器的过程中发现，用户经常需要知道当前list容器中存有多少个元素，
// 于是想让size()成员方法的执行效率达到O(1)。
// 为了实现这个目的，必须重新设计list，使它总能以最快的效率知道自己含有多少个元素。
// 要想令size()的执行效率达到O(1)，最直接的实现方式是：
// 在list中设置一个专门用于统计存储元素数量的size变量，其位于所有成员方法的外部。
// 与此同时，在每一个可用来为容器添加或删除元素的成员方法中，更新size变量的值。
// 由此，size()成员方法只需返回size这个变量即可达到时间复杂度O(1)。
// 不仅如此，由于list容器底层采用的是链式存储结构（也就是链表），
// 该结构最大的特点就是，链表中的节点，无需经过拷贝就可以直接链接到其它链表中，
// 且整个过程只需要消耗O(1)的时间复杂度，选择list的的主要原因。
// 因此，作为list容器设计者，自然也想将splice()方法的时间复杂度设计为O(1)。
...
// 有些版本选择将splice()的执行效率达到O(1)，而size()的执行效率为O(n)。
...
void func1() {
  ...
  // 5.通过拷贝其他类型容器或普通数组中指定区域内的元素创建新的list容器
  int a[] = {1, 2, 3, 4, 5};
...
void func3() {
  ...
  // list容器不支持随机访问，未提供[]和at()，也没有提供data()。

  // 通过front()和back()，可以获得list中第一个和最后一个元素的引用。

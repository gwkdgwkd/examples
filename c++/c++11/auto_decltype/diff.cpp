...
void func1() {
  ...
  // 其中，varname表示变量名，value表示赋给变量的值，
  // exp表示一个表达式，方括号[]表示可有可无。
  decltype(i) i2 = 4;
  decltype(i) i3;
...
void func4() {
  // 总结：
  // 1.auto虽然在书写格式上比decltype简单，
  //   但是它的推导规则复杂，有时候会改变表达式的原始类型；
  // 2.而decltype比较纯粹，它一般会坚持保留原始表达式的任何类型，
  //   让推导的结果更加原汁原味。
  ...
  // 在实际开发中人们仍然喜欢使用auto关键字，
  // 因为它用起来简单直观，更符合人们的审美。
  // 如果你的表达式类型不复杂，还是推荐使用auto关键字，
  // 优雅的代码总是叫人赏心悦目，沉浸其中。
}
...
// 在C++11中增加了返回类型后置语法，
// trailing-return-type，又称跟踪返回类型
// 将decltype和auto结合起来完成返回值类型的推导。
// 返回值类型后置语法，
// 是为了解决函数返回值类型依赖于参数而导致难以确定返回值类型的问题。
// 有了这种语法以后，
// 对返回值类型的推导就可以用清晰的方式（直接通过参数做运算）描述出来，

...
// 2.iostreams，覆盖用户自定义缓冲功能、国际化I/O，及预定义对象cin，cout，cerr和clog；
// 3.国际化支持，包括多区域能力，像wchar_t和wstring等类型都对促进Unicode有所帮助。
// 4.数值处理，包括复数模板和纯数值数组；
// 5.异常阶层体系，包括base class exception、
//   derived class logic_error和runtime_error，以及更深继承的各个class；
// 6.C89标准程序库，1989 C标准程序库内的每个东西也都被覆盖于C++内。

// TR1详细叙述了14个新组件，都放在std命名空间内，更正确地说是在其嵌套命名空间tr1内：
...
// 6.Tuples变量组，pair只能持有两个对象，tuple可持有任意个数的对象；
...
// 11.数学特殊函数，包括Laguerre多项式、Bessel函数、完全椭圆积分，以及更多数学函数；
...
// 请记住：
// 1.C++标准程序库的主要机能由STL、iostream、locale组成，并包括C99标准程序库；
// 2.TR1添加了智能指针、一般化函数指针、hash-based容器、正则表达式以及另外10个组件；
// 3.TR1自身只是一份规范，使用TR1需要一份实物，一个好的实物来源是Boost。

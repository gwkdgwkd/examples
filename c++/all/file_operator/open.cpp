...
// 在对文件进行读写操作之前，先要打开文件。
// 打开文件有以下两个目的：
// 1.通过指定文件名，建立起文件和文件流对象的关联，
//   以后要对文件进行操作时，就可以通过与之关联的流对象来进行。
// 2.指明文件的使用方式。使用方式有只读、只写、既读又写、
//   在文件末尾添加数据、以文本方式使用、以二进制方式使用等多种。
// 打开文件可以通过以下两种方式进行：
...
//  ios::in 	                ifstream fstream 	打开文件用于读取数据。
//                                              如果文件不存在，则打开出错。
//  ios::out 	                ofstream fstream 	打开文件用于写入数据。
//                                              如果文件不存在，则新建该文件；
//                                              如果文件原来就存在，则打开时清除原来的内容。
//  ios::app 	                ofstream fstream 	打开文件，用于在其尾部添加数据。
//                                              如果文件不存在，则新建该文件。
//  ios::ate 	                ifstream 	        打开一个已有的文件，并将文件读指针指向文件末尾。
//                                              如果文件不存在，则打开出错。
//  ios::trunc                ofstream 	        打开文件时会清空内部存储的所有数据，
//                                              单独使用时与ios::out相同。
//  ios::binary      ifstream ofstream fstream 	以二进制方式打开文件。
//                                              若不指定此模式，则以文本模式打开。
...
//  因此在Windows平台上，如果用文本方式打开二进制文件进行读写，
//  读写的内容就可能和文件的内容有出入。
// 总的来说，Linux平台使用哪种打开方式都行；
// Windows平台上最好用ios::in|ios::out等打开文本文件，
// 用ios::binary打开二进制文件。

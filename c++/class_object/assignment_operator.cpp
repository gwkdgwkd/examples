...
// 在定义的同时进行赋值叫做初始化（Initialization）；
// 定义完成以后再赋值（不管在定义的时候有没有赋值）就叫做赋值（Assignment）。
// 初始化只能有一次，赋值可以有多次。
// 当以拷贝的方式初始化一个对象时，会调用拷贝构造函数；当给一个对象赋值时，会调用重载过的赋值运算符。

// C++拷贝控制操作（三/五法则）
// 当定义一个类时，显式地或隐式地指定了此类型的对象在拷贝、赋值和销毁时做什么。
// 一个类通过定义三种特殊的成员函数来控制这些操作，分别是拷贝构造函数、赋值运算符和析构函数。
...
// 在C++11标准中，为了支持移动语义，又增加了移动构造函数和移动赋值运算符，
// 这样共有五个特殊的成员函数，所以又称为“C++五法则”。
// 也就是说，“三法则”是针对较旧的C++89标准说的，“五法则”是针对较新的C++11标准说的。
// 为了统一称呼，后来人们干把它叫做“C++三/五法则”。
// 如果一个类没有定义所有这些拷贝控制成员，编译器会自动为它定义默认的操作，因此很多类会忽略这些拷贝控制操作。
// 但是，对于一些持有其他资源（例如动态分配的内存、打开的文件、指向其他数据的指针、网络连接等）的类来说，
// 依赖这些默认的操作会导致灾难，必须显式的定义这些操作。

// C++并不要求我们定义所有的这些操作，你可以只定义其中的一个或两个。
// 但是，这些操作通常应该被看做一个整体，只需要定义其中一个操作，而不需要定义其他操作的情况很少见。
// 当决定是否要为一个类显式地定义拷贝构造函数和赋值运算符时，一个基本原则是首先确定这个类是否需要一个析构函数。
// 通常，对析构函数的需求要比拷贝构造函数和赋值运算符的需求更加明显。
// 如果一个类需要定义析构函数，那么几乎可以肯定这个类也需要一个拷贝构造函数和一个赋值运算符。
// 如果我们为Array定义了一个析构函数，但却使用默认的拷贝构造函数和赋值运算符，
// 那么将导致不同对象之间相互干扰，修改一个对象的数据会影响另外的对象，此还可能会导致内存操作错误：
// Array func(Array arr) {  // 按值传递，将发生拷贝
//   Array ret = arr;       // 发生拷贝
//   return ret;            // ret和arr将被销毁
// }
// 当func()返回时，arr和ret都会被销毁，在两个对象上都会调用析构函数，此析构函数会free()掉m_p成员所指向的动态内存。
// 但是，这两个对象的m_p成员指向的是同一块内存，所以该内存会被free()两次，这显然是一个错误，将要发生什么是未知的。
...
void testN1() {
  ...
  // 由于调用了默认的赋值操作运算符，arr1和arr2的m_p执向同一块内存，free时导致崩溃：
...
// 赋值操作符注意事项：
// 1.operator=()的返回值类型为Array &，即避免在返回数据时调用拷贝构造函数，还能够达到连续赋值的目的。
// 2.if(this!=&arr)判断是否是给同一个对象赋值：如果是，那就什么也不做；
//   如果不是，那就将原有对象的所有成员变量一一赋值给新对象，并为新对象重新分配内存。
// 3.return *this表示返回当前对象（新对象）。
// 4.operator=()的形参类型为const Array&，这样不但能够避免在传参时调用拷贝构造函数，
//   还能够同时接收const类型和非const类型的实参。
// 5.赋值运算符重载函数除了能有对象引用这样的参数之外，也能有其它参数。好像不行啊？
//   但是其它参数必须给出默认值，例如：Array & operator=(const Array &arr, int a = 100);

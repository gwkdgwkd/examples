...
// 如果想通过程序实现矩阵的转置，互换行标和列标只是第一步。因为实现矩阵转置的前提是将矩阵存储起来，数据结构中提供了3种存储矩阵的结构，
// 分别是三元组顺序表、行逻辑链接的顺序表和十字链表。如果采用前两种结构，矩阵的转置过程会涉及三元组表也跟着改变的问题。
...
//  copt数组用于计算稀疏矩阵中每列第一个非0元素在新三元组表中存放的位置。
//  通常默认第一列首个非0元素存放到新三元组表中的位置为1，然后通过cpot[col] = cpot[col-1] + array[col-1]公式可计算出后续各列首个非0元素存放到新三元组表的位置。
// cpot[col] = cpot[col-1] + array[col-1]的意思是，后一列首个非0元素存放的位置等于前一列首个非0元素的存放位置加上该列非0元素的个数。
...
// array : 0( ) 1(1) 2(2)
// copt  : 0( ) 1(1) 2(2)
// 稀疏矩阵快速转置算法的时间复杂度为O(n)。即使在最坏的情况下（矩阵中全部都是非0元素），该算法的时间复杂度也才为O(n^2)。
// 实现快速转置算法的函数

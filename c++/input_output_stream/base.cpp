...
// 用cin接收从键盘输入的数据，用cout向屏幕上输出数据，
// 这2个过程又统称为标准I/O。
// 除此之外，C++也对从文件中读取数据和向文件中写入数据做了支持（统称为文件I/O）。
// C++的这套I/O解决方案就是一个包含很多类的类库，
// 作为C++标准库的组成部分，这些类常被称为流类。
// C++的开发者认为数据输入和输出的过程也是数据传输的过程，
// 数据像水一样从一个地方流动到另一个地方，
// 所以C++中将此过程称为流，实现此过程的类称为流类。
...
// 以cerr为例，一旦程序某处使用cerr输出数据，
// 自然而然地会认为此处输出的是警告或者错误信息。
// 值得一提的是，类似cin、cout、cerr和clog这样，
// 它们都是C++标准库的开发者创建好的，可以直接拿来使用，
// 这种在C++中提前创建好的对象称为内置对象。

...
void doSomething(const char* pString) { cout << pString << endl; }
...
// 有些vector对数据哟额外的限制，C API改变数据时，也要保证这些额外的限制还能被满足。比如要保持有序。
...
int main() {
  ...
  cout << endl;  // 0.1,1.1,2.1,3.1,4.1,

  vector<char> vc(5);
  size_t charsWritten = fillString(&vc[0], vc.size());
  string s1(vc.begin(), vc.begin() + charsWritten);
  cout << s1 << endl;  // abcde

  // 实际上，先让C API把数据写入到一个vector中，然后把数据拷贝到期望最终写入的STL容器中，这一思想总是可行的：
  deque<double> d(vd.begin(), vd.end());
  list<double> l(vd.begin(), vd.end());
  set<double> ss(vd.begin(), vd.end());
  // 而且这意味着，除了vecor和string以外的其他STL容器也能把它们的数据传递给C API。只需把每个容器的元素拷贝到一个vector中，然后传给该API：
  set<int> intSet;
  vector<int> vv(intSet.begin(), intSet.end());
  if (!v.empty()) {
    doSomething(&vv[0], vv.size());
  }
  // 也可以把数据拷贝到一个数组中，然后把该数组传给C API。但为什么这样做呢？除非在编译时知道容器的大小，否则即要动态地分配数组。

...
int main() {
  ...
  // 不能编译的原因是，对于这些容器类型，const_iterator和iterator是完全不同的类。试图将一个种类型转换wie另一种类型是毫无意义的。
  // reinterpret_cast、static_cast甚至C语言风格的类型转换也不能胜任。

  // 其他容器得到的结果也是一样的。也许vector或string强制转换的代码能通过编译，但这非常特殊
  ...
  advance(i, distance<ConstIter>(i, c));
  // 这种方法的效率取决与使用的迭代器，对于随机访问的迭代器，是常数时间操作；对于双向迭代器而言，是一个线性时间的操作。
  // 所以应该重新设计，是否一定需要从const_iterator到iterator的转换呢？参考条款26，尽量用iterator来代替const或reverse型的迭代器。

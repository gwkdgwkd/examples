...
// 和其它容器不同，array容器的大小是固定的，无法动态的扩展或收缩，
// 这也就意味着，在使过程无法借由增加或移除元素而改变其大小，
// 它只允许访问或者替换存储的元素。
...
//           不能用于修改元素。
// size() 	 返回容器中当前元素的数量，
//           其值始终等于初始化array类的第二个模板参数N。
// max_size()返回容器可容纳元素的最大数量，
//           其值始终等于初始化array类的第二个模板参数N。
// empty() 	 判断容器是否为空，和通过size()==0的判断条件功能相同，
//           但其效率可能更快。
// at(n) 	   返回容器中n位置处元素的引用，该函数自动检查n是否在有效的范围内，
//           如果不是则抛出out_of_range异常。
// front() 	 返回容器中第一个元素的直接引用，该函数不适用于空的array容器。
// back() 	 返回容器中最后一个元素的直接应用，该函数同样不适用于空的array容器。
// data() 	 返回一个指向容器首个元素的指针。
//           利用该指针，可实现复制容器中所有元素等类似功能。
...
void test() {
  ...
  *(data + 3) += 9;
  ...
  // 很少会创建空的array容器，因为当生成一个array容器时，
  // 它的元素个数就固定了，而且无法改变，
  // 所以生成空array容器的唯一方法是将模板的第二个参数指定为0，
  // 但这种情况基本不可能发生：
  ...
  // array容器之所以提供empty()，
  // 对于其他元素可变或者元素可删除的容器（vector、deque等）来说，

...
// 一个对象的定义，不论是有编译器暗中地做，或是有程序员明确地做，都会导致template class的具现。
// 只有在成员函数被使用的时候，C++标准才要求它们被具现出来。
// 之所以由使用者来主导具现规则，有两个主要原因：
// 1.空间和时间效率的考虑。
//   如果类有100个成员函数，但相关类型只使用了2个，那么其他198个函数都具现将会花费大量的时间和空间。
...
//   只有template的float实例、new运算符、默认构造函数需要被具现化。
// 函数在什么时候具现出来呢？有两种策略：
// 1.在编译时候，那么函数将具现与origin和p存在的哪个文件中。
...
// foo()的调用与template参数毫无关联，所以调用操作必须根据scope of the template definition来决议。
// 对于type_dependent的调用，与template参数有关，因为该参数将决定_member的真正类型。
// 所以这一次必须在scope of the template instantiantion中决议，有两个foo()函数声明。
// 由于_member的类型为int，所以应该是int版本的foo()出线。
// 如果ScopeRules是以unsigned int或log类型具现出来，那么foo调用操作就暧昧不明。
// 如果ScopeRules是以某一个类类型具现出来，而该类没有针对int或double实现出转换运算符，
// 那么foo()调用操作会被标示为错误。
...
// 2.另一个是链接时期策略，有一些工具可以引导编译器的具现行为。

// 编译器设计者必须回答三个主要问题：
// 1.编译器如何找出函数的定义？
// 2.编译器如何能够只具现出程序用到的成员函数？
// 3.编译器如何阻止成员定义在多个.o文件中被具现呢？
...
// C++的RTTI机制提供一个安全的downcast，但只对那些展现多态的类型有效。
// 把类相关的RTTI object地址放进virtual table中，通常放在第一个slot中。
// 每一个对象只多花一个指针。
// 这个指针只需被设定一次，它是被编译器静态设定，而不是在执行期有构造函数设定（vptr才这么设定）。

// dynamic_cast运算符可以在执行期决定真正的类型：
// 1.如果downcast是安全的（基类指针指向一个派生类对象），这个运算符会传回被适当转换的指针。
// 2.如果downcast不是安全的，这个运算符会传回0。

// references并不是pointers
// 程序中对一个类指针类型执行dynamic_cast运算符，会获得true或false：
// 1.如果传回真正的地址，表示这个对象的动态类型被确认了，一些与类型有关的操作现在可以施行与其上。
// 2.如果传会0，表示没有指向任何对象，意味这该以另一种逻辑执行这个动态类型为确定的对象身上。
...
// 因此当dynamic_cast运算符用在引用时，不能够提供等于指针情况下的那一组true和false。
// 会发生下面的事：
// 1.如果引用真正参考到适当的派生类，downcast会别执行而程序可以继续进行。
// 2.如果引用并不是某一种派生类，那么由于不能传回0，所以丢出一个bad_cast exception。

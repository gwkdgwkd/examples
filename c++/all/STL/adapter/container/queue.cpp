...
// 和stack不同，queue有2个开口，其中一个专门用来输入数据，另一个专门用来输出数据。
// 这种存储结构最大的特点是，最先进入queue的元素，也可以最先从queue中出来，
// 即用此容器适配器存储数据具有先进先出（FIFO）的特点，因此queue又称为队列适配器。
// STL queue容器适配器模拟的就是队列这种存储结构，
// 因此对于任何需要用队列进行处理的序列来说，使用queue容器适配器都是好的选择。

// queue容器适配器以模板类：
// queue<T,Container=deque<T>>位于<queue>中，并定义在std命名空间里。
...
//                     这是通过调用底层容器的成员函数push_back()来完成的。
//  emplace() 	       在queue的尾部直接添加一个元素。
//  push(T&& obj) 	   以移动的方式在queue的尾部添加元素。
//                     调用底层容器的具有右值引用参数的成员函数push_back()来完成的。
//  pop() 	           删除queue中的第一个元素。
//  swap(queue<T> &other_queue) 将两个queue容器适配器中的元素进行互换，
//                     需要注意的是，进行互换的2个queue容器适配器中，
//                     存储的元素类型以及底层采用的基础容器类型，都必须相同。
// 和stack一样，queue也没有迭代器，因此访问元素的唯一方式是遍历容器，
...
int main() {
  ...
  // 手动指定基础容器时，其存储的数据类型必须和容器适配器存储的元素类型保持一致。
  // 作为queue容器适配器的基础容器，其必须提供：
  // front()、back()、push_back()、pop_front()、empty()和size()，
  // 符合条件的序列式容器仅有deque和list。

  // 3.可以用基础容器来初始化queue容器适配器，
  //   只要该容器类型和queue底层使用的基础容器类型相同即可

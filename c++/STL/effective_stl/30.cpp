...
int main() {
  ...
  // results在transform调用后并不会改变，
  // 并且它的end迭代器仍然指向transform被调用之前的位置。
  // 使用插入型迭代器back_inserter、
  // front_inserter和inserter)可以解决上面的问题。

  // 希望transform覆盖results容器中已有的元素，
  // 需要确保results中的元素至少和values中一样多。

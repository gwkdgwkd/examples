...
// C++提供多态的目的是：
// 可以通过基类指针对所有派生类（包括直接派生和间接派生）的成员变量和成
// 员函数进行全方位的访问，尤其是成员函数。
// 如果没有多态，只能访问成员变量。
...
//   那么当基类指针p指向派生类对象时，语句p->func(100);将会出错，
//   而语句p->func();将调用基类的函数。
...
// 3.如果成员函数在类被继承后功能不需修改，
//   或派生类用不到该函数，则不要把它声明为虚函数。
...
void testN1() {
  ...
  // 与期望不同，指向派生类就应该用派生类的方法，这才符合人们的思维习惯。
  B *p = &d;
  p->show();  // B::show i:10
}
}  // namespace n1
...
void testN5() {
  ...
  // 通过基类的指针只能访问从基类继承过去的成员，
  // 不能访问派生类新增的成员，下面的语句编译失败：

...
// 来自C，Java或C#的开发人员，需要特别注意。那些语言中转型比较必要而无法避免，也比较不危险（于C++相比）。

// 转型语法的三种形式：
//  1 C风格的转型：(T)expression
//  2 函数风格的转型：T(expression)
// 上面两种形式并无差别，纯粹只是小括号的摆放位置不同而已。这两种形式为”旧式转型”。
...
//  容易在代码中被识别出来，便于定位问题;
//  各转型动作的目标愈窄化，编译器可能诊断出错误的运用。

// “转型其实什么都没做，只是告诉编译器把某种类型视为另一种类型”，这是错误的观念。
// 任何一个类型转换（不论是通过转型操作而进行的显示转换，或通过编译器完成的隐式转换）往往真的令编译器编译出运行期间执行的码。
// 单一对象可能拥有一个以上的地址（例如以Base*指向它时的地址和以Derived*指向它时的地址）。
// C，Java，C#都不可能发生这种事。但C++可能，实际上一旦使用多重继承，这事几乎一直发生着。即使在单一继承中也可能发生。
class Base {};
...
class SpecialWindow : public Window {
 public:
  virtual void onResize() {
    // 调用的不是当前对象上的函数，而是稍早转型动作建立的一个”*this对象的base class成分“的暂时副本身上的onResize!
    // 虽然成员函数只有一份，但是成员函数都有个隐藏的this指针，会影响成员操作的数据。
...
class SpecialWindow1 : public Window {
 public:
  virtual void onResize() {
    Window::onResize();  // 调用base class版本的onResize函数
    // SpecialWindow专属行为
  }
};
...
// 绝对必须避免的是所谓的连串dynamic_cast。有好多分支，每个分支都要执行dynameic_cast。可以用虚函数代替。

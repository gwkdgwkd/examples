...
// 1.C语言中const全局变量的可见范围是整个程序，在其他文件中使用extern声明后就可以使用；
// 2.而C++中const全局变量的可见范围仅限于当前文件，
//   在其他文件中不可见，所以可以定义在头文件中，多次引入后也不会出错。
// 如果你使用的是GCC，那么可以通过添加extern关键字来增大C++全局const变量的可见范围。
// extern const int n = 10;这样n的可见范围就变成了整个程序，
// 在其他文件中使用extern声明后就可以使用了。
// 不过这种方式只适用于GCC，不适用于VS/VC。

// C++中的const变量虽然也会占用内存，也能使用&获取得它的地址，但是在使用时却更像编译时期的#define；
// #define也是值替换，可见范围也仅限于当前文件。
// 很多C++教程在对比const和#define的优缺点时提到，#define定义的常量仅仅是字符串的替换，
// 不会进行类型检查，而const定义的常量是有类型的，编译器会进行类型检查，

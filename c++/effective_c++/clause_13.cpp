...
// 其他常见的资源还包括文件描述符、互斥锁、图形界面中的字符和笔刷、数据库连接、以及网络socket。
// 不论哪种，不使用时，必须还给系统。
...
// 以对象管理资源的两个关键想法：
// 1.获得资源后立刻放进管理对象内
//   “以对象管理资源”的观念常被称为“资源取得时便是初始化的时机”（RAII），
...
// 若通过copy构造函数或copy assignment操作符复制它们，它们会变成null，复制所得的指针取得资源唯一拥有权。
// 上面的限制导致auto_ptr并非管理动态分配资源的神兵利器。

// auto_ptr的替代方案是“引用计数型智能指针”（RCSP）。

...
// 要谨慎使用reflect包。同样也适用于unsafe包。
// 高级语言使得程序员不用再关心真正运行程序的指令细节,同时也不再需要关注许多如内存
// 布局之类的实现细节。因为高级语言这个绝缘的抽象层,我们可以编写安全健壮的,并且可
// 以运行在不同操作系统上的具有高度可移植性的程序。
// 但是unsafe包,它让程序员可以透过这个绝缘的抽象层直接使用一些必要的功能,虽然可能
// 是为了获得更好的性能。但是代价就是牺牲了可移植性和程序安全,因此使用unsafe包是一
// 个危险的行为。大多数Go程序员可能永远不会需要直接使用unsafe包。当然,也永远都会
// 有一些需要使用unsafe包实现会更简单的场景。如果确实认为使用unsafe包是最理想的方
// 式,那么应该尽可能将它限制在较小的范围,这样其它代码就可以忽略unsafe的影响。

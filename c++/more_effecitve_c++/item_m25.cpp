...
// 从字面来看，谈论“虚拟构造函数”没有意义。
// 当你有一个指针或引用，但是不知道其指向对象的真实类型是什么时，
// 你可以调用虚拟函数来完成特定类型（type-specific）对象的行为。
// 仅当你还没拥有一个对象但是你又确切地知道想要的对象的类型时，你才会调用构造函数。
// 那么虚拟构造函数又从何谈起呢？
...
// cloneSelf或者是象下面这样就叫做clone，很少会有函数能以这么直接的方式实现它：
// 正如我们看到的，类的虚拟拷贝构造函数只是调用它们真正的拷贝构造函数。
// 因此“拷贝”的含义与真正的拷贝构造函数相同。
...
// 这就是为什么TextBlock的clone函数能够返回TextBlock*和Graphic的clone能够返回Graphic*的原因，
...
// 显而易见的方法是虚拟化这个输出操作符。
// 但是输出操作符是operator<<，
// 函数把ostream&做为它的左参数（left-hand argument）（即把它放在函数参数列表的左边），
// 这就不可能使该函数成为成员函数。
// 为了能够回到正常的语法上来，我们必须把operator<<移出TextBlock和Graphic类，
// 但是如果我们这样做，就不能再把它声明为虚拟了。

// 另一种方法是为打印操作声明一个虚拟函数（例如 print）把它定义在TextBlock和Graphic类里。
// 但是如果这样，
// 打印TextBlock和Graphic对象的语法就与使用operator<<做为输出操作符的其它类型的对象不一致了，
// 这些解决方法都不很令人满意。
// 我们想要的是一个称为operator<<的非成员函数，其具有象print虚拟函数的行为特性。
// 有关想要什么的描述实际上已经很接近如何得到它的描述。我们定义operator<<和print函数，让前者调用后者：
inline ostream& operator<<(ostream& s, const NLComponent& c) {
  return c.print(s);
}
// 具有虚拟行为的非成员函数很简单。
// 你编写一个虚拟函数来完成工作，然后再写一个非虚拟函数，它什么也不做只是调用这个虚拟函数。
// 为了避免这个句法花招引起函数调用开销，你当然可以内联这个非虚拟函数。

// 你知道如何根据它们的一个参数让非成员函数虚拟化，你可能想知道是否可能让它们根据一个以上的参数虚拟化呢？
// 可以，但是不是很容易。有多困难呢？参见条款M31。

...
//   不允许在lambda表达式内部修改，可以理解为这部分变量都是const常量。
//   而如果想修改它们，就必须使用mutable关键字。
...
//   如果lambda函数标有noexcept而函数体内抛出了异常，
//   又或者使用throw()限定了异常类型，而函数体内抛出了非指定类型的异常，
//   这些异常无法使用try-catch捕获，会导致程序执行失败。
// 5.->返回值类型
//   指明lambda匿名函数的返回值类型。
...
namespace n1 {
// lambda匿名函数中的[外部变量]是指：
// 和当前lambda表达式位于同一作用域内的所有局部变量。
...
// [val1,val2,...]，表示以值传递的方式导入val1、val2等外部变量，
// 同时多个变量之间没有先后次序；
void func4() {
  int i = 4;
  float f = 4.4;
...
// [&val1,&val2,...]，表示以引用传递的方式导入val1、val2等外部变量，
// 多个变量之间没有前后次序；
void func5() {
  int i = 4;
  float f = 5.5;
...
void func3() {  // 反例2
  ...
  // 由于指定异常类型和真正发生的异常类型不匹配，
  // 导致try-catch无法捕获，最终程序运行崩溃。
}
}  // namespace n3

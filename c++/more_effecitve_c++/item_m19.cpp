...
// 通常有两个方法可以消除它:
// 1.一种是重新设计你的代码，不让发生这种类型转换，这种方法在条款M5中被研究和分析。
// 2.另一种方法是通过修改软件而不再需要类型转换，条款M21讲述了如何去做。
// 仅当通过传值（by value）方式传递对象或传递常量引用参数时，才会发生这些类型转换。
// 当传递一个非常量引用（reference-to-non-const）参数对象，就不会发生。
void uppercasify(string& str) {}
// 当程序员期望修改非临时对象时，对非常量引用进行的隐式类型转换却修改临时对象。
// 这就是为什么C++语言禁止为非常量引用（reference-to-non-const）产生临时对象。
...
int main() {
  ...
  // 仅当消除类型不匹配后，才能成功进行这个调用，你的编译器很乐意替你消除它，
  // 方法是建立一个string类型的临时对象。
  ...
  // 无疑这不是程序员所希望的，程序员传递subtleBookPlug参数到uppercasify函数中，

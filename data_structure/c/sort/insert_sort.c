...
// 插入排序算法的实现思路是：初始状态下，将待排序序列中的第一个元素看作是有序的子序列。
// 从第二个元素开始，在不破坏子序列有序的前提下，将后续的每个元素插入到子序列中的适当位置。
...
// 具体实现思路为：
// 另外设置一个同存储记录的数组大小相同的数组d，将无序表中第一个记录添加进d[0]的位置上，然后从无序表中第二个记录开始，
// 同d[0]作比较，如果该值比d[0]大，则添加到其右侧；反之添加到其左侧。
// 在这里的数组d可以理解成一个环状数组。
// 2-路插入排序相比于折半插入排序，只是减少了移动记录的次数，没有根本上避免，所以其时间复杂度仍为O(n^2)。
...
// 上面三种插入排序算法，其基本结构都采用数组的形式进行存储，因而无法避免排序过程中产生的数据移动的问题。
// 如果想要从根本上解决只能改变数据的存储结构，改用链表存储。表插入排序，即使用链表的存储结构对数据进行插入排序。

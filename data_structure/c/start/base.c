...
// 数据在计算机存储空间的存放，决不是胡乱的，
// 这就要求选择一种好的方式来存储数据，而这也是数据结构的核心内容。
// 数据结构是一门学科，它教会我们“如何存储具有复杂关系的数据更有助于后期对数据的再利用”。

// 数据结构大致包含以下几种存储结构：
// 1.线性表，还可细分为顺序表、链表、栈和队列；
//   具备一对一关系的数据就可以使用线性表来存储。
...
//   队列中的元素只能从线性表的一端进，从另一端出，且要遵循先入先出的特点，即先进队列的元素也要先出队列。
...
// 集中存储（底层实现使用的是数组）需要使用一大块连续的物理空间，假设要存储大小为1G的数据，
// 若存储设备上没有整块大小超过1G的空间，就无法使用顺序存储，此时就要选择链式存储，
...
// 数据结构中，用时间复杂度来衡量程序运行时间的多少；
// 用空间复杂度来衡量程序运行所需内存空间的大小。
...
// 在数据结构中，频度表达式可以这样简化：
// 1.去掉频度表达式中，所有的加法常数式子。例如2n^2+2n+1简化为2n2+2n；
// 2.如果表达式有多项含有无限大变量的式子，只保留一个拥有指数最高的变量的式子。例如2n^2+2n简化为2n2；
// 3.如果最高项存在系数，且不为1，直接去掉系数。例如2n^2系数为2，直接简化为n^2；
...
// 1.如果随着输入值n的增大，程序申请的临时空间成线性增长，则程序的空间复杂度用O(n)表示;
// 2.如果随着输入值n的增大，程序申请的临时空间成n^2关系增长，则程序的空间复杂度用O(n^2)表示；
// 3.如果随着输入值n的增大，程序申请的临时空间成n^3关系增长，则程序的空间复杂度用O(n^3)表示；
// 在多数场景中，一个好的算法往往更注重的是时间复杂度的比较，而空间复杂度只要在一个合理的范围内就可以。

// 数据结构和算法之间完全是两个相互独立的学科，如果非说它们有关系，那也只是互利共赢、1+1>2的关系。
// 从分析问题的角度去理清数据结构和算法之间的关系。
// 通常，每个问题的解决都经过以下两个步骤：

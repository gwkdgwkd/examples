...
// 链式队列的实现思想同顺序队列类似，只需创建两个指针（命名为top和rear）分别指向链表中队列的队头元素和队尾元素。
...
//  将该数据元素用节点包裹，例如新节点名称为elem；
//  与rear指针指向的节点建立逻辑关系，即执行rear->next=elem；
//  最后移动rear指针指向该新节点，即rear=elem；
// 由此，新节点就入队成功了。
...
//  通过top指针直接找到队头节点，创建一个新指针p指向此即将出队的节点；
//  将p节点（即要出队的队头节点）从链表中摘除；
//  释放节点p，回收其所占的内存空间；

...
// 即无论是向里存数据还是从中取数据，都只能从这一个开口实现操作，开口端为栈顶。
// 由于数据的存和取只能从栈顶处进行操作，因此对于存取数据，
// stack适配器有这样的特性，即每次只能访问适配器中位于最顶端的元素，
// 也只有移除stack顶部的元素之后，才能访问位于栈中的元素。
// 栈中存储的元素满足后进先出LIFO的准则，stack适配器也同样遵循这一准则。

// stack适配器以模板类：
// stack<T,Container=deque<T>>位于<stack>头文件中，并定义在std命名空间里。
// 其中T为存储元素的类型，Container表示底层容器的类型。
// stack容器适配器和其他序列容器相比，是一类存储机制简单、提供成员函数较少的容器：
//  empty()            当stack栈中没有元素时，返回true，反之，返回false。
//  size() 	           返回stack栈中存储元素的个数。
//  top() 	           返回栈顶元素的引用，类型为T&，如果栈为空，程序报错。
//  push(const T& val) 先复制val，再将val副本压入栈顶，通过push_back()完成的。
//  push(T&& obj)      以移动方式压入栈顶，通过有右值引用参数的push_back()完成。
//  pop() 	           弹出栈顶元素。
//  emplace(arg...)    arg...可以是一个或多个参数，但它们都只用于构造一个对象，
//                     并在栈顶直接生成该对象，作为新的栈顶元素。
//  swap(stack<T> & other_stack) 将两个stack中的元素进行互换，需要注意的是，
//                     进行互换的2个stack中，元素以及底层基础容器类型，都必须相同。
...
int main() {
  ...
  //   只要支持empty()、size()、back()、push_back()、pop_back()即可
  ...
  // stack第2个模板参数必须显式指定为list<int>，必须为int类型，
  // 和存储类型保持一致，否则stack底层将默认使用deque容器，

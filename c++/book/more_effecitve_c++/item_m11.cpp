...
// 异常没有被Session的析构函数捕获，所以它被传递到析构函数的调用者那里。
// 但是如果析构函数本身的调用就是源自于某些其它异常的抛出，
// 那么terminate函数将被自动调用，彻底终止程序。
}  // namespace n1

namespace n2 {
// 事态果真严重到了必须终止程序运行的地步了么？
// 如果没有，必须防止在logDestruction内抛出的异常，
...
// 但是这样做并不比原来的代码安全，如果在catch中调用operator<<时导致一个异常被抛出，
...
Session::~Session() {
  try {
    logDestruction(this);
  } catch (...) {  // 在释放Session时必须忽略掉所有它抛出的异常
  }
}
// catch表面上好像没有做任何事情，这是一个假象，
// 实际上它阻止了任何从logDestruction抛出的异常，被传递到session析构函数的外面。
// 无论session对象是不是在堆栈退栈中被释放，terminate函数都不会被调用。
}  // namespace n3
...
// 禁止异常传递到析构函数外有两个原因：
// 1.能够在异常转递的堆栈辗转开解（stack-unwinding）的过程中，防止terminate被调用；
// 2.它能帮助确保析构函数总能完成希望它做的所有事情。

...
// 如果你没有在函数里使用参数，许多编译器会显示警告信息，很令人讨厌。为了避免这些警告信息，一种经常使用的方法时省略掉你不想使用的参数名称。

// 很明显一个后缀increment必须返回一个对象（它返回的是增加前的值），但是为什么是const对象呢？不是const对象，下面的代码就是正确的：
class A {
 public:
  A operator++(int) {}
};
...
// 这个原则是后缀increment和decrement应该根据它们的前缀形式来实现。你仅仅需要维护前缀版本，因为后缀形式自动与前缀形式的行为一致。

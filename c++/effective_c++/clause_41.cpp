...
void doProcessing1(Widget& w) {
  ...
  // 唯一需要做的是，返回一个类型为x的对象，而x对象加上一个int（10的类型）必须能够调用一个operator>。
  // 这个operator>不需要非得取得一个类型为x的参数不可，因为它也可以取得类型Y，只要存在隐转换就可以。
...
// 运行期多态和编译期多态之间的差异，
// 类似于“哪一个重载函数被调用（编译期）”和“哪一个virtual函数该被绑定（运行期）”之间的差异。

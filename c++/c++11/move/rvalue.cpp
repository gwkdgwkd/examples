...
int main() {
  ...
  // 反之，字面量5、10，既无名称，也无法获取其存储地址，所以是右值。
  // 通常存储在寄存器中，或者和代码存储在一起。

  // C++右值引用：
  // 右值引用可以从字面意思上理解，
  // 指的是以引用传递（而非值传递）的方式使用C++右值。
  // 其实C++98/03标准中就有引用，使用&表示。
  // 但非常量左值引用有一个缺陷，
  // 即正常情况下只能操作C++中的左值，无法对右值添加引用：
  int num = 10;
  // int &c = 10; // 错误
  int &b1 = num;  // 正确

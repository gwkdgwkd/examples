...
// 图存储结构基本常识：
// 1.弧头和弧尾，有向图中，无箭头一端的顶点通常被称为初始点或弧尾，
//   箭头直线的顶点被称为终端点或弧头。
// 2.入度和出度，对于有向图中的一个顶点V来说，箭头指向V的弧的数量为V的入度InDegree，
//   记为ID(V)，箭头远离V的弧的数量为V的出度OutDegree，记为OD(V)。
// 3.(V1,V2)和<V1,V2>的区别，无向图中描述两顶点(V1和V2)之间的关系可以用(V1,V2)来表示，
//   而有向图中描述从V1到V2的单向关系用<V1,V2>来表示。
...
//   如果路径中第一个顶点和最后一个顶点相同，则此路径称为回路或环。
//   并且，若路径中各顶点都不重复，此路径又被称为简单路径；
//   同样，若回路中的顶点互不重复，此回路被称为简单回路或简单环。
// 6.权和网的含义，在某些实际场景中，图中的每条边或弧会赋予一个实数来表示一定的含义，
//   这种与边或弧相匹配的实数被称为权，而带权的图通常称为网。
// 7.子图指的是由图中一部分顶点和边构成的图，称为原图的子图。
...
// 2.稀疏图和稠密图，这两种图是相对存在的，即如果图中具有很少的边或弧，
//   此图就称为稀疏图；反之，则称此图为稠密图。
// 3.稀疏和稠密的判断条件是，e<nlogn，其中e表示图中边或弧的数量，n表示图中顶点的数量。
//   如果式子成立，则为稀疏图；反之为稠密图。

// 从一个顶点到达另一顶点，若存在至少一条路径，则称这两个顶点是连通着的。
// 无向图中，如果任意两个顶点之间都能够连通，则称此无向图为连通图。
// 若无向图不是连通图，但图中存储某个子图符合连通图的性质，则称该子图为连通分量。
// 这里的子图指的是图中最大的连通子图（也称极大连通子图）。
// 需要注意的是，连通分量的提出是以整个无向图不是连通图为前提的，因为如果无向图是连通图，

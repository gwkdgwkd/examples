...
class Widget {
 public:
  bool isCertified() const { return false; }
};

// 当容器中存放的是指向动态分配的对象的指针的时候，
// 应该避免使用remove和类似的算法（remove_if和unique）。
// 很多情况下，partition是个不错的选择。
// 无法避免对这种容器使用remove，那么一种可以消除该问题的做法是，
// 在进行erase-remove之前，先把指向未被验证的指针置空，然后清除空指针。
...
int main() {
  ...
  // 担心是正确的，但是已经晚了，在erase被调用之前，
  // remove_if可能已经造成资源泄露了。
  // 要被删除的指针已经被那些不会被删除的指针覆盖了。
  // 没有任何指针指向要被删除的指针，永远不会被删除了，资源也不会别释放。
  ...
  v1.push_back(std::shared_ptr<Widget>(new Widget));

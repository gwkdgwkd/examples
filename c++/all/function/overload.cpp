...
// C++允许多个函数拥有相同的名字，只要它们的参数列表不同就可以，这就是函数的重载。
// 重载就是在一个作用范围内，同一个类、同一个命名空间等，有多个名称相同但参数不同的函数。
// 重载的结果是让一个函数名拥有了多种用途，使得命名更加方便，调用更加灵活。
// 在使用重载函数时，同名函数的功能应当相同或相近，不要用名函数去实现完全不相干的功能，
...
// 5.对于类的成员函数，名称、参数和返回值相同，但const属性不同，可以构造重载。

namespace n1 {
// C++编译时根据参数列表对函数进行重命名，不同的编译器有不同的重命名方式：
...
// 如果在一个优先级中找到多个（两个以及以上）合适的重载函数，编译器就会陷入两难境地，
// 不知道如何抉择，编译器会将这种模棱两可的函数调用视为一种错误，
...
void func() {
  ...
  //   #3是把short转换为long（类型转换），类型提升的优先级高于类型转换，所以#2胜出；

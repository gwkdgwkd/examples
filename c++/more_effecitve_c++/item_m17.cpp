...
// 在计算机科学中，我们尊称这样的延迟为lazy evaluation（懒惰计算法）。
...
// 通过使用lazy evaluation和条款M30中讲述的proxy class，
// 我们可以推迟做出是读操作还是写操作的决定，直到我们能判断出正确的答案。
...
// 因为LargeObject对象实例很大，为这样的对象获取所有的数据，数据库的操作的开销将非常大，
// 特别是如果从远程数据库中获取数据和通过网络发送数据时。
// 而在这种情况下，不需要读去所有数据。
...
// 不过这时建立的仅是一个对象“壳”，当需要某个数据时，这个数据才被从数据库中取回。
...
class LargeObject1 {
 ...
 private:
  int oid;
  // 实现Lazy Fetching时，你面临着一个问题：
  // 在任何成员函数里都有可能需要初始化空指针使其指向真实的数据，包括在const成员函数里，例如field1。
  // 然而当你试图在const成员函数里修改数据时，编译器会出现问题。
  // 最好的方法是声明字段指针为mutable，这表示在任何函数里它们都能被修改，甚至在const成员函数里。
...
// 如果编译器不支持mutable需要找到另一种方法让编译器允许你在const成员函数里修改数据成员。
// 一种方法叫做“fake this”（伪造this指针）,你建立一个指向non-const指针，指向的对象与this指针一样。
// 当你想修改数据成员时，你通过“fake this”访问它：
class LargeObject2 {
 public:
...
const string& LargeObject2::field1() const {
  ...
  // 如果你的编译器不支持cosnt_cast，你可以使用老式C风格的cast：
...
// LargeObject里的指针，必须把这些指针都初始化为空，然后每次使用它们时都必须进行测试，
// 这是令人厌烦的而且容易导致错误发生。
// 幸运的是使用smart(灵巧)指针可以自动地完成这种苦差使。
// 如果在LargeObject里使用smart指针，你也将发现不再需要用mutalbe声明指针。
// 这只是暂时的，因为当你实现smart指针类时你最终会碰到mutalbe。
...
// Matrix<int> m3 = m1 + m2;
// 通常operator的实现使用eagar evaluation：在这种情况下，它会计算和返回m1与m2的和。
...
// 实际上，如果你的计算都是重要的，lazy evaluation可能会减慢速度并增加内存的使用，
// 因为除了进行所有的计算以外，你还必须维护数据结构让lazy evaluation尽可能地在第一时间运行。
// 在某些情况下要求软件进行原来可以避免的计算，这时lazy evaluation才是有用的。
// lazy evaluation对于C++来说没有什么特殊的东西。
// 这个技术能被运用于各种语言里，
// 几种语言例如著名的APL、dialects of Lisp（事实上所有的数据流语言）都把这种思想做为语言的一个基本部分。

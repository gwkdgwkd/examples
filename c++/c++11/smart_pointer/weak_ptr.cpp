...
// 和shared_ptr、unique_ptr一样，
// weak_ptr智能指针也是以模板类的方式实现的。
// weak_ptr<T>定义在<memory>头文件，并位于std命名空间中。

// C++11标准虽然将weak_ptr定位为智能指针的一种，
// 但通常不单独使用，只能和shared_ptr类型指针搭配使用。
// weak_ptr是shared_ptr指针的辅助工具，借助weak_ptr类型指针，
// 可以获取shared_ptr指针的一些状态信息，
// 比如有多少指向相同的shared_ptr指针、
// shared_ptr指针指向的堆内存是否已经被释放等等。
...
// expired() 	  判断当前weak_ptr指针为否过期，
//              指针为空，或者指向的堆内存已经被释放。
// lock() 	    如果当前weak_ptr已经过期，
//              则该函数会返回一个空的shared_ptr指针；
...
void func2() {
  std::shared_ptr<int> p1(new int(5));
  std::weak_ptr<int> wp1(p1);
  std::cout << std::boolalpha << wp1.expired() << " " << wp1.use_count() << " "
            << p1.use_count() << std::endl;  // false 1 1

  wp1.reset();
  std::cout << std::boolalpha << wp1.expired() << " " << wp1.use_count() << " "
            << p1.use_count() << std::endl;  // true 0 1

  std::weak_ptr<int> wp2(p1);
  std::cout << std::boolalpha << wp2.expired() << " " << wp2.use_count() << " "
            << p1.use_count() << std::endl;  // false 1 1

  p1.reset();
  std::cout << std::boolalpha << wp2.expired() << " " << wp2.use_count() << " "
            << p1.use_count() << std::endl;  // true 0 0
}

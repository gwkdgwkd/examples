...
// 优先级 	           包含的内容 	        举例说明
// 精确匹配 	         不做类型转换，直接匹配
//                    只是做微不足道的转换 	 从数组名到数组指针、从函数名到指向函数的指针、
//                                        从非const类型到const类型
// 类型提升后匹配 	    整型提升 	           从bool、char、short提升为int，或者从char16_t、
//                                        char32_t、wchar_t提升为int、long、long long
...
// 因为这些合适的重载函数同等“优秀”，没有一个脱颖而出，调用谁都一样。
// 这就是函数重载过程中的二义性错误。
...
// C++和C可以进行混合编程。
// 但需要注意的是，由于C++和C在程序的编译、链接等方面都存在一定的差异，
// 而这些差异往往会导致程序运行失败。
...
// 2.如果用于修饰一段C++代码，只需为extern "C"添加一对大括号{}，并将要修饰的代码囊括到括号内即可。

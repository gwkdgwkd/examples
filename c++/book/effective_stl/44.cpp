...
// 1.成员函数往往速度快；
// 2.成员函数通常与容器（特别是关联容器）结合的更加紧密，这是算法所不能的，
//   这是因为算法和成员函数虽然有同样的名称，但是它们所做的事情往往不完全相同。

namespace n1 {
// find成员函数所要求的比较次数与关联容器的具体类型有关，大多数关联容器的底层用红黑树结构。
// 搜索包含1000000个元素的集合最大比较次数为38，但对于大多数搜索，比较次数不会超过22。
// 基于完全平衡树的实现永远不会超过21次比较，但在实践中，
// 完全平衡树的总体性能略逊色于红黑树的性能，所以大多数STL采用了红黑树而不是平衡树。

// 效率并不是find成员函数和find算法之间的唯一差别。
// STL算法以相等性来判断两个对象是否具有相同的值，而关联容器使用等价性来进行相同性测试。
// 因此find算法查找的是在容器中是否存在与727等值的元素，
// 而find成员函数则是搜索容器中是否有等价与727的元素。
// 由于使用的准则不同，有可能导致截然不同的搜索结果，因此，在使用关联容器的时候，
// 优先考虑成员函数形式的find、count以及lower_bound等，而不是算法。

void func() {
  std::set<int> s;  // 包含了一百万个整数值

  // 现在想知道727这个整数是否包含其中，如果在的话，第一次出现是在哪里。

  // 无论容器中是否存在727，set::find的比较次数不会超过40次，通常大约20次就可以了：
...
// count会使用相等性测试具有相同键和值的pair的个数，find、lower_bound等算法也与此类似。

// 对于标准关联容器，选择成员函数而不选择对应的同名算法，这可以带来几方面的好处：
// 1.可以获得对数时间的性能，而不是线性时间的性能；
// 2.可以使用等价性来确定两个值是否相同，而等价性是关联容器的一个本质定义；
// 3.在使用map和multimap时，将很自然地只考虑元素的键部分，而不是完整的key，value对。
...
// remove、remove_if、unique、sort、merge和reverse这些算法，
// 无一例外地需要拷贝list容器中的对象，而list容器量身定做的成员函数则无需任何对象拷贝，
// 它们只是简单地维护好那些将list节点连接起来的指针，这些算法的时间复杂度并没有改变，
// 但多数情况下维护指针的开销比拷贝对象要低的多，所以list成员函数提供更好的性能。

// list成员函数的行为不同于其同名的算法的行为：
// 1.在调用了remove、remove_if或者unique算法后并没有删除元素，必须紧接着再调用erase；
// 2.而list的remove、remove_if或unique成员函数则删除了元素，所以无需再调用erase了。
}  // namespace n3

...
// 删除容器中所有指定值的元素，各个容器方法不同，没有对所有容器类型都适用的方式。
...
// 连续内存的容器（vector、deque或string），最好的办法是erase-remove：
void func1() {
...
void func5() {
  ...
  // 使用任何名为remove的操作都是错误的，没有名为remove的成员函数，
  // 使用remove算法可能会覆盖容器的值，同时可能会破坏容器。
  // 对于关联容器，正确方法是用erase，正确且高效，只需要对数时间的开销。
...
bool badValue2(int i) { return i > 2; }

namespace n2 {
// 要删除容器中满足特定判别式（条件）的所有对象：
// 1.如果容器是vector、string或deque，则使用erase-remove_if习惯用法；
// 2.如果容器是list，则使用list::remove_if；
// 3.如果容器是一个标准关联容器，则使用remove_copy_if和swap，
//   或者写一个循环来遍历容器中的元素，记住当把迭代器传给erase时，进行后缀递增。

// 对于序列容器（vector、string、deque和list），把remove换成remove_if，
// 就是删除使badValue返回true的对象的最好办法：
void func1() {
  std::vector<int> v{1, 2, 3, 4, 5, 3};
  v.erase(std::remove_if(v.begin(), v.end(), badValue1), v.end());
  std::copy(v.begin(), v.end(), std::ostream_iterator<int>(std::cout, " "));
...
void func2() {
  std::deque<int> d{1, 2, 3, 4, 5, 3};
  d.erase(std::remove_if(d.begin(), d.end(), badValue1), d.end());
  std::copy(d.begin(), d.end(), std::ostream_iterator<int>(std::cout, " "));
  std::cout << std::endl;  // 2 4
}
...
void func3() {
  std::string str = "123453";
  str.erase(std::remove_if(str.begin(), str.end(), badValue1), str.end());
  std::copy(str.begin(), str.end(),
            std::ostream_iterator<char>(std::cout, " "));
  std::cout << std::endl;  // 2 4
}
...
void func4() {
  std::list<int> l{1, 2, 3, 4, 5, 3, 4};
  l.erase(std::remove_if(l.begin(), l.end(), badValue1), l.end());
  std::copy(l.begin(), l.end(), std::ostream_iterator<int>(std::cout, " "));
  std::cout << std::endl;  // 2 4 4
  l.remove_if(badValue2);
  std::copy(l.begin(), l.end(), std::ostream_iterator<int>(std::cout, " "));
  std::cout << std::endl;  // 2
}
...
void func5() {
  // 简单但效率稍低的办法是，利用remove_copy_if把需要的值拷贝到一个新容器中，
  // 然后把原来容器的内容和新容器的内容互换，缺点是需要拷贝所有不被删除的元素：
...
void func6() {
  ...
  // 关联容器没有提供类似remove_if的成员函数，所以需要遍历元素，并删除元素。
  // 比如，下面是很多程序员首先能想到的：
  ...
  //   }
  // }
  // 这会导致不确定的行为，当容器中的一个元素被删除时，
  // 指向该元素的所有迭代器都将变得无效，为了避免这个问题，
  // 要确保在调用erase之前，有一个迭代器指向容器中的下一个元素。
  // 最简单的办法是，调用时对i使用后缀递增，后缀返回的是i的副本：
...
void func2() {
  // 但对于vector、string和deque，不能使用erase-remove惯用方法了，
  // 因为无法向日志中写信息，而且也不能使用关联容器的方法，因为会导致不确定的行为。
  // 这类容器调用erase不仅会使指向被删除元素的迭代器失效，也会使之后的所有迭代器都无效。
...
void func3() {
  // 对于list，遍历和删除时，可以把list当作vector/string/deque来对对待，
  // 也可以把它当作关联容器来对待，两种方式对list都适用，

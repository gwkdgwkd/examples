...
// };
// 这不能工作，编译器根据调用成员函数的对象的const属性，
// 来选择此成员函数的const和非const版本，而不考虑调用时的环境。

// 也许不可能在operator[]内部区分左值还是右值操作，
// 但仍然能区别对待读操作和写操作，如果将判断读还是写的行为，
// 推迟到知道operator[]的结果被怎么使用之后的话。
// 所需要的是有一个方法将读或写的判断推迟到operator[]返回之后。
// proxy类可以让我们得到我们所需要的时机，
// 让operator[]返回一个proxy对象代理字符，而不是字符本身。
// 可以等着看这个proxy怎么被使用。
...
String::CharProxy& String::CharProxy::operator=(const CharProxy& rhs) {
  // 将写操作的实现移入CharProxy的赋值操作中，
  // 于是可以避免非const的operator[]的调用，
  // 只是作右值时所多付出的写操作的代价。
...
void func2() {
  ...
  cout << s1[5] << endl;
  // calls non-const operator[], because s1 isn't const
  s2[5] = 'x';
  // also calls non-const operator[]: s2 isn't const
  s1[3] = s2[8];
  // both calls are to non-const operator[], because both s1
...
// 出现带引用计数的数组模板中，
// 如果想用proxy类来区分其operator[]作左值还是右值时：
...
// 同时，proxy类也有缺点。
// 作为函数返回值，proxy对象是临时对象，它们必须被构造和析构。

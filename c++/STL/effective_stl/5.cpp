...
// 对于标准序列容器，为了取得同样的结果，使用单元素的成员函数比使用区间成员函数需要更多地调用内存分配子，更频繁地拷贝对象，而且或者做冗余操作。
// 相对于区间版本的insert，单元素版本的insert总共在三个方面影响效率：
//  1 不必要的函数调用。调用了多次insert函数。使用区间的成员函数，只调用了一次。使用内联可以避免这样的影响，但实际中不见得会使用内联。
//  2 元素移动的多，没插入一次都要移动后面所以的元素。使用区间的成员函数，只需要每元素只需要移动一次，移到最终的位置。
//  3 使用单元素成员函数，可能会导致多次分配内存。
// 这些问题对string、deque同样适用。
// list使用区间成员函数也有效率的优势。重复调用的论断继续生效。还有新问题，对某些节点的next和prev指针的重复的、多余的赋值操作。

// 对于标准序列容器，在单元素的插入和区间的插入之间做选择所依据的不只是程序风格的考虑。对于关联容器，效率问题更加难以说清楚。
...
//  void container::insert(InputIterator Begin, InputIterator end);  // 关联容器利用比较函数来决定元素插入位置，不用position
// 区间删除：所有的标准容器都提供了区间形式的删除操作。
//  iterator container::insert(InputIterator Begin, InputIterator end);  // 序列容器
//  void container::insert(InputIterator Begin, InputIterator end);  // 关联容器

...
class Y : public virtual X {
  ...
  //   它或者指向virtual base class subobject，
  //   或者指向一个表格，
...
// 不管是virtual或nonvirtual base class。
// 不过并没有强调定义其间的排列顺序。
...
// 在C++最早的编译器上，
// 如果Point3d1::X()的两个函数实例中对x进行读取操作，
// 这操作将会指向全局的x。
// 这样的绑定结果几乎遍地都是，但不在大家预览之中，
...
const char* asscess_order(data_type1 class_type::*mem1,
  ...
  // assert(mem1 != mem2);
  // return mem1 < mem2 ? "member 1 occurs first" :
...
void func2() {
  cout << asscess_order(&Test::z, &Test::y) << endl;
  // member 2 occurs first
}
...
// }
// 看上去是对于x，y，z的直接存取，
// 事实上是经由一个implicit class object（由this指针表达）完成。
// 事实上这个函数的参数是：
...
// 这样设计的好处就是可以把管理x和y的坐标的程序代码局部化，
// 还可以明显表现出两个抽象类之间的紧密关系。
...
// *pc1_2 = *pc1_1;  // 执行默认的memberwise复制，复制Concrete1那部分
// 如果pc1_1实际指向一个Concrete2或Concrete3对象，
// 则上述操作应该复制内容给Concrete1 subobject。
...
void func7() {
  ...
  p3d = &v3d;  // 简单地拷贝地址就行了
  pv = pv3d;
  // 对于引用，则不需要针对可能的0值做防卫，因为引用不可能参考到无物。
  // 转换为：
...
// 一般的布局策略是先安排好派生类的不变部分，
// 然后再建立其共享部分，然而，存在一个问题：
// 如何能够存取类中的共享部分呢？
...
// 最少也是x和y的大小的总和，
// 因为C++要求用一个access level中的成员的排列次序应该和声明次序相同。
void func9() {
  Point3d_6 dd6;
  dd6.fn();
  // 0x8, 0x7fff7ac65498
  ...
  // 如何区分一个没有指向任何数据成员的指针，
  // 和一个指向第一个数据成员的指针？
  // Point3d_6::*是指向Point3d_6数据成员的指针类型：
  float Point3d_6::*p1 = 0;
  float Point3d_6::*p2 = &Point3d_6::x;
  if (p1 == p2) {
    // 每一个真正的成员offset值都被加上1，
    // 编译器和使用者都必须在使用前，先减掉1。
  }
}

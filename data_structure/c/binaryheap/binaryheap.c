...
// 分别是insert插入一个元素和delMax删除最大元素（如果底层用最小堆，那么就是delMin）。
...
// 对于最大堆，会破坏堆性质的有有两种情况：
// 如果某个节点A比它的子节点（中的一个）小，那么A就不配做父节点，应该下去，
// 下面那个更大的节点上来做父节点，这就是对A进行下沉。
// 如果某个节点A比它的父节点大，那么A不应该做子节点，应该把父节点换下来，
// 自己去做父节点，这就是对A的上浮。
...
// 因为我们时间复杂度主要花费在sink或者swim上，而不管上浮还是下沉，

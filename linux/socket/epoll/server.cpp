...
// 这个特殊的描述符就是epoll实例的句柄，
// 后面的两个接口都以它为中心，即epfd形参。
// size参数表示所要监视文件描述符的最大值，
// 不过在后来的Linux版本中已经被弃用。
...
// 1.timeout=-1表示调用将一直阻塞，
//   直到有文件描述符进入ready状态或者捕获到信号才返回；
// 2.timeout=0用于非阻塞检测是否有描述符处于ready状态，
//   不管结果怎么样，调用都立即返回；
...
// epoll支持边缘触发(edge trigger，ET)或水平触发（level trigger，LT)，
...
// 3.当缓冲区有数据可读，
//   且应用进程对相应的描述符进行EPOLL_CTL_MOD修改EPOLLIN事件时。
// 对于写操作：
// 1.当缓冲区由不可写变为可写时。
// 2.当有旧数据被发送走，即缓冲区中的内容变少的时候。
// 3.当缓冲区有空间可写，
//   且应用进程对相应的描述符进行EPOLL_CTL_MOD修改EPOLLOUT事件时。
// 当被监控的文件描述符上有可读写事件发生时，epoll_wait()会通知处理程序去读写。
// 如果这次没有把数据全部读写完(如读写缓冲区太小)，
// 那么下次调用epoll_wait()时，它不会通知你，也就是它只会通知你一次，
// 直到该文件描述符上出现第二次可读写事件才会通知你。
// 这种模式比水平触发效率高，系统不会充斥大量你不关心的就绪文件描述符。
// 在ET模式下，缓冲区从不可读变成可读，会唤醒应用进程，
// 缓冲区数据变少的情况，则不会再唤醒应用进程。
...
// 2.内核态检测文件描述符读写状态的方式：
//   (1).select：采用轮询方式，遍历所有fd，
//       最后返回一个描述符读写操作是否就绪的mask掩码，
//       根据这个掩码给fd_set赋值。
//   (2).poll：同样采用轮询方式，查询每个fd的状态，
//       如果就绪则在等待队列中加入一项并继续遍历。
//   (3).epoll：采用回调机制。
//       在执行epoll_ctl的add操作时，
//       不仅将文件描述符放到红黑树上，而且也注册了回调函数，
//       内核在检测到某文件描述符可读/可写时会调用回调函数，
//       该回调函数将文件描述符放在就绪链表中。
...
//       这里返回的文件描述符是通过mmap让内核和用户空间共享同一块内存实现传递的，
//       减少了不必要的拷贝。
// 4.重复监听的处理方式

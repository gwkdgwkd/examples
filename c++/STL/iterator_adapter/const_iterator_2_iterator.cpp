...
// C++ STL标准库为了方便用户更轻松地操作容器，
// 每个容器的模板类都提供有丰富且实用的方法。
// 在这些方法中，有些是以const_iterator类型迭代器作为参数，
// 也就意味着在使用此类方法时，需要为其传入一个const_iterator类型的迭代器。
// vector容器模板类中提供有insert()方法，该方法的语法格式如下：
// iterator insert(const_iterator position, const value_type& val);
// 是不是给insert()方法传递其它类型迭代器就不行呢？
// 当然不是，对于给const_iterator类型的迭代器传值，
// 还可以使用iterator类型迭代器，
// 但不能使用const_reverse_iterator和reverse_iterator类型迭代器，
// 这是为什么呢？
// 当将某一类型的迭代器传递给insert()方法中，
// const_iterator类型的position形参时，即便类型不匹配，
// 编译器也不会立即报错，而是先尝试将其类型转换成const_iterator类型，
// 如果转换成功，则程序仍可以正常执行；
// 反之如果转换失败，编译器才会报错。
// 通常将编译器自行尝试进行类型转换的整个过程称为隐式转换（或者自动类型转换）。

// 对于C++ STL标准库中的这4种基础迭代器来说，
// C++编译器的隐式转换仅支持以下2种情况：
// 1.将iterator类型的迭代器隐式转换为const_iterator类型的迭代器；
// 2.将reverse_iterator类型的迭代器，
//   隐式转换为const_reverse_iterator类型的迭代器。
...
// 其中，citer为指向某个容器任意位置的，
// const_iterator或者const_reverse_iterator类型迭代器，
// 而iter通常初始为，
// 指向容器中第一个元素的iterator（或reverse_iterator）类型迭代器。
...
// 该实现方式的本质是，先创建一个迭代器citer，
// 并将其初始化为指向容器中第一个元素的位置。
// 在此基础上，通过计算和目标迭代器iter的距离（调用distance()），
// 将其移动至和iter同一个位置（调用advance()），
// 由此就可以间接得到一个指向同一位置的iter迭代器。
// 此方法的实现效率仍取决于目标容器的迭代器类型，
// 如果是随机访问迭代器，则该方法的执行效率为O(1)；
// 反之，则执行效率为O(n)。

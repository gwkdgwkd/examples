...
// 拷贝构造函数的实现原理很简单，
// 就是为新对象复制一份和其它对象一模一样的数据。
// 当类中拥有指针类型的成员变量时，
// 拷贝构造函数中需要以深拷贝的方式复制该指针成员。
// 即拷贝该指针成员本身的同时，还要拷贝指针指向的内存资源，
// 否则一旦多个对象中的指针成员指向同一块堆空间，
// 这些对象析构时就会对该空间释放多次，这是不允许的。

// 非const右值引用只能操作右值，
// 程序执行中产生的临时对象（函数返回值、lambda表达式等），
...
// 如果使用左值初始化同类对象，但也想调用移动构造函数完成，
// 有没有办法可以实现呢？
// 默认情况下，左值初始化同类对象只能通过拷贝构造函数完成，
// 如果想调用移动构造函数，则必须使用右值进行初始化。
// C++11标准中为了满足用户使用左值初始化同类对象时，
// 也通过移动构造函数完成的需求，新引入了std::move()函数，
// 它可以将左值强制转换成对应的右值，便可以使用移动构造函数。
// 其语法格式为：move(arg);

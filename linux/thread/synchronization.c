...
// 通常将多个线程同时访问某一公共资源的现象，称为线程间产生了资源竞争或者线程间抢夺公共资源，
// 线程间竞争资源往往会导致程序的运行结果出现异常，感到匪夷所思，严重时还会导致程序运行崩溃。

// Linux提供了很多种解决方案，确定各个线程可以同步访问进程提供的公共资源，简称线程同步。
// 所谓线程同步，简单地理解就是：
// 当一个线程访问某公共资源时，其它线程不得访问该资源，
// 它们只能等待此线程访问完成后，再逐个访问该资源。

// Linux环境中，实现线程同步的常用方法有4种：
// 1.互斥锁（Mutex）又称互斥量或者互斥体，是最简单也最有效地一种线程同步机制。
...
//   仅有少量的线程对公共资源进行修改，这种情况下可以使用读写锁解决线程同步问题。
...
// 1.unlock表示当前资源可以访问，第一个访问资源的线程负责将互斥锁的值改为lock，
//   访问完成后再重置为unlock；
// 2.lock表示有线程正在访问资源，其它线程需等待互斥锁的值为unlock后才能开始访问。
...
// mutex参数表示要初始化的互斥锁；
// attr参数用于自定义新建互斥锁的属性，attr的值为NULL时表示以默认属性创建互斥锁。
// pthread_mutex_init()函数成功完成初始化操作时，返回数字0，否则返回非零数。
...
// pthread_mutex_unlock()函数用于对指定互斥锁进行解锁操作，
// pthread_mutex_lock()和pthread_mutex_trylock()
// 函数都用于实现加锁操作，不同之处在于当互斥锁已经处于加锁状态时：
// 1.执行pthread_mutex_lock()函数会使线程进入等待（阻塞）状态，直至互斥锁得到释放；
// 2.执行pthread_mutex_trylock()函数不会阻塞线程，直接返回非零数（表示加锁失败）。
...
// 更重要的是，信号量还支持做加1或者减1运算，且修改值的过程以原子操作的方式实现。

// 原子操作是指当多个线程试图修改同一个信号量的值时，各线程修改值的过程不会互相干扰。
// 例如信号量的初始值为1，此时有2个线程试图对信号量做加1操作，
// 则信号量的值最终一定是3，而不会是其它的值。
// 反之若不以原子操作方式修改信号量的值，那么最终的计算结果还可能是2，
...
// POSIX标准中，信号量用sem_t类型的变量表示，该类型定义在<semaphore.h>头文件中。
...
// 参数sem都表示要操作的目标信号量。
// 各个函数的功能如下：
// 1.sem_post()函数的功能是：将信号量的值加1，同时唤醒其它等待访问资源的线程；
// 2.当信号量的值大于0时，sem_wait()函数会对信号量做减1操作；
//   当信号量的值为0时，sem_wait()函数会阻塞当前线程，
//   直至有线程执行sem_post()函数，暂停的线程才会继续执行；
// 3.sem_trywait()函数的功能和sem_wait()函数类似，唯一的不同在于，
//   当信号量的值为0时，sem_trywait()函数并不会阻塞当前线程，而是立即返回-1；
...
//   // 待条件成立后，执行后续的代码
// }
// 当线程执行此函数时，会判断x的值是否等于10，如果不等则间隔5秒后再重复判断，
...
// 为了避免多线程之间发生抢夺资源的问题，条件变量在使用过程中必须和一个互斥锁搭配使用。

// POSIX标准中，条件变量用pthread_cond_t类型的变量表示，定义在<pthread.h>头文件中。
...
// 那么首先要得到当前系统的时间，然后再加上5秒，最终得到的时间才是传递的实参值。
// 调用两个函数之前，必须先创建好一个互斥锁并完成加锁操作，然后才能作为实参传递给mutex参数。
// 两个函数会完成以下两项工作：
// 1.阻塞线程，直至接收到条件成立的信号；
...
// 2.pthread_cond_timedwait()函数只能在abstime参数指定的时间内阻塞线程，超出时限后，
//   该函数将重新对互斥锁执行加锁操作，解除对线程的阻塞，函数的返回值为ETIMEDOUT。
// 如果函数成功接收到了条件成立的信号，重新对互斥锁完成了加锁并使线程继续执行，
// 函数返回数字0，反之则返回非零数。
...
//   如果等待队列中包含多个线程，优先解除哪个线程将由操作系统的线程调度程序决定；
// 2.pthread_cond_broadcast()函数可以解除等待队列中所有线程的被阻塞状态。
// 由于互斥锁的存在，解除阻塞后的线程也不一定能立即执行。
// 当互斥锁处于加锁状态时，解除阻塞状态的所有线程会组成等待互斥锁资源的队列，等待互斥锁解锁。
...
// 值得一提的是，销毁后的条件变量还可以调用pthread_cond_init()函数重新初始化后使用。
...
// 当发出读请求的线程正在执行时，发出写请求的线程必须等待前者执行完后才能开始执行；
// 当发出写请求的线程正在执行时，发出读请求的线程也必须等待前者执行完后才能开始执行。
...
//   注意，由于读请求和写请求的线程不能同时执行，读写锁默认会优先分配给发出读请求的线程。
...
//   此时无论其它线程发出的是读请求还是写请求，都必须等待读写锁状态改为无锁后才能执行。
...
//   当线程因pthread_cond_wait()函数被阻塞时，一定要保证有其它线程唤醒此线程；
//   无论线程占用的是读锁还是写锁，都必须及时解锁。
//   注意，函数中可以设置多种结束执行的路径，
//   但无论线程选择哪个路径结束执行，都要保证能够将占用的资源释放掉。
// 2.POSIX标准中，很多阻塞线程的函数都有tryxxx()和timexxx()两个版本，
//   例如pthread_mutex_lock()和pthread_mutex_trylock()、sem_wait()和sem_trywait()、
//   pthread_cond_wait()和pthread_cond_timedwait()等，它们可以完成同样的功能，
//   但tryxxx()版本的函数不会阻塞线程，timexxx()版本的函数不会一直阻塞线程。
//   实际开发中，优先选择tryxxx()或者timexxx()版本的函数，可以大大降低线程产生死锁的概率。

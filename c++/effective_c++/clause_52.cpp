...
// placement new意味带任意额外参数的new，
// placement delete直接派生自它。
...
class Widget2 {
 public:
  ...
  // 所以必须同时提供两个版本：
  // placement delete，构造函数抛出异常；
  // 正常的operator delete，没有异常。
...
int main() {
  ...
  // 共有两个函数被调用，一个是operator new，
  // 一个是Widget的default构造函数。
  // 假设new调用成功，default构造函数抛出异常。
  // 那么new分配的内存必须取消并恢复原样，否则会造成内存泄露。
  // 这个时候客户无法归还内存，因为pw尚未被赋值，
  // 客户手上也就没有指针指向未被归还的内存。
  // 取消步骤需要C++运行期系统身上。

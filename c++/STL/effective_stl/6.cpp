...
class Widget {};
// 没有声明对象，声明了一个函数，不带任何参数，返回Widget：
Widget w();
...
int main() {
  ...
  // 第二个参数没有名称，它的类型是指向不带参数的函数的指针，
  // 该函数返回一个istream_iterator<int>。

  // 把形式参数的声明用括号括起来是非法的，
  // 但给函数参数加上括号却是合法的，所以通过增加一对括号：
  std::list<int> data1((std::istream_iterator<int>(dataFile)),
                       std::istream_iterator<int>());
  // 有的编译器不支持这样声明data

  // 更好的方式是在对data的声明中避免使用匿名istream_iterator对象，
  // 而是给些迭代器一个名称：

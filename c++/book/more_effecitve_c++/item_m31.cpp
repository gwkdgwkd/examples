...
// 但现在，取决于两个对象的动态类型，虚函数体系只能作用在一个对象上，它不足以解决问题。
...
// C++没有直接提供double dispatch，所以必须自己完成编译器在实现虚函数时所做的工作。
...
// 认识到自己的脆弱，在show函数中加上了最后的那个else语句，以处理未知类型。
...
void DA::showDA(B &b) {
  ...
  // 当然，它们从不会失败，因为函数被调用时不会带错误类型的参数，谨慎点更好。
...
// 因为这张表包含的是指向成员函数的指针，所以在增加新类型时仍然需要修改类定义，
...
// 这个有特别的含义吗？是不是这样更好些：
// 类型A和类型B的函数应该既不在A中也不在B中处理，而在两者之外的某个中立的地方处理？
// 如果将处理函数从类里移出来，在给用户提供类定义的头文件时，不用带上任何碰撞处理函数。

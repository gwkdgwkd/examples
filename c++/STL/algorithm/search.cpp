...
// find_end()函数用于在序列A中查找序列B最后一次出现的位置。那么，如果想知道序列B在序列A中第一次出现的位置，该如何实现呢？可以借助search()函数。
...
// 同时，search()函数会返回一个正向迭代器，当函数查找成功时，该迭代器指向查找到的子序列中的第一个元素；反之，如果查找失败，则该迭代器的指向和last1迭代器相同。
// search()函数的第一种语法格式，其底层是借助==运算符实现的。这意味着，如果[first1, last1]和[first2, last2]区域内的元素为自定义的类对象或结构体变量时，使用该函数之
// 前需要对==运算符进行重载。
...
// search_n() 函数的语法格式如下：
// 在[first, last]中查找count个val 第一次连续出现的位置
// ForwardIterator search_n (ForwardIterator first, ForwardIterator last,
//                           Size count, const T& val);
// 在[first, last]中查找第一个序列，该序列和count个val满足pred匹配规则
// ForwardIterator search_n ( ForwardIterator first, ForwardIterator last,
//                            Size count, const T& val, BinaryPredicate pred );
// 其中，各个参数的含义分别为：
//  first、last：都为正向迭代器，其组合[first, last)用于指定查找范围（也就是上面例子中的序列 A）；
//  count、val：指定要查找的元素个数和元素值；
//  pred：用于自定义查找规则。该规则实际上是一个包含2个参数且返回值类型为bool的函数（第一个参数接收[first, last)范围内的元素，
//        第二个参数接收val）。函数定义的形式可以是普通函数，也可以是函数对象。
// 实际上，第一种语法格式也可以看做是包含一个默认的pred参数，该参数指定的是一种相等规则，即在[first, last)范围内查找和count个val相等的子序列；
// 而借助第二种语法格式，我们可以自定义一个当前场景需要的匹配规则。
// 同时，search_n()函数会返回一个正向迭代器，当函数查找成功时，该迭代器指向查找到的子序列中的第一个元素；反之，如果查找失败，则该迭代器的指向和last迭代器相同。
// search_n()函数的第一种语法格式，其底层是借助==运算符实现的。这意味着，如果[first, last]区域内的元素为自定义的类对象或结构体变量时，
...
int main() {
  ...
  vector<int>::iterator it =
      search(myvector.begin(), myvector.end(), myarr, myarr + 3);
  if (it != myvector.end()) {
    cout << "第一个{1,2,3}的起始位置为：" << it - myvector.begin()
         << ",*it = " << *it << endl;
  }
  ...
  if (it != myvector.end()) {
    cout << "第一个{2,3,4}的起始位置为：" << it - myvector.begin()
         << ",*it = " << *it << endl;
  }
  // 第一个{2,3,4}的起始位置为：3,*it = 4

  int a[] = {1, 2, 3, 4, 4, 4, 1, 2, 3, 4, 4, 4};
  int* it1 = search_n(a, a + 12, 3, 4);
  if (it1 != a + 12) {
    cout << "one：" << it1 - a << ",*it = " << *it1 << endl;
  }
  // one：3,*it = 4
  vector<int> mv{1, 2, 4, 8, 3, 4, 6, 8};
  vector<int>::iterator iter = search_n(mv.begin(), mv.end(), 3, 2, mycomp2());
  if (iter != mv.end()) {
    cout << "two：" << iter - mv.begin() << ",*iter = " << *iter << endl;
  }
  // two：1,*iter = 2

...
// 真正的并发是在多核CPU上，能够真正的同时执行多个任务，称为硬件并发。
...
// 这些独立的进程可以通过常规的进程间通信机制进行通信，
// 如管道、信号、消息队列、共享内存、存储映射I/O、信号量、套接字等。
// 缺点：
...
void func() {
  ...
  // 移动赋值操作符：
  t1 = std::thread(func1);  // 临时对象，所以使用的是移动赋值
  t1.join();
  for (int i = 0; i < 2; i++) {
  ...
  // 移动拷贝构造函数：
  std::thread t5(std::thread(func1));
...
// 使用joinable()函数检查线程是否可被join，检查线程是否为活动的执行线程。
// 有趣的是，detach()可否调用也是使用joinable()函数来判断。

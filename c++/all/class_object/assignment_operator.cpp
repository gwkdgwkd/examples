...
// 如果一个类没有定义所有这些拷贝控制成员，编译器会自动为它定义默认的操作，
// 因此很多类会忽略这些拷贝控制操作，但是，对于一些持有其他资源的类来说，
...
// 如果需要定义析构函数，那么几乎可以肯定也需要一个拷贝构造函数和一个赋值运算符。
...
// 总之：
// 1.如果需要定义析构函数，那么几乎可以肯定也需要定义拷贝构造函数和赋值运算符；
// 2.如果类需要拷贝构造函数，几乎可以肯定它也需要一个赋值运算符，反之亦然；
// 3.无论需要拷贝构造函数还是需要赋值运算符，都不必然意味着也需要析构函数。
...
void func() {
  ...
  // 由于调用了默认的赋值操作运算符，
  // arr1和arr2的m_p指向同一块内存，free时导致崩溃：

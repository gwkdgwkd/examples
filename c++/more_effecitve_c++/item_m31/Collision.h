...
void shipShip(GameObject3& spaceShip, GameObject3& shipStation) {
  cout << "spaceShip3 and spaceShip3" << endl;
}
void stationStation(GameObject3& spaceStation1, GameObject3& spaceStation2) {
  cout << "shipStation3 and shipStation3" << endl;
}
void asteroidAsteroid(GameObject3& asteroid1, GameObject3& asteroid2) {
  cout << "asteroid3 and asteroid3" << endl;
}
...
// 这个类允许我们在映射表中进行增加和删除操作，
// 以及根据类型名对查找相应的碰撞处理函数。
// 它也使用了Item E26中讲的技巧来限制CollisionMap对象的个数为1，
// 因为我们的系统中只有一个映射表。
// 更复杂的游戏需要多张映射表是可以想象到的。
// 最后，它允许我们简化在映射表中增加对称性的碰撞的过程，也就是说，
// 类型T1的对象撞击T2的对象和T2的对象撞击T1的对象，其效果是相同的。
// 它自动增加对称的映射关系，
// 只要addEntry被调用时可选参数symmetric被设为true。
class CollisionMap {
 public:
  typedef void (*HitFunctionPtr)(GameObject3&, GameObject3&);
...
// 一个方法是让GameObject的子类在构造函数中进行确认。
// 这将导致在运行期的一个小小的性能开销。
// 另外一个方法是创建一个RegisterCollisionFunction类：

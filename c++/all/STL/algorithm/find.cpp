...
namespace n1 {
// find()本质上是模板函数，用于在指定范围内查找和目标元素值相等的第一个元素。
...
//   return last;
// }
// 其中，first和last为输入迭代器，[first,last)用于指定该函数的查找范围；
// val为要查找的目标元素。
// 正因为first和last的类型为输入迭代器，因此该函数适用于所有的序列式容器。
// 另外，该函数会返回一个输入迭代器。
// find()的底层实现，其实就是用==将val和[first,last)区域内的元素逐个进行比对。
// 这也就意味着，[first,last)区域内的元素必须支持==运算符。
// find()函数除了可以作用于序列式容器，还可以作用于普通数组。

void func() {
  // 如果查找成功时，其指向的是在[first,last)区域内查找到的第一个目标元素：
  std::vector<int>::iterator it = std::find(v1.begin(), v1.end(), 30);
  ...
  std::cout << *(v1.begin() + 3) << std::endl;
  it = std::find(v1.begin(), v1.begin() + 3, 60);
...
// 值得一提的是，该函数可以是一个普通函数，也可以是函数对象，称为一元谓词函数。
...
// 其中，first和last都为输入迭代器，其组合[first,last)用于指定要查找的区域；
// pred用于自定义查找规则。
// 由于first和last都为输入迭代器，意味着该函数适用于所有的序列式容器。
// 甚至当采用适当的谓词函数时，该函数还适用于所有的关联式容器（包括哈希容器）。
// 该函数会返回输入迭代器，当查找成功时，指向的是第一个符合查找规则的元素；
...
void func() {
  auto it = std::find_if(v1.begin(), v1.end(), mycomp1);
  print(it, v1);  // not found

  it = std::find_if(v2.begin(), v2.end(), mycomp2());
  print(it, v2);  // *it = 21

  auto it1 = std::find_if(v2.rbegin(), v2.rend(), mycomp2());
  std::cout << *it1 << std::endl;  // 41
}
}  // namespace n2
...
//                           InputIterator last, UnaryPredicate pred);
// 其中，first和last都为输入迭代器，[first,last)用于指定查找范围；
// pred用于自定义查找规则。
...
void func() {
  auto it = std::find_if_not(v3.begin(), v3.end(), mycomp1);
  print(it, v3);  // not found

  it = std::find_if(v3.begin(), v3.end(), mycomp3);
  print(it, v3);  // not found

  it = std::find_if_not(v2.begin(), v2.end(), mycomp2());
  print(it, v2);  // *it = 10
...
namespace n4 {
// find_end()定义在<algorithm>中，常用于在序列A中查找序列B最后一次出现的位置。
// find_end()函数的语法格式有2种：
...
// 查找序列[first1,last1)中，和[first2,last2)满足pred规则的最后一个子序列，
// ForwardIterator find_end(ForwardIterator first1,
//                          ForwardIterator last1,
//                          ForwardIterator first2,
//                          ForwardIterator last2,
...
// pred：用于自定义查找规则，该规则是一个包含2个参数且返回值类型为bool的函数，
//       参数1接收[first1,last1)内的元素，参数2接收[first2,last2)内的元素。
//       函数定义的形式可以是普通函数，也可以是函数对象。

// 实际上，第一种语法格式也可以看做是包含一个默认的pred参数，相等规则：
...
void func() {
  ...
  it = std::find_end(v.begin(), v.end(), a + 3, a + 5);
  if (it == v.end()) {
    std::cout << "not found" << std::endl;
  }
  // not found

  it = std::find_end(v.begin(), v.end(), a + 3, a + 5, mycomp1);
  // it = std::find_end(v.begin(), v.end(), a + 3, a + 5, mycomp2());
  if (it != v.end()) {
    std::cout << "最后一个{2,4,6}的起始位置为：" << it - v.begin()
              << ",*it = " << *it << std::endl;
  }
  // 最后一个{2,4,6}的起始位置为：4,*it = 8
}
}  // namespace n4

namespace n5 {
// 需要在A序列中查找和B序列中任意元素相匹配的第一个元素，可以用find_first_of()。
...
// 如果函数返回true则匹配成功。
// 当采用第一种语法格式时，[first1,last1)或者[first2,last2)范围内的元素，
// 如果类型为自定义的类或者结构体，此时应对==进行重载，使其适用于当前场景。
...
//                               ForwardIterator last,
//                               BinaryPredicate pred);
// 其中，first和last都为正向迭代器，[first,last)用于指定该函数的查找范围；
// pred用于接收一个包含2个参数且返回值类型为bool的函数，以实现自定义查找规则。
// 值得一提的是，pred参数接收的函数既可以定义为普通函数，也可以用函数对象。
// 函数返回正向迭代器，当函数查找成功时，指向的是连续相等元素的第一个元素；

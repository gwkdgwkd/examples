...
// 一类是在查找的过程中已经包含在树中的（假设为A类），剩下的是另一类（假设为B类）。
// 对于给定的连通网，起始状态全部顶点都归为B类。
// 在找最小生成树时，选定任意一个顶点作为起始点，并将之从B类移至A类；
...
// 最小生成树之普里姆算法：
// 该算法从顶点的角度为出发点，时间复杂度为O(n^2)，更适合与解决边的绸密度更高的连通网。
...
// 2.对于具有n个顶点的连通网，其生成树中只能有n-1条边，这n-1条边连通着n个顶点。
// 连接n个顶点在不产生回路的情况下，只需要n-1条边。
// 所以克鲁斯卡尔算法的具体思路是：
// 将所有边按照权值的大小进行升序排序，然后从小到大一一判断，条件为：
// 如果这个边不会与之前选择的所有边组成回路，就可以作为最小生成树的一部分；反之，舍去。
// 直到具有n个顶点的连通网筛选出来n-1条边为止。
// 筛选出来的边和所有的顶点构成此连通网的最小生成树。
// 判断是否会产生回路的方法为：在初始状态下给每个顶点赋予不同的标记，对于遍历过程的每条边，
// 其都有两个顶点，判断这两个顶点的标记是否一致，如果一致，
// 说明它们本身就处在一棵树中，如果继续连接就会产生回路；

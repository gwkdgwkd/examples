...
// 1929年，波兰逻辑学家J・卢卡西维兹提出了一种全新的表示表达式的方法，称为后缀表达式或者逆波兰表达式。和普通表达式不同，后缀表达式习惯将运算符写在它的运算项之后，
...
// 以3!为例，!为运算符，3为运算项，因此3!本身就是一个后缀表达式；再以4*2为例，*为运算符，4和2作为它的运算项，其对应的后缀表达式为42+。
// 在此基础上，试着将 3!+4*2/(1-5)^2转换成后缀表达式，其过程也就是将表达式中所有运算符放置在它的运算项之后：
//  !运算符对应的运算项为3，转换后得到3 !；
//  +运算符对应的运算项是3!和 4*2/(1-5)^2，转换之后得到：3! 4*2/(1-5)^2 +；
...
// 不难发现，后缀表达式完全舍弃了表达式本该有的可读性，但有失必有得，相比普通表达式，后缀表达式的值可以轻松借助栈存储结构求得。
// 具体求值的过程是：当用户给定一个后缀表达式时，按照从左到右的顺序依次扫描表达式中的各个运算项和运算符，对它们进行如下处理：
//  遇到运算项时，直接入栈；
//  遇到运算符时，将位于栈顶的运算项出栈，对于!运算符，取栈顶1个运算项；其它运算符，取栈顶2个运算项，第一个取出的运算项作为该运算符的右运算项，另一个作为左运算项。求此表达式的值并将其入栈。
// 经过以上操作，直到栈中仅存在一个运算项为止，此运算项即为整个表达式的值。
// 以3 ! 4 2 * 1 5 - 2 ^ / +表达式为例，求值的过程为：
//  1) 从3开始，它是运算项: [ 3 ]
//  2) !作为运算符，从栈顶取1个运算项（也就是3），求3!的值（3! = 3*2*1=6）并将其入栈： [ 6 ]
//  3) 将4和2先后入栈： [ 6 4 2 ]
//  4) 对于*运算符，取栈顶2个运算项（2和4），其中先取出的2作为*的右操作数，4作为左操作数。求的4*2的值8 ，并将其入栈： [ 6 8 ]
//  5) 将1和5先后入栈： [ 6 8 1 5 ]
//  6) 对于-运算符，取栈顶2个运算项（5 和 1），计算出1-5的值为-4，将其入栈： [ 6 8 -4 ]
//  7) 将2入栈： [ 6 8 -4 2 ]
...
// 后缀表达式的转换
//  对于普通用户来讲，另其输入一个正确的后缀表达式显然是不实现的，我们只能要求他们输入一个正确的普通表达式。这就引出了一个问题，即如何将一个普通表达式转换成后缀表达式？
//  幸运的是，针对这个问题，伟人迪杰斯特拉给出了一个完美的解决方案，称为调用场算法，该算法可以实现将一个普通表达式转换成后缀表达式。
//  调用场算法的实现，需要借助一个空栈（假设栈名为Optr）和一个空数组（假设数组名为postexp）。对于给定的一个普通表达式，调用场算法的转换过程是：逐个遍历表达式中的每个字符：
//   如果为'0'~'9'的字符，将其添加到postexp数组的末尾；
//   如果该字符为除‘(’和')'以外的运算符，将其与Optr栈顶的运算符进行大小比较，如果该运算符大于栈顶运算符，则将其入栈；
//    反之，如果该运算符小于栈顶运算符，则将栈顶运算符出栈并添加到postexp数组的尾部，然后继续拿当前运算符同新的栈顶运算符做大小比较，以此类推。
//   如果该字符为'('运算符，直接入栈；如果为')'运算符，依次取Optr栈顶运算符并将其添加到postexp数组末尾，直到遇到'('字符为止。注意，'('字符也从栈顶取出，但不将其添加postexp数组中。
//  依照以上处理过程，直到将普通表达式遍历完毕，如果Optr栈中仍有运算符，依次将它们出栈并添加到postexp数组尾部。最终，postexp数组内存储的表达式就是转换后的后缀表达式。
...
//  2) 遍历至!，将其与Optr栈顶字符进行比较，由于此时Optr为空栈，因此直接将!入栈：ostexp[ 3 ],Optr[ ! ]
//  3) 遍历至+，Optr栈顶运算符! > +，将!从Optr栈中取出并添加到postexp数组末尾。此时，Optr栈为空，将+入栈：ostexp[ 3 ! ],Optr[ + ]
...
//  12) 遍历至)，对Optr栈一直做出栈操作并将出栈元素添加到postexp数组末尾，直到将(取出：ostexp[ 3 ! 4 2 * 1 5 - ],Optr[ + / ]
//  13) 遍历至^，Optr栈顶运算符/ < ^，将^入栈：ostexp[ 3 ! 4 2 * 1 5 - ],Optr[ + / ^ ]
//  14) 遍历至2，将其添加到 postexp 数组末尾：ostexp[ 3 ! 4 2 * 1 5 - 2 ],Optr[ + / ^ ]
//  15) 将Optr栈做出栈操作，并逐个将出栈元素添加到postexp数组末尾，直至Optr栈为空：ostexp[ 3 ! 4 2 * 1 5 - 2 ^ / + ],Optr[]

...
// 如果没有，你必须防止在logDestruction内抛出的异常传递到Session析构函数的外面。
...
// 但是这样做并不比你原来的代码安全。
// 如果在catch中调用 operator<<时导致一个异常被抛出，
// 我们就又遇到了老问题，一个异常被转递到Session析构函数的外面。
// 可以在catch中放入try，但是这总得有一个限度，否则会陷入循环。
// 因此我们在释放Session时必须忽略掉所有它抛出的异常：
class Session2 {
 public:
  Session2();
  ~Session2();
...
// 实际上它阻止了任何从logDestruction抛出的异常被传递到session析构函数的外面。
...
// 禁止异常传递到析构函数外有两个原因：
// 1.第一能够在异常转递的堆栈辗转开解（stack-unwinding）的过程中，防止terminate被调用。
// 2.第二它能帮助确保析构函数总能完成我们希望它做的所有事情。

...
// 如果区间是排序的，那么通过binary_search、lower_bound、upper_bound和equal_range，
// 可以获得更快的查找速度（对数时间）。
...
// 但对于标准关联容器，建议选择count、find、lower_bound、upper_bound和equal_range的地方，
// 只需选择同名的成员函数即可。
...
int main() {
  ...
  if (i1 != vw.end() && *i1 == w) {
    ...
    // 但是19条说明了相等性和等价性不相同的情形也是不难发现的。这时代码是错误的。
  ...
  if (p.first != p.second) {  // 使用了等价性，所以总是正确的
    // 如果equal_range返回非空区间找到了特定值，p.first指向第一个与w等价的对象，
  ...
  // 这种情况下，并不想找到所有等价的对象，只是想指定一个位置，第一个不比ageLimit老的对象的位置。

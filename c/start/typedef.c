...
// C语言允许为一个数据类型起一个新的别名，就像给人起绰号一样。
// 起别名的目的不是为了提高程序运行效率，而是为了编码方便。
// typedef的用法一般为：typedef oldName newName;
// 需要强调的是，typedef是赋予现有类型一个新的名字，而不是创建新的类型。
// 为了见名知意，请尽量使用含义明确的标识符，并且尽量大写。
...
// typedef和#define的区别
// typedef在表现上有时候类似于#define，
// 但它和宏替换之间存在一个关键性的区别。
// 正确思考这个问题的方法就是把typedef看成一种彻底的封装类型，
// 声明之后不能再往里面增加别的东西。
// 1.可以使用其他类型说明符对宏类型名进行扩展，
//   但对typedef所定义的类型名却不能这样做。如下所示：
//   #define INTERGE int
//   unsigned INTERGE n;  // 没问题
//   typedef int INTERGE;
//   unsigned INTERGE n;  // 错误，不能在INTERGE前面添加unsigned
// 2.在连续定义几个变量的时候，typedef能够保证定义的所有变量均为同一类型，
//   而#define则无法保证。例如：
//   #define PTR_INT int *
//   PTR_INT p1, p2;
//   经过宏替换以后，第二行变为：

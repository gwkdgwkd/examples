...
/*
// 将实现细目分开描述。
// 如果可以这么做，Person的客户就只需要在Person接口被修改时才重新编译。
namespace std {
  class string;
}
class Date;
...
// 但C++的Interface class并不需要负担Java和.NET的Interfac所需负担的责任。
...
// 但Handle class和Interface class这是特别被设计出来用来隐藏实现细节的，如函数本体。

// 如果因为若干额外成本便不考虑Handle class和Interface class，将是严重的错误。

// 请记住：
// 支持”编译依存性最小化的一般构想是：相依于声明，不要相依于定义。
// 基于此构想的两个手段是handle class和interface class。
// 程序库头文件应该以“完全且仅有声明”的形式存在。

...
// 原因分析:
// 究其原因，是由于插件的动态链接引起的。
// hello.so在动态链接过程中，没有发现example1中有singleton::instance这个实例（事实上该实例已经存在了），
...
// D选项用于查看动态符号（dynamicsymbol），你会发现singleton::pInstance在静态表中存在，
// 在动态表中不存在，这也就是说，动态连接器（dynamiclinker）在加载hello.so的时候，
// 无法找到singleton::pInstance的唯一实例，只能构造一个新的实例，该实例在hello.so中是唯一的，
// 但是不能保证在example1和hello.so中唯一。

// 解决方法:
// 解决问题的关键在于如何暴露example1中的singleton::pInstance。
// 好在，GNUmake有一个链接选项-rdynamic，可以帮我们解决这个问题。
// nm -C example1 | grep singleton
...
// 0000000000400f54 W singleton::singleton()
// 0000000000400f54 W singleton::singleton()
// 静态符号没有什么变化，但是动态符号却显示了更多的内容，其中包括我们想要的singleton::pInstance，
// 也就是singleton的唯一实例。
// singleton.num in main : 100
// singleton.num in hello.so : 100
// singleton.num in hello.so after ++ : 101

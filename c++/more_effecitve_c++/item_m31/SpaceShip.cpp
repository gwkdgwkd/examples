...
// 和后来的基于虚函数的方法相似，每种碰撞都由一个独立的函数处理，
// 不过不同的是，这次，这些函数有着不同的名字，而不是都叫collide。
...
SpaceShip2::HitFunctionPtr SpaceShip2::lookup(const GameObject2& whatWeHit) {
  ...
  // 而不是习惯上的mapEntry->second以满足STL的奇怪行为。
  return (*mapEntry).second;
}

// 不过这意味着我们要付出拷贝赋值的代价。我们不想这么做。
// 如果initializeCollisionMap()返回指针的话，就不需要付出这个代价，

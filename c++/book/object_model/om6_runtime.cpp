...
// 全局对象如果有构造函数和析构函数的话，可以说它需要静态的初始化操作和内存释放操作。
// C++程序中的所有全局对象都被放置在程序的data segment中。
// 如果明确指定了值，对象将以该值为初值，否则，对象的初值为0。
// 和C不同，C并不自动设置初值，在C语言中全局对象只能被常量表达式（编译时期求值）初始化。
// 构造函数并不是常量表达式，虽然对象在编译时期可以被放置于data segment中并切内容为0，
// 但构造函数一直要到程序激活时才会执行。
...
//   一个_main()函数，调用__sti()函数，以及一个exit函数，调用__std()函数。
// 如何收集一个程序中各个文件的__sti()函数和__std()函数？
// nm命令（导出对象文件的符合表格）输出到munch，munch搜索以__sti或__std开头的名称，
...
// 虽然test可能会被调用多次，但是a的构造函数和析构函数只能执行一次。
...
// 除了将获得的指针所指的地址传回外，还要在arena所值得地址上执行Point的构造函数。
// 然而有一个轻微的不良行为：
// 在原已存在的对象上构造新的对象，而该现有的对象有析构函数，析构函数并不会被调用。
...
// 然而，要支持这一点，或许必须放弃对于经由对象静态调用所有虚函数通常会有的优化处理。
void func3() {
  Base b;
  b.f();  // Base::f
  b.~Base();

  // 下面的程序行为未定义，不确定调用哪个f，大部分使用者以为会调用Derived::f()，
  // 但大部分编译器调用的却是Base::f()：
  new (&b) Derived;
  b.f();  // Base::f
}
...
// 但实际上，由于市场竞争，几乎保证：表达式T c = a + b;实现时根本不产生一个临时对象。
...
// 临时对象的生命期规则有两个例外：
// 1.发生在表达式被用来初始化一个对象时：String s = b ? a + b;
...
// 2.当一个临时性对象被一个引用绑定时：const string &space = " ";
//   如果被销毁，那么引用也就没什么用了。
...
void func() {
  ...
  // 于是就不需要临时性对象，以及构造和析构的调用了，NRV优化后，
  // 将导致直接在z对象中求表达式结果，避免执行拷贝构造函数和具名对象的析构函数。

...
// 隐式类型转换利用的是编译器内置的转换规则或者已知的转换规则（转换构造函数及类型转换函数），
...
// 类型转换的本质：
// 数据是放在内存中的，变量（以及指针、引用）是这块内存的名字，有了变量就可以找到并使用数据。
// 内存中的数据有多种解释方式，使用之前必须要确定，数据的解释方式是由数据类型来确定的。
...
//   不能原样拷贝f的二进制位，也不能截取部分二进制位，必须先将f的二进制位读取出来，
//   然后直接截掉小数部分，把剩下的整数部分再转换成二进制形式，拷贝到i所在的内存中。
...
// 3.存在多继承时，如果把派生类指针pd赋值给基类指针pb，就必须考虑基类子对象在派生类对象中的偏移，
//   偏移不为0时就要调整pd的值，让它加上或减去偏移量，这样赋值后才能让pb恰好指向基类子对象。
// 4.Complex占用16个字节，double占用8个字节，将double类型的数据赋值给Complex类型的对象时，
...
// 使用const_cast进行强制类型转换可以突破C/C++的常数限制，修改常数的值有一定的危险性；
...
namespace n5 {
// reinterpret_cast关键字：
// reinterpret是重新解释的意思，顾名思义，这种转换仅仅是对二进制位的重新解释，
// 不会借助已有的转换规则对数据进行调整，非常简单粗暴，所以风险很高。
// reinterpret_cast可以认为是static_cast的一种补充，一些static_cast不能完成的转换，
// 就可以用reinterpret_cast来完成，例如两个具体类型指针之间的转换、int和指针之间的转换，
...
namespace n6 {
// dynamic_cast关键字：
// dynamic_cast用于在类的继承层次之间进行类型转换，它既允许向上转型，也允许向下转型。
...
// dynamic_cast的语法格式为：dynamic_cast <newType> (expression)
...
// 发现了C类型和B类型，所以就转换成功了。
// 从表面上看dynamic_cast能够向下转型，成功地将pa从A类型指针转换成了B和C类型指针。

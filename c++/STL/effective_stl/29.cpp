...
int main() {
  ...
  string fileData1((istream_iterator<char>(inputFile)),
                   istream_iterator<char>());
  ...
  // istreambuf_iterator的使用方法与istream_iterator大致相同，但是istream_iterator<char>对象使用operator>>从输入流中读取单个字符，
  // 而istreambuf_iterator<char>则直接从流的缓存区中读取下一个字符。
  string fileData2((istreambuf_iterator<char>(inputFile)),
                   istreambuf_iterator<char>());
  // 不用清除skipws标志，因为istreambuf_iterator不会跳过任何字符，只是简单地取回流缓冲区中的下一个字符，而不管它们是什么字符。

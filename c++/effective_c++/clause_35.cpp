...
class GameCharacter2 {
 public:
  ...
  // 它是所谓Template Method设计模式的一种独特表现形式，
  // 与C++ template并无关系。
  ...
  // NVI手法的一个优点在于，
  // 外覆器确保在调用virtual函数之前设定好适当的场景，并在调用结束后清理场景。
  // 事前工作包括锁定互斥锁、制造日志记录项、
...
//   NVI手法对public virtual函数而言是一个有趣的代替方案，
//   但从某中设计角度来看，它只比窗花更强一些而已。
//   另一个思路是人物健康指数的计算与人物类型无关，
//   计算时完全不需要人物这个成分。
...
// 本条款的忠告是，当为解决问题而寻找设计方案时，
// 不妨考虑virtual函数的替代方案：
// 1.使用non-virtual interface(NVI)手法，
//   那是Template Method设计模式的一种特殊形式。
//   它以public non-virtual成员函数，
//   包裹较低访问性（private或protected）的virtual函数。
// 2.将virtual函数替换为函数指针成员变量，
//   这是Strategy设计模式的一种分解表现形式。
// 3.以function成员变量替换virtual函数，
//   因而允许使用任何可调用物搭配一个兼容的签名式。
//   这也是Strategy设计模式的某种形式。
// 4.将继承体系内的virtual函数替换为另一个继承体系内的virtual函数。
//   这是Strategy设计模式的传统实现手法。
// 以上并未彻底而详尽地列出virtual函数的所有替换方案，
// 各有优缺点，要全方面考虑。

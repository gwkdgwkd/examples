...
// 所谓相交，是指有公共的部分，而2个单链表相交，则意味着它们有1个或多个公共的节点。
...
// 2.实际上，第1种实现方案还可以进一步优化，2个单链表相交有一个必然结果，
//   最后一个节点必定相同，否则这2个链表的最后一个节点必定不相同。
...
bool LinkIntersect3(link* L1, link* L2) {
  ...
  // 相比方案2，此方法的实现逻辑虽然复杂，但优点是，可以找到2个单链表相交的交点，
  // 也就是返回True时的temp指针指向的那个节点，此方案的时间复杂度也为O(n)。
}

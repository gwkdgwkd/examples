...
// 多文件编程中代码的划分原则是：
// 将变量、函数或者类的声明部分存放在.h文件，对应的实现部分放在.cpp文件中。
...
//   const常量的定义也可以遵循声明在.h文件，
//   定义在.cpp文件，借助extern关键字即可。
//   C++ const关键字会限定变量的可见范围为当前文件，
//   即无法在其它文件中使用该常量。
//   而extern关键字会const限定可见范围的功能，
//   它可以使const常量的可见范围恢复至整个项目。
// 3.借助extern直接定义const常量：
//   第2种方法demo.cpp和main.cpp中都引入了demo.h，
//   而demo.h文件中只包含对const num常量的声明。
//   要知道C++编译器在运行项目时，
//   会在预处理阶段直接将#include引入的头文件替换成该头文件中的内容。
//   显然相比第二种方法，此项目中省略了demo.h头文件的创建，
//   一定程序上提高了项目的编译效率。

// 相比后2种借助extern修饰const常量的方式，
// 第一种方式更简单、更常用，推荐使用。

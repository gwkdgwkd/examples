...
// unordered_set容器，可直译为“无序set容器”，即unordered_set容器和set容器很像，
// 唯一的区别就在于set容器会自行对存储的数据进行排序，而unordered_set容器不会。
// 总的来说，unordered_set容器具有以下几个特性：
// 1.不再以键值对的形式存储数据，而是直接存储数据的值；
// 2.容器内部存储的各个元素的值都互不相等，且不能被修改。
// 3.不会对内部存储的数据进行排序
// 对于unordered_set容器不以键值对的形式存储数据，即unordered_set存储的都是键和值相等的键值对，
...
//  Key 确定容器存储元素的类型，如果将unordered_set看做是存储键和值相同的键值对的容器，
//      则此参数则用于确定各个键值对的键和值的类型，因为它们是完全相同的，因此一定是同一数据类型的数据。
...
//                    反之，则返回一个指向容器中最后一个元素之后位置的迭代器（如果end()方法返回的迭代器）。
...
//                    负载因子，指的是的当前容器中存储元素的数量（size()）和使用桶数（bucket_count()）的比值，
...
void func1() {
  ...
  //   将现有unordered_set容器中存储的元素全部用于为新建unordered_set容器初始化：
  ...
  // C++11标准中还向unordered_set模板类增加了移动构造函数，即以右值引用的方式，

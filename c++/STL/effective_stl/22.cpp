...
int main() {
  ...
  if (i != se.end()) {
    // 尽管set和multiset的元素不是const，STL也有办法防止它们被修改。比如set<T>::iterator和operator*返回一个const T&。
    ...
    // 下面的方法可以通过编译，但运行是不会修改*i。在这两种情况下，类型转换的结果是一个临时的匿名对象，是*i的一个拷贝：
    ...
    // 大多数强制类型转换都可以避免，包括刚刚考虑过的转换，以一种总是可行且安全的方式修改set、multiset、map和multimap中的元素，可以分为5个简单步骤：
    // 1 找到想修改的容器的元素。
    // 2 为将要被修改的元素做一份拷贝。在map和multimap的情况下，不要把该拷贝的第一个部分声明为const。毕竟，你想要改变它。
    Employee e(*i);
    // 3 修改该拷贝，是它具有你期望它在容器中的值。
    e.setTitle("Corporate Deity");
    // 4 把该元素从容器中删除，通常是通过erase来进行。
    se.erase(i++);
    // 5 把新值插入到容器中。如果按照容器的排序顺序，新元素的位置可能与被删除元素的位置相同或紧邻，则使用“提示”形式的insert，

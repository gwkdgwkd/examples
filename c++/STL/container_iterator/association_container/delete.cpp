...
// 原因很简单，C++ STL标准中明确规定，
// map和multimap容器用于存储类型为pair<const K,V>的键值对。
...
// 换句话说，对于set<T>或者multiset<T>类型的容器，
// 其存储元素的类型是T而不是const T。
...
void func2() {
  ...
  // set容器中每个元素也可以看做是键和值相等的键值对，
  // 但对于这里的s容器来说，
  // 其实每个student对象的id才是真正的键，
  // 其它信息（name和age）只不过是和id绑定在一起而已。
  ...
  // 在已创建好的s容器的基础上，
  // 如下代码尝试修改s容器中某个学生的name名字，也是无法通过编译：
  ...
  // 虽然使用const_cast能直接修改set或者multiset容器中的元素，
  // 但一定不要修改元素的键！
  // 如果要修改，只能采用先删除再添加的方式。
}

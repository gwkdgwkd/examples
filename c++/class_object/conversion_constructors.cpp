...
class Complex {
 public:
  ...
  // 编译器会检索当前的类是否定义了转换构造函数，如果没有定义的话就转换失败，
  // 如果定义了的话就调用转换构造函数。
...
void func1() {
  ...
  // 但是，使用强制类型转换后，编译器就认为我们知道这种风险的存在，
...
void func4() {
  // 需要注意的是，为了获得目标类型，编译器会“不择手段”，
  // 会综合使用内置的转换规则和用户自定义的转换规则，并且会进行多级类型转换，例如：
...
// 既然已经知道了要返回type类型的数据，所以没有必要再像普通函数一样明确地给出返回值类型。
...
//   但是如果多个类型转换函数要转换的目标类型本身又可以相互转换（类型相近），那么有时候就会产生二义性。
class A {
 public:
...
void func1() {
  ...
  // 也可以调用operator int()将c1转换为int类型，这两种类型都可以跟12.5进行加法运算，
...
// 2.可以将一个int、double、bool等基本类型的数据直接赋值给Complex类的对象。

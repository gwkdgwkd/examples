...
// 有时你想这样管理某些对象，要让某种类型的对象能够自我销毁，也就是能够“delete this”。
...
// 要求在堆中建立对象
// 让我们先从必须在堆中建立对象开始说起。
// 为了执行这种限制，你必须找到一种方法禁止以调用“new”以外的其它手段建立对象，这很容易做到。
// 非堆对象（non-heap object）在定义它的地方被自动构造，在生存时间结束时自动被释放，
...
// 这种方法的缺点是一个类经常有许多构造函数，类的作者必须记住把它们都声明为private。
// 否则如果这些函数就会由编译器生成，构造函数包括拷贝构造函数，也包括缺省构造函数；
// 编译器生成的函数总是public。
...
// 没有简单的办法，UPNumber构造函数不可能判断出它是否做为堆对象的基类而被调用。
...
// 这种方法利用了这样一个事实：“当在堆上分配对象时，会调用operator new来分配raw memory”，
...
// 不过（倘若你的编译器支持它）你能象编写operator new一样容易地编写operator new[]函数。
// 更大的问题是numberArray有100个元素，所以会调用100次构造函数。
// 但是只有一次分配内存的调用，所以100个构造函数中只有第一次调用构造函数前把onTheHeap设置为true。
// 当调用第二个构造函数时，会抛出一个异常，你真倒霉。
// 即使不用数组，bit-setting操作也会失败。考虑这条语句：
// UPNumber2 *pn = new UPNumber(*new UPNumber2);
// 这些困难没有否定让每个构造函数检测*this指针是否在堆中这个方法的核心思想，
// 它们只是表明检测在operator new(或operator new[])里的bit set不是一个可靠的判断方法。
// 我们需要更好的方法进行判断。
...
// 当调用onHeap时，它的栈框架（stack frame）(也就是它的activation record)被放在程序栈的顶端，
// 因为栈在结构上是向下扩展的（趋向低地址），onTheStack的地址肯定比任何栈中的变量或对象的地址小。
...
// 静态对象不仅仅包括显示地声明为static的对象，也包括在全局和命名空间里的对象。
...
// 正如我们所知，只有一个全局域，
// 只有一种具有正常特征形式（也就是参数类型）的operator new和operator delete。
// 这样做会使得我们的软件与其它也实现全局版本的operator new和
// operator delete的软件（例如许多面向对象数据库系统）不兼容。
// 第二个因素是效率：如果我们不需要这些，为什么还要为跟踪返回的地址而负担额外的开销呢？
// 最后一点可能有些平常，但是很重要。实现isSafeToDelete让它总能够正常工作是不可能的。
// 难点是多继承下来的类或继承自虚基类的类有多个地址，
// 所以无法保证传给isSafeToDelete的地址与operator new返回的地址相同，即使对象在堆中建立。
// 我们希望这些函数提供这些功能时能够不污染全局命名空间，没有额外的开销，没有正确性问题。
// 幸运的是C++使用一种抽象mixin基类满足了我们的需要。
// 抽象基类是不能被实例化的基类，也就是至少具有一个纯虚函数的基类。
// mixin(“mix in”)类提供某一特定的功能，并可以与其继承类提供的其它功能相兼容。
// 这种类几乎都是抽象类。
// 因此我们能够使用抽象混合（mixin）基类给派生类提供判断指针指向的内存是否由operator new分配的能力。
...
bool HeapTracked::isOnHeap() const {
  ...
  // 这个问题在isOnHeap中仍然会遇到，但是因为isOnHeap仅仅用于HeapTracked对象中，
  // 我们能使用dynamic_cast操作符的一种特殊的特性来消除这个问题。
  // 只需简单地放入dynamic_cast，把一个指针dynamic_cast成void*类型（或const void*或volatile void*），
  ...
  // 所以能把this指针dynamic_cast成const void*，变成一个指向当前对象起始地址的指针。
...
// 不过使用象HeapTracked的原因一般都是要判断是否可以调用“delete this”，
...
// 如果operator new和operator delete没有在派生类中被声明为public（进行改写，overwrite），
// 它们就会继承基类中private的版本，如下所示：
class NonNegativeUPNumber4 : public UPNumber4 {};  // 这个类没有声明operator new

// 如果派生类声明它自己的operator new，当在堆中分配派生对象时，
// 就会调用这个函数，于是得另找一种不同的方法来防止UPNumber基类的分配问题。
...
// 所以我们很不走运，不过这也丝毫不奇怪，毕竟如果我们能辨别出某个地址在堆上，
// 我们也就能辨别出某个地址不在堆上。
// 但是我们什么都不能辨别出来。
...
int main() {
  ...
  // 它包含new操作符的两次调用，因此要调用两次operator new和调用两次UPNumber构造函数。
  // 程序员一般期望这些函数以如下顺序执行：
  ...
  // 因为在第一步和第二步设置的bit，第三步中被清除，那么在第四步调用对象的构造函数时，
  // 就会认为对象不再堆中，即使它确实在。
  ...
  // 同样很明显在指向pa->value上调用delete是不安全的，因为该指针不是被new返回的。

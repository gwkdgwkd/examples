...
// 在输入流中碰到ASCII码等于0xfe的字符时，cin.get()返回0xFF，
// 0xFF赋值给c，此时如果c是char类型的，那么其值就是-2（符号位1代表负数），
// 即等于EOF，于是程序就错误地认为输入已经结束。
// 而在c为int类型的情况下，将0xfe赋值给c，c的值是255（符号位0代表正数），
...
// 此函数的作用是跳过输入流中的n个字符，或跳过delim及其之前的所有字符，
// 哪个条件先满足就按哪个执行。
// 两个参数都有默认值，因此cin.ignore()就等效于cin.ignore(1, EOF)，
// 即跳过一个字符。
...
// 此函数返回输入流中的下一个字符，
// 但是并不将该字符从输入流中取走——相当于只是看了一眼下一个字符，因此叫peek。
...
// cin是如何判断输入结束（读取结束）的？
// cin可以用来从键盘输入数据；
// 将标准输入重定向为文件后，cin也可以用来从文件中读入数据。
// 在输入数据的多少不确定，且没有结束标志的情况下，
// 该如何判断输入数据已经读完了呢？
// 从文件中读取数据很好办，到达文件末尾就读取结束了。
// 从控制台读取数据怎么办呢？总不能把控制台关闭吧？这样程序也运行结束了！
// 其实，在控制台中输入特殊的控制字符就表示输入结束了：
//  在Windows中，通过键盘输入时，按Ctrl+Z组合键后再按回车键，就代表输入结束。
...
void testN5() {
  int n;
  int maxN = 0;
  // cin>>n的返回值的确是istream &类型的，
  // 而while语句中的条件表达式的返回值应该是bool类型、
  // 整数类型或其他和整数类型兼容的类型，
  // istream &显然和整数类型不兼容，为什么while(cin>>n)还能成立呢？

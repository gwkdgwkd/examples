...
// 次要的是可以减少冗余的计算，每次迭代都需要检查i是否等于lw.end()。
...
//  但是只有库的实现者才可以使用基于指针的遍历，因为只有他们才知道内部数组的大小，
//  才知道如何从一个数组转移到另一个数组。
//  有些STL包含的算法实现考虑到deque的内部数据结构，这些算法实现比“普通”的算法实现了快20%多。
//  STL算法的实现未必一定都针对deque进行了优化，但无论如何实现者肯定比你更了解内部的实现细节，
//  可以在算法中充分利用这些知识。
// 其次的性能增益在于，除了一些不太重要的算法以外，其他几乎所有的STL算法都使用了复杂的计算科学算法，
...
int main() {
  ...
  // 这段代码是真正想要的，但这个例子就是手写循环难以保证其正确性的一个典型实例，
  // 因为要一刻不停地为迭代器的有效性担心

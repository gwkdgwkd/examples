...
// 作为软件开发人员，也许知道得不多，但知道万物都会变化，没必要知道什么将发生变化，
// 这么变化又怎么发生，以及什么时候发生，在哪里发生，但知道：万物都会变化。
...
// 3.将会编制新的应用软件，函数将在新的环境下被调用，它们应该被写得在新平台上运行正确；
...
// 2.如果类的全部需要创建在堆中，不要只对用户说一句，用Item M27来强迫这一点；
// 3.如果拷贝构造和赋值对类是无意义的，通过声明为私有来阻止生成（Item E27）；
// 4.C++提供了强大的功能、灵活度和表达力，用语言提供的这些特性来强迫程序符合设计。

// 应该判断一个函数的含意，以及它被派生类重定义的话是否有意义。
// 如果是有意义的，声明它为虚，即使没有人立即重定义它。
// 如果不是的话，声明它为非虚，并且不要在以后为了便于某人而更改；
// 确保更改是对整个类的运行环境和类所表示的抽象是有意义的。

// 处理每个类的赋值和拷贝构造函数，即使从没人这样做过。
// 现在没有这么做并不意味着他们以后不这么做（见Item E18）。
// 如果这些函数是难以实现的，那么声明它们为私有。
// 这样，不会有人误调编译器提供的默认版本而做错事。

// 基于最小惊讶法则：
// 努力提供这样的类，它们的操作和函数有自然的语法和直观的语义。
// 和内建数据类型的行为保持一致：拿不定主意时，就仿照int来做。
...
// 4.给对象赋了值而没有使用；
// 5.会赋过大的值、过小的值或空值。
// 一般而言，只要能编译通过，就有人会这么做，所以，要使得自己的类易于被正确使用而难以误用。
...
// 将代码设计得当需要变化时，影响是局部的，尽可能地封装，将实现细节申明为私有。
// 只要可能，使用无名的命名空间和文件内的静态对象或函数。
// 避免导致虚基类的设计，因为这种类需要每个派生类都直接初始化它，即使是那些间接派生类。
// 避免需要RTTI的设计，它需要if.then.else型的瀑布结构，每次，类的继承层次变了，
// 每组if.then.else语句都需要更新，如果忘掉了一个，不会从编译器得到任何告警。
...
// 至少，它们没有任何影响，直到其析构函数被使用。
// 如果需要对类的额外修改，其它用户将受影响。

// 有一个商业类库，在C++标准运行库声明string以前，包含了一个没有虚析构函数的string类。
// 其生产商解释：
// 没有使用虚析构函数，因为不想这个string类有vtbl。
// 甚至不期望想有一个string*，所以这不成为问题，很清楚这么做将造成的困难。
// 这是进行时态的考虑还是未来时态的考虑？
// 当然，vbtl有个技术上的问题，大部分string类的实现都是对象内部只有一个char*指针，
// 所以增加一个vptr造成每个string类的大小翻倍，所以可以理解为什么不肯实现它，
// 尤其是对于string这样频繁出现高密度使用的类。
// 这样的类是属于影响程序性能的那20%的部分的。
// 还有，一个string对象的全部内存，它自己加上所指向的在堆中的字符串，
// 通常远大于保存一个char*指针的大小。
// 从这个方面来说，为vptr增加的花费并不是那么重要的，不过，这仍然是个合法的设计。
// 的确，ISO/ANSI标准委员会似乎是这么想的：标准string类型有一个非虚的析构函数。
// 更有问题的是生产商的注释：
// 甚至不期望有一个string*，所以这不成为问题，这可能是正确的，
// 但他们的这个string类是提供给数以千记的开发人员使用的类库的一部分。
// 有如此多的开发人员，每个人对C++的掌握程度都不同，每个人做事的方法也都不同。
// 这些人都了解string没有虚析构函数的后果吗？
// 生产商确信客户知道没有虚析构函数时，用string*指针delete对象时可能工作不正确，
...
// 1.提供完备的类，即使某些部分现在还没有被使用，如果有了新需求，不用回过头去改它们；
// 2.将接口设计得便于常见操作并防止常见错误，使得类容易正确使用而不易用错，
//   例如，阻止拷贝构造和赋值操作，如果它们对这个类没有意义的话，防止部分赋值；
// 3.如果没有限制不能通用化的代码，那么通用化它，
//   例如，如果在写树的遍历算法，考虑将它通用得可以处理任何有向不循环图。
// 未来时态的考虑增加了代码的可重用性、可维护性、健壮性，已及在环境发生改变时易于修改。
// 它必须与进行时态的约束条件进行取舍，太多的程序员们只关注于现在的需要，
// 这么做牺牲了软件的长期生存能力，是与众不同的，是离经叛道的，应该在未来时态下开发程序。

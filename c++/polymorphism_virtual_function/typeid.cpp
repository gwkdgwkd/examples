...
// 变量名和函数名为我们提供了方便，
// 让在编写代码的过程中可以使用易于阅读和理解的英文字符串，
// 不用直接面对二进制地址，那场景简直让人崩溃。
// 不妨将变量名和函数名统称为符号（Symbol），
// 找到符号对应的地址的过程叫做符号绑定。
...
// 必须要等到程序运行后根据具体的环境或者用户操作才能决定，
// 这称为动态绑定。
// C++是一门静态性的语言，会尽力在编译期间找到函数的地址，
// 以提高程序的运行效率，但是有时候实在没办法，
// 只能等到程序运行后再执行一段代码（很少的代码）才能找到函数的地址。
// 动态绑定的本质：编译器在编译期间不能确定指针指向哪个对象，
// 只能等到程序运行后根据具体的情况再决定。

// 在C++中，typeid运算符，dynamic_cast运算符和异常处理都依赖RTTI机制，
// 并且要能够通过派生类获取基类的信息，
// 或者说要能够判断一个类是否是另一个类的基类，
// 必须要在基类和派生类之间再增加一条绳索，把它们连接起来，
// 形成一条通路，让程序在各个对象之间游走。
// 在面向对象的编程语言中，称此为继承链（Inheritance Chain）。
// 将基类和派生类连接起来很容易，只需要在基类对象中增加一个指向派生类对象的指针，
// 然而考虑到多继承、降低内存使用等诸多方面的因素，真正的对象内存模型很复杂，
// C++标准并没有对对象内存模型的细节做出规定。
...
// raw_name()是VC/VS独有的一个成员函数，
// hash_code()在VC/VS和较新的GCC下有效。

// C++标准规定，type_info类至少要有如下所示的4个public属性的成员函数，
// 其他的扩展函数编不做限制：
// 1.const char* name() const;
//   返回一个能表示类型名称的字符串，
//   但是C++标准并没有规定这个字符串是什么形式的。
//   对于int和Base，在VC/VS下的name()输出结果是int和class Base，
//   而在GCC下的输出结果分别是i和4Base。
// 2.bool before (const type_info& rhs) const;
//   判断一个类型是否位于另一个类型的前面，
//   rhs参数是一个type_info对象的引用。
//   但是C++标准并没有规定类型的排列顺序，
//   不同的编译器有不同的排列规则，程序员也可以自定义。
//   要特别注意的是，这个排列顺序和继承顺序没有关系，
//   基类并不一定位于派生类的前面。
...
// 可以发现，不像Java、C#等动态性较强的语言，
// C++能获取到的类型信息非常有限，也没有统一的标准，如同鸡肋一般，
// 大部分情况下只是使用重载过的==运算符来判断两个类型是否相同。
...
// 而表达式typeid(a)==typeid(b)的结果为true，
// 可以说明，一个类型不管使用了多少次，编译器都只为它创建一个对象，
// 所有typeid都返回这个对象的引用。
// 需要提醒的是，为了减小编译后文件的体积，
// 编译器不会为所有的类型创建type_info对象，
// 只会为使用了typeid运算符的类型创建。

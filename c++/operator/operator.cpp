...
// 所谓重载，就是赋予新的含义。
// 函数重载（Function Overloading）可以让一个函数名有多种功能，
// 在不同情况下进行不同的操作。
...
//   好让编译器区分这是程序员自定义的运算符，
//   防止程序员修改用于内置类型的运算符的性质。
//   如果有两个参数，这两个参数可以都是对象，
//   也可以一个是对象，一个是C++内置类型的数据。
//   另外，将运算符重载函数作为全局函数时，
//   一般都需要在类中将该函数声明为友元函数。
...
class Complex {
 ...
 public:
  ...
  // 在C++中，类型的名字（包括类的名字）本身也是一种运算符，
  // 即类型强制转换运算符。
  // 类型强制转换运算符是单目运算符，也可以被重载，
  ...
  // 如果要使用cout和cin处理自定类型，就必须以全局函数的形式重载<<和>>，

...
// STL主要由用以表现容器、迭代器和算法的template构成，
// 但也覆盖若干工具性template。
// 其中一个名为advance，用来将某个迭代器移动某个给定距离。
// 只有随机访问迭代器才支持+=操作。
// 其他迭代器种类，advance必须反复执行++或--。

// STL共有5重迭代器：
// 1.输入迭代器只能向前移动，一次一步，
//   客户只能读取（不能写）所指的东西，而且只能读取一次。
//   例如istream_iterator。
// 2.输出迭代器智能向前移动，一次一步，
//   客户只能写，不能读，而且只能写一次。
//   例如ostream_iterator。
...
// 这样的template在标准库中有若干个，
// 其中针对迭代器的被命名为iterator_traits。
// iterator_traits是个struct。
// 习惯上总被实现为struct，但它们却往往被称为traits class。

// iterator_traits的实现方式是，针对每一个类型IterT，
// 在struct iterator_traits<IterT>内：
// 一定声明某个typedef名为iterator_category。
...
// 3.提供一个template和一组特化版本（iterator_traits），
//   内含希望支持的类型相关信息。
...
// 2.建立一个控制函数或函数模板，
//   调用上述那些函数并传递traits class所提供的信息。

// Traits广泛用于标准程序库。
// iterator_traits除了iterator_categor外，
// 还提供另外四份迭代器相关信息：
// 1.最有用的value_type；

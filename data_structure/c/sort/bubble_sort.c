...
// 举个例子，对{14,33,27,35,10}序列进行升序排序，冒泡排序算法的实现过程是：
// 1.从{14,33,27,35,10}中找到最大值35；
// 2.从{14,33,27,10}中找到最大值33；
// 3.从{14,27,10}中找到最大值27；
// 4.从{14,10}中找到最大值14。
// 通过一趟趟的比较，一个个的最大值被找到并移动到相应位置，直到检测到表中数据已经有序，
// 或者比较次数等同于表中含有记录的个数，排序结束，这就是起泡排序。
// 整个查找最大值的过程中，最大的元素就像水里的气泡一样，
// 一点一点地冒了出来，这也是将该算法命名为冒泡排序算法的原因。

// 使用起泡排序算法，其时间复杂度同实际表中数据的无序程度有关：
// 1.若表中记录本身为正序存放，则整个排序过程只需进行n-1（n为元素个数）次比较，且不需要移动记录；
// 2.若表中记录为逆序存放（最坏的情况），则需要n-1趟排序，进行n(n-1)/2次比较和数据的移动。
// 所以该算法的时间复杂度为O(N^2)。

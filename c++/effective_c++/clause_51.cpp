...
//       return 一个指针，指向分配的内存
//     // 分配失败
//     new_handler globalHandler = set_new_handler(0); // 没有办法直接取得new-handing函数指针
...
// 针对class设计的operator new，行为很典型地只为大小刚好sizeof(X)的对象设计。
// 一旦被继承下去，有可能base class的operator new被调用用以分配derived class对象。
class Base {
 public:

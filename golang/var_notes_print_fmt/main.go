...
一个Go程序对应一个或多个以.go为文件后缀名的源文件，每个源文件以包的声明语句开始，
说明该源文件属于哪个包。包声明语句之后是import语句导入依赖的其他包，然后是包一级
的类型、变量、常量、函数的声明语句，包一级的各种类型的声明语句的顺序无关紧要。函数
内部的名字必须先声明之后才能使用。
*/
...
func main() {
	...
	fmt.Printf("%g,%g\n", f, 6.02214129e23) // 12.34,6.02214129e+23
	// %G,根据实际情况采用%E或%F格式（以获得更简洁、准确的输出）
	fmt.Printf("%G,%G\n", f, 6.02214129e23) // 12.34,6.02214129E+23

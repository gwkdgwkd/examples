...
// BF该算法的实现过程就是傻瓜式地用模式串（假定为子串的串）与主串中的字符一一匹配，算法执行效率不高。
...
// 每个字符对应的next值都可以表示该字符"同后缀字符串相同的前缀字符串最后一个字符所在的位置"，
...
// A: ABACBCE   A: ABACBCE
// B: ABAB      B:   ABAB
// 当匹配失败时，Next函数会由图AC的Ａ开始继续进行模式匹配，但这样做是没有必要的，纯属浪费时间。
// 出现这种多余的操作，问题在当T[i-1]==T[j-1]成立时，
// 没有继续对i++和j++后的T[i-1]和T[j-1]的值做判断。
// 改进后的Next函数如下所示：

...
// 为了省事，总容易有人不push和pop，直接改，某人就会在不知情的情况下，包含了这个头文件，进而受影响。
...
// alignas指定符可应用到变量或非位域类数据成员的声明，或能应用于class/struct/union或枚举的定义。
// 它不能应用于函数参数或catch子句的异常参数。
// 以此声明声明的对象或类型的对齐要求将等于用于声明的所有alignas指定符最严格（最大）的非零expression，
// 除非这会削弱类型的自然对齐。
...
struct alignas(2) C {  // 大小不是6，而是8，因为a已经是4对齐了，所以b之后会补到8
  int a;
  char b;
};
struct alignas(16) D {

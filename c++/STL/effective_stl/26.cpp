...
// 但这些函数仅接受iterator类型的参数，而不是const_iterator、reverse_iterator以及const_reverse_iterator，总是iterator。
// iterator与其他的迭代器有所不同。
...
// 即便并不需要使用迭代器做参数来调用容器的任何成员函数。其中一个最赋值的理由涉及了iterator与const_iterator之间的比较：
typedef deque<int> IntDeuqe;
...
int main() {
  ...
  // 解决办法：只要交换两个iterator的位置，就万事大吉了。
  // if (c == i)
  // 不仅在进行比较的时候会发生这样的问题，只要在同一个表达式中混用iterator和const_iterator，这样的问题就会出现。例如：
  ...
  // 从const正确性的角度来看，仅仅为了避免一些可能存在的STL实现缺陷（基本都有较为直接的解决途径）而放弃cosnt_iterator显得有欠公允。
  // 但考虑到在容器类的某些成员函数中指定使用iterator的现状，得出iterator比const_iterator更为使用的结论就不足为奇了。

...
// 观察者模式是目前应用最广、影响范围最大的模式之一，
// 也是在大型系统开发过程中要用到的，是软件开发过程中必须要掌握和使用的：
// 1.Model/View/Control（MVC），在系统开发架构设计中有着很重要的地位和意义，
//   MVC实现了业务逻辑和表示层的解耦，在MFC中，Doc/View提供了实现MVC的框架结构；
// 2.Qt中的核心机制信号和槽的实现也同样有观察者的影子；
// 3.在Java语言中还提供了专门的接口实现Observer模式：
// 通过专门的类Observable及Observer接口来实现MVC编程模式

// Observer模式要解决的问题为：
// 建立一个一对多的依赖关系，并且做到当一变化的时候，依赖这个一的多也能够同步改变。
// Observer提供给关联对象一种同步通信的手段，使某个对象与依赖它的其他对象保持同步。

// Observer模式也称为发布-订阅模式，
// 目标就是通知的发布者，观察者则是通知的订阅者（接受通知）。

// 最常见的一个例子就是：
// 对同一组数据进行统计分析时候，希望能够提供多种形式的表示，
// 例如以表格进行统计显示、柱状图统计显示、百分比统计显示等，
// 这些表示都依赖于同一组数据，当然需要当数据改变的时候，
// 所有的统计的显示都能够同时改变，Observer模式就是解决了这一个问题。

...
// 从软件工程来看，一个ADT或类的数据封装，比在C程序中程序性地使用全局数据好。
...
// 封装没有增加成本，并未带给程序任何空间或执行期的不良反应。

// C++在布局以及存取时间上主要的额外负担是由virtual引起，包括：
// 1.virtual function机制，用以支持一个有效率的执行期绑定；
// 2.virtual base
// class，使多次出现在继承体系中的基类，有一个单一而被共享的实体。
// 此外，还有些多重继承的额外负担，发生在一个派生类和其第二或后继之基类的转换之间。
...
// 在C++的类中有：
// 两种成员数据：static和non static；
// 三种成员函数：static、non static和virtual。
...
// 这个模型的主要优点在于空间和存取时间的效率，主要缺点则是，如果代码本身未曾改变，
// 但所用到的class objects的非静态成员函数有所修改，那么应用程序同样得重新编译。

// C++支持单一继承，也支持多重继承，继承关系也可以为虚拟（virtual，共享）继承。
// 在虚拟继承中base class不管被派生多少次，永远只有一个实体，被称为subobject。
}  // namespace n1
...
// 1.如果没有八种整数需要支持的话，overloaded function的解决方式将会简单得多；
...
// 当它让人感觉比较好的时候，关键词struct本身并不一定要象征其后随之声明的任何东西。
// 可以用struct代替class，但仍声明public、protected、private等等。
// 人们和教科书说到struct时，是指数据集合体，没有私有数据，也没有函数，就是纯C的用法。

// C程序员的技巧有时候却成为C++程序的陷阱。
// 例如把单一元素的数组放在一个struct的尾端，于是每个struct可以拥有可变大小的数组：
struct mumble {
  char pc[1];  // stuff
};
class stumble {
...
// C的伎俩或许有效，或许无效，需要看保护数据成员被放在私有数据成员的前面或后面而定。
// 同理基类和继生类的数据成员的布局也没有谁先谁后的强制规定，也不能保证C伎俩有效。
// 组合，而非继承，才是把C和C++结合在一起的唯一可行方法。
// C struct在C++中的一个合理用途，是当要传递复杂的对象的全部或部分到C函数中时，
// struct可以将数据封装起来，并保证拥有与C兼容的空间布局，前提是使用组合。
// 如果是继承而不是组合，编译器会决定是否应该有额外的数据成员被安插到基类中。
...
// 2.抽象数据类型模型（ADT），该模型所谓抽象是和一组表达式（public接口）一起提供，
//   而其运算定义仍然隐而未明；
...
namespace test1 {
// C++以下列方法支持多态：
// 1.经由一组隐含的转换操作，例如把一个派生类指针转换为指向其基类的指针；
//   shape *ps = new circle();
...
namespace test2 {
// 需要多少内存才能够表示一个class object，一般而言有：
// 1.其非静态数据成员的总和大小；
...
// 转型（cast）其实是一种编译器指令，大部分情况下它并不改变指针所含的真正地址，
// 它只影响被指出的内存的大小和其内容的解释方式。
// 指针或引用之所以支持多态，是因为它们并不引发内存中任何与类型有关的内存委托操作，
...
void func() {
  ...
  B *pb2 = &d;
  // pb2和pd每个都指向D对象的第一个byte，其间的差别是，pd所涵盖的地址包含整个D对象，
...
// 当一个基类对象被直接初始化为一个派生类对象时，就会别切割，
// 以塞入较小的base type内存中，派生类没有留下任何蛛丝马迹，多态于是不再呈现。
...
// 1.速度快是因为所有的函数引发的操作都在编译时期完成，对象不需要设置virtual机制；

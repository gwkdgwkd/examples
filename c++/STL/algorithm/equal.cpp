...
// equal()算法可以告诉两个序列是否匹配。
// mismatch()算法也可以告诉我们两个序列是否匹配，
// 而且如果不匹配，它还能告诉我们不匹配的位置。
// mismatch()的4个版本和equal()一样有相同的参数，
// 第二个序列有或没有结束迭代器，有或没有定义比较的额外的函数对象参数。
// mismatch()返回的pair对象包含两个迭代器。
// 它的first成员是一个来自前两个参数所指定序列的迭代器，
// second是来自于第二个序列的迭代器。
// 当序列不匹配时，pair包含的迭代器指向第一对不匹配的元素；
// 因此这个pair对象为pair<iter1+n，iter2+n>，
...
// 当第二个序列比第一个序列长时，
// 返回pair<(iter1+(end_iter2-iter2)),end_iter2>，
...
// 它的前两个参数定义了第一个序列，
// 第3和第4个参数分别是第二个序列的开始和结束迭代器。
// 默认用<运算符来比较元素，但在需要时，
// 也可以提供一个实现小于比较的函数对象作为可选的第5个参数。

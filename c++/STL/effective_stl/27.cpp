...
int main() {
  ...
  // 不能编译的原因是，对于这些容器类型，
  // const_iterator和iterator是完全不同的类。
  ...
  // 从cosnt T*到T*的转换，因而可以通过编译，而且结果也是正确的。
  // 然而即便在这样的STL实现中，
  // reverse_iterator和const_reverse_iterator仍然是真正的类，
  // 所以不能直接将const_reverse_iterator，
  // 通过cosnt_cast强制转换成reverse_iterator。
  // 这些STL实现为了便于调试，
  // 通常只会在Release模式下才使用指针来表示vector和string的迭代器。
  // 所以即使对于vector和string容器，
  // 将cosnt迭代器强制转换成迭代器也是不可取的，
  // 因为这些代码的移植性是一个问题。

  // 安全的、可移植的，
  // 而且用不着涉及类型系统的强制转换的得到对应iterator的方法：
  Iter i(d.begin());
  // 编译不过，两个参数类型不一样：
  // advance(i, distance(i, c));
  // 显式指明distance所使用的类型参数，
  // 从而避免编译器来推断该类型的参数，排除二义性。

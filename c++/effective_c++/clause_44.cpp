...
// 使用template可能会导致代码膨胀：其二进制码带着重复的代码、数据，或两者。源码看起来很整洁，但目标码却不是那么回事。
...
// 对于给定元素对象类型，所有矩阵共享同一个（也是唯一一个）SquareMatrix2Base class。也将因此共享这唯一一个class内的invert。
// SquareMatrix2Base::invert如何知道该操作什么数据？只知道尺寸，如何知道哪个特定矩阵的数据在哪儿？只有derived class知道。
// Derived class如何联络base class做逆运算？
//  1 一个可能的做法是为SquareMatrix2Base::invert添加另一个参数，也许是个指针，指向一块用来放置矩阵数据的内存起起始点。
...
// 这允许derived class具定内存分配方式。某些实现版本也许会决定将矩阵数据存储在SquareMatrix对象内部：
template <typename T, size_t n>
...
// 上面这种类型不需要动态分配内存，但对象自身可能非常大。另一种做法是，把每一个矩阵的数据放进heap：
template <typename T, size_t n>
...
// non-tpe template param能带来膨胀，其实type param也会导致膨胀。vector<int>和vector<long>的成员函数有可能完全相同。
// 有些连接器会合并完全相同的函数实现，但有些不会，意味着template被具现化为int和long两个版本，造成代码膨胀。
// 大多数平台上，所有指针类型都有相同的二进制表述，因此凡template持有指针者（list<int*>,list<const int*>等等）往往应该对每一个成员函数使用唯一一份底层代码。

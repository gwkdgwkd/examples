...
// C++程序中最常使用的资源就是动态分配内存，但内存只是必须管理的众多资源之一。
// 资源还包括文件描述符、互斥锁、图形界面中的字符和笔刷、数据库连接、以及网络socket。
// 不论哪种，不使用时，必须还给系统。
...
// 一种想法是把资源放进对象内，便可依赖C++的析构函数自动调用机制来确保资源被释放。

// 许多资源被动态分配于heap内而后被用于单一区块或函数内，
// 应该在控制流离开那个区块或函数时被释放，auto_ptr正是针对此场景而设计的，
// 是个类指针对象，也就是所谓的智能指针，其析构函数自动对其所指对象调用delete。
// void f1() {
...
// 2.管理对象运用析构函数确保资源释放：
//   不论控制流如何离开区块，一旦对象被销毁，其析构函数自然会被自动调用，
//   于是资源被释放，如果资源资源释放可能导致抛出异常，参考条款8。

// auto_ptr被销毁时会自动删除它所指之物，所以别让多个auto_ptr同时指向同一对象。
// 否则对象被被删除一次以上，未定义行为，为预防这个问题auto_ptr有个不寻常的性质：
// 若通过copy构造函数或copy assignment操作符复制它们，它们会变成null，
// 复制所得的指针取得资源唯一拥有权，这导致auto_ptr并非管理动态分配资源的神兵利器。

// auto_ptr的替代方案是引用计数型智能指针（RCSP），记录资源的引用数量，
// 只有无人指向时才自动删除，但是无法打破环状引用，就是已经没有被使用的对象彼此互指。
void f1() {
...
// 意味着在动态分配而得到的array身上使用是个坏主意，但可悲的是，可以通过编译。

...
// 所谓的TMP是以C++写成、执行于C++编译器内的程序。
// 一旦TMP程序结束执行，其输出，也就是从template具现出来的若干C++源码，便会一如往常地被编译。

// TMP有两个优点：
// 第一，它让某些事情更容易。如果没有它，那些事情将是困难的，甚至不可能的。
// 第二，由于TMP执行于C++编译期，因此可将工作从运行期转移到编译期。
//   这导致的一个结果是，某些错误原本通常在运行期才能检测到，现在在编译期就能找出来。
//   另一个结果是，使用TMP的C++程序可能在每一方面都个更高效：较小的可执行文件、较短的运行期、较少的内存需求。
//   将工作从运行期转移至编译器的另一个结果是，编译时间变长来。可能远长于不使用TMP的对应版本。

// traits解法就是TMP。traits引发“编译期发生于类型身上的if...else计算。

// TMP已被证明是个”图灵完全“机器，意思是它的威力大到足以计算任何事物。
// 使用TMP可以声明变量/执行循环/编写及调用函数等。但与正常的C++看起来很不同。
// TMP并没有真正的循环构件，所以循环效果通过递归完成。
// TMP主要是个”函数式语言“，而递归对于这类语言无法分割。
// TMP的递归甚至不是正常种类，因为TMP循环并不涉及递归函数调用，而是涉及”递归模板具现化“。
template <unsigned n>
struct Factorial {
...
// 尽管如此，将工作从运行期移往编译期所带来的效率改善还是令人深刻的。而TMP对”难以或甚至不可能运行期实现出来的行为“的表现能力也很吸引人。

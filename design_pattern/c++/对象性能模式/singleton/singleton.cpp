...
namespace n1 {
// 线程非安全版本
// 这是单例模式最经典的实现方式，将构造函数和拷贝构造函数都设为私有的，而且采用了延迟初始化的方式，
// 在第一次调用getInstance()的时候才会生成对象，不调用就不会生成对象，不占据内存。
// 然而，在多线程的情况下，这种方法是不安全的。
// 正常情况下，如果线程A调用getInstance()时，将m_instance初始化了，
// 那么线程B再调用getInstance()时，就不会再执行new了，直接返回之前构造好的对象。
// 然而存在这种情况，线程A执行m_instance = new Singleton()还没完成，
// 这个时候m_instance仍然为nullptr，
// 线程B也正在执行m_instance = new Singleton()，
...
namespace n2 {
// 线程安全版本，但锁的代价过高
// 这种写法不会出现上面两个线程都执行new的情况，
// 当线程A在执行m_instance = new Singleton()的时候，
// 线程B如果调用了getInstance()，一定会被阻塞在加锁处，等待线程A执行结束后释放这个锁。
// 从而是线程安全的。
// 但这种写法的性能不高，因为每次调用getInstance()都会加锁释放锁，
// 而这个步骤只有在第一次new Singleton()才是有必要的，
// 只要m_instance被创建出来了， 不管多少线程同时访问，
// 使用if(m_instance == nullptr)进行判断都是足够的（只是读操作，不需要加锁），
...
// 在相当长的一段时间，迷惑了很多人，在2000年的时候才被人发现漏洞，而且在每种语言上都发现了。
// 原因是内存读写的乱序执行（编译器的问题）。
class Singleton {
 private:
  Singleton(){};
...
std::mutex Singleton::m_mutex;
Singleton* Singleton::getInstance() {
  // 线程A执行到new那行，被reorder，申请了内存并赋值给m_instance，此时线程B执行到这里，
  // m_instance不为空，直接返回，但此时m_instance不可用（没调用构造函数）。
  if (m_instance == nullptr) {
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_instance == nullptr) {
      ...
      // 可能会认为这三个步骤是按顺序执行的，但实际上只能确定步骤1是最先执行的，步骤2，3却不一定。
      // 问题就出现在这：
      // 假如线程A在调用执行m_instance=new Singleton()的时按照1,3,2的顺序的，
      // 那么刚刚执行完步骤3给Singleton类型分配了内存（此时m_instance就不是nullptr了）就切换到了线程B，
      // 由于m_instance已经不是nullptr了，
      // 所以B直接执行return m_instance得到一个对象，
...
// java和c#发现这个问题后，就加了一个关键字volatile，在声明m_instance变量的时候，
...
// 同样，他也需要一个once_flag的参数，用法和pthread_once类似，并且支持跨平台。
...
namespace n7 {
// 饿汉模式一
// 在C++中非局部静态对象的初始化顺序的不确定性可能会导致上述饿汉模式工作不正常。
// 考虑：如果有两个这样的单例类，将分别生成单例对象A，单例对象B。
// 它们分别定义在不同的编译单元（cpp中），而A的初始化依赖于B，即A的构造函数中要调用B::GetInstance()，
// 而此时B::m_instance可能还未初始化，显然调用结果就是非法的，
...
namespace n8 {
// 饿汉模式二（boost实现）
// 单例对象作为静态局部变量，然后增加一个辅助类，并声明一个该辅助类的类静态成员变量，
// 在该辅助类的构造函数中，初始化单例对象。
class Singleton {
 public:
  static Singleton* GetInstance() {

...
// 和map、multimap不同，使用set存储的各个键值对，要求key和value必须相等。
// 当使用set存储键值对时，只需要为其提供各键值对中的value值即可。
// map、multimap都会自行根据键的大小对存储的键值对进行排序，set也会如此，
// 只不过set中各键值对的key和value是相等的，根据key排序等价为根据value排序。
// 使用set容器存储的各个元素的值必须各不相同。
// 从语法上讲set并没有强制对存储元素的类型做const修饰，即元素的值是可以修改的。
// 但是，C++标准为了防止用户修改容器中元素的值，对所有可能的操作做了限制，
// 使得在正常情况下，用户是无法做到修改set容器中元素的值的。
// 切勿尝试直接修改set中已存储元素的值，这很有可能破坏set中元素的有序性，
// 最正确的修改set中元素值的做法是：先删除该元素，然后再添加一个修改后的元素。

// set容器定义于<set>头文件，并位于std命名空间中，set容器的类模板定义如下：
// template <
//   class T,                    键key和值value的类型
//   class Compare = less<T>,    指定set容器内部的排序规则
//   class Alloc = allocator<T>  指定分配器对象的类型
// > class set;
// 由于set容器存储的各个键值对，其键和值完全相同，也就意味着它们的类型相同，
// 因此set容器类模板的定义中，仅有第1个参数用于设定存储数据的类型。
...
// cbegin()         和begin()功能相同，只不过增加了const属性，不能元素值。
// cend()	          和end()功能相同，只不过增加了const，不能修改元素值。
// crbegin()        和rbegin()功能相同，只不过增加了const属性，不能修改元素值。
// crend()	        和rend()功能相同，只不过增加了const，不能修改元素值。
// find(val)        在set中查找val，如果找到则返回指向该元素的双向迭代器；
//                  反之，则返回和end()方法一样的迭代器。
// lower_bound(val) 返回指向set中第一个大于或等于val的元素的双向迭代器。
// upper_bound(val) 返回指向set中第一个大于val的元素的迭代器。
...
// max_size()       返回set所能容纳元素的最大个数，不同的操作系统返回值不相同。
// insert()         向set容器中插入元素。
// erase()	        删除set容器中存储的元素。
// swap()	          交换2个set中存储的所有元素，2个set的类型必须相同。
// clear()	        清空set容器中所有的元素，即令set容器的size()为0。
// emplace()        在set中的指定位置直接构造新元素，和插入一样，但效率更高。
// emplace_hint()   在本质上和emplace()在set容器中构造新元素的方式是一样的，
//                  不同之处在于必须提供一个指示新元素生成位置的迭代器作为参数。
// count(val)       在set中查找值为val的元素的个数，并返回，set最多返回1。

// 对于begin()、end()、rbegin()、rend()、find()、lower_bound()、upper_bound()，
// 如果set容器用const限定，则该方法返回的是const类型的双向迭代器。

// 和map不同，set类模板中未提供at()成员函数，也未对[]运算符进行重载。
// 因此，要想访问set容器中存储的元素，只能借助set容器的迭代器。
// set配置的迭代器类型为双向迭代器，只能进行++p、p++、--p、p--、*p操作，
// 并且2个双向迭代器之间做比较，也只能使用==或者!=运算符。
// 在set提供的返回的迭代器的成员函数，指向的只是set中存储的元素，而不再是键值对。
...
void func1() {
  ...
  std::set<char> s1;
  // 创建了set容器，该容器采用默认的std::less<T>规则，进行升序排序。
  // 由于set支持随时向内部添加新的元素，因此创建空set的方法是经常使用的。
  print(s1);  // [0]

  // 2.set还支持在创建的同时，进行初始化，其采用默认的std::less<T>：
  ...
  std::set<char> s4 = s2;
  print(s4);  // a b c z [4]
  // C++11标准还为set类模板新增了移动构造函数，利用临时的set容器为其初始化：
  std::set<char> s5(retSet());
  print(s5);  // d h i [3]
  std::set<char> s6 = retSet();
  print(s6);  // d h i [3]
  // 由于retSet()返回值是一个临时set容器，因此在初始化copyset容器时，
  // 其内部调用的是set类模板中的移动构造函数，而非拷贝构造函数。
  // 无论是调用拷贝构造还是调用移动构造，都必须保证这2个容器的类型完全一致。

  // 4.在第3种方式的基础上，set类模板还支持用set中的部分元素来初始化：
  std::set<char> s7(++s2.begin(), s2.end());
  print(s7);  // b c z [3]

  // 5.借助set类模板定义中第2个参数，完全可以手动修改set的排序规则：
...
void func2() {  // insert
  ...
  //   当向set添加元素成功时，该迭代器指向set新添加的元素，bool的值为true；
  //   如果添加失败，即元素已存在，此时返回的迭代器就指向容器中相同的此元素，
  //   同时bool类型的值为false。
  std::set<char> s;
  std::pair<std::set<char>::iterator, bool> retpair;
  char ch = 'a';
  retpair = s.insert(ch);  // 采用普通引用传值方式
  ...
  // 2.insert()还可以指定将新元素插入到set容器中的具体位置
  //   以普通引用的方式传递val值：
  ...
  //   当添加失败时，证明元素存在，该迭代器就指向set容器中相同的这个元素。
  ch = 'c';  // 采用普通引用传值方式
  ...
  // 如果该元素破坏了容器的有序状态，set还会自行对新元素的位置做进一步调整。
  // 也就是说，insert()中指定新元素插入的位置，并不一定就是该元素最终的位置。

  // 3.insert()支持向set容器中插入其它set容器指定区域内的所有元素，
  //   只要这2个set容器存储的元素类型相同即可：
  //   template <class InputIterator>
  //   void insert(InputIterator first, InputIterator last);
  //   其中first和last都是迭代器，[first,last)可以表示另一set的一块区域。
...
void func3() {
  ...
  // 相比具有同样功能的insert()，emplace()和emplace_hint()的效率会更高。
  // emplace()方法的语法格式如下：
  // template <class... Args>
  ...
  // 和insert()方法一样，虽然emplace_hint()中指定了添加新元素的位置，
...
void func4() {
  ...
  // 后2种格式，返回值都是迭代器，其指向的是set中删除元素之后的第一个元素。
  // 如果要删除的元素就是set最后一个元素，则erase()返回和end()返回的相同。
...
void func5() {
  ...
  // 表示set中和参数val相等元素所在的区域，但由于set存储的元素各不相等，

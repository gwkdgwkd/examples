...
// 无法从cosnt_iterator转换到iterator，
// 也无法从const_reverse_iterator得到reverse_iterator。
// 意味着如果得到一个const迭代器，
// 很难将这些迭代器与容器的某些成员函数一起使用。
// 常量迭代器可以用于许多算法，这些算法并不关心面对的是什么类型的迭代器，
// 只关心这些迭代器属于何种类别。
// 容量类的很多成员函数也都可以接受常量迭代器，
// 只有insert和erase的某些形式显得有些吹毛求疵。

// 通常情况下，
// 是可以把cosnt_iterator或const_reverse_iterator转换为iterator的。
// 但并不总是可能的，甚至即使可行，
// 其做法也不会非常显然，同时可能还会非常低效。
...
int main() {
  ...
  // iterator在比较之前应该被隐式转换成了const_iterator，
  // 真正比较的是两个const_iterator。
  // 对于涉及良好的STL实现而言，情况确实如此。
  // 但对于其他一些实现，这段代码甚至无法通过编译。
  // 原因在于，这些STL实现将const_iterator的等于操作符作为：
  // 一个成员函数而不是一个非成员函数。
  // 解决办法：只要交换两个iterator的位置，就万事大吉了。也是相等?

  // 不仅在进行比较的时候会发生这样的问题，
  // 只要在同一个表达式中混用iterator和const_iterator，
  // 这样的问题就会出现：
  ...
  // 避免这种问题的最简单办法是减少混用不同类型的迭代器，
  // 尽量使用iterator来代替const_iterator。
  // 从const正确性的角度来看，
  // 仅仅为了避免一些可能存在的STL实现缺陷而放弃cosnt_iterator显得有欠公允。

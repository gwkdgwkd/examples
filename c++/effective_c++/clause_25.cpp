...
void swap<Widget>(Widget& a, Widget& b) {
  // 函数名后的<Widget>表示这一特化版本针对”T是Widget“而设计。
...
// C++根据名称查找规则，会找到WidgetStuff内的Widget1专属版本，就是我们想要的。
...
void doSomething(T& obj1, T& obj2) {
  ...
  // 这正是”你的class对std::swap进行全特化“的重要原因。
  // 使得类型专属的swap实现版本也让可以被上面的“迷途代码”所用。
}
// 一旦编译器看到对swap的调用，便查找适当的swap并调用。
// C++的查找法则确保将找到global作用域或T所在命名空间内的任何T专属的swap。
// 如果T是Widget1，编译器会使用”实参数取决之查找规则“找出WidgetStuff内的swap。
// 如果没有T专属的swap存在，编译器就使用std内的swap，这要感谢using声明让std::swap在函数内可用。
...
// 最后，如果你调用swap，确定包含一个using声明，以便让std::swap在你的函数内可见，
// 染红不加任何namespace修饰符，赤裸裸地调用swap。

// 成员版swap绝不可抛出异常。
// 因为swap的一个最好的应用是帮助class（和class template）提供强烈的异常安全性保障。
// 上面的约束只针对成员版，不针对非成员版，因为swap缺省版本是以copy构造函数和copy assignment操作符为基础，
// 而一般情况下两者都允许抛出异常。

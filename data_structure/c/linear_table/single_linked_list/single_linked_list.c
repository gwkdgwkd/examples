...
//   b.首元节点，由于头节点的缘故，链表中称第一个存有数据的节点为首元节点，
//     首元节点只是对链表中第一个存有数据节点的一个称谓，没有实际意义；
//   c.其他节点，链表中其他的节点。
// 链表中有头节点时，头指针指向头节点，若链表中没有头节点，则头指针指向首元节点。
...
link* initLinkNoHead() {
  ...
  for (int i = 2; i < 5; i++) {  // 从第二个节点开始创建
    ...
    temp->next = a;
    // 指针temp每次都指向新链表的最后一个节点，其实就是a节点：
    temp = temp->next;  // temp = a;也可以
  }

  return p;  // 只返回头指针p即可，通过头指针即可找到整个链表
}
// 含头节点的链表
...
// 其中，从链表上摘除某节点的实现非常简单，只需找到该节点的直接前驱节点temp，执行：

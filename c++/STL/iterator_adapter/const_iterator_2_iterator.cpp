...
// C++ STL标准库为了方便用户更轻松地操作容器，每个容器的模板类都提供有丰富且实用的方法。
...
// 2.将reverse_iterator类型的迭代器隐式转换为const_reverse_iterator类型的迭代器。
...
// 但可以明确的是，强制类型转换并不适用于迭代器，因为const_cast的功能仅是去掉某个类型的const修饰符，
...
// 其中，citer为指向某个容器任意位置的const_iterator（或者const_reverse_iterator）类型迭代器，
// 而iter通常初始为指向容器中第一个元素的iterator（或者reverse_iterator）类型迭代器。
// 通过套用此格式，最终iter会变成一个指向和citer一样的iterator（或者reverse_iterator）类型迭代器。
...
int main() {
  ...
  v.insert(cit, 10);
  v.insert(v.begin(), 11);  // 隐式转换，从iterator到cosnt iterator的转换

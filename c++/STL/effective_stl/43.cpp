...
//  有些STL包含的算法实现考虑到deque的内部数据结构，这些算法实现比“普通”的算法实现了快20%多。
//  STL算法的实现未必一定都针对deque进行了优化，但无论如何实现者肯定比你更了解内部的实现细节，可以在算法中充分利用这些知识。
// 其次的性能增益在于，除了一些不太重要的算法以外，其他几乎所有的STL算法都使用了复杂的计算科学算法，有些算法非常复杂，
//  并非一般C++程序员能够达到。例如你很难在算法级别上写出比STL的sort或类似的算法更有效的代码。
...
// 算法的名称比普通的循环更有意义，这已是不争的事实。想要表明在一次迭代中该完成什么工作，则使用循环比算法更为清晰。

// 使用了STL的精巧的C++程序比不用STL的程序所包含的循环要少的多。
// 任何时候都应该尽量用较高层次的insert、find和for_each来替换较低层次的for、while和do，

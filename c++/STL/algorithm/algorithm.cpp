...
// find()、merge()、sort()等算法函数都至少要用一对迭代器来指明作用区间，并且为了实现自己的功能，
// 每个函数内部都势必会对指定区域内的数据进行遍历操作。
// 虽然这些算法函数的内部实现我们不得而知，但无疑都会用到循环结构。
...
int main() {
  ...
  // 前者会依照mycomp()规则查找和"abcd"匹配的元素，而find()函数底层仅会依据"=="运算符
  // 查找s容器中和"abcd"相等的元素，所以会查找失败。

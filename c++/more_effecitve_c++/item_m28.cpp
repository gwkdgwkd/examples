...
//   对于某些类型的灵巧指针来说，期望的行为是自动拷贝它们所指向的对象或用对这些对象进行赋值操作，
//   也就是进deep copy(深层拷贝)。
...
// 灵巧指针从模板中生成，因为要与内建指针类似，必须是strongly typed(强类型)的；
// 模板参数确定指向对象的类型。
// 大多数灵巧指针模板看起来都象这样：
template <class T>
class SmartPtr {
...
// 对于灵巧指针类来说，不能允许进行拷贝和赋值操作，它们应该被声明为private。
// 两个dereference操作符被声明为const，是因为dereference一个灵巧指针时不会对其自身进行修改，
// const型的灵巧指针才可以使用这两个操作符。
// 最后，每个指向T对象的灵巧指针包含一个指向T的dumb pointer。
// 这个dumb pointer指向的对象才是灵巧指针指向的真正对象。

// 使用灵巧指针与使用dump pointer没有很大的差别，这表明了封装是非常有效的。
// 灵巧指针的用户可以象使用dumb pointer一样使用灵巧指针，有时这种替代会更透明化。

// 灵巧指针的构造、赋值和析构
// 灵巧指针的的构造通常很简单：
// 1.找到指向的对象（一般由灵巧指针构造函数的参数给出），让灵巧指针的内部成员dumb pointer指向它。
// 2.如果没有找到对象，把内部指针设为0或发出一个错误信号（可以是抛出一个异常）。
...
// 如果我们只拷贝内部的dumb pointer，会导致两个auto_ptr指向一个相同的对象。
// 这是一个灾难，因为当释放auto_ptr时每个auto_ptr都会删除它们所指的对象。
// 这意味着一个对象会被我们删除两次，这种两次删除的结果将是不可预测的（通常是灾难性的）。
// 另一种方法是通过调用new，建立一个所指对象的新拷贝。
// 这确保了不会有许多指向同一个对象的auto_ptr，但是建立（以后还得释放）新对象会造成不可接受的性能损耗。
// 并且不知道要建立什么类型的对象，因为auto_ptr<T>对象不用必须指向类型为T的对象，它也可以指向T的派生类型对象。
// 虚拟构造函数可能帮助我们解决这个问题，但是好象不能把它们用在auto_ptr这样的通用类中。
// 如果quto_ptr禁止拷贝和赋值，就可以消除这个问题，
// 但是采用“当auto_ptr被拷贝和赋值时，对象所有权随之被传递”的方法，是一个更具灵活性的解决方案：
template <class T>
...
// 在返回的这种对象上调用虚拟函数，不会触发与（原先）所指对象的动态类型相符的函数。
// 实际上就是说你的灵巧指针将不能支持虚拟函数，象这样的指针再灵巧也没有用。
// 而返回一个引用还能够具有更高的效率。能够兼顾正确性与效率当然是一件好事。
// operator->的情况与operator*是相同的。
// operator->仅能返回两种东西：一个指向某对象的dumb pointer或另一个灵巧指针。
// 多数情况下，你想返回一个普通dumb pointer。在此情况下，你这样实现operator->：
template <class T>
T* SmartPtr<T>::operator->() const {
  // perform "smart pointer" processing;
  return pointee;
...
// 如果你想更深入地了解灵巧指针，
// 你必须知道更多的有关dumb pointer的知识和灵巧指针如何能或不能进行模拟dumb pointer。
...
// 有趣的是，iostream库的实现除了提供void*隐式的类型转换，也有operator!函数，
// 不过这两个函数通常测试的流状态有些不同。
...
// 并且这个函数也消除了测试空值的问题。
// 然而，它也有类型转换函数所具有的缺点。它使得用户能够很容易地直接访问dumb指针，
// 绕过了“类指针（pointer-like）”对象所提供的“灵巧”特性:
...
// 例如在music类层次内，
// 在Cassette和CD的灵巧指针类内你可以加入operator SmartPtr<MusicProduct>函数：
template <class T>
class SmartPtr3 {
 public:
...
class SmartPtr4 {
 public:
  ...
  // 感谢最近的语言扩展，让你能够做到，这个扩展能声明（非虚）成员函数模板（通常就叫成员模板（member template))，
  // 你能使用它来生成灵巧指针类型转换函数，如下：
  template <class newType>
  operator SmartPtr4<newType>() {  // 为了实现隐式类
    return SmartPtr4<newType>(pointee);
  }
  // 编译器首先检查SmartPtr<T>的类定义，看其有没有声明明确的类型转换符，但是它没有声明。
  // 这不是指：在上面的模板没有声明类型转换符。
  // 编译器然后检查是否存在一个成员函数模板，并可以被实例化成它所期望的类型转换。
  // 它发现了一个这样的模板（带有形式类型参数newType），所以它把newType绑定成T的基类类型来实例化模板。
  // 这时，惟一的问题是实例化的成员函数代码能否被编译：
  // 传递（dumb）指针pointee到指向“T的基类”的灵巧指针的构造函数，必须合法的。
  // 指针pointee是指向T类型的，把它转变成指向其基类（public或protected）对象的指针必然是合法的，
  // 因此类型转换操作符能够被编译，可以成功地把指向T的灵巧指针隐式地类型转换为指向“T 的基类”的灵巧指针。
...
// 如果你有dumb指针T1*和另一种dumb指针T2*，当且仅当你能隐式地把T1*转换为T2*时，
...
// displayAndPlay被重载，一个函数带有SmartPtr<Cassette> 对象参数，其他函数的参数为SmartPtr<CasSingle>，
// 我们期望调用SmartPtr<Cassette>，因为CasSingle是直接从Cassette上继承下来的，而间接继承自MusicProduct。
// 当然这是dumb指针时的工作方法。我们的灵巧指针不会这么灵巧，它们的转换操作符是成员函数，
// 对C++编译器而言，所有类型转换操作符是同等地位的。
// 因此displayAndPlay的调用具有二义性，
// 因为从SmartPtr<CasSingle>到SmartPtr<Cassette>的类型转换并不比到SmartPtr<MusicProduct>的类型转换优先。
...
//   有些程序员以前从来没有看到过这种技巧，而却被要求维护使用了这种技巧的代码，我真是很可怜他们。
...
// 我们也能用指向non-cosnt对象的指针初始化指向const对象的指针；就像进行赋值一样。例如：
...
// 到目前为止这是一个老问题了，把它们变成赋值兼容的唯一方法是你必须提供函数，
...
// 使用这种设计方法，指向non-const-T对象的灵巧指针包含一个指向const-T的dumb指针，
// 指向const-T的灵巧指针需要包含一个指向const-T的dumb指针。
// 最方便的方法是把指向const-T的dumb指针放在基类里，把指向non-const-T的dumb指针放在派生类里，
// 然而这样做有些浪费，因为SmartPtr对象包含两个dumb指针:
// 一个是从SmartPtrToConst继承来的，一个是SmartPtr自己的。
// 一种在C世界里的老式武器可以解决这个问题，这就是union，它在C++中同样有用。
// Union在protected中，所以所以两个类都可以访问它，它包含两个必须的dumb指针类型，这就是union美丽的地方。
...
int main() {
  ...
  if (ptn4 == ptn5)
    ;
  // 即使在SmartPtr<int> 和 SmartPtr<double>之间没有operator=函数，也能够编译，
  // 因为灵巧指针被隐式地转换为void*指针，而对于内建指针类型有内建的比较函数。
  // 这种进行隐式类型转换的行为特性很危险。
  ...
  delete pt3;  // 这段代码应该不能被编译，pt不是指针，它是一个对象，你不能删除一个对象。
  // 只有指针才能被删除，对么？
  // 编译器使用隐式类型转换来尽可能使函数调用成功；
  // 再回想一下条款M8：使用delete会调用析构函数和operator delete，两者都是函数。
  ...
  // 这个bug极为有害，
  // 因为隐藏在灵巧指针后面的全部思想就是让它们不论是在外观上还是在使用感觉上都与dumb指针尽可能地相似。
  // 你越接近这种思想，你的用户就越可能忘记正在使用灵巧指针。
  // 如果他们忘记了正在使用灵巧指针，肯定会在调用new之后调用delete，以防止资源泄漏，谁又能责备他们这样做不对呢？
  ...
  // 编译器侦测到类型不匹配，于是寻找把funMusic转换成SmartPtr<MusicProduct>对象的方法。
  // 它在SmartPtr<MusicProduct>类里寻找带有SmartPtr<Cassette>类型参数的单参数构造函数，但是没有找到。
  // 然后它们又寻找成员函数模板，以实例化产生这样的函数。
  // 它们在SmartPtr<Cassette>发现了模板，把newType绑定到MusicProduct上，生成了所需的函数。
  // 实例化函数，生成这样的代码：

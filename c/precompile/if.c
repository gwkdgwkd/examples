...
//     程序段4
// #endif
// #if命令要求判断条件为整型常量表达式，也就是说，
// 表达式中不能包含变量，而且结果必须是整数；
// 而if后面的表达式没有限制，只要符合语法就行。
// 这是#if和if的一个重要区别。
...
// 最后需要注意的是，#if后面跟的是整型常量表达式，
// 而#ifdef和#ifndef后面跟的只能是一个宏名，不能是其他的。
// #ifdef可以认为是#if defined的缩写。

int main() {
  // 开发一个C语言程序，让它输出红色的文字，并且要求跨平台，
  // 在Windows和Linux下都能运行，怎么办呢？

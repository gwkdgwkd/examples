...
// m[k]=v;检查键k是否已经在map中了，如果没有，它就被加入，并以v作为响应的值。如果k已经在映射表中了，则与关联的值被更新为v。
// 如果键k已经有了相关联的值，则该值被更新，直接了当。
// 如果还没有在映射表中，那就没有operator[]可以指向的值的对象，在这种情况下，先用值类型的默认构造函数创建一个新的对象，然后operator[]
// 返回一个指向该新对象的引用，最后这个引用成了被赋值的目标，被用值赋值。
// 现在应该明白为什么这种方式会降低性能了。先默认构造对象，然后立刻赋值。
// 如果“直接使用我们所需要的值构造一个对象”比“先默认构造一个对象再赋值”效率更高，那么最好把operator[]的使用换成insert的直接调用。
...
typename MapType::iterator efficientAddOrUpdate(MapType& m, const KeyArgType& k,
  ...
  if (lb != m.end() && !(m.key_comp()(k, lb->first))) {
    ...
    // 使用了“提示”形式的insert，C++标准保证，如果“提示”是正确的，则插入操作将耗费常数时间，而不是对数时间。
  }
...
int main() {
  ...
  m.insert(map<int, Widget>::value_type(4, 36.5)).first->second = 36.5;
  // 从效率上讲，insert需要一个map<int, Widget>::value_type类型的参数（即pair<int,Widget>)，所以当调用insert时，
  // 必须构造和析构一个该类型的对象。付出pair构造函数析构函数的代价。这有导致对Widget的构造和析构动作，因为pair包含了Wiget对象。
  // operator[]不使用pair对象，所以它不会构造和析构任何pair或Widget。

  // 用MapType::key_type和MapType::mapped_type来代替KeyArgType和ValueArgType,会导致调用时把1.5转换成了Widget，
  // 需要为Widget构造和析构付出代价

...
// C++ STL标准库共提供了4种排序函数
// sort()         对指定范围内所有的数据进行排序，排序后相对位置很可能发生改变；
// stable_sort()  对指定范围内所有的数据进行排序，排序后相对位置不发生改变；
...
// 对于list或者forward_list，其容器模板类中都提供有sort()排序方法，
...
// 3.如果需要对最大（或最小）的n个元素进行排序，则优先选择partial_sort()；
// 4.如果只需要找到最大或最小的n个元素，但不要求排序，则优先选择nth_element()。

// 总的来说，函数功能越复杂，做的工作越多，它的性能就越低（主要体现在时间复杂度上）。
// 对于以上4种排序函数，综合考虑它们的时间和空间效率，从左到右，性能由高到低：
// nth_element() > partial_sort() > sort() > stable_sort()
// 建议在实际选择排序函数时，应更多从所需要完成的功能这一角度去考虑，
// 而不是一味地追求函数的性能，换句话说，如果选择的算法更有利于实现所需要的功能，
...
// sort()函数是基于快速排序实现的，stable_sort()函数是基于归并排序实现的。
...
// 排序规则默认以元素值的大小做升序排序，也可以选择标准库提供的其它排序规则，
// 比如std::greater<T>降序排序规则，甚至还可以自定义排序规则。
// 需要注意的是，sort()受到底层实现方式的限制，它仅适用于普通数组和部分容器。
// 换句话说，只有普通数组和具备以下条件的容器，才能使用sort()函数：
// 1.容器支持的迭代器类型必须为随机访问迭代器，这意味着，
//   sort()只对array、vector、deque这3个容器提供支持。
// 2.如果对容器中指定区域的元素做默认升序排序，则元素类型必须支持<小于运算符；
//   如果选用标准库的其它排序规则，元素类型也必须支持该规则底层实现所用的比较运算符；
// 3.sort()函数在实现排序时，需要交换容器中元素的存储位置，这种情况下，
//   如果容器中存储的是自定义的类对象，则该类必须提供移动构造函数和移动赋值运算符。
// 对于指定区域内值相等的元素，sort()函数无法保证它们的相对位置不发生改变。
// sort()函数有2种用法，其语法格式分别为：
// void sort(RandomAccessIterator first,RandomAccessIterator last);
// void sort(RandomAccessIterator first,RandomAccessIterator last,
//           Compare comp);
// 其中，first和last都为随机访问迭代器，[first,last)用来指定要排序的目标区域；
// 另外在第2种格式中，comp可以是C++STL标准库提供的排序规则，比如std::greater<T>，
// 也可以是自定义的排序规则，和为关联式容器设定排序规则不同，
// 给sort()函数指定排序规则时，需要为其传入一个函数名或者函数对象。
...
// partial_sort()可以从指定区域中提取出部分数据，并对它们进行排序。
...
// 并对这部分元素做升序（或降序）排序。
// partial_sort()受到底层实现方式的限制，它仅适用于普通数组和部分类型的容器。
// 换句话说，只有普通数组和具备以下条件的容器，才能使用partial_sort()函数：
// 1.容器支持的迭代器类型必须为随机访问迭代器，这意味着，
//   partial_sort()函数只适用于array、vector、deque这3个容器；
// 2.当选用默认的升序排序规则时，容器中存储的元素类型必须支持<小于运算符，
//   如果选用其它排序规则，元素类型也必须支持该规则底层实现所用的比较运算符；
// 3.partial_sort()函数在实现过程中，需要交换某些元素的存储位置，
//   如果容器中是自定义的类对象，则该类的内部必须提供移动构造函数和移动赋值运算符。
void func() {
...
// partial_sort_copy()和partial_sort()类似，唯一的区别是前者不会修改源数据，
...
// comp用于自定义排序规则。
// partial_sort_copy将[first,last)内最小或大的result_last-result_first个元素，
// 复制到[result_first,result_last)区域中，并对该区域的元素做升序（或降序）排序。
// [first,last]中的这2个迭代器类型仅限定为输入迭代器，这意味着相比partial_sort()，
// partial_sort_copy()函数放宽了对存储原有数据的容器类型的限制。
// 换句话说，partial_sort_copy()函数还支持：
// 对list或者forward_list中存储的元素进行部分排序，而partial_sort()函数不行。
...
//                  RandomAccessIterator last, Compare comp);
// 其中，各个参数的含义如下：
// first和last：都是随机访问迭代器，[first,last)用于指定该函数的作用范围；
// nth：也是随机访问迭代器，其功能是令函数查找第nth大的元素，并将其移动到nth位置；
// comp：用于自定义排序规则。
// 注意，鉴于nth_element()中各个参数的类型，其只能对普通数组或者部分容器进行排序。
// 换句话说，只有普通数组和符合以下全部条件的容器，才能使用nth_element()函数：
// 1.容器支持的迭代器类型必须为随机访问迭代器，这意味着，
//   nth_element()函数只适用于array、vector、deque这3个容器；
// 2.当选用默认的升序排序规则时，容器中存储的元素类型必须支持<小于运算符，
//   如果选用其它排序规则，元素类型也必须支持该规则底层实现所用的比较运算符；
// 3.nth_element()函数在实现过程中，需要交换某些元素的存储位置，因此，
//   如果是自定义的类对象，则该类的内部必须提供移动构造函数和移动赋值运算符。
void func() {
...
// 因此在设计程序时，应该有意识的去避免执行一些不必要的排序操作。
// 怎样才能判断一个序列是否为有序序列呢？
// 很简单，使用is_sorted()函数即可，此函数专门用于判断某个序列是否为有序序列。
...
// comp用于指定自定义的排序规则。
// 注意，如果使用默认的升序排序规则，则[first,last)内的元素必须支持<运算符；
// 如果指定排序规则，也要保证[first,last)内的元素支持该规则内部使用的比较运算符。
// 返回一个bool类型值，如果[first,last)内的序列符合指定的排序规则，则返回true；
// 反之，函数返回false。
...
// [first, last)用于指定要检测的序列；
// comp用于指定自定义的排序规则。
// 如果使用默认的升序排序规则，则[first,last)区域内的元素必须支持<运算符；
// 如果指定排序规则，也要保证[first,last)内的元素支持该规则内部使用的比较运算符。
// 该函数会返回一个正向迭代器。
...
// 该算法都会提供有带comp参数的语法格式，用于接收用户自定义的函数，
// 其定义的方式有2种，既可以是普通函数，也可以是函数对象。

// 为了提高执行效率，其函数都定义为内联函数，在类内部定义的函数本身就是内联函数。
// 函数对象可以理解为伪装成函数的对象，根据以往的认知，
// 函数对象的执行效率应该不如普通函数，但事实恰恰相反，
// 即便将普通函数定义为更高效的内联函数，其执行效率也无法和函数对象相比。
// 通过在4个不同的STL平台上，对包含100万个double类型数据的vector容器进行排序，
// 最差情况下使用函数对象的执行效率要比普通内联函数高50%，最好情况下则高160%。
// 那么，是什么原因导致了它们执行效率上的差异呢？
// 以mycomp2()函数对象为例，其mycomp::operator()也是一个内联函数，
// 编译器在对sort()函数进行实例化时会将该函数直接展开，
// 这也就意味着，展开后的sort()函数内部不包含任何函数调用。
// 而如果使用mycomp1作为参数来调用sort()函数，情形则大不相同。
...
//           bool (*comp)(int, int));
// 可以看到，参数comp只是一个指向函数的指针，所以sort()内部每次调用comp时，
// 编译器都会通过指针产生一个间接的函数调用。
// 也正是基于这个原因，C++ sort()函数要比C语言qsort()函数的执行效率更高。
// 可能会问，程序中comp()函数也是内联函数，为什么C++不像函数对象那样去处理呢？
// 具体原因无从得知，事实上也没必要关心，也许是编译器开发者觉得这种优化不值得去做。
// 除了效率的优势之外，相比普通函数，以函数对象的方式自定义规则还有很多隐藏的优势。
// 例如在某些特殊情况下，以普通函数编写的代码看似非常合理，但就是无法通过编译，
// 这也许是由于STL标准库的原因，也许是编译器缺陷所至，甚至两者都有可能。
// 而使用函数对象的方式，则可以有效避开这些坑，而且还大大提升的代码的执行效率。

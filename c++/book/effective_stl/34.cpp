...
// 并不一定要求排序的区间，但通常情况下与区间一起使用：unique、unique_copy

// 所有要求排序区间的算法unique、unique_copy除外，
// 均使用等价性来判断两个对象是否相同，这与标准关联容器一致。
// unique、unique_copy默认使用相等来判断两个对象是否相同，可以改变这种默认行为。

// 这11个算法之所以要求排序的区间，目的是为了提供更好的性能。
// 只要提供给它们的是排序区间，并保证这些算法的比较函数与排序所使用的比较函数一致，
// 就可以有效地使用这些与查找、集合操作以及区间合并有关的算法，
// 并且会惊喜地发现，unique和unique_copy如愿地删除了所有重复的值。

namespace n1 {
// binary_search、lower_bound、upper_bound、equal_range要求排序是因为二分法。

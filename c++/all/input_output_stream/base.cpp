...
// 在C语言中，通常会使用scanf和printf来对数据进行输入输出操作。
...
// 在编写C++程序时，如果需要使用输入输出时，
// 则需要包含头文件iostream，它包含了用于输入输出的对象，
// 例如常见的cin表示标准输入、cout表示标准输出、cerr表示标准错误。
// cout和cin都是C++的内置对象，而不是关键字。
// C++库定义了大量的类，程序员可以使用它们来创建对象，
// cout和cin就分别是ostream和istream类的对象，
// 只不过它们是由标准库的开发者提前创建好的，可以直接拿来使用。
// 这种在C++中提前创建好的对象称为内置对象。
// 使用cout进行输出时需要紧跟<<运算符，使用cin进行输入时需要紧跟>>运算符，
// 这两个运算符可以自行分析所处理的数据类型，
// 因此无需像使用scanf和printf那样给出格式控制字符串。
// endl表示换行，与C语言里的\n作用相同。
// cout能够连续地输出，同样cin也是支持对多个变量连续输入的。
// cout、cin的用法非常强大灵活，推荐使用cin、cout，
// 它们比C语言中的scanf、printf更加灵活易用。

// 通常情况下，C++程序中使用cin输入流实现数据的输入，用cout输出流实现数据的输出。
// 除此之外，C++还提供有适用于特定场景的输入输出流，比如cerr、clog等。
// C++输入输出流本质上就是已经定义好的类对象，之所以称它们为流，
// C++开发者认为数据传输的过程像水一样，从一个地方流到另一个地方，
// 所以称实现输入的为输入流，实现数据输出的为输出流。
// 作为类对象，C++输入流和输出流不仅可以实现基本的输入输出操作，
// 通过类内部的成员函数，还可以满足特殊场景中的输入输出需求。

// C++又可以称为带类的C，即可以理解为C++是C语言的基础上增加了面向对象。
// 在此基础上，学过C语言的读者应该知道，它有一整套完整的I/O解决方案：
// 1.使用scanf()、gets()等函数从键盘读取数据，
//   使用printf()、puts()等函数向屏幕上输出数据；
// 2.使用fscanf()、fgets()等函数读取文件中的数据，
//   使用fprintf()、fputs()等函数向文件中写入数据。
// 要知道，C语言的这套I/O解决方案也适用于C++程序，但C++并没有偷懒，
// 它自己独立开发了一套全新的I/O解决方案，其中就包含大家一直使用的cin和cout。
// 用cin接收从键盘输入的数据，用cout向屏幕上输出数据，这2个过程又统称为标准I/O。
// 除此之外，C++也对从文件中读取数据和向文件中写入数据做了支持（统称为文件I/O）。
// C++的I/O解决方案是个包含很多类的类库，作为C++标准库的组成部分，常被称为流类。
...
// 特别需要指出的是，为了避免多继承的二义性，
// 从ios派生出istream和ostream时，均使用了virtual关键字（虚继承）。
// 这些流类各自的功能分别为：
//  istream：常用于接收从键盘输入的数据；
//  ostream：常用于将数据输出到屏幕上；
//  ifstream：用于读取文件中的数据；
//  ofstream：用于向文件中写入数据；
//  iostream：继承自istream和ostream类，因为该类的功能兼两者于一身，
//            既能用于输入，也能用于输出；
//  fstream：兼ifstream和ofstream类功能于一身，
//           既能读取文件中的数据，又能向文件中写入数据。

// cin就是istream类的对象，cout是ostream类的对象，
// 它们都声明在<iostream>头文件中，还要声明std命名空间。
// <iostream>头文件中还声明有2个ostream类对象，分别为cerr和clog。
...
//   等缓冲区满或者手动换行（换行符'\n'或endl）时，才会将数据全部显示到屏幕上；
//   而cerr则不设缓冲区，它会直接将数据输出到屏幕上。
// 除了以上2点特性上的不同之外，cerr、clog和cout没有任何不同。
// 之所以常用cout，是因为cerr和clog有各自不同的适用场景，以cerr为例，
// 一旦程序某处使用cerr输出数据，自然而然地会认为此处输出的是警告或者错误信息。
// 类似cin、cout、cerr和clog这样，它们都是C++标准库的开发者创建好的，
// 可以直接拿来使用，这种在C++中提前创建好的对象称为内置对象。
// 实际上，<iostream>头文件中还声明有处理宽字符的4个内置对象，
// 分别为wcin、wcout、wcerr以及wclog。

// istream和ostream类提供了很多实用的函数，
// cin、cout、cerr和clog作为类对象，当然也能调用。
// cin输入流对象常用成员方法：
//  getline(str,n,ch) 	从输入流中接收n-1个字符给str变量，当
//                      遇到指定ch字符时会停止读取，默认情况下ch为'\0'。
...
//  ignore(n,ch)        从输入流中逐个提取字符，但提取出的字符被忽略，不被使用，

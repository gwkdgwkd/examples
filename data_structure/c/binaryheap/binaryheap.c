...
// 二叉堆（Binary Heap）没什么神秘，性质比二叉搜索树BST还简单。
// 其主要操作就两个，sink（下沉）和swim（上浮），用以维护二叉堆的性质。
// 其主要应用有两个，首先是一种排序方法堆排序，第二是一种很有用的数据结构优先级队列。
// 二叉堆其实就是一种特殊的二叉树（完全二叉树），只不过存储在数组里。
// 一般的链表二叉树操作节点的指针，在数组里，把数组索引作为指针。
// 二叉堆还分为：
// 大顶堆，每个节点都大于等于它的两个子节点；
// 小顶堆，每个节点都小于等于它的子节点。

// 优先级队列这种数据结构有一个很有用的功能，插入或者删除元素的时候，
// 元素会自动排序，这底层的原理就是二叉堆的操作。
// 数据结构的功能无非增删查该，优先级队列有两个主要API：
// 1.insert插入一个元素；
// 2.delMax删除最大元素，如果底层用小顶堆，那么就是delMin。
...
// 对于大顶堆，会破坏堆性质的有有两种情况：
// 1.如果某个节点A比它的子节点（中的一个）小，那么A就不配做父节点，
//   应该下去，下面那个更大的节点上来做父节点，这就是对A进行下沉；
// 2.如果某个节点A比它的父节点大，那么A不应该做子节点，
//   应该把父节点换下来，自己去做父节点，这就是对A的上浮。
...
// 但是下沉某个节点A，需要A和其两个子节点比较大小，
// 如果A不是最大的就需要调整位置，要把较大的那个子节点和A交换。
void sink(int k) {
  while (left(k) <= pqSize) {  // 如果沉到堆底，就沉不下去了
...
// 因为时间复杂度主要花费在sink或者swim上，而不管上浮还是下沉，
// 最多也就树（堆）的高度，也就是log级别。

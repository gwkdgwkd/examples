...
int main(void) {
  ...
  // 这种限制无疑是很没有必要的。在C++的各种成对括号中，目前只有右尖括号连续写两个会出现这种二义性。
  // static_cast、reinterpret_cast等C++标准转换运算符，都是使用<>来获得待转换类型（type-id）的。
  // 若这个type-id本身是一个模板，用起来会很不方便。

  // 在C++11中，这种限制终于被取消了，要求编译器对模板的右尖括号做单独处理，
  // 使编译器能够正确判断出>>是一个右移操作符还是模板参数表的结束标记（delimiter，界定符）。
...
// 各种C++98/03编译器除了支持标准（ISO/IEC14882：2003及其之前的标准）之外，还自行做了不少的拓展。

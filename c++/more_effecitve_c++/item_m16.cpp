...
// 一个又一个程序员一本正经地宣称程序的性能问题已被找到，因为网络的延迟，
// 不正确的内存分配，编译器没有进行足够的优化或者一些笨蛋主管拒绝在关键的循环里使用汇编语句。
...
// 所以如果你用不具有代表性的输入数据profile一个程序，那你所进行的profile也没有代表型。
// 必须确保每组数据在客户（或至少是最重要的客户）如何使用软件的方面能有代表性。
// 通常获取有代表性的数据是很容易的，因为许多客户都愿意让你用他们的数据进行profile。
// 毕竟你是为了他们需求而优化软件。

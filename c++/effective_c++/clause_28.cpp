...
class Rectangle {
 public:
  ...
  // 1.一方面upperLefr和lowerRight被声明为const成员函数，因为只是为了提供获取的坐标点的方法，而不让客户修改。
...
// 而这些operator[]就是返回引用指向“容器内的数据”，那些数据随容器被销毁而销毁。

// 请记住：
// 避免返回handles（包括引用、指针、迭代器）指向对象内部。
// 遵守这个条款可增加封装性，帮助const成员函数的行为像个const，并将发生“虚吊号码牌”的可能性降至最低。

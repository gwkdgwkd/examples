...
// 多继承的派生类继承了所有父类的成员，尽管概念上非常简单，
// 但是多个基类的相互交织可能会带来错综复杂的设计问题，命名冲突就是不可回避的一个。
// 多继承时很容易产生命名冲突，即使将所有类中的成员变量和成员函数都命名为不同的名字，
// 命名冲突依然有可能发生，比如典型的是菱形继承。
// 在一个派生类中保留间接基类的多份同名成员，
// 虽然可以在不同的成员变量中分别存放不同的数据，但大多数情况下这是多余的：
...
// istream和ostream必须采用虚继承，否则iostream中将保留两份base_ios类的成员。
// 因为在虚继承的最终派生类中只保留了一份虚基类的成员，可以被直接访问，没有二义性。
// 此外，如果虚基类的成员只被一条派生路径覆盖，那么仍然可以直接访问这个被覆盖的成员。
// 但是如果该成员被两条或多条路径覆盖了，那就不能直接访问了，必须指明该成员属于哪个类。
...
// 所以C++之后的很多面向对象的编程语言，例如Java、C#、PHP等，都不支持多继承。
...
// 编译器总是先调用虚基类的构造函数，再按照声明的顺序调用其他的构造函数；
// 而对于普通继承，就是按照类声明时基类出现的顺序依次调用的。
class A {
 public:
...
class D : public B, public C {
 public:
  ...
  // 再由直接基类的构造函数初始化间接基类，尝试调用间接基类的构造函数将导致错误。
  // 虚基类A在最终派生类D中只保留了一份成员变量m_a，如果由B和C初始化m_a，
  // 那么B和C在调用A的构造函数时很可能传不同的实参，编译器不知道使用哪个初始化m_a。
...
// 该指针指向了一个虚表（virtual table），虚表中记录了vbptr与本类的偏移地址；

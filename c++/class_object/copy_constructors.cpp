...
//  初始化的时候还可以为对象分配其他的资源（打开文件、连接网络、动态分配内存等），或者提前进行一些计算等。
//  说白了，初始化就是调用构造函数。
...
// 对于基本类型的数据，很少会区分「初始化」和「赋值」这两个概念，即使将它们混淆，也不会出现什么错误。
// 但是对于类，它们的区别就非常重要了，因为初始化时会调用构造函数（以拷贝的方式初始化时会调用拷贝构造函数），
// 而赋值时会调用重载过的赋值运算符。

// 如果没有显式地定义拷贝构造函数，那么编译器会自动生成一个默认的拷贝构造函数。
// 这个默认的拷贝构造函数很简单，使用“老对象”的成员变量对“新对象”的成员变量进行一一赋值。
// 对于简单的类，默认拷贝构造函数一般是够用的，我们也没有必要再显式地定义一个功能类似的拷贝构造函数。
// 但是当类持有其它资源时，如动态分配的内存、打开的文件、指向其他数据的指针、网络连接等，
// 默认拷贝构造函数就不能拷贝这些资源，我们必须显式地定义拷贝构造函数，以完整地拷贝对象的所有数据。
...
// 1.将其它对象作为实参，即使没有定义拷贝构造函数，也是有效的，因为编译器会生成默认的拷贝构造函数；
...
// 3.函数的形参为类类型，调用时要将另外一个对象作为实参传递进来赋值给形参，这也是以拷贝的方式初始化形参对象；
void func2(A a) {
  // 函数是一段可以重复使用的代码，只有等到真正调用函数时才会为局部数据（形参和局部变量）在栈上分配内存。
  // 形参a在定义函数时对象并没有被创建，只有等到调用函数时才会真正地创建a对象，并在栈上为它分配内存。
  // 而创建a对象，就是以拷贝的方式进行的。

  // 其实形参在定义时就为它分配了内存，但是此时并没有初始化，只有调用时才会初始化。是这样还是上面那样？
}
void func2() {
  A a;       // A
  func2(a);  // A copy
...
void func3() {
  ...
  // 理论上讲，运行代码后会调用两次拷贝构造函数，一次是返回a对象时，另外一次是创建a对象时。
  // 当函数的返回值为类类型时，return语句会返回一个对象，不过为了防止局部对象被销毁，
  // 也为了防止通过返回值修改原来的局部对象，编译器并不会直接返回这个对象，
  // 而是根据这个对象先创建出一个临时对象（匿名对象），再将这个临时对象返回。
  ...
  // 在较老的编译器上，你或许真的能够看到调用两次拷贝构造函数，例如iPhone上的C/C++编译器。
...
// 1.为什么必须是当前类的引用呢？
//  如果拷贝构造函数的参数不是当前类的引用，而是当前类的对象，那么在调用拷贝构造函数时，
//  会将另外一个对象直接传递给形参，这本身就是一次拷贝，会再次调用拷贝构造函数，
//  这个过程会一直持续下去，没有尽头，陷入死循环。
//  只有当参数是当前类的引用时，才不会导致再次调用拷贝构造函数，这不仅是逻辑上的要求，也是C++语法的要求。
// 2.为什么是const引用呢？
//  拷贝构造函数的目的是用其它对象的数据来初始化当前对象，并没有期望更改其它对象的数据，
//  添加const限制后，这个含义更加明确了。
//  另外一个原因是，添加const后，可以将const对象和非const对象传递给形参，否则只能把非const对象传给形参，
//  因为非const类型可以转换为const类型，而const类型不能转换为非const类型。
...
// 但是当类持有其它资源时，例如动态分配的内存、指向其他数据的指针等，默认的拷贝构造函数就不能拷贝这些资源了，
// 我们必须显式地定义拷贝构造函数，以完整地拷贝对象的所有数据。
...
// 才能将指针指向的内容再复制出一份来，让原有对象和新生对象相互独立，彼此之间不受影响。
...
// 又因为printArray()函数不会修改任何数组元素，所以添加了const限制，以使得语义更加明确。

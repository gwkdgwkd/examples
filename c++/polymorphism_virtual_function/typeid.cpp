...
// typeid会把获取到的类型信息保存到一个type_info类型的对象里面，并返回该对象的常引用；
// 当需要具体的类型信息时，可以通过成员函数来提取。
// typeid的使用非常灵活，它的操作数可以是普通变量、对象、内置类型、自定义类型，还可以是一个表达式。

// 成员函数：
// name()用来返回类型的名称。
// raw_name()用来返回名字编码（Name Mangling）算法产生的新名称。
// hash_code()用来返回当前类型对应的hash值。
//  hash值是一个可以用来标志当前类型的整数，有点类似学生的学号、公民的身份证号、银行卡号等。
...
// C++标准规定，type_info类至少要有如下所示的4个public属性的成员函数，其他的扩展函数编不做限制。
// 1.const char* name() const;
//   返回一个能表示类型名称的字符串，但是C++标准并没有规定这个字符串是什么形式的。
//   对于int和Base，在VC/VS下的name()输出结果是int和class Base，而在GCC下的输出结果分别是i和4Base。
...
// 可以发现，不像Java、C#等动态性较强的语言，C++能获取到的类型信息非常有限，也没有统一的标准，
// 如同“鸡肋”一般，大部分情况下我们只是使用重载过的“==”运算符来判断两个类型是否相同。
...
namespace n3 {
// typeid返回type_info对象的引用，而表达式typeid(a)==typeid(b)的结果为true，可以说明，
// 一个类型不管使用了多少次，编译器都只为它创建一个对象，所有typeid都返回这个对象的引用。
// 需要提醒的是，为了减小编译后文件的体积，编译器不会为所有的类型创建type_info对象，
// 只会为使用了typeid运算符的类型创建。
// 不过有一种特殊情况，就是带虚函数的类（包括继承来的），不管有没有使用typeid运算符，
// 编译器都会为带虚函数的类创建type_info对象。

// type_info的构造函数是private属性的，所以不能在代码中直接实例化，只能由编译器在内部实例化（借助友元）。
...
void func2() {
  ...
  // 表达式typeid(*p1)==typeid(Base)和typeid(p1)==typeid(Base*)的结果为true可以说明：
  // 即使将派生类指针p2赋值给基类指针p1，p1的类型仍然为Base*。
}

...
// 这真正是接口与实现分离，这个分离的关键在于以声明的依赖性替换定义的依赖性，
// 那正是编译依赖性最小化的本质：
...
/*
// 另一个制作Handle class的办法是，令Person成为抽象基类，称为Interface class。
// 这种class的目地是详细描述derived class的接口，因此通常不带成员变量，
// 也没有构造函数，只有一个virtual析构函数以及一组pure virtual函数，用来描述整个接口。
// Interface class类似Java和.NET的Interfaces，但C++的Interface class，
...
class Person{
 public:
  ...
  // 返回指针（或智能指针），指向动态分配的对象，而该对象支持Interface class的接口。
...
// 就像Handle classes的客户一样，除非Interface class的接口被修改否则不需要重新编译。
...
// 2.如果能够，尽量以class声明替换class定义，声明一个函数用到一个class时，
//   并不需要该class的定义，以by value方式传参也不需要定义。
//   class Date;
//   Date today();
//   void clearAppointments(Date d);
//   声明today和clearAppointments函数无需定义Date，一旦任何人调用那些函数，
//   调用之前一定要找到Date的定义，何必费心声明一个没人调用的函数呢？
//   并非没人调用，而是并非每个人都调用，如果能将提供class定义（#include完成）的义务，
...
//   Date today();
//   void clearAppointments(Date d);
//   iosfwd深具启发意义的另一个原因，分外彰显本条款适用于template也适用于non-template。
//   C++也提供export关键字，允许将template声明和定义分割于不同的文件内。
//   不幸的是支持这个关键在的编译器目前非常少。

// Handle和Interface解除来接口和实现之间的耦合关系，从而降低文件间的编译依赖。

...
int main() {
  ...
  // 实际上，是完全无法调用与数据收发有关的函数，这种生硬的方式有时候会显得不太优雅。
  ...
  // shutdown()用来关闭连接，而不是套接字，不管调用多少次shutdown()，
  // 套接字依然存在，直到调用close()/closesocket()将套接字从内存清除。
  ...
  // 不管输出缓冲区中是否还有数据，而shutdown()会等输出缓冲区中的数据传输完毕再发送FIN包。

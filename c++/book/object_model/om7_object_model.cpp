...
// 1.空间和时间效率的考虑，如果类有100个成员函数，但相关类型只使用了2个，
...
// 函数在什么时候实例化出来呢？有两种策略：
// 1.在编译时候，那么函数将实例化与origin和p存在的那个文件中；
...
void func1() {
  ...
  // 因为指向对象的指针，本身并不是对象，编译器不需知道任何成员数据或对象布局数据。
  // 所以将Point的一个float实体实例化也就没有必要，之前标准没有强制定义，
  // 编译器自己决定是否将template实例化出来，如今，C++标准已经禁止编译器实例化了。

  const Point<float> &ref = 0;
  // 引用会实例化出Point的float实体来，因为引用并不是无物（no object）的代名词。
...
// template之中，对于一个non member name的决议结构是根据，
...
// 所以这次必须在scope of the template instantiantion中决议，有两个foo()函数声明。
// 由于_member的类型为int，所以应该是int版本的foo()出线。
// 如果ScopeRules是以unsigned int或log类型实例化出来，那么foo调用操作就暧昧不明。
// 如果ScopeRules是以某一个类类型实例化出来，
// 而该类没有针对int或double实现出转换运算符，那么foo()调用操作会被标示为错误。
...
// 对于template的支持，最困难的是template function的实例化，编译器提供了两个策略：
...
// 1.如果downcast是安全的（基类指针指向派生类对象），这个运算符会返回被适当转换的指针；
...
// 2.如果传回0，表示没有指向任何对象，意味这该以另一种逻辑执行这个动态类型为确定的对象身上。
...
// 传统的C++对象模型提供了有效率的执行期支持，这份效率，再加上与C之间的兼容性，
// 造成了C++的广泛接受度，然而，在某些方面，
// 像动态共享函数库、共享内存、以及分布式对象方面，这个对象模型的弹性还是不够。

// 动态共享函数库
// 在目前的C++对象模型中，如果库中的对象布局有所改变，应用程序就必须重新编译。
// 因为类的大小及其每一个直接（或继承而来）的成员的偏移量，都在编译时期就已经固定，
// 虚拟继承的成员除外，这虽然带来了效率，却在二进制层面影响了弹性。

...
//  在函数调用中不允许转递一个临时对象到一个非const引用类型的参数里,但是在异常中却被允许。

// 当用传值的方式传递函数的参数,我们制造了被传递对象的一个拷贝,并把这个拷贝存储到函数的参数里。同样我们通过传值的方式传递一个异常时,也是这么做的。
// catch (Widget w) // 通过传值捕，会建立两个被抛出对象的拷贝,一个是所有异常都必须建立的临时对象,第二个是把临时对象拷贝进w中。
// catch (Widget& w) // 通过引用捕获，仍旧会建立一个被抛出对象的拷贝:拷贝同样是一个临时对象。相反当我们通过引用传递函数参数时,没有进行对象拷贝。
// 当抛出一个异常时,系统构造的(以后会析构掉)被抛出对象的拷贝数比以相同对象做为参数传递给函数时构造的拷贝数要多一个。

// 通过指针抛出异常与通过指针传递参数是相同的。不论哪种方法都是一个指针的拷贝被传递。
// 但你不能认为抛出的指针是一个指向局部对象的指针,因为当异常离开局部变量的生存空间时,该局部变量已经被释放。
// catch子句将获得一个指向已经不存在的对象的指针。这种行为在设计时应该予以避免。也就是说:必须是全局的或堆中的。

// 对象从函数的调用处传递到函数参数里与从异常抛出点传递到catch子句里所采用的方法不同,这只是参数传递与异常传递的区别的一个方面;
// 第二个差异是在函数调用者或抛出异常者与被调用者或异常捕获者之间的类型匹配的过程不同。
...
void f(int value) {
  try {
    ...
    // 该子句只能捕获类型真真正正为double的异常,不进行类型转换。因此如果要想捕获int异常,必须使用带有int或int&参数的catch子句。
  }
}

// 不过在catch子句中进行异常匹配时可以进行两种类型转换:
// 第一种是继承类与基类间的转换。一个用来捕获基类的catch子句也可以处理派生类类型的异常。
// 例如在标准C++库(STL)定义的异常类层次中的诊断部分(diagnostics portion)。
...
//   // 这个块永远不会被执行，因为所有的invalid_argument异常都被上面的catch子句捕获。
// }
// 如果一个处理派生类异常的catch子句位于处理基类异常的catch子句后面,编译器会发出警告。因为这样的代码在 ++里通常是不合法的。
// 不过你最好做好预先防范:不要把处理基类异常的catch子句放在处理派生类异常的catch子句的前面。应该写成这样：
// try {
// } catch (invalid_argument& ex {

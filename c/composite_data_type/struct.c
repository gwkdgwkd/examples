...
//     结构体所包含的变量或数组
// };
// 结构体是一种集合，它里面包含了多个变量或数组，它们的类型可以相同，
// 也可以不同，每个这样的变量或数组都称为结构体的成员（Member）。
// 结构体成员的定义方式与变量和数组的定义方式相同，只是不能初始化。
// 注意大括号后面的分号;不能少，这是一条完整的语句。
// 结构体也是一种数据类型，它由程序员自己定义，可以包含多个其他类型的数据。
// 像int、float、char等是由C语言本身提供的数据类型，
// 不能再进行分拆，称之为基本数据类型；
// 而结构体可以包含多个基本类型的数据，也可以包含其他的结构体，
// 将它称为复杂数据类型或构造数据类型。
// 理论上讲结构体的各个成员在内存中是连续存储的，和数组非常类似。
// 但是在编译器的具体实现中，各个成员之间可能会存在缝隙，需要内存对齐。
// 需要注意的是，结构体是一种自定义的数据类型，
// 是创建变量的模板，不占用内存空间；
...
// 3.如果只需要两个变量，后面不需要再使用结构体名定义其他变量，
//   那么在定义时也可以不给出结构体名：
struct {        // 没有写stu3
  char *name;   // 姓名
  int num;      // 学号
...
// 要想取得结构体变量的地址，必须在前面加&，所以给pstu赋值只能写作：
// struct stu *pstu = &stu1;
// 而不能写作：
// struct stu *pstu = stu1;
// 结构体是一种数据类型，是创建变量的模板，不会分配内存空间，
// 就像int、float、char这些关键字本身不占用内存一样；
// 结构体变量才包含数据，才需要内存来存储。
// 下面的写法是错误的，不能取一个结构体名的地址，也不能将它赋值给其他变量：
// struct stu *pstu = &stu;
// struct stu *pstu = stu;
...
// 如果去掉括号写作*pointer.memberName，
// 那么就等效于*(pointer.memberName)，这样意义就完全不对了。
// 第二种写法中，->是一个新的运算符，习惯称它为箭头，
// 有了它，可以通过结构体指针直接取得结构体成员；
// 这也是->在C语言中的唯一用途。
// 上面的两种写法是等效的，通常采用后面的写法，这样更加直观。

// 结构体指针作为函数参数
// 结构体变量名代表的是整个集合本身，作为函数参数时传递的整个集合，
// 也就是所有成员，而不是像数组一样被编译器转换成一个指针。
// 如果结构体成员较多，尤其是成员为数组时，
// 传送的时间和空间开销会很大，影响程序的运行效率。
// 所以最好的办法就是使用结构体指针，这时由实参传向形参的是地址，非常快速。

...
void func1() {  // 输出单独字符
  ...
  // 该函数是C语言中的，在<stdio.h>头文件中定义，C++在<iostream>头文件中定义。
  putchar('n');
  putchar('\n');
  // n
}
...
// seekp()方法会返回一个引用形式的ostream类对象，这意味着方法可以这样使用：
...
namespace n3 {
// cout格式化输出：
// 当使用printf()时，可以通过设定合理的格式控制符，来达到以指定格式输出数据的目的。
// C++通常使用cout据，和printf()函数相比，cout实现格式化输出数据的方式更加多样化。
// 一方面，cout作为ostream类的对象，有一些成员方法，可实现对输出数据的格式化；
...
//  flags(fmtfl) 	    当前格式状态全部替换为fmtfl，表示一种或多种格式。
//  precision(n) 	    设置输出浮点数的精度为n。
//  width(w) 	        指定输出宽度为w个字符。
//  fill(c) 	        在指定输出宽度的情况下，输出宽度不足时用c填充（默认是空格）。
...
// 当cout采用成员函数进行格式化时，其后不能立即输出数据，只在下条语句生效。
...
// 然后又设置setiosflags(ios::scientific)，那么可能两个标志都不起作用。
// 设置了某标志，又要设置与之矛盾的标志时，应该用resetiosflags清除原先的标志。

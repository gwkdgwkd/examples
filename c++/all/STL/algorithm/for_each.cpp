...
// 仿函数是一种在模板类编程时的封装手法，本质上的仿函数是一种类。
// 但是实际起到的是函数的作用，在这个所谓的类中通常都是一个operator()的重载，
// 这个重载是根据需求产生的，通过调用类的构造函数就使得这个类有了函数的行为。
// 进一步的解释就是，在写代码的时候，总有一部分代码的重用率非常高，
// 一般会把这段代码写成全局函数放在类外，任何的成员函数都可以去调用或访问，
// 但是在编写代码的同时也会产生大量的全局变量，很难维护。
// 为了提高代码的内聚度，选择将这部分代码封装到一个类中，
// 通过调用类的含参或默认构造函数来执行这段代码，将这种做法成为仿函数。
// 仿函数的机制为编写代码提供了很好的有利于资源管理的手段，
// 如果得到恰当应用的话会写出质量很高的代码。

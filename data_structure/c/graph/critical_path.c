...
// 拓扑排序只适用于AOV网，求关键路径针对的是和AOV网相近的AOE网。
// AOE网是在AOV网的基础上，其中每一个边都具有各自的权值，是一个有向无环网。
// 其中权值表示活动持续的时间。
// 使用AOE网可以帮助解决这样的问题：如果将AOE网看做整个项目，那么完成整个项目至少需要多少时间？
// 解决这个问题的关键在于从AOE网中找到一条从起始点到结束点长度最长的路径，
// 这样就能保证所有的活动在结束之前都能完成。
// 起始点是入度为0的点，称为源点；结束点是出度为0的点，称为汇点。
// 这条最长的路径，被称为关键路径。
// 为了求出一个给定AOE网的关键路径，需要知道以下4个统计数据：
// 1.对于AOE网中的顶点有两个时间：最早发生时间（用Ve(j)表示）和最晚发生时间（用Vl(j)表示）；
// 2.对于边来说，也有两个时间：最早开始时间（用e(i)表示）和最晚开始时间（l(i)表示）。
//   表示活动ai的最早开始时间，如果活动ai是由弧<Vk,Vj>表示的，
//   那么活动ai的最早开始的时间就等于时间Vk的最早发生时间，也就是说：e[i] = ve[k]。
//   表示活动ai的最晚开始时间，如果活动a是由弧<Vk,Vj>表示，
//   ai的最晚开始时间的设定要保证Vj的最晚发生时间不拖后，所以l[i]=Vl[j]-len<Vk,Vj>。
// 在得知以上四种统计数据后，就可以直接求得AOE网中关键路径上的所有的关键活动，方法是：
// 对于所有的边来说，如果它的最早开始时间等于最晚开始时间，称这条边所代表的活动为关键活动。
// 由关键活动构成的路径为关键路径。

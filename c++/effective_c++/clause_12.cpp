...
// 如果自己声明了copying函数，意思告诉编译器，不喜欢缺省实现中的某些行为。
// 编译器仿佛被冒犯似的，会以一种奇怪的方式回敬：实现的代码几乎必然出错时却不告诉你。
class Date {};
class Base {
 public:
...
class Derived : public Base {
  // 为derived class撰写copying函数，必须很小心地复制其base class成分，往往是private，无法直接访问，需要调用base class对应的copying函数。
...
// copy构造函数调用copy assignment操作符同样无意义。构造函数用来初始化新对象，而assignment操作符只用于已初始化对象身上。
// 消除重复代码的做法是，建立一个新的成员函数给两者调用，这样的函数往往是private而且常被命名为init。

// 请记住：
// copying函数应该确保复制“对象内的所以成员变量”及“所有base class成分”。
// 不要尝试以某个copying函数实现另一个copying函数。应该将共同机制放进第三个函数中，并由两个copying函数共同调用。

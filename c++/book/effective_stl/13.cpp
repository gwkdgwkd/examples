...
// 2.必须确保使用了正确的delete格式，如果格式错误，结果不确定，某些平台会崩溃；
...
// 而当vector和string被析构时，析构函数会自动析构容器中的元素并释放内存。
// vector和string是功能完全的STL序列容器，凡是适合于序列容器的STL算法，都可以使用。
...
// 1.检查库的实现，通常通过改变某个预处理变量的值来禁止引用计数，不可移植的办法；
// 2.寻找或开发另一个不使用引用计数的string实现；
// 3.考虑使用vector<char>而不是string，vector的实现不允许使用引用计数，
//   所以不会发生隐藏的多线程性能问题，使用vector<char>，
//   那么就舍弃了使用string的成员函数的机会，但大多数功能可以通过STL算法来实现。

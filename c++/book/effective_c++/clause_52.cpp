...
void func() {
  ...
  // 假设new成功，default构造函数抛出异常，
  // 那么new分配的内存必须取消并恢复原样，否则会造成内存泄露：
  // 1.这个时候无法归还内存，因为p尚未被赋值，用户手上没有指针指向未被归还的内存；
  // 2.取消步骤需要C++运行期系统身上，它会高高兴兴地调用new对应的delete版本，
  //   前提当然是它必须知道哪一个delete该被调用。
}
}  // namespace n1

namespace n2 {
// 写了一个class专属的operator new，要求接受一个ostream，
// 用来标记相关分配信息，同时有写了一个正常形式的class专属operator delete：
class A {
 public:
  A() { throw 5; }

...
//   operator new，operator new[]，operator delete和operator delete[]。
...
// 4.什么是新type的合法值？
//   对成员变量而言，通常只有某些数值集是有效的，这些数值决定了必须维护的约束条件，
//   也就决定了成员函数，特别是构造函数、赋值操作符和所谓setter函数必须进行的错误检查。
//   也影响函数抛出的异常、以及（极少被使用的）函数异常明细列表。
// 5.新的type需要配合某个继承体系吗？
//   如果继承自某些类，就受到那些类的设计约束，特别是virtual或non-virtual的影响。
//   如果允许其他类继承类，那会影响声明的函数，尤其是析构函数，是否为virtual。
// 6.的新type需要什么样的转换？
//   有很多类型，彼此之间该有转换行为么？
//   如果希望允许类型T1能被隐式转换为类型T2，就必须在T1内写一个类型转换函数，
//   或在T2内定义non-explicit-one-argument可被单一实参调用的构造函数。
//   如果允许explicit构造函数存在，就得写出专门负责执行转换的函数，
//   且不得为类型转换操作符或non-explicit-one-argument构造函数。
// 7.什么样的操作符和函数对此新type而言是合理的？
//   这个问题的答案决定为你的class声明哪些函数，其中某些该是member函数，某些则否。
// 8.什么样的标准函数应该驳回？
//   那些正是必须声明为private的函数。
// 9.谁该取用新type的成员？

...
// 3.即使insert和erase这样的基本操作，也会随容器类型的不同而表现出不同的原型和语义。

// 假设想编写对于大多数通用的序列容器（vector、deque和list）都适用的代码，
// 那么很显然，程序只能适用它们功能的交集。
// 1.意味着不能使用reserve或capacity，因为deque和list中没有这样的操作；
// 2.有list存在，意味着要放弃operator[]，而且要把操作现在在双向迭代器的能力范围之内；
// 3.意味着要放弃要求随机迭代器的操作，包括sort、stable_sort、partial_sort和nth_elemment。

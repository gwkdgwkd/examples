...
//  按位与运算通常用来对某些位清0，或者保留某些位。例如要把n的高16位清0，保留低16位，可以进行n&0XFFFF运算。
// 按位或运算（|）：
//  参与|运算的两个二进制位有一个为1时，结果就为1，两个都为0时结果才为0。
//  按位或运算可以用来将某些位置1，或者保留某些位。例如要把n的高16位置1，保留低16位，可以进行n|0XFFFF0000运算。
// 按位异或运算（^）：
//  参与^运算两个二进制位不同时，结果为1，相同时结果为0。
//  按位异或运算可以用来将某些二进制位反转。例如要把n的高16位反转，保留低16位，可以进行n^0XFFFF0000运算。
// 取反运算（~）：
...
int main() {
  ...
  int n1 = 0X8FA6002D;
  printf("%d, %d, %X\n", 9 & 5, -9 & 5, n1 & 0XFFFF);  // 1, 5, 2D
  int n2 = 0X2D;
  printf("%d, %d, %X\n", 9 | 5, -9 | 5, n2 | 0XFFFF0000);  // 13, -9, FFFF002D
  unsigned n3 = 0X0A07002D;
  printf("%d, %d, %X\n", 9 ^ 5, -9 ^ 5, n3 ^ 0XFFFF0000);  // 12, -14, F5F8002D
  printf("%d, %d\n", ~9, ~ - 9);                           // -10, 8
  printf("%d, %d\n", 9 << 3, (-9) << 3);                   // 72, -72
  printf("%d, %d\n", 9 >> 3, (-9) >> 3);                   // 1, -2

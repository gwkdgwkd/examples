...
// 派生类和基类成员的排列次序并未在C++标准中强制指定，理论上编译器可以自由安排。大部分编译器，基类成员总是先出现，虚基类除外。
struct Point2d_1 {
  float x, y;
} pt2d_1;
struct Point3d_1 {
...
//  2 把一个类分解为两层或更多层，有可能会为了“表现类体系的抽象化”而膨胀所需空间。
//    C++保证“出现在派生类中的基类subobject有其完整原样性”，这是重点。
class Concrete {
  char c1, c2, c3;
  int val;
...
// 如果不保证“出现在派生类中的基类subobject有其完整原样性”，会怎么样？
// Concrete2* pc2;
// Concrete1 *pc1_1, *pc1_2;  // 都可以指向前面的三种对象
// *pc1_2 = *pc1_1;  // 应该执行一个默认的“memberwise”复制操作，复制Concrete1的那一部分
// 如果pc1_1实际指向一个Concrete2或Concrete3对象，则上述操作应该复制内容给Concrete1 subobject。
// 如果没有保持派生类中的基类subobject的完整原样性，把bit2和bit3和Concrete1 subobject捆绑在一起，去填补空间。
...
void foo(Point2d_3& p1, Point2d_3& p2) {
  p1 += p2;
  // p1和p2可能是2d也可能是3d坐标点，这并不是先前任何设计所能支持的。这样的弹性，正是面向对象设计的中心。
  // 支持这样的弹性，势必给2d类带来空间和存取时间的额外负担：
  // 1 导入一个二和2d有关的虚函数表，用来存放它所声明的每一个虚函数地址。
  //   这个表的元素数目一般而言是被声明的虚函数的数目，再加上一个或两个slots（用以支持runtime type identification）。
  // 2 在每一个对象中导入一个vptr，提供执行期的链接，使每一个对象能够找到对应virtual table。
  // 3 加强构造函数，使它能够为vptr设定初值，让它指向类所对应的virtual table。
  //   这可能意味着在派生类和每一个基类的构造函数中，重新设定vptr的值。其情况视编译器的优化的积极性而定。
  // 4 加强构造函数，使它能够抹消“指向类的相关virtual table”的vptr。
  //   要知道，vptr很可能已经在派生类析构函数中被设定为派生类的virtual table地址。
  //   析构函数的调用次序是反向的：从派生类到基类。一个积极的优化编译器可以压抑那些大量的指定操作。

  // 把vptr放到哪里在最好？
  // 在C++最初问世时，把vptr放在对象的尾端，可以保留基类C struct的对象布局。因而允许在C程序代码中也能使用。
  // 到了C++2.0，开始支持虚拟继承以及抽象基类，并且由于面向对象典范的兴起，某些编译器开始把vptr放到对象的开头。
  // 把vptr放在对象的前端，对于“在多重继承之下，通过指向成员的指针调研虚函数”，会带来一些帮助。
  // 否则，不仅“从对象起点开始量起”的offset必须在执行期备妥，甚至与类vptr之间的offset也必须备妥。
  // 也丧失了C语言的兼容性。
}

// 多重继承
// 单一继承提供了一种“自然多态”形式，是关于类体系中的基类和派生类之间的转换。基类和派生类都是从相同的地址开始，差异只是派生类对象比较大。
// 把派生类对象指给基类（不管继承深度有多深）的指针或引用，该操作并不需要编译器去调停或修改地址。很自然地发生，而且提供了最佳执行效率。
...
// 多重继承既不是单一继承，也不容易定义其模型。其复杂度在于派生类和其上一个基类乃至于上上一个基类之间的“非自然”关系。
class Point2d_4 {
 public:
  void print() {
...
class Vertex3d : public Point3d_4, public Vertex {
 public:
  void print() {
    Point3d_4::print();
    Vertex::print();
    cout << "Vertex3d mumbles: " << &mumbles << " " << sizeof(mumbles) << endl;
  }
 ...
 protected:
  ...
  // 不，成员的位置在编译时就固定了，因此存取成员只是一个简单的offset运算，就像单一继承一样简单，不管是经由指针、引用或是对象来存取。
};
// 多重继承的问题主要发生于派生类对象和其第二或后继的基类对象之间的转换。
// 不论是直接转换还是经由其所支持的虚函数机制做转换。
void mumble(const Vertex&) {}
// 对一个多重派生类对象，将地址指定给“最左端（第一个）基类的指针”，情况和单一继承时相同，因为二者都指向相同的起始地址。付出的成本只有地址的指定操作而已。
// 至于第二个或后继的基类的地址指定操作，则需要将地址修改：加上（或减去，如果downcast的话）介于中间的基类subobjcet大小。

// 虚拟继承
// 多重继承的一个语意上的副作用就是，它必须支持某种形式的”share subobject继承“。一个典型的例子是iostream library。
// 如果类内含一个或多个虚拟基类subobjects，那么将被分割为两部分：一个不变局部和一个共享局部。
// 不变局部中的数据，不管后继如何变化，总是拥有固定的offset（从对象的开头算起），所以这一部分的数据可以直接存取。
...
// cfront编译器在每一个派生类对象中安插一些指针，每个指针指向一个虚拟基类。通过这个指针间接存取虚基类的成员。
// 这样的实现模型有两个主要的缺点：
//  a 每一个对象必须针对其每一个虚基类背负一个额外的指针，理想上我们希望对象有固定的负担，不因为虚基类的数目而变化。
//  b 由于虚继承链的加长，导致间接存取层次的增加。理想上希望有固定的存取时间，不因为虚基类的深度而改变。
// MetaWare和其他编译器到今天仍然使用cfront的原始实现模型来解决第二个问题：
//  把经由拷贝操作取得得所有的nested virtual base class指针，放到派生类对象中。这就解决了“固定存取时间”的问题，虽然付出了一些空间上的代价。
// 至于第一个问题，一般有两个解决方法：
//  a microsoft编译器引入所谓的virtual base class table。每一个对象如果有一个或多个虚拟基类，就会有编译器安插一个指针，
//    指向virtual base class table。至于真正的virtual base class指针，就被放在该表格中。
//  b 在virtual function table中放置virtual base class的offset，而不是地址。
// 一般而言，virtual base class最有效的一种运用形式就是：一个抽象的virtual base class，没有任何数据成员。

// 3.5 对象成员的效率

// 如果把优化开关打开，“封装”就不会带来执行期的效率成本。使用inline存取函数也是。
// 如果没有把优化开关打开，就很难猜测一个程序的效率表现，因为程序代码潜在性地受到专家所谓的“一种奇行怪癖...与也特定编译器有关”的魔咒影响。
...
// 指向data members的指针，是一个有点神秘但颇有用处的语言特性，特比是如果需要详细调查成员的底层布局的话。
// 可以用来决定vptr是放在类的起始处或是尾端。也可以用来决定类中访问段的次序。
class Point3d_6 {
 public:
  virtual ~Point3d_6() {}
  void fn() {
    // 取一个非静态数据成员的地址，将会得到它在类中的offset；取一个绑定于真正对象身上的数据成员的地址，，将会得到该成员在内存中的真正地址。
    // 过去的编译器会在每个偏移值上加1么？
...
// 每个对象含有三个坐标，依次为x，y，z，以及一个vptr。静态成员，放在对象之外。唯一随编译器不同而不同的是vptr的位置。
// C++标准允许vptr被放在对象中的任何位置：起始处、尾端，或是在各个成员之间。实际上，所有编译器不是放在对象头部，就是放在对象尾部。
// &Point3d_6::z;
// 上面的取地址操作将得到z坐标在对象中的偏移量。最少也是x和y的大小的总和，因为C++要求用一个access level中的成员的排列次序应该和声明次序相同。

// 在多重继承下，如果要将第二个（或后继）base class的指针和一个“与派生类对象绑定”的成员结合起来，那么将会因为“需要加入offset值”而变得相当复杂。
struct Base1 {
  int val1;
};
struct Base2 {
...
void func1(int Derived::*dmp, Derived* pd) {
  // 期望第一个参数得到的是一个“指向派生类成员”的指针。如果传入一个“指向基类成员”的指针会怎么样呢？
  pd->*dmp;
}
void func2(Derived* pd) {
  int Base2::*bmp = &Base2::val2;  // bmp将成为1，但是在派生类中，val2==5
  // 当bmp被作为func1的第一个参数时，它的值就必须因介入的Base1 class的大小而调整，
  // 否则func1中将取到Base1::val1，而非程序员所期望的Base::val2。要解决这个问题，必须经由编译器内部转换：
...
int main() {
  ...
  Vertex3d v3d;
  v3d.print();
  // Point2d_4 x: 0x7ffe15c38e50 4
  // Point2d_4 y: 0x7ffe15c38e54 4
  // Point3d_4 z: 0x7ffe15c38e58 4
  // Vertex next: 0x7ffe15c38e60 8
  // Vertex3d mumbles: 0x7ffe15c38e68 4
  cout << &v3d << endl;  // 0x7ffe15c38e50
  Vertex3d* pv3d;
  Vertex* pv;
  Point2d_4* p2d;
  Point3d_4* p3d;
  pv = &v3d;
  cout << pv << endl;  // 0x7ffe15c38e60
  // 需要这样的内部转化：
  Vertex* pv1 = (Vertex*)(((char*)&v3d) + sizeof(Point3d_4));  // 转换的不对啊!
  cout << pv1 << endl;                                         // 0x7ffe15c38e5c
  p2d = &v3d;  // 简单地拷贝地址就行了
  p3d = &v3d;  // 简单地拷贝地址就行了
  pv = pv3d;
  ...
  // 如何区分一个“没有指向任何数据成员”的指针和一个指向“第一个数据成员”的指针？
  float Point3d_6::*p1 = 0;  // Point3d_6::*是“指向Point3d_6数据成员”的指针类型

...
class SmartPtr2 {
 public:
  // 使用成员初始值列表来初始化SmartPtr<T>的类型为T*的成员变量，
  // 并以类型U*的指针作为初值。
  // 这个转换只有当：
  // 存在某个隐式转换可将一个U*转换为一个T*指针时才能通过编译，正是想要的。
  // 最终受益的是SmartPtr2<T>，
  // 它的泛化copy构造函数只在其所获得的实参隶属适当类型时才能通过编译。
  template <typename U>
...
// member function template(成员函数模板)的效用不限于构造函数，
// 常扮演的另一个角色是支持赋值操作。
// shared_prt支持所有来自兼容的内置指针、shared_ptr、
// auto_ptr和weak_ptr的构造行为，以及上述各物的赋值操作。

// member function template是个奇妙的东西，
// 但它们并不改变语言基本规则：
// 如果需要一个copy构造函数，却没有声明，编译器会为你暗自生成一个。
// 在class内声明泛化copy构造函数，
// 并不会阻止编译器生成它们自己的copy构造函数（non-template)。
// 所以想要控制copy构造的方方面面，
// 必须同时声明泛化copy构造函数和正常的copy构造函数。
// 相同的规则也适用于赋值操作。

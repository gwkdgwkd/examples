...
// assert是运行期断言，它用来发现运行期间的错误，不能提前到编译期发，
// 也不具有强制性，也谈不上改善编译信息的可读性。
// 既然是运行期检查，对性能肯定是有影响的，所以经常在发行版本中，assert都会被关掉。

// #error可看作是预编译期断言（甚至都算不上断言），仅仅能在预编译时显示一个错误信息，
// 可以配合#ifdef/ifndef参与预编译的条件检查，由于它无法获得编译信息，
// 当然，也就做不了更进一步分析了。
...
// 使用注意点：
// 1.使用范围：static_assert可以用在全局作用域中，命名空间中，
//   类作用域中，函数作用域中，几乎可以不受限制的使用。
// 2.常量表达式：static_assert的断言表达式的结果必须是在编译时期可以计算的表达式，
//   即必须是常量表达式：
...
// 3.模板参数：编译器在遇到一个static_assert语句时，
...
// 4.性能方面：由于static_assert是编译期间断言，不生成目标代码，

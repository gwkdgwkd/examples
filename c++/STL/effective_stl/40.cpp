...
// 传给std::unary_function和std::binary_function的类型与operator()的参数和返回类型完全相同。

// STL函数对象是C++函数的一种抽象和建摸形式，而每个C++函数只有一组确定的参数类型和一个返回类型。
// 所以STL总是假设每个函数子类只有一个operator()成员函数，
// 并且其参数和返回类型应该吻合std::unary_function和std::binary_function的模板参数。
// 这也就意味着，虽然只要创建一个包含两个operator函数的结构，
// 就可以把WidgetNameCompare和PtrWidgetNameCompare的功能合并到一个函数子结构中，但最好不要这样做。
...
int main() {
  ...
  // 如果MeetsThreshold和WidgetNameCompare不是从unary_function和binary_function继承而来的，

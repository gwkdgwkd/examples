...
// 程序的错误大致可以分为三种：
// 1.语法错误在编译和链接阶段就能发现，只有符合语法规则的代码才能生成可执行程序。
//   是最容易发现、最容易定位、最容易排除的错误，程序员最不需要担心的错误；
// 2.逻辑错误是说编写的代码思路有问题，不能够达到最终的目标，可以通过调试来解决；
// 3.运行时错误是指程序在运行期间发生的错误，如除数为0、内存分配失败、数组越界等。
//   C++异常（Exception）机制就是为解决运行时错误而引入的。
// 运行时错误如果不管，系统就会执行默认操作，终止程序运行，就是程序崩溃（Crash）。
...
namespace n2 {
// catch语句中的异常类型使用引用是为了提高效率，如果不使用引用，

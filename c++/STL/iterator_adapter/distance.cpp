...
// 其中，first和last都为迭代器，其类型可以是输入迭代器、
// 前向迭代器、双向迭代器以及随机访问迭代器；
// 该函数会返回[first,last)范围内包含的元素的个数。
// 注意，first和last的迭代器类型，直接决定了distance()函数底层的实现机制：
// 1.当first和last为随机访问迭代器时，
//   distance()底层直接采用last-first求得[first,last)范围内包含元素的个数，
//   其时间复杂度为O(1)常数阶；
// 2.当first和last为非随机访问迭代器时，
//   distance()底层通过不断执行++first（或者first++）直到first==last，

...
// 1.std::less<T> 底层采用<运算符实现升序排序，各关联式容器默认采用的排序规则。
// 2.std::greater<T> 底层采用>运算符实现降序排序，同样适用于各个关联式容器。
// 3.std::less_equal<T> 采用<=运算符实现升序排序，多用于multimap和multiset。
// 4.std::greater_equal<T> 采用>=运算符实现降序排序，多用于multimap和multiset。
// 排序规则底层也是采用函数对象的方式实现的，以std::less<T>为例，其底层实现为：
// template <typename T>
// struct less {
...
namespace n1 {
// 定义函数对象类，并在其重载()运算符的方法中自定义了新的排序规则：
...
void func() {
  ...
  // 由于是以字符串的长度进行排序，因此其无法存储相同长度的多个字符串，没有DDDD
}
}  // namespace n1
...
namespace n4 {
// n3以全局函数的形式实现对<运算符的重载，还可以使用成员函数或者友元函数实现。
// 当以成员函数的方式重载<运算符时，函数必须声明为const，且参数也必须为const：
class myString {
 public:
  myString(std::string tempStr) : str(tempStr){};
  std::string getStr() const;

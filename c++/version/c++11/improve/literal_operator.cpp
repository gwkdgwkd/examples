...
// 4.字符串"123"。
// 字面量又可添加后缀来表明具体类型：
// 1.无符号整形(unsigned int)：123u；
// 2.长整形(long)：123l

// 在C++03中，可以定义一个浮点数height：
// double height = 3.4
// 那么，痛点来了，此处的height的单位是什么呢？米？厘米？又或是英尺？
// 在面对此类问题时，如果能编写如下代码，事情就会简单许多：
// height = 3cm;
// ratio = (3 * 10) / 2
// ratio = 3cm / 2mm;
...
// 自定义字面量的限制：
// 1.如果字面量是字符串，那么字面量操作符函数只能接受const char*，
//   size_t为参数，系统会主动对字符串进行解析，并完成形参赋值匹配；
// 2.如果目标字面量的形式是整数型，
//   那么字面量操作符函数只能接受unsigned long long或const char*为其参数，
//   当unsigned long long溢出后，编译器会自动切换到const char*版本;
// 3.如果目标字面量的形式是浮点数型，
//   那么字面量操作符函数只能接受long double或const char*为其参数，规则同2；
...
namespace n2 {
// 在C++编程中，用户自定义类或结构体是难以避免的事情，
// 所以一旦遇到接受用户自定义的类或结构体作为参数的函数，那么必然难逃先初始化变量，
// 然后传值或传引用调用函数，这就导致了编程过程中出现的不直观。
// struct T {};
// void func(T t1, T t2) {}
// T temp1(xxxx);
// T temp2(xxxx);
// func(temp1, temp2);
// 为什么对于void func(int, double)可以直接使用func(12, 3.14)调用，
// 而不像下面一样使用：
// int a = 12;
// double b = 3.14;
// func(a, b);
...
// blend函数形参是右值引用，是专门为RGBA设计的，普通的RGBA对象是无法调用的：

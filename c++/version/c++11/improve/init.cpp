...
namespace n1 {
// 在C++98/03中的对象初始化方法有很多种，都有各自的适用范围和作用。
...
// 对于普通数组和POD类型，C++98/03可以使用初始化列表进行初始化，
...
namespace n2 {
// 为了统一初始化方式，并且让初始化行为具有确定的效果，C++11中提出了列表初始化的概念。
...
Foo func(void) {
  return {123, 321.0};  // 列表初始化还可以直接使用在函数的返回值上
}
}  // namespace n2

namespace n3 {
// 在C++11中，初始化列表的适用性被大大增加了，可以用于任何类型的初始化。
...
void func1() {
  ...
  // 虽然使用了等号，但仍是列表初始化，不受私有拷贝构造影响到：
  Foo a3 = {123};
  // 直接在变量名后面跟上初始化列表，C++98/03所不具备的：
  Foo a4{123};
...
void func2() {
  ...
  // 通过初始化列表方式在内存初始化时指定了值为123：
  int *a = new int{123};
  // 对匿名对象使用列表初始化后，再进行拷贝初始化：
  double b = double{12.12};
  // 堆上动态分配的数组也可以用初始化列表初始化：
  int *arr = new int[3]{1, 2, 3};
}
}  // namespace n3

namespace n4 {
// 类型收窄指的是导致数据内容发生变化或者精度丢失的隐式类型转换，类型收窄的几种情况：
// 1.从浮点数隐式转换为整型数，如int i=2.2;
// 2.从高精度浮点数隐式转换为低精度浮点数，如从long double转double或float。
// 3.从整型数隐式转换为浮点数，并且超出浮点数的表示范围，
//   如float=(unsigned long Long)-1，
//   注意这表示将-1先强制转换为unsigned long long，再隐式转换为float。
// 4.从整型数隐式转换为长度较短的整型数，并且超出其表示范围，如char x=65536。
// C++98/03中类型收窄不会报错，而C++11可以通过列表初始化来检查及防止类型收窄。

void func1() {
  int x1(5.3);   // ok，x1=5
  int x2 = 5.3;  // ok，x2=5
  // int x3{5.3};
  // error：narrowing，gcc给的是warning
  // int x4 = {5.3};
  // 同上
...
void func2() {
  ...
  // float f2 = {1e40};
  // error，double->float，超过float能表示的范围

  float f3 = (unsigned long long)-1;
  // 将-1（0xFFFFFFFF）强转，再隐式转成float
...
void func3() {
  ...
  char c1 = x;
  // ok，overflow in implicit constant conversion [-Woverflow]
  ...
  char c5 = z;  // ok
  char c6 = {z};
  // narrowing conversion of ‘z’ from ‘int’ to ‘char’
}
}  // namespace n4

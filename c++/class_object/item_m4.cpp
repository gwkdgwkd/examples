...
// 缺省构造函数（指没有参数的构造函数）在C++语言中是一种让无中生有的方法。
...
//   因为实例化模板时，模板的类型参数应该提供缺省构造函数，这是个常见的要求。
//   这个要求总是来自于模板内部，被建立的模板参数类型数组里。
//   在多数情况下，通过仔细设计模板可以杜绝对缺省构造函数的需求，如vector模板。
template <class T>
class Array {
 public:
  Array(int size);
...
// 缺省构造函数一般不会提供这种保证，它们可能使类没有意义时，尽量去避免使用。
// 使用这种（没有缺省构造函数的）类的确有一些限制，
// 但是当使用它时，它也给你提供了一种保证：能相信这个类被正确地建立和高效地实现。
}  // namespace n2
...
int main() {
  ...
  int ID1, ID2, ID3;
  n1::EquipmentPiece bestPieces[] = {n1::EquipmentPiece(ID1), n1::EquipmentPiece(ID2),
                                 n1::EquipmentPiece(ID3)};
  // 不过很遗憾，这种方法不能用在堆数组(heap arrays)的定义上。

  // 一个更通用的解决方法是利用指针数组来代替一个对象数组：
  typedef n1::EquipmentPiece* PEP;
  PEP bestPieces1[10];             // 正确，没有调用构造函数
  PEP* bestPieces2 = new PEP[10];  // 也正确
  // 在指针数组里的每一个指针被重新赋值，以指向一个不同的EquipmentPiece对象：
  for (int i = 0; i < 10; ++i) {
    bestPieces1[i] = new n1::EquipmentPiece(i);
  }
  // 不过这种方法有两个缺点：
  // 1.必须删除数组里每个指针所指向的对象，如果忘了，就会发生内存泄漏。
  // 2.增加了内存分配量，因为正如需要空间来容纳EquipmentPiece对象一样，
  //   也需要空间来容纳指针。

  // 如果为数组分配raw memory，就可以避免浪费内存。
  // 使用placement new方法在内存中构造EquipmentPiece对象：
  void* rawMemory = operator new[](10 * sizeof(n1::EquipmentPiece));
  n1::EquipmentPiece* bestPieces3 = static_cast<n1::EquipmentPiece*>(rawMemory);
  for (int i = 0; i < 10; ++i) {
    // 注意仍旧得为每一个EquipmentPiece对象提供构造函数参数：
    new (&bestPieces[i]) n1::EquipmentPiece(i);
  }
  // 这个技术和指针数组的主意一样，允许在没有缺省构造函数的情况下建立一个对象数组。
  // 它没有绕过对构造函数参数的需求，实际上也做不到。
  // 如果能做到的话，就不能保证对象被正确初始化。
  // 使用placement new的缺点除了是大多数程序员对它不熟悉外，
  // 还有就是当不想让它继续存在使用时，必须手动调用数组对象的析构函数，

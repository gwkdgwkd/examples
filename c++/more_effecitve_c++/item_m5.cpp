...
// 不过当你增加自己的类型时，你就可以有更多的控制力，因为你能选择是否提供函数让编译器进行隐式类型转换。
...
// 隐式类型转换运算符只是一个样子奇怪的成员函数：operator关键字，其后跟一个类型符号。
// 你不用定义函数的返回类型，因为返回类型就是这个函数的名字。
// 例如为了允许Rational类隐式地转换为double类型（用有理数进行混合类型运算时可能有用），
// 你可以如此声明Rational类：
class Rational1 {
 public:
...
// 构造函数用explicit声明，如果这样做，编译器会拒绝为了隐式类型转换而调用构造函数。
// 显式类型转换依然合法。
template <class T>
class Array1 {
 public:
...
int main() {
  ...
  // 编译器会发现它们能调用Rational::operator double函数来把r转换为double类型。
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    // 而且编译器无法把int转换成一个临时ArraySize对象然后通过这个临时对象建立必须的Array<int>对象，

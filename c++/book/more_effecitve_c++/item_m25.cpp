...
class TextBlock : public NLComponent {  // 不包含纯虚函数
 public:
...
class Graphic : public NLComponent {  // 不包含纯虚函数
 public:
...
// 显而易见的方法是虚拟化这个输出操作符。
// 但是输出操作符是operator<<，函数把ostream&做为它的左参数，
// 即把它放在函数参数列表的左边，这就不可能使该函数成为成员函数。
// 为了能够回到正常的语法上来，必须把operator<<移出TextBlock和Graphic类，
// 但是如果这样做，就不能再把它声明为虚拟了。

// 另一种方法是为打印操作声明虚拟函数，如print，把它定义在TextBlock和Graphic里。
// 但是如果这样，打印TextBlock和Graphic对象的语法，
// 就与使用operator<<做为输出操作符的其它类型的对象不一致了，这些方法都不很令人满意。
...
// 具有虚拟行为的非成员函数很简单，编写一个虚拟函数来完成工作，
// 然后再写一个非虚拟函数，它什么也不做只是调用这个虚拟函数。

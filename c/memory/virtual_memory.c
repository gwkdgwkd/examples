...
// 1.运算单元是CPU的大脑，负责加减乘除、比较、位移等运算工作，
//   每种运算都有对应的电路支持，速度很快。
// 2.寄存器（Register）是CPU内部非常小、非常快速的存储部件，
//   它的容量很有限，对于32位的CPU，
//   每个寄存器一般能存储32位（4个字节）的数据，对于64位的CPU，
//   每个寄存器一般能存储64位（8个字节）的数据。
//   为了完成各种复杂的功能，现代CPU都内置了几十个甚至上百个的寄存器，
//   嵌入式系统功能单一，寄存器数量较少。
...
//   虽然内存的读取速度已经很快了，但是和CPU比起来，还是有很大差距的，
//   不是一个数量级的，如果每次都从内存中读取数据，
//   会严重拖慢CPU的运行速度，CPU经常处于等待状态，无事可做。
//   在CPU内部设置一个缓存，可以将使用频繁的数据暂时读取到缓存，
//   需要同一地址上的数据时，就不用大老远地再去访问内存，直接从缓存中读取即可。
//   在购买CPU时，也会经常关心缓存容量，例如Intel Core i7 3770K的三级缓存为8MB，
//   二级缓存为256KB，一级缓存为32KB。容量越大，CPU越强悍。
//   缓存的容量是有限的，CPU只能从缓存中读取到部分数据，
//   对于使用不是很频繁的数据，会绕过缓存，直接到内存中读取。
//   所以不是每次都能从缓存中得到数据，这就是缓存的命中率，
//   能够从缓存中读取就命中，否则就没命中。
//   关于缓存的命中率又是一门学问，哪些数据保留在缓存，
//   哪些数据不保留，都有复杂的算法。

// CPU指令
// 要想让CPU工作，必须借助特定的指令，
// 例如add用于加法运算，sub用于除法运算，cmp用于比较两个数的大小，
// 这称为CPU的指令集（Instruction Set）。
...
// 作为普通的程序，它需要的是一个简单的执行环境，有自己的内存，
// 有自己的CPU，好像整个程序占有整个计算机而不用关心其他的程序。
...
// CPU的数据处理能力
// CPU是计算机的核心，决定了计算机的数据处理能力和寻址能力，
// 也即决定了计算机的性能。
// CPU一个时钟内处理的数据的大小由寄存器的位数和数据总线的宽度决定，
// 我们通常所说的多少位的CPU，除了可以理解为寄存器的位数，
// 也可以理解数据总线的宽度，通常情况下它们是相等的。
// 数据总线和主频都是CPU的重要指标：
// 数据总线决定了CPU单次的数据处理能力，
// 主频决定了CPU单位时间内的数据处理次数，
// 它们的乘积就是CPU单位时间内的数据处理量。
// 数据总线和地址总线不是一回事，数据总线用于在CPU和内存之间传输数据，
// 地址总线用于在内存上定位数据，它们之间没有必然的联系，宽度并不一定相等。
...
// 这是一个很大的值，几乎是无限的，就目前的技术来讲，
// 不但物理内存不可能达到这么大，CPU的寻址能力也没有这么大，
// 实现64位长的虚拟地址只会增加系统的复杂度和地址转换的成本，
...
// 1.地址隔离：程序A和程序B分别被映射到了两块不同的物理内存，
//   它们之间没有任何重叠，如果程序A访问的虚拟地址超出了范围，
//   系统就会判断这是一个非法的访问，拒绝这个请求，
...
// 关于页的大小
// 页的大小是固定的，由硬件决定，或硬件支持多种大小的页，
// 由操作系统选择决定页的大小。
// 比如Intel Pentium系列处理器支持4KB或4MB的页大小，
// 那么操作系统可以选择每页大小为4KB，也可以选择每页大小为4MB，
// 但是在同一时刻只能选择一种大小，所以对整个系统来说，也就是固定大小的。
// 目前几乎所有PC上的操作系统都是用4KB大小的页。
// 假设我们使用的PC机是32位的，那么虚拟地址空间总共有4GB，
// 按照4KB每页分的话，总共有2^32/2^12=2^20=1M=1048576个页；
// 物理内存也是同样的分法。
...
//   但是这样的数组有2^32个元素，每个元素大小为4个字节，
//   总共占用16GB的内存，显现是不现实的！
// 2.使用一级页表
//   既然内存是分页的，只要我们能够定位到数据所在的页，
//   以及它在页内的偏移（也就是距离页开头的字节数），就能够转换为物理地址。
//   虚拟地址空间大小为4GB，
//   总共包含2^32/2^12=2^20=1K*1K=1M=1048576个页面，
//   可以定义一个这样的数组：它包含2^20=1M个元素，
//   每个元素的值为页面编号（也就是位于第几个页面），
//   长度为4字节，整个数组共占用4MB的内存空间。
//   这样的数组就称为页表（Page Table），它记录了地址空间中所有页的编号。
//   虚拟地址长度为32位，不妨进行一下切割，
//   将高20位作为页表数组的下标，低12位作为页内偏移。
//   因为页表数组共有2^20=1M个元素，使用虚拟地址的高20位作为下标，
//   正好能够访问数组中的所有元素；
//   并且，一个页面的大小为2^12=4KB，
//   使用虚拟地址的低12位恰好能够表示所有偏移。
//   这种方案，不管程序占用多大的内存，
//   都要为页表数组分配4M的内存空间（页表数组必须放在物理内存中），
...
//   它和程序本身占用的内存空间成正比，从整体上来看，
//   会比使用一级页表占用的内存少得多。
// 3.使用多级页表
//   对于64位环境，虚拟地址空间达到256TB，
//   使用二级页表占用的存储空间依然不小，所以会更加细化，
//   从而使用三级页表甚至多级页表，这样就会有多个页目录，
//   虚拟地址也会被分割成多个部分。

// MMU
// 通过页表完成虚拟地址和物理地址的映射时，要经过多次转换，
// 还要进行计算，如果由操作系统来完成这项工作，
// 那将会成倍降低程序的性能，得不偿失，所以这种方式是不现实的。
// 在CPU内部，有一个MMU（Memory Management Unit，内存管理单元），
// 由它来负责将虚拟地址映射为物理地址。
...
// 有了硬件的直接支持，使用虚拟地址和使用物理地址相比，
// 损失的性能已经很小，在可接受的范围内。
// MMU只是通过页表来完成虚拟地址到物理地址的映射，
// 但不会构建页表，构建页表是操作系统的任务。
// 在程序加载到内存以及程序运行过程中，操作系统会不断更新程序对应的页表，
// 并将页目录的物理地址保存到CR3寄存器。
// MMU向缓存中加载页表时，会根据CR3寄存器找到页目录，
// 再找到页表，最终通过软件和硬件的结合来完成内存映射。
// CR3是CPU内部的一个寄存器，专门用来保存页目录的物理地址。
// 操作系统在构建页表时将内存权限定义好，当MMU对虚拟地址进行映射时，
// 首先检查低12位，看当前程序是否有权限使用，如果有，就完成映射，
// 如果没有，就产生一个异常，并交给操作系统处理。

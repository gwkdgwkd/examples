...
// 简单地理解哈希函数，它可以接收一个元素，并通过内部对该元素做再加工，最终会得出一个整形值并反馈回来。
// 需要注意的是，哈希函数只是一个称谓，其本体并不是普通的函数形式，而是一个函数对象类。
// 因此，如果我们想自定义个哈希函数，就需要自定义一个函数对象类。
...
//   }
// };
// 该规则在底层实现过程中，直接用==运算符比较容器中任意2个元素是否相等，这意味着，如果容器中存储的元素类型，
// 支持直接用==运算符比较是否相等，则该容器可以使用默认的std::equal_to<key>比较规则；反之，就不可以使用。
...
// 因此默认的hash<key>哈希函数不再适用，这时就需要以函数对象类的方式自定义一个哈希函数：
class hash_fun {
 public:
...
// 1.在Person 类中重载==运算符，这会使得std::equal_to<key>比较规则中使用的==运算符变得合法，
//   myset容器就可以继续使用std::equal_to<key>比较规则；
bool operator==(const Person &A, const Person &B) {  // 2个参数必须用const修饰

...
// 连续内存容器，或称为基于数组的容器，
// 把它的元素存放在一块或多块（动态分配的）内存中，每块内存中存有多个元素。
...
// 当发生插入时，避免移动是否重要？如果是，要避免连续内存的容器。
// 容器中数据布局是否需要和C兼容？如果需要，就只能选择vector。
// 元素查找速度是否是关键的考虑因素？
// 如果是，就要考虑哈希容器、排序的vector和标准关联容器，或许这就是优先顺序。
// 如果容器内部使用了引用计数技术，是否介意？
// 如果是，避免使用string和rope。可以考虑vector<char>。
// 对插入和删除操作，需要事物语义么？
// 也就是说，在插入和删除失败时，需要回滚能力吗？
// 如果需要，使用基于节点的容器。
// 如果对多个元素的插入操作需要事物语义，需要选择list，
// 只有list对多个元素的插入操作提供了事物语义。
...
// 如果序列容器的迭代器是随机访问类型，
// 而且只要没有删除操作发生，且插入操作只发生在容器的末尾，
// 则指向数据的指针和引用就不会失效，这样的容器是否有帮助？
// 如果是，选择deque。
// 当插入仅在容器末尾发生时，deque的迭代器有可能失效。

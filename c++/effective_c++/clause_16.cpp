...
//  delete的最大问题在于：即将被删除的内存之内究竟有多少对象？答案决定了有多少个析构函数必须被调用起来。

// 即将被删除的那个指针，所指的是单一对象或对象数组？这是必不可缺的问题，因为单一对象的内存布局一般与数组的内存布局不同。
...
int main() {
  ...
  // 最好尽量不要对数组做typedef动作，可以使用string，vector等模板代替数组。

...
// 11包含copy的算法：
// copy、copy_backward、replace_copy、reverse_copy、replace_copy_if、unique_copy、
...
OutputIterator copy_if(InputIterator begin, InputIterator end,
  ...
  // 虽然STL不允许“复制所有使判别式为真的元素”，但允许“复制所有使判别式条件不为真的元素”。
...
int main() {
  ...
  // not1不能被直接应用到一个函数指针上（条款41），函数指针必须首先用ptr_fun进行转换。

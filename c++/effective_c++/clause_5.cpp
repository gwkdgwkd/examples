...
// default构造函数和析构函数主要是给编译器一个地方用来放置“藏身幕后”的代码，
// 像调用base class和non-static成员变量的构造函数和析构函数。
// copy构造函数和copy assignment操作符，
// 编译器创建的版本只是单纯地将来源对象的每一个non-static成员变量拷贝到目标对象。
// 编译器生成的copy assignment操作符行为基本上与copy构造函数如出一辙，
// 但一般而言只有当生出的代码合法且有适当机会证明它有意义，才会产生。
template <class T>
class NamedObject {
 public:
 ...
 private:
  // 1.C++并不允许让引用改指向不同的对象。
  //   如果打算在一个内含引用成员的class内支持赋值操作，必须自己定义copy assignment操作符。
  string& nameValue;
...
//   毕竟编译器为derived class所生成订的copy assignment操作符想象中可以处理base class成分，
//   但当无法调用时，就不生成，也就不允许赋值操作。
class Base {
 private:

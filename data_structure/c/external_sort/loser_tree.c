...
// 胜者树与败者树可以在log(n)的时间内找到最值，任何一个叶子结点的值改变后，
// 利用中间结点的信息，还是能够快速地找到最值，在k路归并排序中经常用到。
// 胜者树的一个优点是，如果一个选手的值改变了，可以很容易地修改这棵胜者树，
// 只需要沿着从该结点到根结点的路径修改这棵二叉树，而不必改变其他比赛的结果。
// 败者树是胜者树的一种变体，在败者树中，用父结点记录其左右子结点进行比赛的败者，
// 而让胜者参加下一轮的比赛，败者树的根结点记录的是败者，需要加一个结点来记录整个比赛的胜利者。
// 采用败者树可以简化重构的过程：
...
// 但会增加内部归并的时间，得不偿失，为了避免在增加k值的过程中影响内部归并的效率，
// 在进行k-路归并时可以使用败者树来实现，该方法在增加k值时不会影响其内部归并的效率。

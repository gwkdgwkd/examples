...
// 在选择具体的查找策略时，有迭代器指定的区间是否是排序的，
// 这是一个至关重要的决定条件。
// 如果区间是排序的，
// 那么通过binary_search、lower_bound、upper_bound和equal_range，
// 可以获得更快的查找速度（对数时间）。
// 如果不是排序的，那么只能使用count、count_if、find以及find_if，
// 这些算法仅提供线性时间的效率。
// 无论是依赖默认得的搜索判别式，还是自己指定的判别式，
// 这都不会影响选择查找算法的决定。
class Widget {
 public:
...
// 对于标准序列容器可以使用本条款中给出的建议，
// 用begin和end迭代器来指明区间。
// 但对于标准关联容器，
// 建议选择count、find、lower_bound、upper_bound和equal_range的地方，
// 只需选择同名的成员函数即可。
...
int main() {
  ...
  // 因为find找到第一个匹配的结果后马上就返回了，
  // 而count必须到达区间的末尾，以便找到所有的匹配。
  ...
  // 与标准C/C++函数库中的bsearch不同的是，
  // binary_search仅仅返回一个bool值：是否找到了特定的值。
  ...
  // 如果要在multiset和multimap中测试一个值是否存在，
  // 则一般情况下用find比用count好，
  // 因为find只要找到第一个期望的值就返回了，
  // 而count在最坏的情形下必须检查容器中的每一个对象。
  // 对于set和map而言，这不是一个问题，
  // 因为set不允许有重复的值，而map不允许有重复的键。
  // 要在关联容器中统计个数，使用count是非常安全的。
  // 特别是，它比先调用equal_range，
  // 再在结果迭代器上调用distance的做法要好得多。
  // 1.首先，用法非常清晰，count的含义就是计数。
  // 2.其次，使用count更加简单，没有必要先创建一对迭代器，
  //   再将first和second两个迭代器传给distance。
  // 3.第三，速度更快一些。

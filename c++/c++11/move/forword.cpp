...
// 所谓完美，即不仅能准确地转发参数的值，
// 还能保证被转发参数的左、右值属性不变。
// C++11标准为C++引入右值引用语法的同时，
// 还解决了一个C++98/03标准长期存在的短板，
...
//   对于函数内部的参数t来说，它有自己的名称，
//   也可以获取它的存储地址，因此它永远都是左值，也就是说，
//   传递给otherdef()函数的参数t永远都是左值。
...
// 那么如何才能将函数模板接收到的形参连同其左、右值属性，
// 一起传递给被调用的函数呢？
// C++11标准引入了模板函数forword<T>()，
// 只需要调用该函数，就可以很方便地解决此问题。
...
// 总的来说，在定义模板函数时应该：
// 1.采用右值引用定义参数类型，
//   由此该函数既可以接收外界传入的左值，也可以接收右值；
// 2.使用C++11提供的forword()模板函数修饰被调用函数中需要维持左、
//   右值属性的参数。

...
// 但是经过计算得知，如果毫无限度地增加k值，虽然会减少读写外存数据的次数，但会增加内部归并的时间，得不偿失。
...
// 使用败者树来实现增加 k-路归并的规模来提高外部排序的整体效率。但是对于k值得选择也并不是一味地越大越好，而是需要综合考虑选择一个合适的k值。
...
typedef struct { int key; } ExNode, External[k + 1];
...
void Adjust(LoserTree ls, int s) {
  int t = (s + k) / 2;
  while (t > 0) {
    // 判断每一个叶子结点同其双亲结点中记录的败者的值相比较，调整败者的值，其中s一直表示的都是胜者

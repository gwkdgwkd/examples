...
// s[3] = 'x';
// 首先调用operator[]用来读取string的部分值，
// 但是第二次调用该函数是为了完成写操作。
// 我们应能够区别对待读调用和写调用，
// 因为读取reference-counted string是很容易的，
...
// 当LargeObject对象被建立时，不从磁盘上读取所有的数据，这
// 样懒惰法解决了这个问题。
// 不过这时建立的仅是一个对象壳，
// 当需要某个数据时，这个数据才被从数据库中取回。
...
// 对象中每个字段都用一个指向数据的指针来表示，
// LargeObject构造函数把每个指针初始化为空。
...
// 如果编译器不支持mutable，
// 需要找到另一种方法让编译器允许你在const成员函数里修改数据成员。
// 一种方法叫做fake this（伪造this指针），
// 你建立一个指向non-const指针，指向的对象与this指针一样。
// 当你想修改数据成员时，你通过fake this访问它：
class LargeObject2 {
 public:
 ...
 private:
  string* field1Value;  // 不声明为mutable，因为老的编译器不支持
};
const string& LargeObject2::field1() const {
  // 声明指针fakeThis，
  // 其与this指向同样的对象但是已经去掉了对象的常量属性
...
// 实际上，如果你的计算都是重要的，
// lazy evaluation可能会减慢速度并增加内存的使用，
// 因为除了进行所有的计算以外，
// 你还必须维护数据结构让lazy evaluation尽可能地在第一时间运行。
// 在某些情况下要求软件进行原来可以避免的计算，
// 这时lazy evaluation才是有用的。
// lazy evaluation对于C++来说没有什么特殊的东西。
// 这个技术能被运用于各种语言里，
// 几种语言例如著名的APL、dialects of Lisp（事实上所有的数据流语言），
// 都把这种思想做为语言的一个基本部分。
// 然而主流程序设计语言采用的是eager evaluation，C++是主流语言。
// 不过C++特别适合用户实现lazy evaluation，
// 因为它对封装的支持使得能在类里加入lazy evaluation，
// 而根本不用让类的使用者知道。

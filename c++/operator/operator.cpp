...
class Complex {
 ...
 public:
  ...
  // 在实际开发中，应该同时提供以上两种形式，这样做是为了适应const对象，因为const对象只能调用const成员函数，
  // 如果不提供第二种形式，那么将无法访问const对象的任何元素。
...
void testN2() {
  ...
  // 这样做是为了保证+运算符的操作数能够被对称的处理，换句话说，小数（double类型）在+左边和右边都是正确的。
  ...
  // 为什么就是不转换呢？
  // 没错，编译器不会转换，原因在于，C++只会对成员函数的参数进行类型转换，而不会对调用成员函数的对象进行类型转换。

...
// 有环链表并不一定就是循环链表。
// 循环链表指的是首尾相连的单链表，而有环链表则指的是单链表中存在一个循环子链表。
...
// 当函数的返回值为True，表示该链表有环；
// 反之若函数返回值为False，表明链表中无环。
// 显然，此实现方案的时间复杂度为O(n^2)。

// 2.相比上一种实现方案，这是一种时间复杂度为O(n)的算法。
//   该算法的实现思想需要借助一个论点，即在一个链表中，
//   如果2个指针（假设为H1和H2）都从表头开始遍历链表，
//   其中H1每次移动2个节点的长度（H1=H1->next->next）,
//   而H2每次移动1个节点的长度（H2=H2->next），如果该链表为有环链表，则H1、H2最终必定会相等；
//   反之，如果该链表中无环，则H1、H2永远不会相遇。
...
// 当函数返回False时，表明当前链表中无环；
// 反之若返回True，则表明该链表为有环链表。
// 和第一种算法相比，本算法的时间复杂度为O(n)。

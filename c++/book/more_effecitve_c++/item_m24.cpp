...
// 因为在程序里每个类只需要一个vtbl拷贝，所以编译器会遇到棘手的问题：把它放哪里。
...
// 编译器生成的代码会做如下这些事情：
// 1.通过对象的vptr找到类的vtbl。
//   这是简单的操作，因为编译器知道在对象内哪里能找到vptr，毕竟是由编译器放置的。
//   因此这个代价只是：
//   一个偏移调整（以得到vptr）；
//   一个指针的间接寻址（以得到vtbl）。
...
// 因此增加了每个类和每个对象中的虚函数额外占用的空间，运行时调用的代价也增加些。
